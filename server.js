...
function rgAliasV3(universeKey, n) {
    const t = (typeof RG_ALIASES_V3 !== 'undefined') ? RG_ALIASES_V3[universeKey]?.[n] : null;
    return t ? normalizeRG(t) : n;
}
...
function rgApplyKnownAliases(universeKey, value) {
    ...
    // V3 : noms VF / anglais / japonais / anciennes écritures (prioritaire).
    const v3 = rgAliasV3(universeKey, n);
    if (v3 !== n) return v3;
    ...
    // Les anciens alias peuvent pointer vers un nom renommé en VF : on suit la chaîne.
    return rgAliasV3(universeKey, n);
}
...
function rgCanonicalInput(universeKey, value, pool) {
    ...
    // 4) Faute de frappe sur un alias (nom anglais / japonais mal orthographié) :
    //    accepté seulement si un seul personnage du pool correspond.
    const aliasMap = (typeof RG_ALIASES_V3 !== 'undefined') ? RG_ALIASES_V3[universeKey] : null;
    if (aliasMap && brut.length >= 4 && typeof distanceRG === 'function' && typeof toleranceRG === 'function') {
        const cibles = new Set();
        for (const [alias, target] of Object.entries(aliasMap)) {
            if (Math.abs(alias.length - brut.length) > 3) continue;
            if (distanceRG(alias, brut) <= toleranceRG(Math.max(alias.length, brut.length))) {
                const t = normalizeRG(target);
                if (norms.has(t)) cibles.add(t);
            }
        }
        if (cibles.size === 1) {
            const t = [...cibles][0];
            // pas si la saisie ressemble déjà directement à un autre nom du pool
            const direct = pool.some(name => { const x = normalizeRG(name); return x !== t && distanceRG(x, brut) <= toleranceRG(Math.max(x.length, brut.length)); });
            if (!direct) return t;
        }
    }
...
const RG_POOLS_V2 = {"naruto": ["Naruto Uzumaki", "Sasuke Uchiwa", "Sakura Haruno", "Kakashi Hatake", "Itachi Uchiwa", "Hinata Hyūga", "Gaara", "Minato Namikaze", "Madara Uchiwa", "Hashirama Senju", "Neji Hyūga", "Jiraiya", "Agara", "Ageha", "Akahoshi", "Akamaru", "Akatsuchi", "Akino", "Anko Mitarashi", "Ao", "Aoi Rokushô", "Arashi Fûma", "Ashina Uzumaki", "Asura Ōtsutsuki", "Asuma Sarutobi", "Ayame", "Zetsu Noir", "Boruto Uzumaki", "Chino", "Chiriku", "Chiyo", "Chôchô Akimichi", "Chôji Akimichi", "Chôza Akimichi", "Daemon", "Danzô Shimura", "Darui", "Deidara", "Denki Kaminarimon", "Ebisu", "Fû", "Fū Yamanaka", "Fubuki Kakuyoku", "Fugaku Uchiwa", "Gaï Maito", "Gama", "Gamabunta", "Gamakichi", "Hagoromo Ôtsutsuki", "Haku", "Hamura Ôtsutsuki", "Hanabi Hyûga", "Hanzô", "Hayate Gekkô", "Hiashi Hyûga", "Hidan", "Himawari Uzumaki", "Hiruzen Sarutobi", "Ibiki Morino", "Indra Ôtsutsuki", "Ino Yamanaka", "Inoichi Yamanaka", "Inojin Yamanaka", "Iruka Umino", "Izumi Uchiwa", "Jigen", "Jinpachi Munashi", "Jirôbô", "Jirocho", "Jōseki", "Jûbi", "Jûgo", "Jūjin", "Jūzō Biwa", "Kabuto Yakushi", "Kagami Uchiwa", "Kagari", "Kaguya Ôtsutsuki", "Kakuzu", "Kankurô", "Karin Uzumaki", "Karui", "Katsuyu", "Kawaki", "Kisame Hoshigaki", "Konan", "Konohamaru Sarutobi", "Kurama", "Kushina Uzumaki", "Matatabi", "Meï Terumi", "Metal Lee", "Daï Maito", "Mito Uzumaki", "Mitsuki", "Momoshiki Ôtsutsuki", "Mû", "Nagato", "Obito Uchiwa", "Omoï", "Onoki", "Orochimaru", "Pakkun", "Rin Nohara", "Rock Lee", "Saï", "Samui", "Sarada Uchiwa", "Sasori", "Shikadai Nara", "Shikaku Nara", "Shikamaru Nara", "Shino Aburame", "Shisui Uchiwa", "Shukaku", "Son Gokû", "Suzume", "Temari", "Tenten", "Tobirama Senju", "Tsunade", "Utakata", "Zetsu Blanc", "Yahiko", "Yamato", "Zōmei", "Zôri", "Kiba Inuzuka", "Kurenai Yūhi", "A", "Rasa", "Baki", "Chōjūrō", "Yagura Karatachi", "Zabuza Momochi", "Suigetsu Hōzuki", "Mangetsu Hōzuki", "Kurotsuchi", "Gengetsu Hōzuki", "Mifune", "Izuna Uchiwa", "Kinshiki Ōtsutsuki", "Isshiki Ōtsutsuki", "Toneri Ōtsutsuki", "Gyūki", "Isobu", "A — Premier Raikage", "A — Troisième Raikage", "Agari Kichi", "Ajisaï", "Akio", "Amachi", "Amado Sanzu", "Amaru", "Ameyuri Ringo", "Ami", "Aoba Yamashiro", "Aoda", "Araya", "Atsui", "Baïu", "Baku", "Bansai", "Bekkō", "Benga", "Benten", "Bifū", "Bikō", "Biwako Sarutobi", "Blue B", "Boro", "Bull", "Bunpuku", "Buntan Kurosuki", "Burō", "Byakuren", "C", "Chōbi", "Chōhan", "Chômei", "Chōsa", "Chûkichi", "Chûshin", "Code", "Daikoku Funeno", "Dajimu", "Dan Katô", "Datsuji", "Deepa", "Delta", "Denka", "Dodaï", "Dōjō", "Doki", "Dōmu", "Doshu Goetsu", "Dosu Kinuta", "Dotô Kazahana", "Ebizô", "Eida", "Eiki", "En Chinoike", "En no Gyôja", "Enko Onikuma", "Enma", "Ensui Nara", "Fudo", "Fuen", "Fugai", "Fuguki Suikazan", "Fûjin", "Fûka", "Fukai", "Fukasaku", "Fuki", "Fukusuke Hiashira", "Funamushi", "Fūshin", "Fūta", "Gamahiro", "Gamaken", "Gamamaru", "Gamariki", "Gamatabi", "Gamatatsu", "Gantetsu", "Garaga", "Gari", "Gatô", "Gengo", "Genma Shiranui", "Gennô", "Genzô", "Gerotora", "Giichi", "Ginkaku", "Gôzu", "Gulko", "Guren", "Guruguru", "Gyokurō", "Hachidai", "Haido", "Hakkaku", "Hako Kuroi", "Han", "Hana Inuzuka", "Hanare", "Hanzaki", "Haruna", "Harusame", "Hina", "Hiruko", "Hizashi Hyûga", "Hoheto Hyûga", "Hōichi", "Hōki Taketori", "Hokuto", "Homura Mitokado", "Hotaru", "Hotarubi", "Hyô", "Ibuse", "Idate Morino", "Ikkaku Umino", "Inabi Uchiwa", "Inari", "Iō", "Isaribi", "Ishikawa", "Ittan", "Ittetsu", "Iwabee Yuino", "Iwana Akame", "Iwashi Tatami", "Izumo Kamizuki", "Jako", "Jibachi Kamizuru", "Jiga", "Jigumo", "Jin", "Jinin Akebino", "Kagerô Fûma", "Kaiza", "Kaji", "Kakkou", "Kamatari", "Kamikiri", "Kandachi", "Kanna", "Kanpachi", "Karashi", "Karura", "Kashin Koji", "Katasuke Tôno", "Kazuma", "Kido Tsumiki", "Kidômaru", "Kigiri", "Kihô", "Kikunojô", "Kimimaro Kaguya", "Kin Tsuchi", "Kinkaku", "Kirara", "Kirisame", "Kitsuchi", "Kiyoi Yotsuki", "Kô Hyûga", "Kogen", "Kohari Mitokado", "Koharu Utatane", "Kokuô", "Komachi", "Komori", "Kôsuke Maruboshi", "Kotetsu Hagane", "Kotohime", "Koyuki Kazahana", "Kujaku", "Kurobachi Kamizuru", "Kurobane Raijin", "Kushimaru Kuriarare", "Kyodaigumo", "Kyodaija", "Kyūsuke", "Lando", "Lone", "Mabui", "Madame Shijimi", "Mahiru", "Maki", "Mamushi", "Manabu Akado", "Manda", "Matsu", "Matsuri", "Meizu", "Menma Uzumaki", "Meno", "Menza", "Midare", "Mikoshi", "Mikoto Uchiwa", "Minoichi", "Mirai Sarutobi", "Misumi Tsurugi", "Mizore Fuyukuma", "Mizuki Tôji", "Mizura", "Mō", "Moegi Kazamatsuri", "Monju", "Monzaemon Chikamatsu", "Mōryō", "Mōsō", "Motoi", "Mozuku", "Mubi", "Mugino", "Mukade", "Murasame", "Muta Aburame", "Nadare Rôga", "Namida Suzumeno", "Nangô", "Naori Uchiwa", "Natori", "Natsuhi", "Nawaki", "Nekobaa", "Nekomata", "Ninkame", "Nishi", "Nono Yakushi", "Nue", "Nurari", "Oboro", "Okisuke", "Oogama Sennin", "Osoi", "Otoha", "Pakura", "Pochi", "Ponta", "Princesse Fuku", "Raidô Namiashi", "Raïga Kurosuki", "Raimei", "Ranka", "Ranke", "Ranmaru", "Renga", "Reto", "Rinji", "Rō", "Rôshi", "Ryûgan", "Ryūmyaku", "Ryūzetsu", "Sabu", "Saiken", "Sakon", "Sakumo Hatake", "Sana", "Sandayuu Azama", "Sari", "Sasame Fûma", "Satori", "Saya", "Sazanami", "Seimei", "Sekka", "Sen", "Shamon", "Shiba", "Shibai Ôtsutsuki", "Shibi Aburame", "Shibito Azuma", "Shibuki", "Shigure", "Shiho", "Shiin", "Shikuro Aburame", "Shima", "Shimon Hijiri", "Shin Uchiwa", "Shinki", "Shinnô", "Shion", "Shiore", "Shira", "Shiranami", "Shisou", "Shizuka", "Shizune", "Shun", "Shura", "Sora", "Sôtetsu Kazahana", "Suien", "Suika", "Suiko", "Suiren", "Sumaru", "Sumashi", "Sumire Kakei", "Suzumebachi Kamizuru", "Taiseki", "Tajima Uchiwa", "Tamaki", "Tatewaki", "Tayuya", "Tazuna", "Tenga", "Tenji", "Tera", "Teraï", "Teuchi", "Teyaki Uchiwa", "Todoroki", "Tōka Senju", "Tokuma Hyûga", "Tonbo Tobitake", "Tonton", "Toroi", "Torune Aburame", "Towa", "Tsubaki Kurogane", "Tsuguri", "Tsukino", "Tsunami", "Tsurushi Hachiya", "Udon Ise", "U-hei", "Ukon", "Ulshi", "Umibôzu", "Urakaku", "Urashiki Ôtsutsuki", "Uruchi Uchiwa", "Urushi", "Victor", "Wagarashi", "Waraji", "Wasabi Izuno", "Watari Nagare", "Wataru", "Yakumo Kurama", "Yashamaru", "Yashiro Uchiwa", "Yodo", "Yome", "Yoro", "Yoroï Akadô", "Yoshino Nara", "Yota", "Yotaka", "Yûgao Uzuki", "Yugito Nii", "Yukata", "Yûkimaru", "Yûra", "Yurinojô", "Zaji", "Zaku Abumi", "Zansūru", "Tsume Inuzuka", "Kizashi Haruno", "Mebuki Haruno", "Hikaku Uchiwa", "Butsuma Senju", "Kawarama Senju", "Itama Senju", "Killer Bee", "Tekka Uchiwa", "Torifu Akimichi", "Kuromaru", "Bisuke", "Garuda", "Natsu Hyûga", "Kagura Karatachi", "Jura", "Hidari", "Temujin", "Kahyô", "Michiru Tsuki", "Hikaru Tsuki", "Shabadaba", "Taruho", "Sara", "Mui", "Hakuja Sennin", "Tora"], "onepiece": ["Monkey D. Luffy", "Roronoa Zoro", "Nami", "Sanji", "Nico Robin", "Trafalgar D. Water Law", "Shanks", "Portgas D. Ace", "Marshall D. Teach", "Kaido", "Boa Hancock", "Sabo", "Abdullah", "Absalom", "Ain", "Aisa", "Aladine", "Alpacaman", "Alvida", "Charlotte Amande", "Andre", "Aphelandra", "Aramaki", "Arlong", "Ashura Doji", "Atlas", "Atmos", "Avalo Pizarro", "Morgan", "Babanuki", "Baby 5", "Baccarat", "Baggaley", "Bakkin", "Bao Huang", "Bariete", "Bartholomew Kuma", "Bartolomeo", "Basil Hawkins", "Bastille", "Batman", "Bellamy", "Bell-mère", "Belo Betty", "Benn Beckman", "Bentham", "Bepo", "Big Pan", "Billy", "Binz", "Black Maria", "Blackback", "Blamenco", "Blenheim", "Blue Gilly", "Blueno", "Boa Marigold", "Boa Sandersonia", "Bobbin", "Bogard", "Jewelry Bonney", "Boo", "Boodle", "Borsalino", "Brannew", "Briscola", "Brogy", "Brook", "Brownbeard", "Buchi", "Buffalo", "Baggy", "Byrnndi World", "Cabaji", "Caesar Clown", "Caimanlady", "Kalgara", "Camie", "Capone Bege", "Caribou", "Carina", "Carmel", "Carne", "Carrot", "Catarina Devon", "Cavendish", "Chaka", "Charlos", "Charlotte Anana", "Charlotte Anglais", "Charlotte Basans", "Charlotte Bavarois", "Charlotte Brownie", "Charlotte Brûlée", "Charlotte Chiboust", "Charlotte Chiffon", "Charlotte Cinnamon", "Charlotte Citron", "Charlotte Compote", "Charlotte Counter", "Charlotte Cracker", "Charlotte Custard", "Charlotte Daifuku", "Charlotte Decuple", "Charlotte Dolce", "Charlotte Dragée", "Charlotte Flampe", "Charlotte Galette", "Charlotte Harumatsu", "Charlotte Joconde", "Charlotte Joscarpone", "Charlotte Kanten", "Charlotte Katakuri", "Charlotte Kato", "Charlotte Linlin", "Charlotte Lola", "Charlotte Mascarpone", "Charlotte Mash", "Charlotte Mobile", "Charlotte Mont-d'Or", "Charlotte Moscato", "Charlotte Myukuru", "Charlotte Newichi", "Charlotte Newji", "Charlotte Nougat", "Charlotte Nusstorte", "Charlotte Opera", "Charlotte Oven", "Charlotte Perospero", "Charlotte Poire", "Charlotte Praline", "Charlotte Pudding", "Charlotte Raisin", "Charlotte Smoothie", "Charlotte Snack", "Charlotte Tablet", "Charlotte Yuen", "Chess", "Chew", "Chimney", "Clover", "Conis", "Coribou", "Crocodile", "Crocus", "Curiel", "Curly Dadan", "Dagama", "Daifugo", "Daikoku", "Dalmatian", "Dalton", "Damask", "Daruma", "Daz Bones", "Dellinger", "Demalo Black", "Den", "Denjiro", "Diamante", "Dice", "Disco", "Doberman", "Dobon", "Doc Q", "Doma", "Domino", "Don Chinjao", "Don Krieg", "Donquixote Doflamingo", "Donquixote Homing", "Donquixote Mjosgard", "Donquixote Rosinante", "Dorry", "Dosun", "Douglas Bullet", "Dr. Hiriluk", "Dr. Hogback", "Dr. Indigo", "Dr. Kureha", "Dracule Mihawk", "Perona", "Du Feld", "Duval", "Edison", "Edward Newgate", "Edward Weevil", "Elizabello II", "Emporio Ivankov", "Ener", "Epoida", "Saint Ethanbaron V. Nusjuro", "Eustass Kid", "Farafra", "Fisher Tiger", "Fossa", "Foxy", "Franky", "Fukaboshi", "Fukuro", "Fukurokuju", "Fullbody", "Funkfreed", "Scopper Gaban", "Gaimon", "Galdino", "Gambia", "Gan Fall", "Gasparde", "Gazelleman", "Gecko Moria", "Gedatsu", "Genzo", "Gerd", "Gin", "Ginny", "Ginrummy", "Giolla", "Gladius", "Gloriosa", "Gol D. Roger", "Gonbe", "Gordon", "Gotti", "Guernica", "Gyaro", "Gyukimaru", "Hack", "Hajrudin", "Hamburg", "Hammond", "Hannyabal", "Hanzo", "Haredas", "Haruta", "Hattori", "Heat", "Helmeppo", "Héra", "Heracles", "Hildon", "Hina", "Hody Jones", "Holdem", "Hongo", "Hotori", "Hyogoro", "Hyouzou", "Iceburg", "Ideo", "Igaram", "Ikaros Much", "Imu", "Inazuma", "Inuarashi", "Ipponmatsu", "Issho", "Isuka", "Itomimizu", "Izo", "Jabra", "Jack", "Jaguar D. Saul", "Jango", "Saint Jaygarcia Saturn", "Jean Bart", "Jeet", "Jesus Burgess", "Jinbei", "Jiro", "John", "Johnny", "Jonathan", "Jora", "Joy Boy", "Jozu", "Kaku", "Kalifa", "Kanjuro", "Karasu", "Karoo", "Kashii", "Kawamatsu", "Kaya", "Kikunojo", "Kikyo", "Killer", "Kin'emon", "King", "King Neptune", "Kingdew", "Koala", "Kobby", "Kokoro", "Komachiyo", "Kong", "Kotori", "Koushirou", "Koza", "Kozuki Hiyori", "Kozuki Momonosuke", "Kozuki Oden", "Kozuki Sukiyaki", "Kozuki Toki", "Kuina", "Kumadori", "Kumashi", "Kuro", "Kuromarimo", "Kuroobi", "Kurozumi Higurashi", "Kurozumi Orochi", "Kuzan", "Kyoshiro", "Kyros", "Laboon", "Lacroix", "Laffitte", "Laki", "Lao G", "Leo", "Lilith", "Lindbergh", "Little Oars Jr.", "Loki", "Lonz", "Lorenz", "Lucky Roux", "Lulu", "Machvise", "Macro", "Magellan", "Makino", "Manboshi", "Mansherry", "Marco", "Saint Marcus Mars", "Marguerite", "Masira", "Maynard", "McGuy", "McKinley", "Merry", "Minatomo", "Miss Friday", "Miss Goldenweek", "Miss Merry Christmas", "Miss Monday", "Miss Valentine", "Miyagi", "Mohji", "Momonga", "Monet", "Monkey D. Dragon", "Monkey D. Garp", "Mont Blanc Cricket", "Mont Blanc Noland", "Morgans", "Morley", "Mouseman", "Mousse", "Mozambia", "Mr. 13", "Mr. 4", "Mr. 5", "Mr. 7", "Mr. 9", "Namur", "Napoléon", "Nefertari Cobra", "Nefertari D. Lili", "Nefertari Vivi", "Nekomamushi", "Nero", "Nezumi", "Nico Olvia", "Ninjin", "Nojiko", "Nure-Onna", "Oars", "Ochoku", "Ohm", "Oimo", "Onigumo", "Orlumbus", "O-Tama", "Otohime", "O-Toko", "O-Tsuru", "Pagaya", "Page One", "Pappag", "Patty", "Paulie", "Pearl", "Pedro", "Pekoms", "Pell", "Penguin", "Pica", "Pierre", "Porche", "Porchemy", "Portgas D. Rouge", "Pound", "Prometheus", "Pythagoras", "Queen", "Rabbitman", "Raizo", "Raki", "Rakuyo", "Rebecca", "Richie", "Riku Doldo III", "Rindo", "Rob Lucci", "Rocks D. Xebec", "Rockstar", "Roddy", "Roshio", "Rosward", "Rush", "Ryuboshi", "Ryuma", "Sadi", "Saga", "Sai", "Saint Figarland Garling", "Saint Shepherd Ju Peter", "Saint Topman Warcury", "Sakazuki", "Saldeath", "Salome", "Sanjuan Wolf", "Sarahebi", "Sarquiss", "Sarutobi", "Sasaki", "Satori", "S-Bat", "S-Bear", "Scarlett", "Scotch", "Scratchmen Apoo", "Sengoku", "Senor Pink", "Sentomaru", "S-Flamingo", "Shachi", "Shaka", "Shakuyaku", "Shalria", "Sham", "S-Hawk", "Sheepshead", "Shiki", "Shiryu", "Shimotsuki Kozaburo", "Shimotsuki Yasuie", "Shinobu", "Shirahoshi", "Shoujou", "Shura", "Shyarly", "Sicilian", "Silver Axe", "Silvers Rayleigh", "Smoker", "Solitaire", "Spandam", "Spandine", "Speed", "Speed Jill", "Squardo", "S-Shark", "S-Snake", "Strawberry", "Streusen", "Stronger", "Stussy", "Sugar", "Suleiman", "Surume", "Sweet Pea", "Tamago", "Tank Lepanto", "Tansui", "Tararan", "Taro", "Tashigi", "T-Bone", "Terracotta", "Tesoro", "Thatch", "Tilestone", "Tom", "Tony Tony Chopper", "Toto", "Trebol", "Tristan", "Tsuru", "Ucy", "Ulti", "Urashima", "Urouge", "Usopp", "Uta", "Van Augur", "Vander Decken IX", "Vasco Shot", "Vegapunk", "Vergo", "Victoria Cindry", "Vinsmoke Ichiji", "Vinsmoke Judge", "Vinsmoke Niji", "Vinsmoke Reiju", "Vinsmoke Sora", "Vinsmoke Yonji", "Viola", "Vista", "Vito", "Wadatsumi", "Wanda", "Wanze", "Wapol", "Whitey Bay", "Who's Who", "Wire", "Wyper", "X Drake", "Yama", "Yamakaji", "Yamato", "Yasopp", "Yokozuna", "Yosaku", "Yuda", "Zala", "Zambai", "Zeff", "Zeo", "Zephyr", "Zepo", "Zeus", "Zodia", "Zunesha", "Toko", "Vegapunk York", "Hachi", "Momousagi", "Chaton", "Shamrock", "Woop Slap", "Shakky", "Margaret", "Rika", "Shushu", "Higuma", "Dogra", "Magra", "Bluejam", "Outlook III", "Sterry", "Limejuice", "Bonk Punch", "Building Snake", "Howling Gab", "Piiman", "Tamanegi", "Banchina", "Chabo", "Momoo", "Dr. Nako", "Carue", "Robson", "Lassoo", "Kamakiri", "Braham", "Genbo", "Holy", "Nola", "Pickles", "Tonjit", "Kiev", "Mozu", "Kiwi", "Baskerville", "Pandaman", "Inuppe", "Buhichuck", "Yorki", "Ran", "Bacura", "Minotaurus", "Minokoala", "Minozebra", "Minorhinoceros", "Comil", "John Giant", "Stainless", "Prince Grus", "Hibari", "Kujaku", "Rock", "Mocha", "Kabu", "Gancho", "Wicca", "Rampo", "Rolling Logan", "Kelly Funk", "Bobby Funk", "Gatz", "Charlotte Cadenza", "Charlotte Cabaletta", "King Baum", "Kurozumi Semimaru", "Shimotsuki Ushimaru", "Emet", "Harald", "Gunko", "Sommers", "Killingham", "Wang Zhi", "El Drago", "Tobio", "Bear King", "Borodo", "Needles", "Bigalo", "Shuzo", "Omatsuri", "Muchigoro", "Ratchet", "Xiao", "Musshuru", "Buena Festa", "Tanaka", "Miss Father's Day", "Ripper", "Megalo", "Monster"], "bleach": ["Ichigo Kurosaki", "Rukia Kuchiki", "Renji Abarai", "Byakuya Kuchiki", "Toshiro Hitsugaya", "Kenpachi Zaraki", "Sosuke Aizen", "Uryu Ishida", "Grimmjow Jaegerjaquez", "Ulquiorra Cifer", "Kisuke Urahara", "Yhwach", "Acidwire", "Akon", "As Nodt", "Asguiaro Ebern", "Ashido Kano", "Bambietta Basterbine", "Baraggan Louisenbairn", "Bazz-B", "BG9", "Bonnie", "Chojiro Sasakibe", "Cirucci Sanderwicci", "Coyote Starrk", "Cyan Sung-Sun", "Danon", "Di Roy Rinker", "Edrad Liones", "Eikichirō Saidō", "Ganryu", "Garogai", "Genryusai Shigekuni Yamamoto", "Gerard Valkyrie", "Gin Ichimaru", "Ginrei Kuchiki", "Giriko Kutsuzawa", "Giselle Gewelle", "Grand Fisher", "Gremmy Thoumeaux", "Shuhei Hisagi", "Hisana Kuchiki", "Ichibe Hyosube", "Isane Kotetsu", "Isshin Kurosaki", "Jugram Haschwalth", "Jushiro Ukitake", "Lille Barro", "Oetsu Nimaiya", "Orihime Inoue", "Ouko Yushima", "Pernida Parnkgjas", "Rangiku Matsumoto", "Retsu Unohana", "Shinji Hirako", "Shunsui Kyoraku", "Soi Fon", "Soken Ishida", "Roi des Âmes", "Szayelaporro Granz", "Tier Harribel", "Yachiru Kusajishi", "Yoruichi Shihoin", "Yylfordt Granz", "Zangetsu", "Zennosuke Kurumadani", "Zommari Rureaux", "Mayuri Kurotsuchi", "Kaname Tosen", "Nnoitra Gilga", "Askin Nakk Le Vaar", "Aaroniero Arruruerie", "Abirama Redder", "Aura Michibane", "Baishin", "Baura", "Bawabawa", "Berenice Gabrielli", "Candice Catnipp", "Cang Du", "Charlotte Chuhlhourne", "Chizuru Honsho", "Choe Neng Poww", "Don Kanonji", "Dondochakka Birstanne", "Dordoni Alessandro Del Socaccio", "Driscoll Berci", "Emilou Apacci", "Enryu", "Enzo", "Fērum", "Findorr Calius", "Fishbone D", "Franceska Mila Rose", "Ganju Shiba", "Gantenbainne Mosqueda", "Genshiro Okikiba", "Ggio Vega", "Guenael Lee", "Gunjo", "Hachigen Ushoda", "Hanataro Yamada", "Hanza Nukui", "Hexapodus", "Hikone Ubuginu", "Hiyori Sarugaki", "Hiyosu", "Homura", "Ikkaku Madarame", "Ikumi Unagiya", "Inose", "Izuru Kira", "Jackie Tristan", "Jerome Guizbatt", "Jidanbo Ikkanzaka", "Jinnai Doko", "Jinta Hanakari", "Kageroza Inaba", "Kaien Shiba", "Kanae Katagiri", "Karin Kurosaki", "Kazui Kurosaki", "Keigo Asano", "Kenryu", "Kensei Muguruma", "Kirio Hikifune", "Kiyone Kotetsu", "Koga Kuchiki", "Kokuto", "Kon", "Kugo Ginjo", "Kukaku Shiba", "Kumoi", "Liltotto Lamperd", "Lilynette Gingerbuck", "Lisa Yadomaru", "Loly Aivirrne", "Love Aikawa", "Loyd Lloyd", "Luppi Antenor", "Mahana Natsui", "Maki Ichinose", "Makoto Kibune", "Marechiyo Omaeda", "Marenoshin Ōmaeda", "Masaki Kurosaki", "Mashiro Kuna", "Mask De Masculine", "Meninas McAllon", "Menis", "Menoly Mallia", "Michiru Ogawa", "Mimihagi", "Miyako Shiba", "Mizuiro Kojima", "Moe Shishigawara", "Momo Hinamori", "Muramasa", "Nakeem Grindina", "NaNaNa Najahkoop", "Nanao Ise", "Nelliel Tu Odelschwanck", "Nemu Kurotsuchi", "Nianzol Weizol", "Ninny Spangcole", "Nirgge Parduoc", "Nozomi Kujo", "Numb Chandelier", "Pepe Waccabrada", "Pesche Guatiche", "Quilge Opie", "Rin Tsubokura", "Riruka Dokugamine", "Riyan", "Robert Accutrone", "Rōjūrō Otoribashi", "Royd Lloyd", "Rudbornn Chelute", "Rurichiyo Kasumioji", "Ryo Kunieda", "Ryuken Ishida", "Ryusei Kenzaki", "Sajin Komamura", "Seinosuke Yamada", "Senjumaru Shutara", "Senna", "Sentaro Kotsubaki", "Shawlong Koufang", "Shaz Domino", "Shino Madarame", "Shizuku", "Shrieker", "Shukuro Tsukishima", "Shuren", "Shusuke Amagai", "Sojiro Kusaka", "Sojun Kuchiki", "Sora Inoue", "Taikon", "Tatsuki Arisawa", "Tenjiro Kirinji", "Tesla Lindocruz", "Tessai Tsukabishi", "Tetsuzaemon Iba", "Tokinada Tsunayashiro", "Ururu Tsumugiya", "Wonderweiss Margela", "Yammy Llargo", "Yasutora Sado", "Yukio Hans Vorarlberna", "Yumichika Ayasegawa", "Yūshirō Shihōin", "Yuzu Kurosaki", "Ichika Abarai", "Rikichi", "Makizo Aramaki", "Yasochika Iemura", "Mizuho Asano", "Yûichi Shibata", "Koganehiko", "Shiroganehiko", "Lumina", "Verona", "Metastacia", "Jin Kariya", "Yoshino Sôma", "Gô Kôga", "Ryô Utagawa", "Yoshi", "Mabashi", "Sawatari"], "hxh": ["Gon Freecss", "Killua Zoldyck", "Kurapika", "Leorio Paradinight", "Hisoka Morow", "Chrollo Lucilfer", "Illumi Zoldyck", "Meruem", "Isaac Netero", "Neferpitou", "Ging Freecss", "Kite", "Biscuit Krueger", "Zeno Zoldyck", "Silva Zoldyck", "Kikyo Zoldyck", "Milluki Zoldyck", "Alluka Zoldyck", "Nanika", "Kalluto Zoldyck", "Gotoh", "Canary", "Tsubone", "Amane", "Satotz", "Menchi", "Buhara", "Lippo", "Hanzo", "Pokkle", "Ponzu", "Tonpa", "Bodoro", "Geretta", "Basho", "Squala", "Baise", "Dalzollene", "Neon Nostrade", "Light Nostrade", "Nobunaga Hazama", "Feitan Portor", "Machi Komacine", "Phinks Magcub", "Franklin Bordeau", "Shizuku Murasaki", "Bonolenov Ndongo", "Pakunoda", "Uvogin", "Shalnark", "Kortopi", "Razor", "Genthru", "Sub", "Bara", "Tsezguerra", "Goreinu", "Abengane", "Shaiapouf", "Menthuthuyoupi", "Colt", "Reina", "Cheetu", "Leol", "Welfin", "Bloster", "Ikalgo", "Meleoron", "Zazan", "Pike", "Rammot", "Morel Mackernasey", "Knov", "Knuckle Bine", "Shoot McMahon", "Palm Siberia", "Komugi", "Gyro", "Wing", "Zushi", "Kastro", "Riehlvelt", "Sadaso", "Gido", "Mito Freecss", "Beans", "Cheadle Yorkshire", "Pariston Hill", "Mizaistom Nana", "Botobai Gigante", "Ginta", "Kanzai", "Saccho Kobayakawa", "Cluck", "Pyon", "Gel", "Linne Horsdoeuvre", "Saiyu", "Beyond Netero", "Nasubi Hui Guo Rou", "Benjamin Hui Guo Rou", "Camilla Hui Guo Rou", "Tserriednich Hui Guo Rou", "Halkenburg Hui Guo Rou", "Tubeppa Hui Guo Rou", "Tyson Hui Guo Rou", "Momoze Hui Guo Rou", "Oito Hui Guo Rou", "Woble Hui Guo Rou", "Kacho Hui Guo Rou", "Fugetsu Hui Guo Rou", "Luzurus Hui Guo Rou", "Theta", "Salé-Salé", "Bill", "Hinrigh Vielopolsky", "Morena Prudo", "Koala", "Hagya", "Peggy", "Brovada", "Flutter", "Hina", "Eeta", "Elena", "Bopobo", "Nickes", "Maha Zoldyck", "Zenji", "Senritsu", "Todo", "Nicholas", "Amori", "Imori", "Umori", "Majtani", "Johness", "Bendot", "Sedokan", "Leroute", "Zebro", "Seaquant", "Tocino", "Ivlenkov", "Zepile", "Binolt", "Dwun", "Owl", "Baro", "Yunju", "Teradein Neutral", "Bushidora Ambitious", "Spinner Clow", "Stick Dinner", "Monta Yuras", "Lin Koshi", "Podungo Lapoy", "Banana Kavaro", "Zhang Lei Hui Guo Rou", "Marayam Hui Guo Rou", "Izunavi", "Omokage", "Sarasa", "Pairo", "Pietro", "Abe"], "snk": ["Eren Jäger", "Mikasa Ackerman", "Armin Arlert", "Livaï Ackerman", "Reiner Braun", "Annie Leonhart", "Zeke Jäger", "Erwin Smith", "Hansi Zoe", "Jean Kirstein", "Connie Springer", "Sasha Braus", "Historia Reiss", "Ymir", "Bertolt Hoover", "Marco Bott", "Floch Forster", "Petra Ral", "Oruo Bozado", "Erd Gin", "Gunther Schultz", "Mike Zacharius", "Moblit Berner", "Keith Sadies", "Dot Pixis", "Dallis Zachary", "Kenny Ackerman", "Rod Reiss", "Frieda Reiss", "Grisha Jäger", "Dina Fritz", "Pieck Finger", "Porco Galliard", "Marcel Galliard", "Gabi Braun", "Falco Grice", "Theo Magath", "Yelena", "Onyankopon", "Niccolo", "Nanaba", "Nile Dork", "Uri Reiss", "Carla Jäger", "Eren Kruger", "Colt Grice", "Willy Tybur", "Lara Tybur", "Ymir Fritz", "Hannes", "Kiyomi Azumabito", "Rico Brzenska", "Mina Carolina", "Thomas Wagner", "Franz Kefka", "Hannah Diamant", "Samuel Linke-Jackson", "Mylius Zeramuski", "Nac Tius", "Daz", "Hitch Dreyse", "Marlowe Freudenberg", "Boris Feulner", "Djel Sannes", "Traute Carven", "Kaya", "Nifa", "Ian Dietrich", "Mitabi Jarnach", "Gustav", "Ilse Langnar", "Anka Rheinberger", "Karl Fritz", "Tom Ksaver", "Faye Jäger", "Zofia", "Udo", "Calvi", "Artur Braus", "Alma", "Karina Braun", "Lobov", "Gross", "Louise", "Ralph", "Mr. Leonhart", "Dimo Reeves", "Flegel Reeves", "Nick", "Kitz Woermann", "Dita Ness", "Luke Siss", "Gelgar", "Lynne", "Henning", "Moses Braun", "Kuchel Ackerman", "Ramzi"], "sds": ["Meliodas", "Ban", "King", "Diane", "Gowther", "Merlin", "Escanor", "Elizabeth Liones", "Zeldris", "Estarossa", "Mael", "Hawk", "Aldrich", "Anne", "Arthur Pendragon", "Bartra Liones", "Bellion", "Burgie", "Cain Barzad", "Chandler", "Chaos", "Dale", "Dalmally", "Dana", "Derieri", "Dolores", "Dreyfus", "Drole", "Elaine", "Fraudrin", "Galand", "Gilthunder", "Gloxinia", "Griamore", "Guila", "Howzer", "Hawk Mama", "Helbram", "Hendrickson", "Jericho", "Lancelot", "Liz", "Margaret Liones", "Matrona", "Melascula", "Monspeet", "Slader", "Tristan", "Twigo", "Veronica Liones", "Vivian", "Zaneri", "Zaratras", "Zhivago", "Ludociel", "Sariel", "Tarmiel", "Cusack", "Aranak", "Arden", "Atra", "Cath Palug", "Chion", "Dahaka", "Dahlia", "Deathpierce", "Deldry", "Roi Démon", "Denzel Liones", "Dogedo", "Dolor", "Donny", "Doronach", "Dubs", "Edlin", "Ellatte", "Fiddich", "Friesia", "Galla", "Gannon", "Gara", "Gawain", "Gelda", "Gerheade", "Golgius", "Grayroad", "Guinevere", "Gustaf", "Hugo", "Ironside", "Isolde", "Jade", "Jenna", "Jenny", "Jude", "Kay", "Dame du Lac", "Macduff", "Marmas", "Mild", "Mod", "Mortlach", "Nadja Liones", "Nanashi", "Nasiens", "Nerobasta", "Ordo", "Orlondi", "Oslo", "Pelio", "Pellegarde", "Pelliot", "Percival", "Ren", "Rosa", "Rou", "Selion", "Sennett", "Simon", "Solaad", "Déesse Suprême", "Talisker", "Tamdhu", "Teaninich", "Threader", "Varghese", "Waillo", "Weinheidt", "Wild", "Zeal", "Zoria", "Puora", "Albion"], "deathnote": ["Light Yagami", "L Lawliet", "Misa Amane", "Near", "Mello", "Ryuk", "Rem", "Teru Mikami", "Soichiro Yagami", "Touta Matsuda", "Sachiko Yagami", "Sayu Yagami", "Shuichi Aizawa", "Kanzo Mogi", "Hideki Ide", "Hirokazu Ukita", "Watari", "Kiyomi Takada", "Naomi Misora", "Raye Penber", "Kyosuke Higuchi", "Reiji Namikawa", "Wedy", "Aiber", "Matt", "Sidoh", "Gelus", "Halle Lidner", "Stephen Gevanni", "Anthony Rester", "Rod Ross", "Armonia Justin Beyondormason", "Hitoshi Demegawa", "Masahiko Kida", "Shingo Mido", "Arayoshi Hatori", "Eiichi Takahashi", "Suguru Shimura", "Beyond Birthday", "Jack Neylon", "Roger Ruvie", "Gook", "Zellogi", "Calikarcha", "Deridovely", "Midora", "Nu", "Takeshi Ooi", "Lind L. Tailor", "Kiichiro Osoreda", "Kurou Otoharada", "Takuo Shibuimaru", "Steve Mason", "David Hoope", "Yuri", "Linda", "Daril Ghiroza", "Kinddara Guivelostain", "Minoru Tanaka", "Taro Kagami"], "cote": ["Kiyotaka Ayanokoji", "Suzune Horikita", "Kikyo Kushida", "Kei Karuizawa", "Kakeru Ryuen", "Arisu Sakayanagi", "Honami Ichinose", "Rokusuke Koenji", "Manabu Horikita", "Miyabi Nagumo", "Yosuke Hirata", "Ken Sudo", "Kanji Ike", "Haruki Yamauchi", "Airi Sakura", "Akito Miyake", "Haruka Hasebe", "Hiyori Shiina", "Albert Yamada", "Mio Ibuki", "Ryuji Kanzaki", "Masayoshi Hashimoto", "Masumi Kamuro", "Sae Chabashira", "Chie Hoshinomiya", "Ichika Amasawa", "Takuya Yagami", "Keisei Yukimura", "Chiaki Matsushita", "Maya Sato", "Kokoro Inogashira", "Kohei Katsuragi", "Daichi Ishizaki", "Tsubasa Nanase", "Kazuomi Hosen", "Fuka Kiryuin", "Mei-Yu Wang", "Akane Tachibana", "Kazuma Sakagami", "Atsuomi Ayanokoji", "Nazuna Asahina", "Tomonari Mashima", "Narumori Sakayanagi", "Tsukishiro", "Satoru Kaneda", "Hayato Kito", "Miki Yamamura", "Ai Morishita", "Hiroya Tokito", "Kayano Onodera", "Mako Amikura", "Satsuki Shinohara", "Yuki Himeno", "Tetsuya Machida", "Ikuto Kiriyama", "Tetsuya Hamaguchi", "Kosei Sanada", "Hideo Sotomura", "Ryotaro Hondo", "Soshi Miyamoto", "Kyosuke Okitani", "Nene Mori", "Anzu Maezono", "Ruri Ichihashi", "Kyogo Komiya", "Reon Kondo", "Takeko Nishino", "Nanami Yabu", "Rika Morofuji", "Shiho Manabe", "Saki Yamashita", "Norihito Watanabe", "So Shibata", "Chihiro Shiranami", "Yume Kobashi", "Yahiko Totsuka", "Kaya Ishikura", "Riku Utomiya", "Sakurako Tsubaki", "Kyo Ishigami", "Katsunori Shiba"], "solo": ["Sung Jinwoo", "Cha Hae-In", "Choi Jong-In", "Baek Yoonho", "Yoo Jinho", "Thomas Andre", "Liu Zhigang", "Igris", "Beru", "Antares", "Sung Jinah", "Park Kyung-Hye", "Lee Joohee", "Song Chi-Yul", "Kim Sangshik", "Hwang Dongsuk", "Kang Taeshik", "Hwang Dongsoo", "Woo Jinchul", "Go Gunhee", "Lim Tae-Gyu", "Ma Dongwook", "Min Byung-Gyu", "Christopher Reed", "Siddharth Bachchan", "Norma Selner", "Bellion", "Tusk", "Iron", "Tank", "Kaisel", "Ashborn", "Baran", "Rakan", "Sillad", "Querehsha", "Tarnak", "Legia", "Yogumunt", "Kandiaru", "Esil Radiru", "Kamish", "L'Architecte", "Goto Ryuji", "Vulcan", "Metus", "Cerberus", "Kasaka", "Barka", "Fangs", "Jima", "Sung Il-Hwan", "Yoo Soohyun", "Lennart Niermann", "Adam White", "Yoo Myunghan", "Han Songyi", "Laura", "Sung Suho"], "clover": ["Asta", "Yuno", "Noelle Silva", "Yami Sukehiro", "Luck Voltia", "Magna Swing", "Fuegoleon Vermillion", "Mereoleona Vermillion", "Julius Novachrono", "Nacht Faust", "Finral Roulacase", "Vanessa Enoteca", "Acier Silva", "Alecdora Sandler", "Charlotte Roselei", "Charmy Pappitson", "Ciel Grinberryall", "Conrad Leto", "Damnatio Kira", "Dante Zogratis", "Fana", "Ichika Yami", "Jack the Ripper", "Kirsch Vermillion", "Klaus Lunettes", "Langris Vaude", "Leopold Vermillion", "Licht", "Liebe", "Lilith", "Lolopechka", "Lucifero", "Lucius Zogratis", "Marx Francois", "Megicula", "Mimosa Vermillion", "Nozel Silva", "Rill Boismortier", "Sekke Bronzazza", "Sister Lily", "Sister Theresa", "Undine", "Vanica Zogratis", "William Vangeance", "Zenon Zogratis", "Gauche Adlai", "Gordon Agrippa", "Grey", "Dorothy Unsworth", "Kaiser Granvorka", "Zora Ideale", "Patolli", "Zagred", "Adrammelech", "Allen Fiarain", "Aruru", "Augustus Kira Clover XIII", "Baro", "Baval", "Beelzebub", "Broccos", "Catherine", "Charla", "Daizaemon O'oka", "David Swallow", "Digit Taliss", "Dominante Code", "Drowa", "Eclat", "Ecthel", "Edward Avalache", "En Ringard", "Fanzell Kruger", "Father Fuego", "Floga", "Foyal Migusteau", "Fragil Tormenta", "Fujio", "Gaderois Godroc", "Gadjah", "Gifso", "Gimodelo", "Ginnojomorifuyu Kezoukaku", "Gueldre Poizot", "Halbet Chevour", "Hamon Caseus", "Heath Grice", "Henry Legolant", "Hollo", "Jester Garandros", "Kabwe Carillon", "Kahono", "Kiato", "Kivn", "Komari Imari", "Ladros", "Letoile Becquerel", "Lotus Whomalt", "Loyce Grinberryall", "Lucifugus", "Lumiere Silvamillion Clover", "Marie Adlai", "Mariella", "Mars", "Milly Maxwell", "Morgen Faust", "Morris Libardirt", "Mushogatake Yosuga", "Naamah", "Nash", "Nathan Agrippa", "Nebra Silva", "Neige", "Noze", "Orsi Orfai", "Owen", "Plumede", "Potrof", "Princia Funnybunny", "Puli Angel", "Rades Spirito", "Ragus", "Raia", "Ralph Niaflem", "Randall Luftair", "Rebecca Scarlet", "Recca", "Revchi Salik", "Rhya", "Richita", "Robero Ringert", "Roland", "Ronne", "Rufel", "Ryudo Ryuya", "Salim of Hapshass", "Sally", "Secre Swallowtail", "Siren Tium", "Sivoir Snyle", "Slotos", "Sol Marron", "Solid Silva", "Svenkin Gatard", "Tetia", "Valtos", "Vetto", "Walgner", "Xerx Lugner", "Yagos", "Bell", "Salamander", "Zara Ideale", "Reine des Sorcières"], "fireforce": ["Shinra Kusakabe", "Arthur Boyle", "Maki Oze", "Tamaki Kotatsu", "Akitaru Obi", "Takehisa Hinawa", "Benimaru Shinmon", "Leonard Burns", "Sho Kusakabe", "Haumea", "Iris", "Viktor Licht", "Vulcan Joseph", "Lisa Isaribe", "Yu", "Konro Sagamiya", "Hibana", "Karim Flam", "Rekka Hoshimiya", "Ogun Montgomery", "Pan Ko Paat", "Joker", "Charon", "Arrow", "Inca Kasugatani", "Ritsu", "Yona", "Assault", "Dragon", "Giovanni", "Nataku Son", "Amaterasu", "Yuichiro Kurono", "Juggernaut", "Kayoko Huang", "Sumire", "Foien Lee", "Gustav Honda", "Hinata", "Hikage", "Flail", "Takigi Tsunderera", "Raffles III"], "mushoku": ["Rudeus Greyrat", "Roxy Migurdia", "Sylphiette", "Eris Boreas Greyrat", "Ruijerd Superdia", "Orsted", "Paul Greyrat", "Ghislaine Dedoldia", "Nanahoshi Shizuka", "Zenith Greyrat", "Lilia Greyrat", "Norn Greyrat", "Aisha Greyrat", "Hitogami", "Kishirika Kishirisu", "Badigadi", "Luke Notos Greyrat", "Zanoba Shirone", "Elinalise Dragonroad", "Philip Boreas Greyrat", "Hilda Boreas Greyrat", "Sauros Boreas Greyrat", "Perugius Dola", "Almanfi", "Soldat Heckler", "Sara", "Linia Dedoldia", "Pursena Adoldia", "Pax Shirone", "Randolph Marianne", "Ariel Anemoi Asura", "Talhand", "Geese Nukadia", "Atofe", "Cliff Grimoire", "Julie", "Laplace", "Moore", "Nina Farion", "Jino Britts", "Isolde Cruel", "Auber Corbett", "Wi Ta", "Ginger York", "Benedikte", "Lara Greyrat", "Lucie Greyrat", "Darius Silva Garganti", "Gall Falion", "Alphonse", "Reida Lia", "Timothy Mayers", "Ars Greyrat", "Sieghart Saladin Greyrat", "Lily Greyrat", "Christina Greyrat", "James Boreas Greyrat", "Pilemon Notos Greyrat", "Rawls", "Alexander Rybak", "Sylvaril", "Jenius", "Suzanne", "Nokopara", "Gyes Dedoldia", "Claire Latreia", "Therese Latreia", "Grabel Zafin Asura", "Akito Shinohara", "Makoto Kuroki"], "rezero": ["Subaru Natsuki", "Emilia", "Rem", "Ram", "Beatrice", "Roswaal L. Mathers", "Reinhard van Astrea", "Crusch Karsten", "Priscilla Barielle", "Regulus Corneas", "Puck", "Otto Suwen", "Garfiel Tinsel", "Frederica Baumann", "Petra Leyte", "Patrasche", "Felt", "Felix Argyle", "Wilhelm van Astrea", "Theresia van Astrea", "Anastasia Hoshin", "Julius Juukulius", "Ricardo Welkin", "Hetaro", "Tivey", "Al", "Echidna", "Satella", "Minerva", "Typhon", "Daphne", "Sekhmet", "Carmilla", "Pandora", "Petelgeuse Romanee-Conti", "Sirius Romanee-Conti", "Capella Emerada Lugunica", "Lye Batenkaitos", "Roy Alphard", "Louis Arneb", "Elsa Granhiert", "Meili Portroute", "Mimi Pearlbaton", "Fortuna", "Liliana Masquerade", "Clind", "Kiritaka Muse", "Halibel", "Schult", "Heinkel van Astrea", "Russel Fellow", "Flam", "Grassis", "Vincent Vollachia", "Volcanica", "Shaula", "Ryuzu Meyer", "Rom", "Kadomon Risch", "Joshua Juukulius", "Reid Astrea", "Flügel", "Marcos Gildark", "Miklotov McMahon", "Bordeaux Zellgef", "Fourier Lugunica", "Hector", "Grimm Fauzen", "Carol Remendis", "Mizelda", "Taritta", "Kuna", "Holly", "Tanza", "Yorna Mishigure", "Chisha Gold", "Olbart Dankelken", "Cecilus Segmunt", "Arakiya", "Madelyn Eschart", "Goz Ralfon", "Moguro Hagane", "Balleroy Temeglyph", "Groovy Gumbledatt", "Kurgan", "Todd Fang", "Jamal Aurelius", "Zikr Osman", "Katya", "Medium O'Connell", "Flop O'Connell", "Serena Dracroy", "Lamia Godwin", "Sphinx"], "fairy": ["Natsu Dragneel", "Lucy Heartfilia", "Gray Fullbuster", "Erza Scarlet", "Wendy Marvell", "Gajeel Redfox", "Luxus Draer", "Juvia Lockser", "Mirajane Strauss", "Makarov Draer", "Sting Eucliffe", "Rogue Cheney", "Acnologia", "Aldoron", "Alzack Connell", "Anna Heartfilia", "Aquarius", "Arcadios", "Aries", "Athena", "Bisca Connell", "Cana Alberona", "Cancer", "Capricorn", "Carla", "Cobra", "Coco", "Deliora", "Droy", "Duke Barbaroa", "Eve Tearm", "Evergreen", "Mavis Vermillion", "Freed Justine", "Frosch", "Gemini", "Gildarts Clive", "Hades", "Happy", "Ichiya", "Igneel", "Jellal Fernandes", "Jose Porla", "Jura Neekis", "Kagura Mikazuchi", "Lector", "Loki", "Lyon Vastia", "Macao Conbolt", "Mest Gryder", "Metalicana", "Nadi", "Neinhart", "Nichiya", "Org", "Panther Lily", "Racer", "Ren Akatsuki", "Romeo Conbolt", "Scorpio", "Sherria Blendy", "Virgo", "Wakaba Mine", "Wally Buchanan", "Warren Rocko", "Warrod Sequen", "Weisslogia", "Wolfheim", "Yajima", "Yomazu", "Yuka Suzuki", "Yukino Agria", "Yuri Draer", "Yury", "Zancrow", "Zentopia", "Zera", "Zeref Dragneel", "Zirconis", "Zoldeo", "Levy McGarden", "Lisanna Strauss", "Minerva Orland", "Brandish μ", "Irene Belserion", "August", "Dimaria Yesta", "Larcade Dragneel", "Ajeel Raml", "Arana Webb", "Aria", "Asuka Connell", "Atlas Flame", "Azuma", "Belno", "Beth Vanderwood", "Bickslow", "Bloodman", "Bluenote Stinger", "Bob", "Bora", "Brain", "Byro Cracy", "Chapati Lola", "Crux", "Darton", "Deneb", "Dobengal", "Dogramag", "Duke Everlue", "Elefseria", "Elfman Strauss", "Erigor", "Ezel", "Faris", "Faust", "Flare Corona", "Franmalth", "Georg Reizen", "God Serena", "Goldmine", "Grandeeney", "Guttman", "Hibiki Lates", "Hisui E. Fiore", "Horologium", "Hoteye", "Hughes", "Ignia", "Ikaruga", "Imitatia", "Invel Yura", "Jackal", "Jackpot", "Jacob Lessio", "Jason", "Jenny Realight", "Jet", "Jiemma", "Jude Heartfilia", "Kageyama", "Kain Hikaru", "Karen Lilica", "Kawazu", "Keith", "Keyes", "Kinana", "Kurohebi", "Kyoka", "Laki Olietta", "Layla Heartfilia", "Leiji", "Libra", "Lyra", "Mard Geer", "Max Alors", "Mercphobia", "Meredy", "Michelle Lobster", "Michello", "Midnight", "Millianna", "Motherglare", "Mystogan", "Nab Lasaro", "Nullpudding", "Obra", "Ooba Babasaama", "Ophiuchus", "Orga Nanagear", "Plue", "Polaris", "Porlyusica", "Pyxis", "Reedus Jonah", "Risley Law", "Rocker", "Rufus Lore", "Rustyrose", "Sagittarius", "Samuel", "Sayla", "Seilah", "Selene", "Semmes", "Shagotte", "Sherry Blendy", "Sho", "Silver Fullbuster", "Simon", "Skiadrum", "Sorano Agria", "Sugarboy", "Taurus", "Tempester", "Toby Horhorta", "Toma E. Fiore", "Torafuzar", "Touka", "Ultear Milkovich", "Ur", "Vidaldus Taka", "Viernes", "Vijeeter Ecor", "Wahl Icht", "Ivan Draer", "Sol", "Totomaru", "Rayule", "Karacka", "Lullaby", "Moka", "Bobo", "Fukuro", "Rob", "Gran Doma", "Lahar", "Klodoa", "Roubaul", "Kaby Melon", "Spetto", "Pisces", "Caelum", "Roi des esprits célestes", "Bacchus Groh", "War Cry", "Nobarly", "Jäger", "Kamika", "Uosuke", "Kama", "Cosmos", "Scissor Runner", "Lamy", "Crawford Seam", "Marin Hollow", "Hyberion", "Dan Straight", "Zash Caine", "Sonia", "Animus", "Éclair"], "bluelock": ["Yoichi Isagi", "Rin Itoshi", "Meguru Bachira", "Seishiro Nagi", "Reo Mikage", "Shoei Barou", "Hyoma Chigiri", "Ryusei Shidou", "Michael Kaiser", "Sae Itoshi", "Oliver Aiku", "Gin Gagamaru", "Rensuke Kunigami", "Jyubei Aryu", "Aoshi Tokimitsu", "Ikki Niko", "Raichi Jingo", "Tabito Karasu", "Eita Otoya", "Kenyu Yukimiya", "Yo Hiori", "Ranze Kurona", "Kiyora Jin", "Alexis Ness", "Don Lorenzo", "Charles Chevalier", "Noel Noa", "Julian Loki", "Lavinho", "Chris Prince", "Marc Snuffy", "Jinpachi Ego", "Anri Teieri", "Shuto Sendou", "Gurimu Igarashi", "Asahi Naruhaya", "Wataru Kuon", "Yudai Imamura", "Junichi Wanima", "Keisuke Wanima", "Nijiro Nanase", "Miroku Darai", "Teppei Neru", "Kazuma Niou", "Gen Fukaku", "Ryosuke Kira", "Zantetsu Tsurugi", "Okuhito Iemon", "Yukio Ishikari", "Adam Blake", "Hibiki Okawa", "Iyo Isagi", "Issei Isagi"], "fma": ["Edward Elric", "Alphonse Elric", "Roy Mustang", "Riza Hawkeye", "Scar", "Winry Rockbell", "King Bradley", "Greed", "Envy", "Père", "Pinako Rockbell", "Van Hohenheim", "Trisha Elric", "Jean Havoc", "Heymans Breda", "Vato Falman", "Kain Fuery", "Maes Hughes", "Gracia Hughes", "Elicia Hughes", "Alex Louis Armstrong", "Olivier Mira Armstrong", "Buccaneer", "Miles", "Grumman", "Basque Grand", "May Chang", "Ling Yao", "Lan Fan", "Fu", "Yoki", "Tim Marcoh", "Izumi Curtis", "Sig Curtis", "Selim Bradley", "Lust", "Gluttony", "Sloth", "Shou Tucker", "Nina Tucker", "Alexander", "Barry the Chopper", "Maria Ross", "Denny Brosh", "Solf J. Kimblee", "Isaac McDougal", "Frank Archer", "Zampano", "Jerso", "Darius", "Heinkel", "Rose Thomas", "Cornello", "Dominic LeCoulte", "Paninya", "Hakuro", "Raven", "Rebecca Catalina", "Xiao Mei", "Chris Mustang", "Black Hayate", "Berthold Hawkeye", "Martel", "Dolcetto", "Roa", "Bido", "Sheska", "Garfiel", "Den", "Mason", "Knox", "Kanama", "Urey Rockbell", "Sara Rockbell", "Ridel LeCoulte", "Satella LeCoulte", "Catherine Elle Armstrong", "Philip Gargantos Armstrong", "La Vérité", "Dante", "Lyra", "Russell Tringham", "Fletcher Tringham", "Noa", "Alfons Heiderich", "Dietlinde Eckhart", "Julia Crichton", "Melvin Voyager"], "chainsaw": ["Denji", "Power", "Aki Hayakawa", "Makima", "Kobeni Higashiyama", "Himeno", "Reze", "Katana Man", "Angel Devil", "Pochita", "Kishibe", "Hirokazu Arai", "Madoka", "Beam", "Violence Fiend", "Princi", "Démon du Futur", "Démon de la Malédiction", "Démon Renard", "Démon Fantôme", "Akane Sawatari", "Quanxi", "Cosmo", "Pingtsi", "Long", "Tsugihagi", "Santa Claus", "Tolka", "Aldo", "Asa Mitaka", "Yoru", "Nayuta", "Hirofumi Yoshida", "Fami", "Haruka Iseumi", "Seigi Akoku", "Nobana Higashiyama", "Barem Bridge", "Miri Sugo", "Whip Hybrid", "Spear Hybrid", "Fumiko Mifune", "Démon Chauve-souris", "Démon Sangsue", "Démon de l'Éternité", "Démon Tomate", "Démon Zombie", "Gun Devil", "Démon des Ténèbres", "Aging Devil", "Démon de la Chute", "Démon de la Justice", "Démon de l'Enfer", "Démon Poulet", "Michiko Tendo", "Yutaro Kurose", "Yuko", "Taiyo Hayakawa", "Nyako", "Démon Serpent", "Démon Pieuvre", "Démon Typhon"], "wakfu": ["Yugo", "Tristepin de Percedal", "Amalia Sheran Sharm", "Evangelyne", "Ruel Stroud", "Adamai", "Qilby", "Nox", "Oropo", "Goultard", "Az", "Grougaloragran", "Alibert", "Shinonome", "Phaeris", "Chibi", "Mina", "Glip", "Baltazar", "Echo", "Harebourg", "Ush Galesh", "Black Bump", "Rubilax", "Elely", "Flopin", "Pin", "Joris Jurgen", "Kerubim Crepin", "Atcham", "Remington Smisse", "Grany Smisse", "Maskemane", "Kabrok", "Miranda", "Armand Sheran Sharm", "Aurora", "Moon", "Ogrest", "Rushu", "Kriss la Krass", "Dathura", "Lou", "Toross Mordal", "Roi Sadida", "Otomaï", "Julith", "Jahash Jurgen", "Khan Karkass"], "demonslayer": ["Tanjiro Kamado", "Nezuko Kamado", "Zenitsu Agatsuma", "Inosuke Hashibira", "Giyu Tomioka", "Kyojuro Rengoku", "Shinobu Kocho", "Muzan Kibutsuji", "Kokushibo", "Akaza", "Kanao Tsuyuri", "Genya Shinazugawa", "Tengen Uzui", "Mitsuri Kanroji", "Muichiro Tokito", "Sanemi Shinazugawa", "Gyomei Himejima", "Obanai Iguro", "Kanae Kocho", "Kagaya Ubuyashiki", "Amane Ubuyashiki", "Sakonji Urokodaki", "Sabito", "Makomo", "Jigoro Kuwajima", "Shinjuro Rengoku", "Senjuro Rengoku", "Doma", "Hantengu", "Gyokko", "Gyutaro", "Daki", "Nakime", "Kaigaku", "Enmu", "Rui", "Kyogai", "Susamaru", "Yahaba", "Tamayo", "Yushiro", "Yoriichi Tsugikuni", "Murata", "Aoi Kanzaki", "Michikatsu Tsugikuni", "Hotaru Haganezuka", "Hand Demon", "Swamp Demon", "Kamanue", "Mukago", "Rokuro", "Wakuraba", "Kotetsu", "Tecchin Tecchikawahara", "Sumi Nakahara", "Kiyo Terauchi", "Naho Takada", "Tanjuro Kamado", "Kie Kamado", "Kanata Ubuyashiki", "Kiriya Ubuyashiki", "Koyuki", "Ruka Rengoku", "Tsutako Tomioka", "Goto", "Suma", "Makio", "Hinatsuru", "Takeo Kamado", "Hanako Kamado", "Shigeru Kamado", "Rokuta Kamado", "Kanata Kamado", "Sumihiko Kamado", "Yoshiteru Agatsuma", "Toko Agatsuma", "Saburo", "Nichika Ubuyashiki", "Hinaki Ubuyashiki", "Kuina Ubuyashiki", "Yuichiro Tokito", "Keizo", "Kiyoshi", "Teruko", "Shoichi", "Ozaki", "Uta", "Sumiyoshi", "Kotoha Hashibira", "Kazumi"], "pokemon": ["Bulbizarre", "Herbizarre", "Florizarre", "Salamèche", "Reptincel", "Dracaufeu", "Carapuce", "Carabaffe", "Tortank", "Chenipan", "Chrysacier", "Papilusion", "Aspicot", "Coconfort", "Dardargnan", "Roucool", "Roucoups", "Roucarnage", "Rattata", "Rattatac", "Piafabec", "Rapasdepic", "Abo", "Arbok", "Pikachu", "Raichu", "Sabelette", "Sablaireau", "Nidoran femelle", "Nidorina", "Nidoqueen", "Nidoran mâle", "Nidorino", "Nidoking", "Mélofée", "Mélodelfe", "Goupix", "Feunard", "Rondoudou", "Grodoudou", "Nosferapti", "Nosferalto", "Mystherbe", "Ortide", "Rafflesia", "Paras", "Parasect", "Mimitoss", "Aéromite", "Taupiqueur", "Triopikeur", "Miaouss", "Persian", "Psykokwak", "Akwakwak", "Férosinge", "Colossinge", "Caninos", "Arcanin", "Ptitard", "Têtarte", "Tartard", "Abra", "Kadabra", "Alakazam", "Machoc", "Machopeur", "Mackogneur", "Chétiflor", "Boustiflor", "Empiflor", "Tentacool", "Tentacruel", "Racaillou", "Gravalanch", "Grolem", "Ponyta", "Galopa", "Ramoloss", "Flagadoss", "Magnéti", "Magnéton", "Canarticho", "Doduo", "Dodrio", "Otaria", "Lamantine", "Tadmorv", "Grotadmorv", "Kokiyas", "Crustabri", "Fantominus", "Spectrum", "Ectoplasma", "Onix", "Soporifik", "Hypnomade", "Krabby", "Krabboss", "Voltorbe", "Électrode", "Noeunoeuf", "Noadkoko", "Osselait", "Ossatueur", "Kicklee", "Tygnon", "Excelangue", "Smogo", "Smogogo", "Rhinocorne", "Rhinoféros", "Leveinard", "Saquedeneu", "Kangourex", "Hypotrempe", "Hypocéan", "Poissirène", "Poissoroy", "Stari", "Staross", "M. Mime", "Insécateur", "Lippoutou", "Élektek", "Magmar", "Scarabrute", "Tauros", "Magicarpe", "Léviator", "Lokhlass", "Métamorph", "Évoli", "Aquali", "Voltali", "Pyroli", "Porygon", "Amonita", "Amonistar", "Kabuto", "Kabutops", "Ptéra", "Ronflex", "Artikodin", "Électhor", "Sulfura", "Minidraco", "Draco", "Dracolosse", "Mewtwo", "Mew", "Germignon", "Macronium", "Méganium", "Héricendre", "Feurisson", "Typhlosion", "Kaiminus", "Crocrodil", "Aligatueur", "Fouinette", "Fouinar", "Hoothoot", "Noarfang", "Coxy", "Coxyclaque", "Mimigal", "Migalos", "Nostenfer", "Loupio", "Lanturn", "Pichu", "Mélo", "Toudoudou", "Togepi", "Togetic", "Natu", "Xatu", "Wattouat", "Lainergie", "Pharamp", "Joliflor", "Marill", "Azumarill", "Simularbre", "Tarpaud", "Granivol", "Floravol", "Cotovol", "Capumain", "Tournegrin", "Héliatronc", "Yanma", "Axoloto", "Maraiste", "Mentali", "Noctali", "Cornèbre", "Roigada", "Feuforêve", "Zarbi", "Qulbutoké", "Girafarig", "Pomdepik", "Foretress", "Insolourdo", "Scorplane", "Steelix", "Snubbull", "Granbull", "Qwilfish", "Cizayox", "Caratroc", "Scarhino", "Farfuret", "Teddiursa", "Ursaring", "Limagma", "Volcaropod", "Marcacrin", "Cochignon", "Corayon", "Rémoraid", "Octillery", "Cadoizo", "Démanta", "Airmure", "Malosse", "Démolosse", "Hyporoi", "Phanpy", "Donphan", "Porygon2", "Cerfrousse", "Queulorior", "Debugant", "Kapoera", "Lippouti", "Élekid", "Magby", "Écrémeuh", "Leuphorie", "Raikou", "Entei", "Suicune", "Embrylex", "Ymphect", "Tyranocif", "Lugia", "Ho-Oh", "Celebi", "Arcko", "Massko", "Jungko", "Poussifeu", "Galifeu", "Braségali", "Gobou", "Flobio", "Laggron", "Medhyèna", "Grahyèna", "Zigzaton", "Linéon", "Chenipotte", "Armulys", "Charmillon", "Blindalys", "Papinox", "Nénupiot", "Lombre", "Ludicolo", "Grainipiot", "Pifeuil", "Tengalice", "Nirondelle", "Hélédelle", "Goélise", "Bekipan", "Tarsal", "Kirlia", "Gardevoir", "Arakdo", "Maskadra", "Balignon", "Chapignon", "Parecool", "Vigoroth", "Monaflèmit", "Ningale", "Ninjask", "Munja", "Chuchmur", "Ramboum", "Brouhabam", "Makuhita", "Hariyama", "Azurill", "Tarinor", "Skitty", "Delcatty", "Ténéfix", "Mysdibule", "Galekid", "Galegon", "Galeking", "Méditikka", "Charmina", "Dynavolt", "Élecsprint", "Posipi", "Négapi", "Muciole", "Lumivole", "Rosélia", "Gloupti", "Avaltout", "Carvanha", "Sharpedo", "Wailmer", "Wailord", "Chamallot", "Camérupt", "Chartor", "Spoink", "Groret", "Spinda", "Kraknoix", "Vibraninf", "Libégon", "Cacnea", "Cacturne", "Tylton", "Altaria", "Mangriff", "Séviper", "Séléroc", "Solaroc", "Barloche", "Barbicha", "Écrapince", "Colhomard", "Balbuto", "Kaorine", "Lilia", "Vacilys", "Anorith", "Armaldo", "Barpau", "Milobellus", "Morphéo", "Kecleon", "Polichombr", "Branette", "Skelénox", "Téraclope", "Tropius", "Éoko", "Absol", "Okéoké", "Stalgamin", "Oniglali", "Obalie", "Phogleur", "Kaimorse", "Coquiperl", "Serpang", "Rosabyss", "Relicanth", "Lovdisc", "Draby", "Drackhaus", "Drattak", "Terhal", "Métang", "Métalosse", "Regirock", "Regice", "Registeel", "Latias", "Latios", "Kyogre", "Groudon", "Rayquaza", "Jirachi", "Deoxys", "Tortipouss", "Boskara", "Torterra", "Ouisticram", "Chimpenfeu", "Simiabraz", "Tiplouf", "Prinplouf", "Pingoléon", "Étourmi", "Étourvol", "Étouraptor", "Keunotor", "Castorno", "Crikzik", "Mélokrik", "Lixy", "Luxio", "Luxray", "Rozbouton", "Roserade", "Kranidos", "Charkos", "Dinoclier", "Bastiodon", "Cheniti", "Cheniselle", "Papilord", "Apitrini", "Apireine", "Pachirisu", "Mustébouée", "Mustéflott", "Ceribou", "Ceriflor", "Sancoki", "Tritosor", "Capidextre", "Baudrive", "Grodrive", "Laporeille", "Lockpin", "Magirêve", "Corboss", "Chaglam", "Chaffreux", "Korillon", "Moufouette", "Moufflair", "Archéomire", "Archéodong", "Manzaï", "Mime Jr.", "Ptiravi", "Pijako", "Spiritomb", "Griknot", "Carmache", "Carchacrok", "Goinfrex", "Riolu", "Lucario", "Hippopotas", "Hippodocus", "Rapion", "Drascore", "Cradopaud", "Coatox", "Vortente", "Écayon", "Luminéon", "Babimanta", "Blizzi", "Blizzaroi", "Dimoret", "Magnézone", "Coudlangue", "Rhinastoc", "Bouldeneu", "Élekable", "Maganon", "Togekiss", "Yanmega", "Phyllali", "Givrali", "Scorvol", "Mammochon", "Porygon-Z", "Gallame", "Tarinorme", "Noctunoir", "Momartik", "Motisma", "Créhelf", "Créfollet", "Créfadet", "Dialga", "Palkia", "Heatran", "Regigigas", "Giratina", "Cresselia", "Phione", "Manaphy", "Darkrai", "Shaymin", "Arceus", "Victini", "Vipélierre", "Lianaja", "Majaspic", "Gruikui", "Grotichon", "Roitiflam", "Moustillon", "Mateloutre", "Clamiral", "Ratentif", "Miradar", "Ponchiot", "Ponchien", "Mastouffe", "Chacripan", "Léopardus", "Feuillajou", "Feuiloutan", "Flamajou", "Flamoutan", "Flotajou", "Flotoutan", "Munna", "Mushana", "Poichigeon", "Colombeau", "Déflaisan", "Zébibron", "Zéblitz", "Nodulithe", "Géolithe", "Gigalithe", "Chovsourir", "Rhinolove", "Rototaupe", "Minotaupe", "Nanméouïe", "Charpenti", "Ouvrifier", "Bétochef", "Tritonde", "Batracné", "Crapustule", "Judokrak", "Karaclée", "Larveyette", "Couverdure", "Manternel", "Venipatte", "Scobolide", "Brutapode", "Doudouvet", "Farfaduvet", "Chlorobule", "Fragilady", "Bargantua", "Mascaïman", "Escroco", "Crocorible", "Darumarond", "Darumacho", "Maracachi", "Crabicoque", "Crabaraque", "Baggiguane", "Baggaïd", "Cryptéro", "Tutafeh", "Tutankafer", "Carapagos", "Mégapagos", "Arkéapti", "Aéroptéryx", "Miamiasme", "Miasmax", "Zorua", "Zoroark", "Chinchidou", "Pashmilla", "Scrutella", "Mesmérella", "Sidérella", "Nucléos", "Méios", "Symbios", "Couaneton", "Lakmécygne", "Sorbébé", "Sorboul", "Sorbouboul", "Vivaldaim", "Haydaim", "Emolga", "Carabing", "Lançargot", "Trompignon", "Gaulet", "Viskuse", "Moyade", "Mamanbo", "Statitik", "Mygavolt", "Grindur", "Noacier", "Tic", "Clic", "Cliticlic", "Anchwatt", "Lampéroie", "Ohmassacre", "Lewsor", "Neitram", "Funécire", "Mélancolux", "Lugulabre", "Coupenotte", "Incisache", "Tranchodon", "Polarhume", "Polagriffe", "Hexagel", "Escargaume", "Limaspeed", "Limonde", "Kungfouine", "Shaofouine", "Drakkarmin", "Gringolem", "Golemastoc", "Scalpion", "Scalproie", "Frison", "Furaiglon", "Gueriaigle", "Vostourno", "Vaututrice", "Aflamanoir", "Fermite", "Solochi", "Diamat", "Trioxhydre", "Pyronille", "Pyrax", "Cobaltium", "Terrakium", "Viridium", "Boréas", "Fulguris", "Reshiram", "Zekrom", "Démétéros", "Kyurem", "Keldeo", "Meloetta", "Genesect", "Marisson", "Boguérisse", "Blindépique", "Feunnec", "Roussil", "Goupelin", "Grenousse", "Croâporal", "Amphinobi", "Sapereau", "Excavarenne", "Passerouge", "Braisillon", "Flambusard", "Lépidonille", "Pérégrain", "Prismillon", "Hélionceau", "Némélios", "Flabébé", "Floette", "Florges", "Cabriolaine", "Chevroum", "Pandespiègle", "Pandarbare", "Couafarel", "Psystigri", "Mistigrix", "Monorpale", "Dimoclès", "Exagide", "Fluvetin", "Cocotine", "Sucroquin", "Cupcanaille", "Sepiatop", "Sepiatroce", "Opermine", "Golgopathe", "Venalgue", "Kravarech", "Flingouste", "Gamblast", "Galvaran", "Iguolta", "Ptyranidur", "Rexillius", "Amagara", "Dragmara", "Nymphali", "Brutalibré", "Dedenne", "Strassie", "Mucuscule", "Colimucus", "Muplodocus", "Trousselin", "Brocélôme", "Desséliande", "Pitrouille", "Banshitrouye", "Grelaçon", "Séracrawl", "Sonistrelle", "Bruyverne", "Xerneas", "Yveltal", "Zygarde", "Diancie", "Hoopa", "Volcanion", "Brindibou", "Efflèche", "Archéduc", "Flamiaou", "Matoufeu", "Félinferno", "Otaquin", "Otarlette", "Oratoria", "Picassaut", "Piclairon", "Bazoucan", "Manglouton", "Argouste", "Larvibule", "Chrysapile", "Lucanon", "Crabagarre", "Crabominable", "Plumeline", "Bombydou", "Rubombelle", "Rocabot", "Lougaroc", "Froussardine", "Vorastérie", "Prédastérie", "Tiboudet", "Bourrinos", "Araqua", "Tarenbulle", "Mimantis", "Floramantis", "Spododo", "Lampignon", "Tritox", "Malamandre", "Nounourson", "Chelours", "Croquine", "Candine", "Sucreine", "Guérilande", "Gouroutan", "Quartermac", "Sovkipou", "Sarmuraï", "Bacabouh", "Trépassable", "Concombaffe", "Type:0", "Silvallié", "Météno", "Dodoala", "Boumata", "Togedemaru", "Mimiqui", "Denticrisse", "Draïeul", "Sinistrail", "Bébécaille", "Écaïd", "Ékaïser", "Tokorico", "Tokopiyon", "Tokotoro", "Tokopisco", "Cosmog", "Cosmovum", "Solgaleo", "Lunala", "Zéroïd", "Mouscoto", "Cancrelove", "Câblifère", "Bamboiselle", "Katagami", "Engloutyran", "Necrozma", "Magearna", "Marshadow", "Vémini", "Mandrillon", "Ama-Ama", "Pierroteknik", "Zeraora", "Meltan", "Melmetal", "Ouistempo", "Badabouin", "Gorythmic", "Flambino", "Lapyro", "Pyrobut", "Larméléon", "Arrozard", "Lézargus", "Rongourmand", "Rongrigou", "Minisange", "Bleuseille", "Corvaillus", "Larvadar", "Coléodôme", "Astronelle", "Goupilou", "Roublenard", "Tournicoton", "Blancoton", "Moumouton", "Moumouflon", "Khélocrok", "Torgamord", "Voltoutou", "Fulgudog", "Charbi", "Wagomine", "Monthracite", "Verpom", "Pomdrapi", "Dratatin", "Dunaja", "Dunaconda", "Nigosier", "Embrochet", "Hastacuda", "Toxizap", "Salarsen", "Grillepattes", "Scolocendre", "Poulpaf", "Krakos", "Théffroi", "Polthégeist", "Bibichut", "Chapotus", "Sorcilence", "Grimalin", "Fourbelin", "Angoliath", "Ixon", "Berserkatt", "Corayôme", "Palarticho", "M. Glaquette", "Tutétékri", "Crèmy", "Charmilly", "Hexadron", "Wattapik", "Frissonille", "Beldeneige", "Dolman", "Bekaglaçon", "Wimessir", "Morpeko", "Charibari", "Pachyradjah", "Galvagon", "Galvagla", "Hydragon", "Hydragla", "Duralugon", "Fantyrm", "Dispareptil", "Lanssorien", "Zacian", "Zamazenta", "Éthernatos", "Wushours", "Shifours", "Zarude", "Regieleki", "Regidrago", "Blizzeval", "Spectreval", "Sylveroy", "Cerbyllin", "Hachécateur", "Ursaking", "Paragruel", "Farfurex", "Qwilpik", "Amovénus", "Poussacha", "Matourgeon", "Miascarade", "Chochodile", "Crocogril", "Flâmigator", "Coiffeton", "Canarbello", "Palmaval", "Gourmelet", "Fragroin", "Tissenboule", "Filentrappe", "Lilliterelle", "Gambex", "Pohm", "Pohmotte", "Pohmarmotte", "Compagnol", "Famignol", "Pâtachiot", "Briochien", "Olivini", "Olivado", "Arboliva", "Tapatoès", "Selutin", "Amassel", "Gigansel", "Charbambin", "Carmadura", "Malvalame", "Têtampoule", "Ampibidou", "Zapétrel", "Fulgulairo", "Grondogue", "Dogrino", "Gribouraigne", "Tag-Tag", "Virovent", "Virevorreur", "Terracool", "Terracruel", "Craparoi", "Pimito", "Scovilain", "Léboulérou", "Bérasca", "Flotillon", "Cléopsytra", "Forgerette", "Forgella", "Forgelina", "Taupikeau", "Triopikeau", "Lestombaile", "Dofin", "Superdofin", "Vrombi", "Vrombotor", "Motorizard", "Ferdeter", "Germéclat", "Floréclat", "Toutombe", "Tomberro", "Flamenroule", "Piétacé", "Balbalèze", "Délestin", "Oyacata", "Nigirigon", "Courrousinge", "Terraiste", "Farigiraf", "Deusolourdo", "Scalpereur", "Fort-Ivoire", "Hurle-Queue", "Fongus-Furie", "Flotte-Mèche", "Rampe-Ailes", "Pelage-Sablé", "Roue-de-Fer", "Hotte-de-Fer", "Paume-de-Fer", "Têtes-de-Fer", "Mite-de-Fer", "Épine-de-Fer", "Frigodo", "Cryodo", "Glaivodo", "Mordudor", "Gromago", "Chongjian", "Baojian", "Dinglu", "Yuyu", "Rugit-Lune", "Garde-de-Fer", "Koraidon", "Miraidon", "Serpente-Eau", "Vert-de-Fer", "Pomdramour", "Poltchageist", "Théffroyable", "Félicanis", "Fortusimia", "Favianos", "Ogerpon", "Pondralugon", "Pomdorochi", "Feu-Perçant", "Ire-Foudre", "Roc-de-Fer", "Chef-de-Fer", "Terapagos", "Pêchaminus", "Sacha", "Ondine", "Pierre", "Régis", "Professeur Chen", "Jessie", "James", "Giovanni", "Tracey", "Flora", "Max", "Aurore", "Iris", "Rachid", "Serena", "Lem", "Clem", "Lilie", "Kiawe", "Néphie", "Barbara", "Chrys", "Goh", "Tarak", "Cynthia", "Pierre Rochard", "Peter", "Dianthéa", "Goyah", "Roy", "Paul", "Alain", "Major Bob", "Érika", "Koga", "Morgane", "Auguste", "Olga", "Aldo", "Agatha", "Albert", "Hector", "Blanche", "Mortimer", "Chuck", "Jasmine", "Frédo", "Sandra", "Roxanne", "Bastien", "Voltère", "Adriane", "Norman", "Alizée", "Lévy", "Tatia", "Marc", "Juan", "Pierrick", "Flo", "Mélina", "Lovis", "Kiméra", "Charles", "Gladys", "Tanguy", "Hélio", "Ghetis", "N", "Lysandre", "Guzma", "Elsa-Mina", "Arthur", "Professeur Orme", "Professeur Seko", "Professeur Sorbier", "Professeure Keteleeria", "Professeur Platane", "Professeur Euphorbe", "Professeure Magnolia", "Infirmière Joy", "Agent Jenny"], "dragonball": ["Sangoku", "Vegeta", "Sangohan", "Piccolo", "Freezer", "Cell", "Majin Boo", "Beerus", "Jiren", "Broly", "Gogeta", "Vegeto", "Sangoten", "Trunks", "Krilin", "Ten Shin Han", "Chaozu", "Yamcha", "Tortue Géniale", "Bulma", "Chi-Chi", "Videl", "Pan", "Hercule", "Oob", "Baddack", "Gine", "Raditz", "Nappa", "Roi Vegeta", "Tarble", "Roi Cold", "Cooler", "Zarbon", "Dodoria", "Capitaine Ginyu", "Jeece", "Barta", "Recoome", "Guldo", "C-16", "C-17", "C-18", "C-19", "Docteur Gero", "Babidi", "Dabura", "Kaioshin", "Kibito", "Whis", "Champa", "Vados", "Zeno", "Grand Prêtre", "Hit", "Cabba", "Caulifla", "Kale", "Kefla", "Frost", "Botamo", "Magetta", "Toppo", "Dyspo", "Zamasu", "Paragus", "Gotenks", "Moro", "Merus", "Granolah", "Gas", "Elec", "Dende", "Janemba", "Nail", "Tout-Puissant", "Mr. Popo", "Maître Karin", "Yajirobe", "Lunch", "Oolong", "Plume", "Maître Kaio", "Grand Chef", "C-13", "C-21", "Baby", "Li Shenron", "Shenron", "Porunga", "Bojack", "Tullece", "Lord Slug", "Garlic Junior", "Pilaf", "Mai", "Shu", "Tao Pai Pai", "General Blue", "Commander Red", "Upa", "Jaco", "Sorbet", "Tagoma", "Ribrianne", "Belmod", "Marcarita", "Quitela", "Heles", "Obuni", "Hildegarn", "Towa", "Mira", "Fu", "Cumber", "Hearts", "Zangya", "Su Shenron", "Kakunsa", "Hatchiyack", "Gamma 1", "Gamma 2", "Docteur Hedo", "Magenta", "Carmine", "Bra", "Marron", "Cus", "Grand-père Sangohan", "Gyumao", "Satan Petit-Cœur", "Tambourine", "Cymbal", "Drum", "Piano", "Baba", "Arale Norimaki", "Senbei Norimaki", "Bora", "Suno", "Hasky", "Murasaki", "Colonel Silver", "Général White", "Colonel Violet", "Black", "C-8", "Namu", "Giran", "Bacterian", "Ranfan", "Roi Chappa", "Tsuru Sennin", "Docteur Brief", "Madame Brief", "Tights", "Bubbles", "Gregory", "Enma", "Princesse Serpent", "Grand Kaio", "Kaio du Sud", "Kaio de l'Est", "Kaio de l'Ouest", "Vieux Kaioshin", "Tora", "Seripa", "Toteppo", "Panbukin", "Kiwi", "Appule", "Moori", "Cargo", "Tsuno", "Sharpner", "Erasa", "Spopovich", "Yamu", "Pui Pui", "Yakon", "Bibidi", "C-14", "C-15", "Docteur Willow", "Docteur Kochin", "Kishime", "Misokatsun", "Ebifurya", "Ginger", "Nicky", "Sansho", "Spice", "Vinegar", "Angila", "Medamatcha", "Amond", "Daiz", "Cacao", "Lakasei", "Rasin", "Salza", "Dore", "Neiz", "Bujin", "Bido", "Gokua", "Tapion", "Minotia", "Hoi", "Shisami", "Cheelai", "Lemo", "Berryblue", "Kikono", "Gil", "Docteur Mu", "Général Rild", "Ryan Shenron", "San Shenron", "Uu Shenron", "Rō Shenron", "Chi Shenron", "Sangoku Jr", "Vegeta Jr", "Yurin", "Monaka", "Gowasu", "Iwne", "Mosco", "Arack", "Liquiir", "Sidra", "Rumsshi", "Geene", "Awamo", "Sour", "Campari", "Cukatail", "Cognac", "Korn", "Mojito", "Martinu", "Anato", "Pell", "Ea", "Kuru", "Fuwa", "Roh", "Khai", "Basil", "Lavender", "Bergamo", "Kahseral", "Cocotte", "Zoiray", "Rozie", "Paparoni", "Anilaza", "Ganos", "Caway", "Dercori", "Majora", "Saonel", "Pirina", "Murichim", "Monite", "Oil", "Macki", "Seven-Three", "Saganbo", "Glorio", "Roi Gomah", "Degesu", "Panzy", "Neva", "Docteur Arinsu", "Majin Duu", "Majin Kuu"], "hellsparadise": ["Gabimaru", "Yamada Asaemon Sagiri", "Yuzuriha", "Aza Chobei", "Aza Toma", "Yamada Asaemon Shion", "Yamada Asaemon Tenza", "Tamiya Gantetsusai", "Rien", "Mei", "Yamada Asaemon Fuchi", "Nurugai", "Yamada Asaemon Senta", "Yamada Asaemon Eizen", "Yamada Asaemon Genji", "Yamada Asaemon Kisho", "Yamada Asaemon Jikka", "Yamada Asaemon Shugen", "Isuzu", "Kiyomaru", "Zhu Jin", "Mu Dan", "Ju Fa", "Tao Fa", "Gui Fa", "Hoko", "Yui", "Lan Ban", "Xu Fu", "Tokugawa Ienari", "Rokurota"], "gachiakuta": ["Rudo Surebrec", "Enjin", "Zanka Nijiku", "Riyo Reaper", "Jabber Wonger", "Zodyl Typhon", "Tamsy Caines", "Semiu Grier", "Regto Surebrec", "Amo Empool", "Delmon", "Bro", "Dear", "Guita", "Gris", "Follo", "Tomme", "Corvus", "August", "Eishia", "Cthoni", "Noerde", "Fu", "Bundus", "Chiwa", "Alice", "Remlin"], "haikyuu": ["Shoyo Hinata", "Tobio Kageyama", "Kei Tsukishima", "Yu Nishinoya", "Toru Oikawa", "Wakatoshi Ushijima", "Kotaro Bokuto", "Kenma Kozume", "Tetsuro Kuroo", "Atsumu Miya", "Osamu Miya", "Kiyoomi Sakusa", "Tadashi Yamaguchi", "Daichi Sawamura", "Koshi Sugawara", "Asahi Azumane", "Ryunosuke Tanaka", "Chikara Ennoshita", "Hisashi Kinoshita", "Kazuhito Narita", "Kiyoko Shimizu", "Hitoka Yachi", "Ittetsu Takeda", "Keishin Ukai", "Hajime Iwaizumi", "Issei Matsukawa", "Takahiro Hanamaki", "Shinji Watari", "Shigeru Yahaba", "Yutaro Kindaichi", "Akira Kunimi", "Morisuke Yaku", "Nobuyuki Kai", "Taketora Yamamoto", "Shohei Fukunaga", "Lev Haiba", "So Inuoka", "Tamahiko Teshiro", "Keiji Akaashi", "Akinori Konoha", "Satori Tendo", "Tsutomu Goshiki", "Kenjiro Shirabu", "Eita Semi", "Reon Ohira", "Taichi Kawanishi", "Shinsuke Kita", "Rintaro Suna", "Aran Ojiro", "Korai Hoshiumi", "Sachiro Hirugami", "Motoya Komori", "Takanobu Aone", "Kenji Futakuchi", "Kanji Koganegawa", "Yuji Terushima", "Suguru Daisho", "Kentaro Kyotani", "Yusuke Takinoue", "Makoto Shimada", "Saeko Tanaka", "Akiteru Tsukishima", "Natsu Hinata", "Ikkei Ukai", "Yasufumi Nekomata", "Yuki Shibayama", "Tatsuki Washio", "Haruki Komi", "Wataru Onaga", "Yamato Sarukui", "Hayato Yamagata", "Tanji Washijo", "Hitoshi Ginjima", "Michinari Akagi", "Ren Omimi", "Gao Hakuba", "Kaname Moniwa", "Yasushi Kamasaki", "Takeru Nakashima", "Yudai Hyakuzawa", "Yui Michimiya", "Madoka Yachi", "Miwa Kageyama", "Kazuyo Kageyama", "Yukitaka Izumi", "Koji Sekimukai", "Nobuteru Irihata", "Sadayuki Mizoguchi", "Takeru Oikawa", "Kosuke Sakunami", "Yutaka Obara", "Takehito Sasaya", "Takuro Oiwake", "Soichiro Saito", "Manabu Naoi", "Alisa Haiba", "Akane Yamamoto", "Kaori Suzumeda", "Yukie Shirofuku", "Wakatsu Kiryu", "Tenma Udai", "Shugo Meian", "Shion Inunaki", "Thomas Barnes", "Nicolas Romero", "Pedro Nascimento", "José Blanco"], "jjk": ["Yuji Itadori", "Megumi Fushiguro", "Nobara Kugisaki", "Satoru Gojo", "Yuta Okkotsu", "Maki Zenin", "Suguru Geto", "Ryomen Sukuna", "Mahito", "Kento Nanami", "Toji Fushiguro", "Aoi Todo", "Toge Inumaki", "Panda", "Masamichi Yaga", "Shoko Ieiri", "Kiyotaka Ijichi", "Atsuya Kusakabe", "Mai Zenin", "Kasumi Miwa", "Mechamaru", "Noritoshi Kamo", "Momo Nishimiya", "Utahime Iori", "Kenjaku", "Riko Amanai", "Jogo", "Hanami", "Dagon", "Choso", "Eso", "Kechizu", "Uraume", "Mei Mei", "Ui Ui", "Yuki Tsukumo", "Tengen", "Naobito Zenin", "Naoya Zenin", "Ogi Zenin", "Jinichi Zenin", "Kinji Hakari", "Hiromi Higuruma", "Hajime Kashimo", "Reggie Star", "Yoshinobu Gakuganji", "Misato Kuroi", "Yu Haibara", "Kirara Hoshi", "Fumihiko Takaba", "Remi", "Rin Amai", "Iori Hazenoki", "Chizuru Hari", "Ryu Ishigori", "Takako Uro", "Dhruv Lakdawalla", "Kurourushi", "Hana Kurusu", "Charles Bernard", "Yorozu", "Rika Orimoto", "Tsumiki Fushiguro", "Takuma Ino", "Arata Nitta", "Akari Nitta", "Nanako Hasaba", "Mimiko Hasaba", "Miguel", "Larue", "Juzo Kumiya", "Haruta Shigemo", "Granny Ogami", "Jiro Awasaka", "Wasuke Itadori", "Jin Itadori", "Smallpox Deity", "Shiu Kong", "Junpei Yoshino", "Nagi Yoshino", "Kaori Itadori", "Chojuro Zenin", "Ranta Zenin", "Miyo Rokujushi", "Sasaki", "Iguchi"], "jojo": ["Jonathan Joestar", "Joseph Joestar", "Jotaro Kujo", "Josuke Higashikata", "Giorno Giovanna", "Jolyne Cujoh", "Johnny Joestar", "Josuke Higashikata Gappy", "Dio Brando", "Robert E. O. Speedwagon", "Will A. Zeppeli", "Erina Pendleton", "Dire", "Straizo", "Caesar Zeppeli", "Lisa Lisa", "Rudol von Stroheim", "Santana", "Wamuu", "Esidisi", "Kars", "Suzi Q", "Muhammad Avdol", "Noriaki Kakyoin", "Jean Pierre Polnareff", "Iggy", "Hol Horse", "Enya", "Vanilla Ice", "Oingo", "Boingo", "Mariah", "Pet Shop", "Koichi Hirose", "Okuyasu Nijimura", "Rohan Kishibe", "Yukako Yamagishi", "Shigekiyo Yangu", "Tonio Trussardi", "Akira Otoishi", "Reimi Sugimoto", "Yoshikage Kira", "Bruno Bucciarati", "Guido Mista", "Narancia Ghirga", "Leone Abbacchio", "Pannacotta Fugo", "Trish Una", "Diavolo", "Vinegar Doppio", "Risotto Nero", "Prosciutto", "Pesci", "Ghiaccio", "Melone", "Formaggio", "Illuso", "Ermes Costello", "Foo Fighters", "Weather Report", "Narciso Anasui", "Emporio Alnino", "Enrico Pucci", "Gyro Zeppeli", "Diego Brando", "Funny Valentine", "Hot Pants", "Lucy Steel", "George Joestar", "Wang Chan", "Poco", "Tarkus", "Bruford", "Dario Brando", "Messina", "Loggins", "Holy Kujo", "N'Doul", "Gray Fly", "Devo", "Steely Dan", "Rubber Soul", "J. Geil", "Mannish Boy", "Cameo", "Alessi", "Daniel J. D'Arby", "Telence T. D'Arby", "Kenny G", "Arabia Fats", "Nena", "ZZ", "Midler", "Tomoko Higashikata", "Toshikazu Hazamada", "Yuya Fungami", "Mikitaka Hazekura", "Keicho Nijimura", "Kosaku Kawajiri", "Hayato Kawajiri", "Shinobu Kawajiri", "Terunosuke Miyamoto", "Aya Tsuji", "Tamami Kobayashi", "Ken Oyanagi", "Toyohiro Kanedaichi", "Yoshihiro Kira", "Squalo", "Tiziano", "Cioccolata", "Secco", "Carne", "Sale", "Zucchero", "Polpo", "Luca", "Gwess", "Johngalli A", "Thunder McQueen", "Miraschon", "Sports Maxx", "Lang Rangler", "Viviano Westwood", "Kenzou", "D an G", "Guccio", "Ungaro", "Rikiel", "Versus", "Sandman", "Mountain Tim", "Pocoloco", "Blackmore", "Ringo Roadagain", "Wekapipo", "Magent Magent", "Axl RO", "Dr. Ferdinand", "Oyecomova", "Mike O", "Yasuho Hirose", "Joshu Higashikata", "Norisuke Higashikata IV", "Tsurugi Higashikata", "Kei Nijimura", "Toru", "Jobin Higashikata", "Rai Mamezuku", "Hato Higashikata", "Tonpetty", "Doobie", "Jack the Ripper", "George Joestar II", "Smokey Brown", "Forever", "Nukesaku", "Chaka", "Wilson Phillips", "Anjuro Katagiri", "Ryohei Higashikata", "Shizuka Joestar", "Pericolo", "Sorbet", "Gelato", "Donatella Una", "Scolippi", "Coco Jumbo", "Romeo", "Gloria", "Perla Pucci", "Miu Miu", "Stephen Steel", "Pork Pie Hat Kid", "Andre Boom Boom", "Benjamin Boom Boom", "L. A. Boom Boom", "Sugar Mountain", "Scarlet Valentine", "Gregorio Zeppeli", "Nicholas Joestar", "Josefumi Kujo", "Holy Kira", "Daiya Higashikata", "Mitsuba Higashikata", "Kaato Higashikata", "Karera Sakunami", "Dolomite", "Poor Tom", "Yotsuyu Yagiyama", "Aisho Dainenjiyama", "Tamaki Damo", "Urban Guerrilla", "Ojiro Sasame", "Wu Tomoki", "Jodio Joestar", "Dragona Joestar", "Paco Laburantes", "Usagi Alohaoe", "Meryl Mei Qi", "Barbara Ann Joestar"], "tensura": ["Rimuru Tempest", "Veldora Tempest", "Milim Nava", "Diablo", "Benimaru", "Shion", "Guy Crimson", "Luminous Valentine", "Hinata Sakaguchi", "Gobta", "Shizue Izawa", "Shuna", "Souei", "Hakuro", "Kurobe", "Rigurd", "Rigur", "Ranga", "Geld", "Gabiru", "Testarossa", "Carrera", "Ultima", "Zegion", "Apito", "Kumara", "Adalmann", "Ramiris", "Leon Cromwell", "Dagruel", "Dino", "Frey", "Carrion", "Clayman", "Chloe Aubert", "Yuuki Kagurazaka", "Kagali", "Laplace", "Tear", "Footman", "Velgrynd", "Velzard", "Treyni", "Beretta", "Kaijin", "Gazel Dwargo", "Gobzo", "Haruna", "Momiji", "Alvis", "Sufia", "Phobio", "Elmesia El-Ru Sarion", "Eren Grimwald", "Kabal", "Gido", "Fuze", "Vester", "Myulan", "Grucius", "Yohm", "Razen", "Shogo Taguchi", "Kyoya Tachibana", "Kirara Mizutani", "Masayuki Honjo", "Veldanava", "Feldway", "Vega", "Hiiro", "Charybdis", "Albert", "Wenty", "Gadora", "Tatsuya Kondo", "Rudra", "Garm", "Dold", "Mildo", "Soka", "Toka", "Saika", "Nanso", "Hokuso", "Abiru", "Venom", "Agera", "Dolph", "Jaine", "Kenya Misaki", "Ryota Sekiguchi", "Gale Gibson", "Alice Rondo", "Shinji Tanimura", "Mark Lauren", "Edmaris", "Folgen", "Reyhiem", "Arnaud Baumann", "Nicolaus Speltus", "Roy Valentine", "Middray", "Damrada", "Minitz", "Calgurio", "Trya", "Doryada", "Ifrit", "Mjöllmile", "Saare", "Grigori", "Gelmud"], "opm": ["Saitama", "Genos", "Tatsumaki", "Bang", "King", "Fubuki", "Garou", "Boros", "Mumen Rider", "Flashy Flash", "Bomb", "Blast", "Atomic Samurai", "Child Emperor", "Metal Knight", "Zombieman", "Drive Knight", "Pig God", "Superalloy Darkshine", "Watchdog Man", "Tanktop Master", "Metal Bat", "Puri-Puri Prisoner", "Amai Mask", "Iaian", "Okamaitachi", "Bushidrill", "Spring Mustachio", "Golden Ball", "Tanktop Tiger", "Tanktop Black Hole", "Speed-o'-Sound Sonic", "Suiryu", "Melzargard", "Geryuganshoop", "Groribas", "Orochi", "Psykos", "Gyoro Gyoro", "Black Sperm", "Homeless Emperor", "Elder Centipede", "Gouketsu", "Fuhrer Ugly", "Nyan", "Phoenix Man", "Deep Sea King", "Vaccine Man", "Carnage Kabuto", "Mosquito Girl", "Beast King", "Armored Gorilla", "Dr. Genus", "Charanko", "Snek", "Stinger", "Lightning Max", "Death Gatling", "Smile Man", "Blue Fire", "Sansetsukon Lily", "Mountain Ape", "Eyelashes", "Glasses", "Sekingar", "Tareo", "Awakened Cockroach", "Subterranean King", "Hammerhead", "Marugori", "Bakuzan", "Evil Natural Water", "Do-S", "Overgrown Rover", "Royal Ripper", "Waganma", "Sitch", "Madame Shibabawa", "Suiko", "Heavy Tank Fundoshi", "Jet Nice Guy", "Captain Mizuki", "Chain'n'Toad", "Darkness Blade", "Zenko", "Crablante", "Gale Wind", "Hellfire Flame", "Dr. Kuseno", "Agoni", "Narinki"], "sao": ["Kirito", "Asuna Yuuki", "Sinon", "Leafa", "Klein", "Agil", "Yuuki Konno", "Alice Zuberg", "Eugeo", "Heathcliff", "Yui", "Silica", "Lisbeth", "Sachi", "Argo", "Diavel", "Kibaou", "Oberon", "Death Gun", "XaXa", "Vassago Casals", "Selka Zuberg", "Ronie Arabel", "Tiese Shtolienen", "Cardinal", "Administrator", "Chudelkin", "Bercouli Synthesis One", "Fanatio Synthesis Two", "Gabriel Miller", "Deusolbert Synthesis Seven", "Sakuya", "Alicia Rue", "Recon", "Sheyta", "Iskahn", "Llenn", "Pitohui", "Fukaziroh", "M", "Eiji", "Yuna", "Kuradeel", "Godfree", "Thinker", "Yulier", "Johnny Black", "Grimlock", "Schmitt", "Yolko", "Caynz", "Rosalia", "Kizmel", "Seijirou Kikuoka", "Rinko Koujiro", "Siune", "Jun", "Talken", "Tecchi", "Nori", "Raios Antinous", "Humbert Zizek", "Sortiliena Serlut", "Golgorosso Ballto", "Renly Synthesis Twenty-Seven", "Eldrie Synthesis Thirty-One", "Dee Ai Ell", "Lilpilin", "Shasta", "Garitta", "Medina Orthinanos", "Eugene", "Mortimer", "Kyouji Shinkawa", "Keita", "Tetsuo", "Sasamaru", "Ducker", "Lind", "Morte", "Coper", "Nezha", "Kobatz", "Nishida", "Sasha", "Griselda", "Kyouko Yuuki", "Shouzou Yuuki", "Kouichirou Yuuki", "Midori Kirigaya", "Minetaka Kirigaya", "Aki Natsuki", "Kurahashi", "Ai Konno", "Tetsuhiro Shigemura", "Takeru Higa", "Frenica Cesky", "Azurica", "Azalia", "Gasfut Zuberg", "Jink", "Sadore", "Charlotte", "Fizel", "Linel", "Dampe", "Critter", "Sigurd", "Yamikaze", "Behemoth", "Dyne", "Ginrou", "Eva", "Tanya", "Anna", "Rosa", "Sophie", "Tohma", "Clarence", "Shirley", "Mito", "Tilnel"], "tokyoghoul": ["Ken Kaneki", "Touka Kirishima", "Kishou Arima", "Juuzou Suzuya", "Eto Yoshimura", "Shu Tsukiyama", "Hinami Fueguchi", "Ayato Kirishima", "Kotaro Amon", "Nishiki Nishio", "Rize Kamishiro", "Hideyoshi Nagachika", "Kimi Nishino", "Yoshimura", "Renji Yomo", "Uta", "Itori", "Roma Hoito", "Kaya Irimi", "Enji Koma", "Akira Mado", "Kureo Mado", "Seidou Takizawa", "Yukinori Shinohara", "Iwao Kuroiwa", "Koori Ui", "Naki", "Tatara", "Donato Porpora", "Kurona Yasuhisa", "Take Hirako", "Mougan Tanakamaru", "Yakumo Oomori", "Noro", "Shachi", "Kuki Urie", "Ginshi Shirazu", "Tooru Mutsuki", "Saiko Yonebayashi", "Shinsanpei Aura", "Hairu Ihei", "Nimura Furuta", "Kanae von Rosewald", "Nashiro Yasuhisa", "Kazuichi Banjou", "Ichimi", "Jiro", "Nico", "Houji Kasuka", "Itou Kuramoto", "Akihiro Kanou", "Tokuyuki Marude", "Tsuneyoshi Washuu", "Matsuri Washuu", "Yoshitoki Washuu", "Hanbee Abara", "Arata Kirishima", "Chie Hori", "Kiyoko Aura", "Hsiao", "Shiki Kijima", "Big Madam", "Mirumo Tsukiyama", "Matsumae", "Ryouko Fueguchi", "Asaki Fueguchi", "Ippei Kusaba", "Chuuzen Hachikawa", "Takeomi Kuroiwa", "Yoriko Kosaka", "Yasunari Nakajima", "Miza", "Nutcracker", "Torso", "Shikorae", "Mizurou Tamaki", "Sante", "Ukina"], "tokyorevengers": ["Takemichi Hanagaki", "Manjiro Sano", "Ken Ryuguji", "Keisuke Baji", "Chifuyu Matsuno", "Takashi Mitsuya", "Kazutora Hanemiya", "Tetta Kisaki", "Izana Kurokawa", "Taiju Shiba", "Haruki Hayashida", "Ryohei Hayashi", "Nahoya Kawata", "Souya Kawata", "Hakkai Shiba", "Atsushi Sendo", "Takuya Yamamoto", "Makoto Suzuki", "Kazushi Yamagishi", "Hinata Tachibana", "Naoto Tachibana", "Emma Sano", "Shinichiro Sano", "Shuji Hanma", "Nobutaka Osanai", "Yuzuha Shiba", "Seishu Inui", "Hajime Kokonoi", "Kakucho", "Kanji Mochizuki", "Shion Madarame", "Ran Haitani", "Rindo Haitani", "Yasuhiro Muto", "Haruchiyo Sanzu", "South Terano", "Senju Kawaragi", "Takeomi Akashi", "Wakasa Imaushi", "Keizo Arashi", "Masataka Kiyomizu", "Luna Mitsuya", "Mana Mitsuya", "Akane Inui", "Mansaku Sano"]};

// Alias propres à un univers (formes / surnoms renvoyant au personnage unique du pool)
const RG_EXTRA_ALIASES = {"naruto": {"pain": "nagato", "tobi": "obito uchiha", "tenzo": "yamato", "sasuke uchiwa": "sasuke uchiha", "itachi uchiwa": "itachi uchiha", "madara uchiwa": "madara uchiha", "obito uchiwa": "obito uchiha", "shisui uchiwa": "shisui uchiha"}, "dragonball": {"goku black": "zamasu", "black goku": "zamasu", "future trunks": "trunks", "trunks du futur": "trunks", "fused zamasu": "zamasu", "zamasu fusionne": "zamasu", "freezer": "freezer", "frieza": "freezer", "sangoku": "son goku", "san goku": "son goku", "goku": "son goku", "gohan": "son gohan", "sangohan": "son gohan", "krilin": "krillin", "tortue geniale": "master roshi", "satan": "mr satan", "hercule": "mr satan"}, "tokyoghoul": {"haise sasaki": "ken kaneki", "haise": "ken kaneki", "sasaki haise": "ken kaneki", "yamori": "jason"}, "jjk": {"kokichi muta": "mechamaru", "kokichi": "mechamaru", "angel": "hana kurusu"}, "tokyorevengers": {"pah chin": "haruki hayashida", "pahchin": "haruki hayashida", "peh yan": "ryohei hayashi", "pehyan": "ryohei hayashi", "mikey": "manjiro sano", "draken": "ken ryuguji", "smiley": "nahoya kawata", "angry": "souya kawata", "mucho": "yasuhiro muto"}, "solo": {"greed": "hwang dongsuk"}, "sao": {"shino asada": "sinon", "nobuyuki sugou": "oberon", "sugou": "oberon", "poh": "vassago casals", "deusolbert": "deusolbert synthesis seven", "kayaba akihiko": "heathcliff", "akihiko kayaba": "heathcliff", "kayaba": "heathcliff", "suguha": "leafa", "suguha kirigaya": "leafa", "kazuto kirigaya": "kirito", "kazuto": "kirito", "keiko ayano": "silica", "rika shinozaki": "lisbeth", "quinella": "administrator"}, "fairy": {"angel": "sorano agria", "angel sorano": "sorano agria", "zero": "brain", "precht": "hades", "precht gaebolg": "hades", "meldy": "meredy", "doranbalt": "mest gryder", "belserion": "irene belserion", "silver": "silver fullbuster", "tauros": "taurus", "caprico": "capricorn", "chelia": "sherria blendy", "chelia blendy": "sherria blendy", "erza knightwalker": "erza scarlet", "hot eye": "hoteye", "blue note": "bluenote stinger"}, "bleach": {"tensa zangetsu": "zangetsu", "rose": "rojuro otoribashi", "rose otoribashi": "rojuro otoribashi", "hisagi": "shuhei hisagi", "kaien": "kaien shiba", "kensei": "kensei muguruma", "nanao": "nanao ise", "askin": "askin nakk le vaar", "tsukishima": "shukuro tsukishima", "lilinette": "lilynette gingerbuck", "lilinette gingerback": "lilynette gingerbuck"}, "sds": {"chaos arthur": "arthur pendragon", "gowther doll": "gowther", "elaine's brother": "king", "hauser": "howzer", "baltra": "bartra liones"}, "onepiece": {"miss all sunday": "nico robin", "lola": "charlotte lola", "dadan": "curly dadan", "squard": "squardo", "hitetsu": "tenguyama hitetsu", "san juan wolf": "sanjuan wolf", "cindry": "victoria cindry"}, "clover": {"wizard king": "julius novachrono", "gaja": "gadjah", "siren tium": "siren tium", "shiren tium": "siren tium"}, "pokemon": {"ash ketchum": "sacha", "misty": "ondine", "brock": "pierre", "gary oak": "regis", "professor oak": "professeur chen", "may": "flora", "dawn": "aurore", "cilan": "rachid", "clemont": "lem", "bonnie": "clem", "lillie": "lilie", "lana": "nephie", "mallow": "barbara", "sophocles": "chrys", "leon": "tarak", "steven stone": "pierre rochard", "lance": "peter", "diantha": "dianthea", "alder": "goyah", "raihan": "roy", "ash": "sacha", "professeur oak": "professeur chen", "prof chen": "professeur chen", "steven": "pierre rochard"}};
// ===== Rolland Garos V3 : noms VF + alias (anglais, japonais, anciennes écritures) =====
const RG_ALIASES_V3 = {"naruto": {"might guy": "Gaï Maito", "might duy": "Daï Maito", "black zetsu": "Zetsu Noir", "white zetsu": "Zetsu Blanc", "sasuke uchiha": "Sasuke Uchiwa", "itachi uchiha": "Itachi Uchiwa", "madara uchiha": "Madara Uchiwa", "fugaku uchiha": "Fugaku Uchiwa", "izumi uchiha": "Izumi Uchiwa", "kagami uchiha": "Kagami Uchiwa", "obito uchiha": "Obito Uchiwa", "sarada uchiha": "Sarada Uchiwa", "shisui uchiha": "Shisui Uchiwa", "izuna uchiha": "Izuna Uchiwa", "hikaku uchiha": "Hikaku Uchiwa", "inabi uchiha": "Inabi Uchiwa", "mikoto uchiha": "Mikoto Uchiwa", "naori uchiha": "Naori Uchiwa", "shin uchiha": "Shin Uchiwa", "tajima uchiha": "Tajima Uchiwa", "teyaki uchiha": "Teyaki Uchiwa", "uruchi uchiha": "Uruchi Uchiwa", "yashiro uchiha": "Yashiro Uchiwa", "tekka uchiha": "Tekka Uchiwa", "maito gai": "Gaï Maito", "might gai": "Gaï Maito", "gai sensei": "Gaï Maito", "guy": "Gaï Maito", "maito dai": "Daï Maito", "might dai": "Daï Maito", "kuro zetsu": "Zetsu Noir", "shiro zetsu": "Zetsu Blanc", "sakura uchiwa": "Sakura Haruno", "sakura uchiha": "Sakura Haruno", "hinata uzumaki": "Hinata Hyūga", "temari nara": "Temari", "karui akimichi": "Karui", "killer b": "Killer Bee", "kirabi": "Killer Bee", "hachibi": "Gyūki", "eighttails": "Gyūki", "huitqueues": "Gyūki", "gyuuki": "Gyūki", "kyubi": "Kurama", "kyuubi": "Kurama", "kyubi no kitsune": "Kurama", "ninetails": "Kurama", "neufqueues": "Kurama", "renard a neuf queues": "Kurama", "ichibi": "Shukaku", "onetail": "Shukaku", "unequeue": "Shukaku", "nibi": "Matatabi", "twotails": "Matatabi", "deuxqueues": "Matatabi", "sanbi": "Isobu", "threetails": "Isobu", "troisqueues": "Isobu", "yonbi": "Son Gokû", "fourtails": "Son Gokû", "quatrequeues": "Son Gokû", "gobi": "Kokuô", "fivetails": "Kokuô", "cinqqueues": "Kokuô", "rokubi": "Saiken", "sixtails": "Saiken", "sixqueues": "Saiken", "nanabi": "Chômei", "seventails": "Chômei", "septqueues": "Chômei", "tentails": "Jûbi", "dixqueues": "Jûbi", "juubi": "Jûbi", "premier hokage": "Hashirama Senju", "shodai hokage": "Hashirama Senju", "deuxieme hokage": "Tobirama Senju", "nidaime hokage": "Tobirama Senju", "troisieme hokage": "Hiruzen Sarutobi", "sandaime hokage": "Hiruzen Sarutobi", "quatrieme hokage": "Minato Namikaze", "yondaime hokage": "Minato Namikaze", "eclair jaune de konoha": "Minato Namikaze", "yellow flash": "Minato Namikaze", "cinquieme hokage": "Tsunade", "godaime hokage": "Tsunade", "tsunade senju": "Tsunade", "sixieme hokage": "Kakashi Hatake", "rokudaime hokage": "Kakashi Hatake", "kakashi du sharingan": "Kakashi Hatake", "copy ninja kakashi": "Kakashi Hatake", "septieme hokage": "Naruto Uzumaki", "nanadaime hokage": "Naruto Uzumaki", "huitieme hokage": "Shikamaru Nara", "raikage": "A", "quatrieme raikage": "A", "yondaime raikage": "A", "ay": "A", "cinquieme raikage": "Darui", "kazekage": "Gaara", "cinquieme kazekage": "Gaara", "godaime kazekage": "Gaara", "gaara du desert": "Gaara", "quatrieme kazekage": "Rasa", "yondaime kazekage": "Rasa", "tsuchikage": "Onoki", "troisieme tsuchikage": "Onoki", "sandaime tsuchikage": "Onoki", "quatrieme tsuchikage": "Kurotsuchi", "deuxieme tsuchikage": "Mû", "mizukage": "Meï Terumi", "cinquieme mizukage": "Meï Terumi", "sixieme mizukage": "Chōjūrō", "quatrieme mizukage": "Yagura Karatachi", "deuxieme mizukage": "Gengetsu Hōzuki", "hanzo la salamandre": "Hanzô", "hanzo of the salamander": "Hanzô", "ermite pervers": "Jiraiya", "erosennin": "Jiraiya", "pervy sage": "Jiraiya", "ermite rikudo": "Hagoromo Ôtsutsuki", "rikudo sennin": "Hagoromo Ôtsutsuki", "sage of six paths": "Hagoromo Ôtsutsuki", "sage des six chemins": "Hagoromo Ôtsutsuki", "nagato uzumaki": "Nagato", "kinoe": "Yamato", "lee": "Rock Lee", "ada": "Eida", "ogama sennin": "Oogama Sennin", "grand crapaud ermite": "Oogama Sennin", "hinata hyuuga": "Hinata Hyūga", "neji hyuuga": "Neji Hyūga", "hanabi hyuuga": "Hanabi Hyûga", "hiashi hyuuga": "Hiashi Hyûga", "juugo": "Jûgo", "chouji akimichi": "Chôji Akimichi", "danzou shimura": "Danzô Shimura", "kankurou": "Kankurô", "kyuubi no kitsune": "Kurama", "akaboshi": "Akahoshi", "ningame": "Ninkame", "koji kashin": "Kashin Koji"}, "onepiece": {"jinbe": "Jinbei", "enel": "Ener", "buggy": "Baggy", "koby": "Kobby", "axehand morgan": "Morgan", "chapeau de paille": "Monkey D. Luffy", "luffy au chapeau de paille": "Monkey D. Luffy", "straw hat luffy": "Monkey D. Luffy", "mugiwara": "Monkey D. Luffy", "nika": "Monkey D. Luffy", "luffy gear 5": "Monkey D. Luffy", "zorro": "Roronoa Zoro", "roronoa zorro": "Roronoa Zoro", "chasseur de pirates": "Roronoa Zoro", "pirate hunter zoro": "Roronoa Zoro", "mr bushido": "Roronoa Zoro", "vinsmoke sanji": "Sanji", "jambe noire": "Sanji", "black leg sanji": "Sanji", "mr prince": "Sanji", "sanji la jambe noire": "Sanji", "sogeking": "Usopp", "god usopp": "Usopp", "ussop": "Usopp", "usop": "Usopp", "chatte voleuse": "Nami", "cat burglar nami": "Nami", "enfant du demon": "Nico Robin", "demon child": "Nico Robin", "robin lenfant du demon": "Nico Robin", "cutty flam": "Franky", "cyborg franky": "Franky", "soul king": "Brook", "humming brook": "Brook", "roi des ames": "Brook", "tony chopper": "Tony Tony Chopper", "jimbei": "Jinbei", "jimbe": "Jinbei", "chevalier des mers": "Jinbei", "knight of the sea": "Jinbei", "eneru": "Ener", "god enel": "Ener", "dieu ener": "Ener", "baggy le clown": "Baggy", "buggy the clown": "Baggy", "buggy le clown": "Baggy", "coby": "Kobby", "morgan la main de hache": "Morgan", "capitaine morgan": "Morgan", "aokiji": "Kuzan", "akainu": "Sakazuki", "kizaru": "Borsalino", "fujitora": "Issho", "ryokugyu": "Aramaki", "green bull": "Aramaki", "barbe blanche": "Edward Newgate", "whitebeard": "Edward Newgate", "shirohige": "Edward Newgate", "barbe noire": "Marshall D. Teach", "blackbeard": "Marshall D. Teach", "kurohige": "Marshall D. Teach", "teach": "Marshall D. Teach", "gold roger": "Gol D. Roger", "roi des pirates": "Gol D. Roger", "le roux": "Shanks", "shanks le roux": "Shanks", "redhaired shanks": "Shanks", "akagami": "Shanks", "figarland shanks": "Shanks", "trafalgar law": "Trafalgar D. Water Law", "chirurgien de la mort": "Trafalgar D. Water Law", "surgeon of death": "Trafalgar D. Water Law", "eustass captain kid": "Eustass Kid", "eustass kidd": "Eustass Kid", "kidd": "Eustass Kid", "kaidou": "Kaido", "kaido aux cent betes": "Kaido", "big mom": "Charlotte Linlin", "imperatrice pirate": "Boa Hancock", "pirate empress": "Boa Hancock", "oeil de faucon": "Dracule Mihawk", "il de faucon": "Dracule Mihawk", "hawk eyes": "Dracule Mihawk", "mihawk": "Dracule Mihawk", "mr 0": "Crocodile", "sir crocodile": "Crocodile", "mr 1": "Daz Bones", "mr 2": "Bentham", "bon clay": "Bentham", "bon kurei": "Bentham", "mr 2 bon clay": "Bentham", "mr 3": "Galdino", "miss wednesday": "Nefertari Vivi", "mr 8": "Igaram", "marco le phenix": "Marco", "marco the phoenix": "Marco", "alber": "King", "king the wildfire": "King", "queen the plague": "Queen", "jack the drought": "Jack", "shogun orochi": "Kurozumi Orochi", "corazon": "Donquixote Rosinante", "joker": "Donquixote Doflamingo", "doffy": "Donquixote Doflamingo", "dragon": "Monkey D. Dragon", "roi des tenebres": "Silvers Rayleigh", "dark king rayleigh": "Silvers Rayleigh", "ace aux poings ardents": "Portgas D. Ace", "fire fist ace": "Portgas D. Ace", "gol d ace": "Portgas D. Ace", "lion dor": "Shiki", "golden lion shiki": "Shiki", "rocks": "Rocks D. Xebec", "klahadore": "Kuro", "kurahador": "Kuro", "krieg": "Don Krieg", "ivankov": "Emporio Ivankov", "iva": "Emporio Ivankov", "thunder soldier": "Kyros", "hakuba": "Cavendish", "dr vegapunk": "Vegapunk", "stella": "Vegapunk", "calgara": "Kalgara", "wiper": "Wyper", "kohza": "Koza", "chu": "Chew", "jyabura": "Jabra", "jabura": "Jabra", "shishilian": "Sicilian", "mother carmel": "Carmel", "mother caramel": "Carmel", "mere carmel": "Carmel", "ushimitsu kozo": "Shimotsuki Yasuie", "shutenmaru": "Ashura Doji", "kappa kawamatsu": "Kawamatsu", "hiluluk": "Dr. Hiriluk", "hilluluk": "Dr. Hiriluk", "tyrant kuma": "Bartholomew Kuma", "kuma le tyran": "Bartholomew Kuma", "gekko moriah": "Gecko Moria", "gecko moriah": "Gecko Moria", "gang bege": "Capone Bege", "diez drake": "X Drake", "apoo": "Scratchmen Apoo", "nerona imu": "Imu", "jaygarcia saturn": "Saint Jaygarcia Saturn", "marcus mars": "Saint Marcus Mars", "topman warcury": "Saint Topman Warcury", "ethanbaron v nusjuro": "Saint Ethanbaron V. Nusjuro", "nusjuro": "Saint Ethanbaron V. Nusjuro", "shepherd ju peter": "Saint Shepherd Ju Peter", "ju peter": "Saint Shepherd Ju Peter", "figarland garling": "Saint Figarland Garling", "garling": "Saint Figarland Garling", "figarland shamrock": "Shamrock", "saint charlos": "Charlos", "saint rosward": "Rosward", "roswald": "Rosward", "saint shalria": "Shalria", "gion": "Momousagi", "tokikake": "Chaton", "sengoku le bouddha": "Sengoku", "z": "Zephyr", "black arm zephyr": "Zephyr", "vander decken": "Vander Decken IX", "riku dold iii": "Riku Doldo III", "byrndi world": "Byrnndi World", "baron tamago": "Tamago", "gild tesoro": "Tesoro", "chouchou": "Shushu", "violet": "Viola", "momo": "Kozuki Momonosuke", "hatchan": "Hachi", "miss doublefinger": "Zala", "tonoyasu": "Shimotsuki Yasuie", "komurasaki": "Kozuki Hiyori", "okiku": "Kikunojo", "tenguyama hitetsu": "Kozuki Sukiyaki", "nyon": "Gloriosa", "onimaru": "Gyukimaru"}, "bleach": {"soul king": "Roi des Âmes", "reio": "Roi des Âmes", "chad": "Yasutora Sado", "sado": "Yasutora Sado", "suifeng": "Soi Fon", "soifon": "Soi Fon", "shaolin fon": "Soi Fon", "tia halibel": "Tier Harribel", "tier halibel": "Tier Harribel", "harribel": "Tier Harribel", "halibel": "Tier Harribel", "stark": "Coyote Starrk", "coyote stark": "Coyote Starrk", "starrk": "Coyote Starrk", "ulquiorra schiffer": "Ulquiorra Cifer", "grimmjow jeagerjaques": "Grimmjow Jaegerjaquez", "grimmjow jaggerjack": "Grimmjow Jaegerjaquez", "grimmjow jaggerjaquez": "Grimmjow Jaegerjaquez", "nnoitra jiruga": "Nnoitra Gilga", "nnoitra jiluga": "Nnoitra Gilga", "szayel aporro granz": "Szayelaporro Granz", "szayel": "Szayelaporro Granz", "barragan luisenbarn": "Baraggan Louisenbairn", "barragan": "Baraggan Louisenbairn", "juha bach": "Yhwach", "ywach": "Yhwach", "yuhabah": "Yhwach", "bazzard black": "Bazz-B", "aizen": "Sosuke Aizen", "urahara": "Kisuke Urahara", "yamamoto": "Genryusai Shigekuni Yamamoto", "genryusai yamamoto": "Genryusai Shigekuni Yamamoto", "shigekuni yamamoto genryusai": "Genryusai Shigekuni Yamamoto", "yamaji": "Genryusai Shigekuni Yamamoto", "rukia abarai": "Rukia Kuchiki", "orihime kurosaki": "Orihime Inoue", "yachiru unohana": "Retsu Unohana", "unohana": "Retsu Unohana", "isshin shiba": "Isshin Kurosaki", "nel": "Nelliel Tu Odelschwanck", "nel tu": "Nelliel Tu Odelschwanck", "hichigo": "Zangetsu", "shirosaki": "Zangetsu", "kyoraku": "Shunsui Kyoraku", "ukitake": "Jushiro Ukitake", "hitsugaya": "Toshiro Hitsugaya", "zaraki": "Kenpachi Zaraki", "kenpachi": "Kenpachi Zaraki", "kurotsuchi": "Mayuri Kurotsuchi", "komamura": "Sajin Komamura", "tosen": "Kaname Tosen", "haschwalth": "Jugram Haschwalth", "sungsun": "Cyan Sung-Sun", "apacci": "Emilou Apacci", "mila rose": "Franceska Mila Rose", "kariya": "Jin Kariya", "ganju": "Ganju Shiba", "kukaku": "Kukaku Shiba", "hanataro": "Hanataro Yamada", "pesche": "Pesche Guatiche", "dondochakka": "Dondochakka Birstanne", "hachi": "Hachigen Ushoda"}, "hxh": {"gon freeccs": "Gon Freecss", "kirua": "Killua Zoldyck", "killua zaoldyeck": "Killua Zoldyck", "kirua zoldyck": "Killua Zoldyck", "leolio": "Leorio Paradinight", "leorio paladiknight": "Leorio Paradinight", "leolio paladiknight": "Leorio Paradinight", "hisoka morrow": "Hisoka Morow", "kuroro lucifer": "Chrollo Lucilfer", "kuroro lucilfer": "Chrollo Lucilfer", "chrollo lucifer": "Chrollo Lucilfer", "kuroro": "Chrollo Lucilfer", "irumi": "Illumi Zoldyck", "gittarackur": "Illumi Zoldyck", "miruki": "Milluki Zoldyck", "karuto": "Kalluto Zoldyck", "kalluto": "Kalluto Zoldyck", "silva": "Silva Zoldyck", "kikyo": "Kikyo Zoldyck", "zeno": "Zeno Zoldyck", "alluka": "Alluka Zoldyck", "netero": "Isaac Netero", "pitou": "Neferpitou", "youpi": "Menthuthuyoupi", "pouf": "Shaiapouf", "shaiapuf": "Shaiapouf", "roi des fourmis": "Meruem", "ant king": "Meruem", "gin freecss": "Ging Freecss", "kaito": "Kite", "bisky": "Biscuit Krueger", "biscuit kruger": "Biscuit Krueger", "satotsu": "Satotz", "goto": "Gotoh", "pokkuru": "Pokkle", "ubogin": "Uvogin", "uvogine": "Uvogin", "fincks": "Phinks Magcub", "feitan": "Feitan Portor", "machi": "Machi Komacine", "nobunaga": "Nobunaga Hazama", "shizuku": "Shizuku Murasaki", "bonolenov": "Bonolenov Ndongo", "franklin": "Franklin Bordeau", "paku": "Pakunoda", "shalnalk": "Shalnark", "coltopi": "Kortopi", "kurtopi": "Kortopi", "gensuru": "Genthru", "bomber": "Genthru", "castro": "Kastro", "knuckle": "Knuckle Bine", "novu": "Knov", "morel": "Morel Mackernasey", "shoot": "Shoot McMahon", "palm": "Palm Siberia", "cheadle": "Cheadle Yorkshire", "pariston": "Pariston Hill", "mizaistom": "Mizaistom Nana", "botobai": "Botobai Gigante", "saccho": "Saccho Kobayakawa", "neon": "Neon Nostrade", "mito": "Mito Freecss", "maha": "Maha Zoldyck", "tserriednich": "Tserriednich Hui Guo Rou", "benjamin": "Benjamin Hui Guo Rou", "kacho": "Kacho Hui Guo Rou", "fugetsu": "Fugetsu Hui Guo Rou", "nasubi": "Nasubi Hui Guo Rou", "halkenburg": "Halkenburg Hui Guo Rou", "camilla": "Camilla Hui Guo Rou", "momoze": "Momoze Hui Guo Rou", "woble": "Woble Hui Guo Rou", "tubeppa": "Tubeppa Hui Guo Rou", "tyson": "Tyson Hui Guo Rou", "luzurus": "Luzurus Hui Guo Rou", "oito": "Oito Hui Guo Rou", "zhang lei": "Zhang Lei Hui Guo Rou", "marayam": "Marayam Hui Guo Rou", "sale sale": "Salé-Salé", "linne": "Linne Horsdoeuvre", "hinrigh": "Hinrigh Vielopolsky", "morena": "Morena Prudo", "melody": "Senritsu"}, "snk": {"eren yeager": "Eren Jäger", "zeke yeager": "Zeke Jäger", "grisha yeager": "Grisha Jäger", "faye yeager": "Faye Jäger", "levi ackerman": "Livaï Ackerman", "hange zoe": "Hansi Zoe", "bertholdt hoover": "Bertolt Hoover", "sasha blouse": "Sasha Braus", "miche zacharius": "Mike Zacharius", "oluo bozado": "Oruo Bozado", "eld jinn": "Erd Gin", "keith shadis": "Keith Sadies", "darius zackly": "Dallis Zachary", "nile dok": "Nile Dork", "eren jaeger": "Eren Jäger", "titan assaillant": "Eren Jäger", "attack titan": "Eren Jäger", "zeke jaeger": "Zeke Jäger", "titan bestial": "Zeke Jäger", "beast titan": "Zeke Jäger", "grisha jaeger": "Grisha Jäger", "carla yeager": "Carla Jäger", "carla jaeger": "Carla Jäger", "levi": "Livaï Ackerman", "rivaille": "Livaï Ackerman", "rivai": "Livaï Ackerman", "caporal livai": "Livaï Ackerman", "captain levi": "Livaï Ackerman", "hange": "Hansi Zoe", "hanji zoe": "Hansi Zoe", "hanji": "Hansi Zoe", "bertholdt": "Bertolt Hoover", "berthold hoover": "Bertolt Hoover", "artur blouse": "Artur Braus", "mike zacharias": "Mike Zacharius", "miche": "Mike Zacharius", "auruo bossard": "Oruo Bozado", "oluo": "Oruo Bozado", "eld gin": "Erd Gin", "gunter schultz": "Gunther Schultz", "dhalis zachary": "Dallis Zachary", "zackly": "Dallis Zachary", "nile dawk": "Nile Dork", "krista lenz": "Historia Reiss", "christa renz": "Historia Reiss", "christa lenz": "Historia Reiss", "krista": "Historia Reiss", "marco bodt": "Marco Bott", "jean kirschtein": "Jean Kirstein", "annie leonhardt": "Annie Leonhart", "titan cuirasse": "Reiner Braun", "armored titan": "Reiner Braun", "titan feminin": "Annie Leonhart", "female titan": "Annie Leonhart", "titan charrette": "Pieck Finger", "cart titan": "Pieck Finger", "titan marteau darmes": "Lara Tybur", "war hammer titan": "Lara Tybur", "pixis": "Dot Pixis", "erwin": "Erwin Smith", "tom xaver": "Tom Ksaver", "kitz weilman": "Kitz Woermann", "rico brezenska": "Rico Brzenska", "pyxis": "Dot Pixis"}, "sds": {"demon king": "Roi Démon", "supreme deity": "Déesse Suprême", "lady of the lake": "Dame du Lac", "reine supreme": "Déesse Suprême", "peche de la colere": "Meliodas", "dragons sin of wrath": "Meliodas", "peche du dragon": "Meliodas", "peche de lavarice": "Ban", "foxs sin of greed": "Ban", "peche du renard": "Ban", "harlequin": "King", "roi des fees": "King", "fairy king": "King", "peche de la paresse": "King", "grizzlys sin of sloth": "King", "peche du grizzly": "King", "peche de lenvie": "Diane", "serpents sin of envy": "Diane", "peche du serpent": "Diane", "peche de la luxure": "Gowther", "goats sin of lust": "Gowther", "peche du bouc": "Gowther", "peche de la gourmandise": "Merlin", "boars sin of gluttony": "Merlin", "peche du sanglier": "Merlin", "peche de lorgueil": "Escanor", "lions sin of pride": "Escanor", "peche du lion": "Escanor", "mama hawk": "Hawk Mama", "maman hawk": "Hawk Mama", "hendricksen": "Hendrickson", "monspiet": "Monspeet", "gray road": "Grayroad", "galan": "Galand", "ludoshel": "Ludociel", "perceval": "Percival", "gauvain": "Gawain", "tristan liones": "Tristan", "roi arthur": "Arthur Pendragon", "king arthur": "Arthur Pendragon", "elizabeth": "Elizabeth Liones", "gil thunder": "Gilthunder", "cath": "Cath Palug", "elaines brother": "king"}, "deathnote": {"kira": "Light Yagami", "raito yagami": "Light Yagami", "raito": "Light Yagami", "l": "L Lawliet", "ryuzaki": "L Lawliet", "hideki ryuga": "L Lawliet", "ryuga": "L Lawliet", "misa misa": "Misa Amane", "nate river": "Near", "nate rivers": "Near", "mihael keehl": "Mello", "mail jeevas": "Matt", "quillsh wammy": "Watari", "wammy": "Watari", "ryuku": "Ryuk", "remu": "Rem", "shidoh": "Sidoh", "merrie kenwood": "Wedy", "thierry morello": "Aiber", "ray penber": "Raye Penber", "shoko maki": "Naomi Misora", "bb": "Beyond Birthday", "rue ryuzaki": "Beyond Birthday", "kal snyder": "Jack Neylon", "hal lidner": "Halle Lidner", "halle bullook": "Halle Lidner", "gevanni": "Stephen Gevanni", "rester": "Anthony Rester", "tota matsuda": "Touta Matsuda", "matsuda": "Touta Matsuda", "aizawa": "Shuichi Aizawa", "mogi": "Kanzo Mogi", "ukita": "Hirokazu Ukita", "mikami": "Teru Mikami", "xkira": "Teru Mikami", "takada": "Kiyomi Takada", "higuchi": "Kyosuke Higuchi", "soichiro": "Soichiro Yagami", "sayu": "Sayu Yagami", "sachiko": "Sachiko Yagami", "justin": "Armonia Justin Beyondormason", "takeshi ohi": "Takeshi Ooi", "roger": "Roger Ruvie", "jelus": "Gelus", "shinigami gook": "Gook", "jealous": "Gelus"}, "cote": {"nanase tsubasa": "Tsubasa Nanase", "sotomura hideo": "Hideo Sotomura", "hakase": "Hideo Sotomura", "professeur": "Hideo Sotomura", "miichan": "Mei-Yu Wang", "wang mei yu": "Mei-Yu Wang", "teruhiko yukimura": "Keisei Yukimura", "ayanokouji": "Kiyotaka Ayanokoji", "kiyotaka ayanokouji": "Kiyotaka Ayanokoji", "ayanokouji kiyotaka": "Kiyotaka Ayanokoji", "kiyotaka": "Kiyotaka Ayanokoji", "ryuuen": "Kakeru Ryuen", "ryuuen kakeru": "Kakeru Ryuen", "kakeru ryuuen": "Kakeru Ryuen", "kikyou kushida": "Kikyo Kushida", "kushida kikyou": "Kikyo Kushida", "kouenji": "Rokusuke Koenji", "kouenji rokusuke": "Rokusuke Koenji", "sudou": "Ken Sudo", "sudou ken": "Ken Sudo", "kouhei katsuragi": "Kohei Katsuragi", "ryuuji kanzaki": "Ryuji Kanzaki", "suzune": "Suzune Horikita", "horikita": "Suzune Horikita", "manabu": "Manabu Horikita", "sakayanagi": "Arisu Sakayanagi", "alice sakayanagi": "Arisu Sakayanagi", "sakayanagi alice": "Arisu Sakayanagi", "nagumo": "Miyabi Nagumo", "kushida": "Kikyo Kushida", "karuizawa": "Kei Karuizawa", "ichinose": "Honami Ichinose", "ichinose honami": "Honami Ichinose", "chabashira": "Sae Chabashira", "hoshinomiya": "Chie Hoshinomiya", "koenji": "Rokusuke Koenji", "ryuen": "Kakeru Ryuen", "ibuki": "Mio Ibuki", "hirata": "Yosuke Hirata", "yousuke hirata": "Yosuke Hirata", "albert": "Albert Yamada", "amasawa": "Ichika Amasawa", "hosen": "Kazuomi Hosen", "housen": "Kazuomi Hosen", "housen kazuomi": "Kazuomi Hosen", "yagami": "Takuya Yagami", "sakura": "Airi Sakura", "shiina": "Hiyori Shiina", "kanzaki": "Ryuji Kanzaki", "katsuragi": "Kohei Katsuragi", "hashimoto": "Masayoshi Hashimoto", "kamuro": "Masumi Kamuro", "kiryuuin fuka": "Fuka Kiryuin", "fuuka kiryuuin": "Fuka Kiryuin", "kiryuin": "Fuka Kiryuin", "ike": "Kanji Ike", "yamauchi": "Haruki Yamauchi", "hasebe": "Haruka Hasebe", "miyake": "Akito Miyake", "yukimura": "Keisei Yukimura", "ayanokoji atsuomi": "Atsuomi Ayanokoji", "tsukishiro director": "Tsukishiro", "directeur tsukishiro": "Tsukishiro", "mashima": "Tomonari Mashima", "sakagami": "Kazuma Sakagami", "tokitou": "Hiroya Tokito", "hiroya tokitou": "Hiroya Tokito", "kitou": "Hayato Kito", "kito": "Hayato Kito", "hayato kitou": "Hayato Kito", "ishizaki": "Daichi Ishizaki", "nanase": "Tsubasa Nanase", "kei": "Kei Karuizawa", "ichika": "Ichika Amasawa", "ayanokoji kiyotaka": "Kiyotaka Ayanokoji", "horikita suzune": "Suzune Horikita", "kushida kikyo": "Kikyo Kushida", "karuizawa kei": "Kei Karuizawa", "ryuen kakeru": "Kakeru Ryuen", "sakayanagi arisu": "Arisu Sakayanagi", "koenji rokusuke": "Rokusuke Koenji", "horikita manabu": "Manabu Horikita", "nagumo miyabi": "Miyabi Nagumo", "hirata yosuke": "Yosuke Hirata", "sudo ken": "Ken Sudo", "ike kanji": "Kanji Ike", "yamauchi haruki": "Haruki Yamauchi", "sakura airi": "Airi Sakura", "miyake akito": "Akito Miyake", "hasebe haruka": "Haruka Hasebe", "shiina hiyori": "Hiyori Shiina", "yamada albert": "Albert Yamada", "ibuki mio": "Mio Ibuki", "kanzaki ryuji": "Ryuji Kanzaki", "hashimoto masayoshi": "Masayoshi Hashimoto", "kamuro masumi": "Masumi Kamuro", "chabashira sae": "Sae Chabashira", "hoshinomiya chie": "Chie Hoshinomiya", "amasawa ichika": "Ichika Amasawa", "yagami takuya": "Takuya Yagami", "yukimura keisei": "Keisei Yukimura", "matsushita chiaki": "Chiaki Matsushita", "sato maya": "Maya Sato", "inogashira kokoro": "Kokoro Inogashira", "katsuragi kohei": "Kohei Katsuragi", "ishizaki daichi": "Daichi Ishizaki", "hosen kazuomi": "Kazuomi Hosen", "kiryuin fuka": "Fuka Kiryuin", "wang meiyu": "Mei-Yu Wang", "tachibana akane": "Akane Tachibana", "sakagami kazuma": "Kazuma Sakagami", "asahina nazuna": "Nazuna Asahina", "mashima tomonari": "Tomonari Mashima", "sakayanagi narumori": "Narumori Sakayanagi", "kaneda satoru": "Satoru Kaneda", "kito hayato": "Hayato Kito", "yamamura miki": "Miki Yamamura", "morishita ai": "Ai Morishita", "tokito hiroya": "Hiroya Tokito", "onodera kayano": "Kayano Onodera", "amikura mako": "Mako Amikura", "shinohara satsuki": "Satsuki Shinohara", "himeno yuki": "Yuki Himeno", "machida tetsuya": "Tetsuya Machida", "kiriyama ikuto": "Ikuto Kiriyama", "hamaguchi tetsuya": "Tetsuya Hamaguchi", "sanada kosei": "Kosei Sanada", "hondo ryotaro": "Ryotaro Hondo", "miyamoto soshi": "Soshi Miyamoto", "okitani kyosuke": "Kyosuke Okitani", "mori nene": "Nene Mori", "maezono anzu": "Anzu Maezono", "ichihashi ruri": "Ruri Ichihashi", "komiya kyogo": "Kyogo Komiya", "kondo reon": "Reon Kondo", "nishino takeko": "Takeko Nishino", "yabu nanami": "Nanami Yabu", "morofuji rika": "Rika Morofuji", "manabe shiho": "Shiho Manabe", "yamashita saki": "Saki Yamashita", "watanabe norihito": "Norihito Watanabe", "shibata so": "So Shibata", "shiranami chihiro": "Chihiro Shiranami", "kobashi yume": "Yume Kobashi", "totsuka yahiko": "Yahiko Totsuka", "ishikura kaya": "Kaya Ishikura", "utomiya riku": "Riku Utomiya", "tsubaki sakurako": "Sakurako Tsubaki", "ishigami kyo": "Kyo Ishigami", "shiba katsunori": "Katsunori Shiba"}, "solo": {"architect": "L'Architecte", "architecte": "L'Architecte", "shun mizushino": "Sung Jinwoo", "mizushino shun": "Sung Jinwoo", "shun": "Sung Jinwoo", "aoi mizushino": "Sung Jinah", "mizushino aoi": "Sung Jinah", "shizuku": "Cha Hae-In", "sung jin woo": "Sung Jinwoo", "seong jinwoo": "Sung Jinwoo", "seong jin woo": "Sung Jinwoo", "jinwoo sung": "Sung Jinwoo", "jin woo": "Sung Jinwoo", "jinwoo": "Sung Jinwoo", "shadow monarch": "Sung Jinwoo", "monarque des ombres": "Sung Jinwoo", "sung jin ah": "Sung Jinah", "seong jinah": "Sung Jinah", "jinah": "Sung Jinah", "cha hae in": "Cha Hae-In", "hae in": "Cha Hae-In", "haein": "Cha Hae-In", "choi jong in": "Choi Jong-In", "baek yoon ho": "Baek Yoonho", "yoo jin ho": "Yoo Jinho", "jinho": "Yoo Jinho", "go gun hee": "Go Gunhee", "woo jin chul": "Woo Jinchul", "woo jincheol": "Woo Jinchul", "hwang dong suk": "Hwang Dongsuk", "hwang dong soo": "Hwang Dongsoo", "lee joo hee": "Lee Joohee", "joohee": "Lee Joohee", "kim sang shik": "Kim Sangshik", "kang tae shik": "Kang Taeshik", "ma dong wook": "Ma Dongwook", "min byung gyu": "Min Byung-Gyu", "lim tae gyu": "Lim Tae-Gyu", "park kyung hye": "Park Kyung-Hye", "sung il hwan": "Sung Il-Hwan", "yoo soo hyun": "Yoo Soohyun", "yoo myung han": "Yoo Myunghan", "han song yi": "Han Songyi", "liu zhi gang": "Liu Zhigang", "goliath": "Thomas Andre", "ryuji goto": "Goto Ryuji", "kim chul": "Iron", "kim cheol": "Iron", "kargalgan": "Tusk", "roi des fourmis": "Beru", "ant king": "Beru", "igris le sanguinaire": "Igris", "dragon monarch": "Antares", "monarque des dragons": "Antares", "monarque de la destruction": "Antares", "queen of insects": "Querehsha", "reine des insectes": "Querehsha", "monarch of white flames": "Baran", "roi des demons": "Baran", "suho": "Sung Suho", "sung su ho": "Sung Suho"}, "clover": {"sylph": "Bell", "witch queen": "Reine des Sorcières", "licita": "Richita", "lily aquaria": "Sister Lily", "soeur lily": "Sister Lily", "soeur theresa": "Sister Theresa", "loropechka": "Lolopechka", "yami": "Yami Sukehiro", "sukehiro yami": "Yami Sukehiro", "empereur mage": "Julius Novachrono", "empereurmage": "Julius Novachrono", "roi sorcier": "Julius Novachrono", "patry": "Patolli", "yuno grinberryall": "Yuno", "zora": "Zora Ideale", "gray": "Grey", "jack leventreur": "Jack the Ripper", "ryuya ryudo": "Ryudo Ryuya", "yosuga mushogatake": "Mushogatake Yosuga", "yami ichika": "Ichika Yami", "finral rulacase": "Finral Roulacase", "megikyula": "Megicula", "lucifer": "Lucifero", "belzebuth": "Beelzebub", "nahamah": "Naamah", "noelle": "Noelle Silva", "secre": "Secre Swallowtail", "charmy": "Charmy Pappitson", "gauche": "Gauche Adlai", "vangeance": "William Vangeance", "fuegoleon": "Fuegoleon Vermillion", "mereoleona": "Mereoleona Vermillion", "leo": "Leopold Vermillion", "mimosa": "Mimosa Vermillion", "nozel": "Nozel Silva", "charlotte": "Charlotte Roselei", "rill": "Rill Boismortier", "dorothy": "Dorothy Unsworth", "kaiser": "Kaiser Granvorka", "lumiere silva million clover": "Lumiere Silvamillion Clover", "premier empereur mage": "Lumiere Silvamillion Clover", "nero": "Secre Swallowtail"}, "fireforce": {"knight king": "Arthur Boyle", "roi chevalier": "Arthur Boyle", "roi des chevaliers": "Arthur Boyle", "shou kusakabe": "Sho Kusakabe", "victor licht": "Viktor Licht", "soeur iris": "Iris", "sister iris": "Iris", "waka": "Benimaru Shinmon", "princesse hibana": "Hibana", "princess hibana": "Hibana", "karim fram": "Karim Flam", "carim flam": "Karim Flam", "foyen li": "Foien Lee", "foien li": "Foien Lee", "captain pan": "Pan Ko Paat", "huang kayoko": "Kayoko Huang", "kurono": "Yuichiro Kurono", "lisa": "Lisa Isaribe", "risa isaribe": "Lisa Isaribe", "yuu": "Yu", "tsunderella takigi": "Takigi Tsunderera", "takigi": "Takigi Tsunderera", "empereur raffles": "Raffles III", "raffles the third": "Raffles III", "raffles": "Raffles III", "konro": "Konro Sagamiya", "burns": "Leonard Burns", "obi": "Akitaru Obi", "hinawa": "Takehisa Hinawa", "vulcan": "Vulcan Joseph", "oze": "Maki Oze", "tamaki": "Tamaki Kotatsu", "rekka": "Rekka Hoshimiya", "inca": "Inca Kasugatani", "nataku": "Nataku Son", "shinra": "Shinra Kusakabe", "sho": "Sho Kusakabe", "kusakabe shinra": "Shinra Kusakabe", "boyle arthur": "Arthur Boyle", "oze maki": "Maki Oze", "kotatsu tamaki": "Tamaki Kotatsu", "obi akitaru": "Akitaru Obi", "hinawa takehisa": "Takehisa Hinawa", "shinmon benimaru": "Benimaru Shinmon", "burns leonard": "Leonard Burns", "kusakabe sho": "Sho Kusakabe", "licht viktor": "Viktor Licht", "joseph vulcan": "Vulcan Joseph", "isaribe lisa": "Lisa Isaribe", "sagamiya konro": "Konro Sagamiya", "flam karim": "Karim Flam", "hoshimiya rekka": "Rekka Hoshimiya", "montgomery ogun": "Ogun Montgomery", "kasugatani inca": "Inca Kasugatani", "son nataku": "Nataku Son", "kurono yuichiro": "Yuichiro Kurono", "lee foien": "Foien Lee", "honda gustav": "Gustav Honda", "tsunderera takigi": "Takigi Tsunderera"}, "mushoku": {"rudy": "Rudeus Greyrat", "rudeus": "Rudeus Greyrat", "quagmire": "Rudeus Greyrat", "sylphie": "Sylphiette", "sylphy": "Sylphiette", "fitz": "Sylphiette", "fitts": "Sylphiette", "sylphiette greyrat": "Sylphiette", "roxy greyrat": "Roxy Migurdia", "eris greyrat": "Eris Boreas Greyrat", "eris boreas": "Eris Boreas Greyrat", "eris": "Eris Boreas Greyrat", "ruijerd": "Ruijerd Superdia", "rujierd": "Ruijerd Superdia", "dragon god": "Orsted", "dieu dragon": "Orsted", "mangod": "Hitogami", "dieuhomme": "Hitogami", "dieu humain": "Hitogami", "shizuka nanahoshi": "Nanahoshi Shizuka", "nanahoshi": "Nanahoshi Shizuka", "silent seven star": "Nanahoshi Shizuka", "paul": "Paul Greyrat", "zenith": "Zenith Greyrat", "lilia": "Lilia Greyrat", "norn": "Norn Greyrat", "aisha": "Aisha Greyrat", "lucie": "Lucie Greyrat", "lara": "Lara Greyrat", "ars": "Ars Greyrat", "sieghart greyrat": "Sieghart Saladin Greyrat", "sieg": "Sieghart Saladin Greyrat", "chris greyrat": "Christina Greyrat", "ghislaine": "Ghislaine Dedoldia", "kishirika": "Kishirika Kishirisu", "demon empress": "Kishirika Kishirisu", "imperatrice des demons": "Kishirika Kishirisu", "badi": "Badigadi", "luke": "Luke Notos Greyrat", "zanoba": "Zanoba Shirone", "elinalise": "Elinalise Dragonroad", "philip": "Philip Boreas Greyrat", "hilda": "Hilda Boreas Greyrat", "sauros": "Sauros Boreas Greyrat", "perugius": "Perugius Dola", "arumanfi": "Almanfi", "linia": "Linia Dedoldia", "pursena": "Pursena Adoldia", "pax": "Pax Shirone", "randolph": "Randolph Marianne", "ariel": "Ariel Anemoi Asura", "geese": "Geese Nukadia", "cliff": "Cliff Grimoire", "nina": "Nina Farion", "jino": "Jino Britts", "isolde": "Isolde Cruel", "auber": "Auber Corbett", "ginger": "Ginger York", "darius": "Darius Silva Garganti", "gall": "Gall Falion", "sword god": "Gall Falion", "dieu de lepee": "Gall Falion", "reida": "Reida Lia", "water god": "Reida Lia", "dieu de leau": "Reida Lia", "timothy": "Timothy Mayers", "soldat": "Soldat Heckler", "kalman iii": "Alexander Rybak", "alexander kalman rybak": "Alexander Rybak", "alex rybak": "Alexander Rybak", "grabel": "Grabel Zafin Asura", "claire": "Claire Latreia", "therese": "Therese Latreia", "pilemon": "Pilemon Notos Greyrat", "james": "James Boreas Greyrat", "gyes": "Gyes Dedoldia"}, "rezero": {"natsuki subaru": "Subaru Natsuki", "natsumi schwartz": "Subaru Natsuki", "emiliatan": "Emilia", "roswaal": "Roswaal L. Mathers", "roswaal j mathers": "Roswaal L. Mathers", "reinhard": "Reinhard van Astrea", "crusch": "Crusch Karsten", "priscilla": "Priscilla Barielle", "prisca benedict": "Priscilla Barielle", "regulus": "Regulus Corneas", "otto": "Otto Suwen", "garfiel": "Garfiel Tinsel", "garf": "Garfiel Tinsel", "frederica": "Frederica Baumann", "petra": "Petra Leyte", "ferris": "Felix Argyle", "felix": "Felix Argyle", "wilhelm": "Wilhelm van Astrea", "wilhelm trias": "Wilhelm van Astrea", "sword devil": "Wilhelm van Astrea", "demon de lepee": "Wilhelm van Astrea", "theresia": "Theresia van Astrea", "thearesia van astrea": "Theresia van Astrea", "anastasia": "Anastasia Hoshin", "ana": "Anastasia Hoshin", "julius": "Julius Juukulius", "ricardo": "Ricardo Welkin", "aldebaran": "Al", "betty": "Beatrice", "betelgeuse romaneeconti": "Petelgeuse Romanee-Conti", "betelgeuse": "Petelgeuse Romanee-Conti", "petelgeuse": "Petelgeuse Romanee-Conti", "geuse": "Petelgeuse Romanee-Conti", "sirius": "Sirius Romanee-Conti", "capella": "Capella Emerada Lugunica", "ley batenkaitos": "Lye Batenkaitos", "lye": "Lye Batenkaitos", "roy": "Roy Alphard", "louis": "Louis Arneb", "spica": "Louis Arneb", "elsa": "Elsa Granhiert", "meili": "Meili Portroute", "mimi": "Mimi Pearlbaton", "liliana": "Liliana Masquerade", "kiritaka": "Kiritaka Muse", "heinkel": "Heinkel van Astrea", "russel": "Russel Fellow", "vincent": "Vincent Vollachia", "abel": "Vincent Vollachia", "ryuzu": "Ryuzu Meyer", "lewes meyer": "Ryuzu Meyer", "kadomon": "Kadomon Risch", "joshua": "Joshua Juukulius", "reid": "Reid Astrea", "reid van astrea": "Reid Astrea", "witch of envy": "Satella", "sorciere de lenvie": "Satella", "witch of greed": "Echidna", "sorciere de lavarice": "Echidna", "sorciere de lavidite": "Echidna", "witch of wrath": "Minerva", "sorciere de la colere": "Minerva", "witch of pride": "Typhon", "sorciere de lorgueil": "Typhon", "witch of gluttony": "Daphne", "sorciere de la gourmandise": "Daphne", "witch of sloth": "Sekhmet", "sorciere de la paresse": "Sekhmet", "witch of lust": "Carmilla", "sorciere de la luxure": "Carmilla", "witch of vanity": "Pandora", "witch of vainglory": "Pandora", "sorciere de la vanite": "Pandora", "cecilus": "Cecilus Segmunt", "yorna": "Yorna Mishigure", "chisha": "Chisha Gold", "olbart": "Olbart Dankelken", "madelyn": "Madelyn Eschart", "goz": "Goz Ralfon", "moguro": "Moguro Hagane", "balleroy": "Balleroy Temeglyph", "groovy": "Groovy Gumbledatt", "todd": "Todd Fang", "jamal": "Jamal Aurelius", "zikr": "Zikr Osman", "serena": "Serena Dracroy", "lamia": "Lamia Godwin", "flop": "Flop O'Connell", "medium": "Medium O'Connell", "grimm": "Grimm Fauzen", "carol": "Carol Remendis", "marcos": "Marcos Gildark", "miklotov": "Miklotov McMahon", "fourier": "Fourier Lugunica"}, "fairy": {"loke": "Loki", "laxus dreyar": "Luxus Draer", "makarov dreyar": "Makarov Draer", "yuri dreyar": "Yuri Draer", "salamander": "Natsu Dragneel", "end": "Natsu Dragneel", "natsu dragion": "Natsu Dragneel", "etherious natsu dragneel": "Natsu Dragneel", "lucy ashley": "Lucy Heartfilia", "grey fullbuster": "Gray Fullbuster", "titania": "Erza Scarlet", "juvia loxar": "Juvia Lockser", "mira": "Mirajane Strauss", "siegrain": "Jellal Fernandes", "jellal fernandez": "Jellal Fernandes", "gerard fernandes": "Jellal Fernandes", "leo": "Loki", "leo le lion": "Loki", "charle": "Carla", "charles": "Carla", "pantherlily": "Panther Lily", "lily": "Panther Lily", "laxus": "Luxus Draer", "luxus": "Luxus Draer", "ivan dreyar": "Ivan Draer", "alexei": "Ivan Draer", "levi mcgarden": "Levy McGarden", "fried justine": "Freed Justine", "fried": "Freed Justine", "bixlow": "Bickslow", "guildarts": "Gildarts Clive", "alzak connell": "Alzack Connell", "sawyer": "Racer", "erik": "Cobra", "richard buchanan": "Hoteye", "macbeth": "Midnight", "ryos": "Rogue Cheney", "minerva orlando": "Minerva Orland", "ichiya vandalay kotobuki": "Ichiya", "polyussica": "Porlyusica", "porlyusica grandeeney": "Porlyusica", "mavis vermilion": "Mavis Vermillion", "wall eehto": "Wahl Icht", "wahl": "Wahl Icht", "serena": "God Serena", "mato": "Toma E. Fiore", "ur milkovich": "Ur", "mard geer tartaros": "Mard Geer", "zeira": "Zera", "nikora": "Plue", "grampa crux": "Crux", "fro": "Frosch", "ooba": "Ooba Babasaama", "monsieur sol": "Sol", "celestial spirit king": "Roi des esprits célestes", "spirit king": "Roi des esprits célestes", "invel": "Invel Yura", "dimaria": "Dimaria Yesta", "grandine": "Grandeeney", "metallicana": "Metalicana", "ignir": "Igneel", "zeref": "Zeref Dragneel", "kagura": "Kagura Mikazuchi", "jura": "Jura Neekis", "eve thearm": "Eve Tearm", "urutia": "Ultear Milkovich", "extalia queen": "Shagotte"}, "bluelock": {"isagi yoichi": "Yoichi Isagi", "itoshi rin": "Rin Itoshi", "bachira meguru": "Meguru Bachira", "nagi seishiro": "Seishiro Nagi", "mikage reo": "Reo Mikage", "barou shoei": "Shoei Barou", "chigiri hyoma": "Hyoma Chigiri", "shidou ryusei": "Ryusei Shidou", "itoshi sae": "Sae Itoshi", "aiku oliver": "Oliver Aiku", "gagamaru gin": "Gin Gagamaru", "kunigami rensuke": "Rensuke Kunigami", "aryu jyubei": "Jyubei Aryu", "tokimitsu aoshi": "Aoshi Tokimitsu", "niko ikki": "Ikki Niko", "jingo raichi": "Raichi Jingo", "karasu tabito": "Tabito Karasu", "otoya eita": "Eita Otoya", "yukimiya kenyu": "Kenyu Yukimiya", "hiori yo": "Yo Hiori", "kurona ranze": "Ranze Kurona", "jin kiyora": "Kiyora Jin", "ego jinpachi": "Jinpachi Ego", "teieri anri": "Anri Teieri", "sendou shuto": "Shuto Sendou", "igarashi gurimu": "Gurimu Igarashi", "naruhaya asahi": "Asahi Naruhaya", "kuon wataru": "Wataru Kuon", "imamura yudai": "Yudai Imamura", "wanima junichi": "Junichi Wanima", "wanima keisuke": "Keisuke Wanima", "nanase nijiro": "Nijiro Nanase", "darai miroku": "Miroku Darai", "neru teppei": "Teppei Neru", "niou kazuma": "Kazuma Niou", "fukaku gen": "Gen Fukaku", "kira ryosuke": "Ryosuke Kira", "tsurugi zantetsu": "Zantetsu Tsurugi", "iemon okuhito": "Okuhito Iemon", "ishikari yukio": "Yukio Ishikari", "okawa hibiki": "Hibiki Okawa", "ryusei shido": "Ryusei Shidou", "shido ryusei": "Ryusei Shidou", "shuto sendo": "Shuto Sendou", "jubei aryu": "Jyubei Aryu", "seishirou nagi": "Seishiro Nagi", "grim igarashi": "Gurimu Igarashi", "hyouma chigiri": "Hyoma Chigiri", "shouei barou": "Shoei Barou", "ryuusei shidou": "Ryusei Shidou", "juubei aryu": "Jyubei Aryu", "hibiki ookawa": "Hibiki Okawa", "kaiser": "Michael Kaiser", "ness": "Alexis Ness", "noa": "Noel Noa", "loki": "Julian Loki", "snuffy": "Marc Snuffy", "lorenzo": "Don Lorenzo", "chevalier": "Charles Chevalier", "ego": "Jinpachi Ego"}, "fma": {"father": "Père", "le pere": "Père", "ed": "Edward Elric", "fullmetal alchemist": "Edward Elric", "alchimiste dacier": "Edward Elric", "full metal": "Edward Elric", "al": "Alphonse Elric", "flame alchemist": "Roy Mustang", "alchimiste de flamme": "Roy Mustang", "colonel mustang": "Roy Mustang", "liza hawkeye": "Riza Hawkeye", "winry rockbelle": "Winry Rockbell", "fuhrer bradley": "King Bradley", "fuhrer king bradley": "King Bradley", "bradley": "King Bradley", "hohenheim": "Van Hohenheim", "hohenheim of light": "Van Hohenheim", "hohenheim de lumiere": "Van Hohenheim", "hughes": "Maes Hughes", "havoc": "Jean Havoc", "breda": "Heymans Breda", "falman": "Vato Falman", "fuery": "Kain Fuery", "major armstrong": "Alex Louis Armstrong", "strong arm alchemist": "Alex Louis Armstrong", "olivier armstrong": "Olivier Mira Armstrong", "olivia armstrong": "Olivier Mira Armstrong", "kimblee": "Solf J. Kimblee", "zolf j kimblee": "Solf J. Kimblee", "crimson alchemist": "Solf J. Kimblee", "alchimiste ecarlate": "Solf J. Kimblee", "mei chang": "May Chang", "lin yao": "Ling Yao", "ranfan": "Lan Fan", "shao may": "Xiao Mei", "marcoh": "Tim Marcoh", "izumi": "Izumi Curtis", "sig": "Sig Curtis", "selim": "Selim Bradley", "tucker": "Shou Tucker", "sewinglife alchemist": "Shou Tucker", "nina": "Nina Tucker", "barry le boucher": "Barry the Chopper", "barry": "Barry the Chopper", "ross": "Maria Ross", "brosh": "Denny Brosh", "iceman": "Isaac McDougal", "isaac the freezing alchemist": "Isaac McDougal", "archer": "Frank Archer", "madame christmas": "Chris Mustang", "hayate": "Black Hayate", "pinako": "Pinako Rockbell", "trisha": "Trisha Elric", "gracia": "Gracia Hughes", "elicia": "Elicia Hughes", "truth": "La Vérité", "verite": "La Vérité", "juliet douglas": "Dante", "schiezka": "Sheska", "garfield": "Garfiel", "alfons": "Alfons Heiderich", "eckhart": "Dietlinde Eckhart", "ashleigh crichton": "Melvin Voyager", "pride": "Selim Bradley"}, "chainsaw": {"future devil": "Démon du Futur", "curse devil": "Démon de la Malédiction", "fox devil": "Démon Renard", "ghost devil": "Démon Fantôme", "bat devil": "Démon Chauve-souris", "leech devil": "Démon Sangsue", "eternity devil": "Démon de l'Éternité", "tomato devil": "Démon Tomate", "zombie devil": "Démon Zombie", "darkness devil": "Démon des Ténèbres", "falling devil": "Démon de la Chute", "justice devil": "Démon de la Justice", "hell devil": "Démon de l'Enfer", "spider devil": "Princi", "chicken devil": "Démon Poulet", "mirai no akuma": "Démon du Futur", "demon malediction": "Démon de la Malédiction", "demon eternite": "Démon de l'Éternité", "demon tenebres": "Démon des Ténèbres", "demon chute": "Démon de la Chute", "demon justice": "Démon de la Justice", "demon enfer": "Démon de l'Enfer", "snake devil": "Démon Serpent", "octopus devil": "Démon Pieuvre", "typhoon devil": "Démon Typhon", "demon du pistolet": "Gun Devil", "demon pistolet": "Gun Devil", "demon du fusil": "Gun Devil", "demon flingue": "Gun Devil", "demon ange": "Angel Devil", "demon de lange": "Angel Devil", "demon de la vieillesse": "Aging Devil", "demon vieillesse": "Aging Devil", "demon de la violence": "Violence Fiend", "violence devil": "Violence Fiend", "galgali": "Violence Fiend", "chainsaw man": "Denji", "homme tronconneuse": "Denji", "chainsaw devil": "Pochita", "demon tronconneuse": "Pochita", "control devil": "Makima", "demon de la domination": "Makima", "demon du controle": "Makima", "blood fiend": "Power", "demon du sang": "Power", "bomb devil": "Reze", "demon bombe": "Reze", "samurai sword": "Katana Man", "homme katana": "Katana Man", "shark fiend": "Beam", "demon requin": "Beam", "war devil": "Yoru", "demon de la guerre": "Yoru", "famine devil": "Fami", "kiga": "Fami", "demon de la famine": "Fami", "hayakawa aki": "Aki Hayakawa", "higashiyama kobeni": "Kobeni Higashiyama", "kobeni": "Kobeni Higashiyama", "mitaka asa": "Asa Mitaka", "mitaka": "Asa Mitaka", "yoshida": "Hirofumi Yoshida", "yoshida hirofumi": "Hirofumi Yoshida", "kuanshi": "Quanxi", "sawatari": "Akane Sawatari", "meowy": "Nyako", "nobana": "Nobana Higashiyama", "sugo": "Miri Sugo", "tendo": "Michiko Tendo", "kurose": "Yutaro Kurose", "demon araignee": "Princi"}, "wakfu": {"percedal": "Tristepin de Percedal", "sir percedal": "Tristepin de Percedal", "percedal sadlygrove": "Tristepin de Percedal", "sadlygrove": "Tristepin de Percedal", "eva": "Evangelyne", "amalia": "Amalia Sheran Sharm", "armand": "Armand Sheran Sharm", "prince armand": "Armand Sheran Sharm", "noximilien": "Nox", "noximilien lhorloger": "Nox", "noximilien the watchmaker": "Nox", "lhorloger": "Nox", "the watchmaker": "Nox", "grougal": "Grougaloragran", "goultard le barbare": "Goultard", "goultard the barbarian": "Goultard", "comte harebourg": "Harebourg", "count harebourg": "Harebourg", "ush": "Ush Galesh", "shushu rubilax": "Rubilax", "kerubim": "Kerubim Crepin", "maitre joris": "Joris Jurgen", "master joris": "Joris Jurgen", "joris": "Joris Jurgen", "granny smisse": "Grany Smisse", "remington": "Remington Smisse", "kriss": "Kriss la Krass", "sadida king": "Roi Sadida", "king sadida": "Roi Sadida", "ruel": "Ruel Stroud"}, "demonslayer": {"kamado tanjiro": "Tanjiro Kamado", "tanjirou kamado": "Tanjiro Kamado", "kamado nezuko": "Nezuko Kamado", "agatsuma zenitsu": "Zenitsu Agatsuma", "hashibira inosuke": "Inosuke Hashibira", "tomioka giyu": "Giyu Tomioka", "giyuu tomioka": "Giyu Tomioka", "tomioka": "Giyu Tomioka", "rengoku kyojuro": "Kyojuro Rengoku", "kyoujurou rengoku": "Kyojuro Rengoku", "rengoku": "Kyojuro Rengoku", "kocho shinobu": "Shinobu Kocho", "shinobu kochou": "Shinobu Kocho", "kibutsuji muzan": "Muzan Kibutsuji", "kibutsuji": "Muzan Kibutsuji", "tsukihiko": "Muzan Kibutsuji", "kokushibou": "Kokushibo", "hakuji": "Akaza", "douma": "Doma", "ume": "Daki", "tsuyuri kanao": "Kanao Tsuyuri", "uzui tengen": "Tengen Uzui", "uzui": "Tengen Uzui", "kanroji mitsuri": "Mitsuri Kanroji", "kanroji": "Mitsuri Kanroji", "tokito muichiro": "Muichiro Tokito", "muichirou tokitou": "Muichiro Tokito", "himejima gyomei": "Gyomei Himejima", "gyoumei himejima": "Gyomei Himejima", "himejima": "Gyomei Himejima", "iguro obanai": "Obanai Iguro", "iguro": "Obanai Iguro", "urokodaki": "Sakonji Urokodaki", "jigorou kuwajima": "Jigoro Kuwajima", "kuwajima": "Jigoro Kuwajima", "oyakatasama": "Kagaya Ubuyashiki", "sekido": "Hantengu", "karaku": "Hantengu", "aizetsu": "Hantengu", "urogi": "Hantengu", "zohakuten": "Hantengu", "gyuutarou": "Gyutaro", "kyougai": "Kyogai", "yushirou": "Yushiro", "yuushirou": "Yushiro", "tsugikuni yoriichi": "Yoriichi Tsugikuni", "haganezuka": "Hotaru Haganezuka", "tetchin": "Tecchin Tecchikawahara", "kanzaki aoi": "Aoi Kanzaki", "te oni": "Hand Demon", "demon aux mains": "Hand Demon", "demon des mains": "Hand Demon", "numa oni": "Swamp Demon", "demon du marais": "Swamp Demon", "demon du marecage": "Swamp Demon"}, "pokemon": {"bulbasaur": "Bulbizarre", "ivysaur": "Herbizarre", "venusaur": "Florizarre", "charmander": "Salamèche", "charmeleon": "Reptincel", "charizard": "Dracaufeu", "squirtle": "Carapuce", "wartortle": "Carabaffe", "blastoise": "Tortank", "caterpie": "Chenipan", "metapod": "Chrysacier", "butterfree": "Papilusion", "weedle": "Aspicot", "kakuna": "Coconfort", "beedrill": "Dardargnan", "pidgey": "Roucool", "pidgeotto": "Roucoups", "pidgeot": "Roucarnage", "raticate": "Rattatac", "spearow": "Piafabec", "fearow": "Rapasdepic", "ekans": "Abo", "sandshrew": "Sabelette", "sandslash": "Sablaireau", "clefairy": "Mélofée", "clefable": "Mélodelfe", "vulpix": "Goupix", "ninetales": "Feunard", "jigglypuff": "Rondoudou", "wigglytuff": "Grodoudou", "zubat": "Nosferapti", "golbat": "Nosferalto", "oddish": "Mystherbe", "gloom": "Ortide", "vileplume": "Rafflesia", "venonat": "Mimitoss", "venomoth": "Aéromite", "diglett": "Taupiqueur", "dugtrio": "Triopikeur", "meowth": "Miaouss", "psyduck": "Psykokwak", "golduck": "Akwakwak", "mankey": "Férosinge", "primeape": "Colossinge", "growlithe": "Caninos", "arcanine": "Arcanin", "poliwag": "Ptitard", "poliwhirl": "Têtarte", "poliwrath": "Tartard", "machop": "Machoc", "machoke": "Machopeur", "machamp": "Mackogneur", "bellsprout": "Chétiflor", "weepinbell": "Boustiflor", "victreebel": "Empiflor", "geodude": "Racaillou", "graveler": "Gravalanch", "golem": "Grolem", "rapidash": "Galopa", "slowpoke": "Ramoloss", "slowbro": "Flagadoss", "magnemite": "Magnéti", "farfetchd": "Canarticho", "seel": "Otaria", "dewgong": "Lamantine", "grimer": "Tadmorv", "muk": "Grotadmorv", "shellder": "Kokiyas", "cloyster": "Crustabri", "gastly": "Fantominus", "haunter": "Spectrum", "gengar": "Ectoplasma", "drowzee": "Soporifik", "hypno": "Hypnomade", "kingler": "Krabboss", "voltorb": "Voltorbe", "exeggcute": "Noeunoeuf", "exeggutor": "Noadkoko", "cubone": "Osselait", "marowak": "Ossatueur", "hitmonlee": "Kicklee", "hitmonchan": "Tygnon", "lickitung": "Excelangue", "koffing": "Smogo", "weezing": "Smogogo", "rhyhorn": "Rhinocorne", "rhydon": "Rhinoféros", "chansey": "Leveinard", "tangela": "Saquedeneu", "kangaskhan": "Kangourex", "horsea": "Hypotrempe", "seadra": "Hypocéan", "goldeen": "Poissirène", "seaking": "Poissoroy", "staryu": "Stari", "starmie": "Staross", "mr mime": "M. Mime", "scyther": "Insécateur", "jynx": "Lippoutou", "electabuzz": "Élektek", "pinsir": "Scarabrute", "magikarp": "Magicarpe", "gyarados": "Léviator", "lapras": "Lokhlass", "ditto": "Métamorph", "eevee": "Évoli", "vaporeon": "Aquali", "jolteon": "Voltali", "flareon": "Pyroli", "omanyte": "Amonita", "omastar": "Amonistar", "aerodactyl": "Ptéra", "snorlax": "Ronflex", "articuno": "Artikodin", "zapdos": "Électhor", "moltres": "Sulfura", "dratini": "Minidraco", "dragonair": "Draco", "dragonite": "Dracolosse", "chikorita": "Germignon", "bayleef": "Macronium", "cyndaquil": "Héricendre", "quilava": "Feurisson", "totodile": "Kaiminus", "croconaw": "Crocrodil", "feraligatr": "Aligatueur", "sentret": "Fouinette", "furret": "Fouinar", "noctowl": "Noarfang", "ledyba": "Coxy", "ledian": "Coxyclaque", "spinarak": "Mimigal", "ariados": "Migalos", "crobat": "Nostenfer", "chinchou": "Loupio", "cleffa": "Mélo", "igglybuff": "Toudoudou", "mareep": "Wattouat", "flaaffy": "Lainergie", "ampharos": "Pharamp", "bellossom": "Joliflor", "sudowoodo": "Simularbre", "politoed": "Tarpaud", "hoppip": "Granivol", "skiploom": "Floravol", "jumpluff": "Cotovol", "aipom": "Capumain", "sunkern": "Tournegrin", "sunflora": "Héliatronc", "wooper": "Axoloto", "quagsire": "Maraiste", "espeon": "Mentali", "umbreon": "Noctali", "murkrow": "Cornèbre", "slowking": "Roigada", "misdreavus": "Feuforêve", "unown": "Zarbi", "wobbuffet": "Qulbutoké", "pineco": "Pomdepik", "forretress": "Foretress", "dunsparce": "Insolourdo", "gligar": "Scorplane", "scizor": "Cizayox", "shuckle": "Caratroc", "heracross": "Scarhino", "sneasel": "Farfuret", "slugma": "Limagma", "magcargo": "Volcaropod", "swinub": "Marcacrin", "piloswine": "Cochignon", "corsola": "Corayon", "delibird": "Cadoizo", "mantine": "Démanta", "skarmory": "Airmure", "houndour": "Malosse", "houndoom": "Démolosse", "kingdra": "Hyporoi", "stantler": "Cerfrousse", "smeargle": "Queulorior", "tyrogue": "Debugant", "hitmontop": "Kapoera", "smoochum": "Lippouti", "miltank": "Écrémeuh", "blissey": "Leuphorie", "larvitar": "Embrylex", "pupitar": "Ymphect", "tyranitar": "Tyranocif", "treecko": "Arcko", "grovyle": "Massko", "sceptile": "Jungko", "torchic": "Poussifeu", "combusken": "Galifeu", "blaziken": "Braségali", "mudkip": "Gobou", "marshtomp": "Flobio", "swampert": "Laggron", "poochyena": "Medhyèna", "mightyena": "Grahyèna", "zigzagoon": "Zigzaton", "linoone": "Linéon", "wurmple": "Chenipotte", "silcoon": "Armulys", "beautifly": "Charmillon", "cascoon": "Blindalys", "dustox": "Papinox", "lotad": "Nénupiot", "seedot": "Grainipiot", "nuzleaf": "Pifeuil", "shiftry": "Tengalice", "taillow": "Nirondelle", "swellow": "Hélédelle", "wingull": "Goélise", "pelipper": "Bekipan", "ralts": "Tarsal", "surskit": "Arakdo", "masquerain": "Maskadra", "shroomish": "Balignon", "breloom": "Chapignon", "slakoth": "Parecool", "slaking": "Monaflèmit", "nincada": "Ningale", "shedinja": "Munja", "whismur": "Chuchmur", "loudred": "Ramboum", "exploud": "Brouhabam", "nosepass": "Tarinor", "sableye": "Ténéfix", "mawile": "Mysdibule", "aron": "Galekid", "lairon": "Galegon", "aggron": "Galeking", "meditite": "Méditikka", "medicham": "Charmina", "electrike": "Dynavolt", "manectric": "Élecsprint", "plusle": "Posipi", "minun": "Négapi", "volbeat": "Muciole", "illumise": "Lumivole", "gulpin": "Gloupti", "swalot": "Avaltout", "numel": "Chamallot", "torkoal": "Chartor", "grumpig": "Groret", "trapinch": "Kraknoix", "vibrava": "Vibraninf", "flygon": "Libégon", "swablu": "Tylton", "zangoose": "Mangriff", "lunatone": "Séléroc", "solrock": "Solaroc", "barboach": "Barloche", "whiscash": "Barbicha", "corphish": "Écrapince", "crawdaunt": "Colhomard", "baltoy": "Balbuto", "claydol": "Kaorine", "lileep": "Lilia", "cradily": "Vacilys", "feebas": "Barpau", "milotic": "Milobellus", "castform": "Morphéo", "shuppet": "Polichombr", "banette": "Branette", "duskull": "Skelénox", "dusclops": "Téraclope", "chimecho": "Éoko", "wynaut": "Okéoké", "snorunt": "Stalgamin", "glalie": "Oniglali", "spheal": "Obalie", "sealeo": "Phogleur", "walrein": "Kaimorse", "clamperl": "Coquiperl", "huntail": "Serpang", "gorebyss": "Rosabyss", "luvdisc": "Lovdisc", "bagon": "Draby", "shelgon": "Drackhaus", "salamence": "Drattak", "beldum": "Terhal", "metagross": "Métalosse", "turtwig": "Tortipouss", "grotle": "Boskara", "chimchar": "Ouisticram", "monferno": "Chimpenfeu", "infernape": "Simiabraz", "piplup": "Tiplouf", "prinplup": "Prinplouf", "empoleon": "Pingoléon", "starly": "Étourmi", "staravia": "Étourvol", "staraptor": "Étouraptor", "bidoof": "Keunotor", "bibarel": "Castorno", "kricketot": "Crikzik", "kricketune": "Mélokrik", "shinx": "Lixy", "budew": "Rozbouton", "cranidos": "Kranidos", "rampardos": "Charkos", "shieldon": "Dinoclier", "burmy": "Cheniti", "wormadam": "Cheniselle", "mothim": "Papilord", "combee": "Apitrini", "vespiquen": "Apireine", "buizel": "Mustébouée", "floatzel": "Mustéflott", "cherubi": "Ceribou", "cherrim": "Ceriflor", "shellos": "Sancoki", "gastrodon": "Tritosor", "ambipom": "Capidextre", "drifloon": "Baudrive", "drifblim": "Grodrive", "buneary": "Laporeille", "lopunny": "Lockpin", "mismagius": "Magirêve", "honchkrow": "Corboss", "glameow": "Chaglam", "purugly": "Chaffreux", "chingling": "Korillon", "stunky": "Moufouette", "skuntank": "Moufflair", "bronzor": "Archéomire", "bronzong": "Archéodong", "bonsly": "Manzaï", "happiny": "Ptiravi", "chatot": "Pijako", "gible": "Griknot", "gabite": "Carmache", "garchomp": "Carchacrok", "munchlax": "Goinfrex", "hippowdon": "Hippodocus", "skorupi": "Rapion", "drapion": "Drascore", "croagunk": "Cradopaud", "toxicroak": "Coatox", "carnivine": "Vortente", "finneon": "Écayon", "mantyke": "Babimanta", "snover": "Blizzi", "abomasnow": "Blizzaroi", "weavile": "Dimoret", "lickilicky": "Coudlangue", "rhyperior": "Rhinastoc", "tangrowth": "Bouldeneu", "electivire": "Élekable", "magmortar": "Maganon", "leafeon": "Phyllali", "glaceon": "Givrali", "gliscor": "Scorvol", "mamoswine": "Mammochon", "gallade": "Gallame", "probopass": "Tarinorme", "dusknoir": "Noctunoir", "froslass": "Momartik", "rotom": "Motisma", "uxie": "Créhelf", "mesprit": "Créfollet", "azelf": "Créfadet", "snivy": "Vipélierre", "servine": "Lianaja", "serperior": "Majaspic", "tepig": "Gruikui", "pignite": "Grotichon", "emboar": "Roitiflam", "oshawott": "Moustillon", "dewott": "Mateloutre", "samurott": "Clamiral", "patrat": "Ratentif", "watchog": "Miradar", "lillipup": "Ponchiot", "herdier": "Ponchien", "stoutland": "Mastouffe", "purrloin": "Chacripan", "liepard": "Léopardus", "pansage": "Feuillajou", "simisage": "Feuiloutan", "pansear": "Flamajou", "simisear": "Flamoutan", "panpour": "Flotajou", "simipour": "Flotoutan", "musharna": "Mushana", "pidove": "Poichigeon", "tranquill": "Colombeau", "unfezant": "Déflaisan", "blitzle": "Zébibron", "zebstrika": "Zéblitz", "roggenrola": "Nodulithe", "boldore": "Géolithe", "gigalith": "Gigalithe", "woobat": "Chovsourir", "swoobat": "Rhinolove", "drilbur": "Rototaupe", "excadrill": "Minotaupe", "audino": "Nanméouïe", "timburr": "Charpenti", "gurdurr": "Ouvrifier", "conkeldurr": "Bétochef", "tympole": "Tritonde", "palpitoad": "Batracné", "seismitoad": "Crapustule", "throh": "Judokrak", "sawk": "Karaclée", "sewaddle": "Larveyette", "swadloon": "Couverdure", "leavanny": "Manternel", "venipede": "Venipatte", "whirlipede": "Scobolide", "scolipede": "Brutapode", "cottonee": "Doudouvet", "whimsicott": "Farfaduvet", "petilil": "Chlorobule", "lilligant": "Fragilady", "basculin": "Bargantua", "sandile": "Mascaïman", "krokorok": "Escroco", "krookodile": "Crocorible", "darumaka": "Darumarond", "darmanitan": "Darumacho", "maractus": "Maracachi", "dwebble": "Crabicoque", "crustle": "Crabaraque", "scraggy": "Baggiguane", "scrafty": "Baggaïd", "sigilyph": "Cryptéro", "yamask": "Tutafeh", "cofagrigus": "Tutankafer", "tirtouga": "Carapagos", "carracosta": "Mégapagos", "archen": "Arkéapti", "archeops": "Aéroptéryx", "trubbish": "Miamiasme", "garbodor": "Miasmax", "minccino": "Chinchidou", "cinccino": "Pashmilla", "gothita": "Scrutella", "gothorita": "Mesmérella", "gothitelle": "Sidérella", "solosis": "Nucléos", "duosion": "Méios", "reuniclus": "Symbios", "ducklett": "Couaneton", "swanna": "Lakmécygne", "vanillite": "Sorbébé", "vanillish": "Sorboul", "vanilluxe": "Sorbouboul", "deerling": "Vivaldaim", "sawsbuck": "Haydaim", "karrablast": "Carabing", "escavalier": "Lançargot", "foongus": "Trompignon", "amoonguss": "Gaulet", "frillish": "Viskuse", "jellicent": "Moyade", "alomomola": "Mamanbo", "joltik": "Statitik", "galvantula": "Mygavolt", "ferroseed": "Grindur", "ferrothorn": "Noacier", "klink": "Tic", "klang": "Clic", "klinklang": "Cliticlic", "tynamo": "Anchwatt", "eelektrik": "Lampéroie", "eelektross": "Ohmassacre", "elgyem": "Lewsor", "beheeyem": "Neitram", "litwick": "Funécire", "lampent": "Mélancolux", "chandelure": "Lugulabre", "axew": "Coupenotte", "fraxure": "Incisache", "haxorus": "Tranchodon", "cubchoo": "Polarhume", "beartic": "Polagriffe", "cryogonal": "Hexagel", "shelmet": "Escargaume", "accelgor": "Limaspeed", "stunfisk": "Limonde", "mienfoo": "Kungfouine", "mienshao": "Shaofouine", "druddigon": "Drakkarmin", "golett": "Gringolem", "golurk": "Golemastoc", "pawniard": "Scalpion", "bisharp": "Scalproie", "bouffalant": "Frison", "rufflet": "Furaiglon", "braviary": "Gueriaigle", "vullaby": "Vostourno", "mandibuzz": "Vaututrice", "heatmor": "Aflamanoir", "durant": "Fermite", "deino": "Solochi", "zweilous": "Diamat", "hydreigon": "Trioxhydre", "larvesta": "Pyronille", "volcarona": "Pyrax", "cobalion": "Cobaltium", "terrakion": "Terrakium", "virizion": "Viridium", "tornadus": "Boréas", "thundurus": "Fulguris", "landorus": "Démétéros", "chespin": "Marisson", "quilladin": "Boguérisse", "chesnaught": "Blindépique", "fennekin": "Feunnec", "braixen": "Roussil", "delphox": "Goupelin", "froakie": "Grenousse", "frogadier": "Croâporal", "greninja": "Amphinobi", "bunnelby": "Sapereau", "diggersby": "Excavarenne", "fletchling": "Passerouge", "fletchinder": "Braisillon", "talonflame": "Flambusard", "scatterbug": "Lépidonille", "spewpa": "Pérégrain", "vivillon": "Prismillon", "litleo": "Hélionceau", "pyroar": "Némélios", "skiddo": "Cabriolaine", "gogoat": "Chevroum", "pancham": "Pandespiègle", "pangoro": "Pandarbare", "furfrou": "Couafarel", "espurr": "Psystigri", "meowstic": "Mistigrix", "honedge": "Monorpale", "doublade": "Dimoclès", "aegislash": "Exagide", "spritzee": "Fluvetin", "aromatisse": "Cocotine", "swirlix": "Sucroquin", "slurpuff": "Cupcanaille", "inkay": "Sepiatop", "malamar": "Sepiatroce", "binacle": "Opermine", "barbaracle": "Golgopathe", "skrelp": "Venalgue", "dragalge": "Kravarech", "clauncher": "Flingouste", "clawitzer": "Gamblast", "helioptile": "Galvaran", "heliolisk": "Iguolta", "tyrunt": "Ptyranidur", "tyrantrum": "Rexillius", "amaura": "Amagara", "aurorus": "Dragmara", "sylveon": "Nymphali", "hawlucha": "Brutalibré", "carbink": "Strassie", "goomy": "Mucuscule", "sliggoo": "Colimucus", "goodra": "Muplodocus", "klefki": "Trousselin", "phantump": "Brocélôme", "trevenant": "Desséliande", "pumpkaboo": "Pitrouille", "gourgeist": "Banshitrouye", "bergmite": "Grelaçon", "avalugg": "Séracrawl", "noibat": "Sonistrelle", "noivern": "Bruyverne", "rowlet": "Brindibou", "dartrix": "Efflèche", "decidueye": "Archéduc", "litten": "Flamiaou", "torracat": "Matoufeu", "incineroar": "Félinferno", "popplio": "Otaquin", "brionne": "Otarlette", "primarina": "Oratoria", "pikipek": "Picassaut", "trumbeak": "Piclairon", "toucannon": "Bazoucan", "yungoos": "Manglouton", "gumshoos": "Argouste", "grubbin": "Larvibule", "charjabug": "Chrysapile", "vikavolt": "Lucanon", "crabrawler": "Crabagarre", "oricorio": "Plumeline", "cutiefly": "Bombydou", "ribombee": "Rubombelle", "rockruff": "Rocabot", "lycanroc": "Lougaroc", "wishiwashi": "Froussardine", "mareanie": "Vorastérie", "toxapex": "Prédastérie", "mudbray": "Tiboudet", "mudsdale": "Bourrinos", "dewpider": "Araqua", "araquanid": "Tarenbulle", "fomantis": "Mimantis", "lurantis": "Floramantis", "morelull": "Spododo", "shiinotic": "Lampignon", "salandit": "Tritox", "salazzle": "Malamandre", "stufful": "Nounourson", "bewear": "Chelours", "bounsweet": "Croquine", "steenee": "Candine", "tsareena": "Sucreine", "comfey": "Guérilande", "oranguru": "Gouroutan", "passimian": "Quartermac", "wimpod": "Sovkipou", "golisopod": "Sarmuraï", "sandygast": "Bacabouh", "palossand": "Trépassable", "pyukumuku": "Concombaffe", "type null": "Type:0", "silvally": "Silvallié", "minior": "Météno", "komala": "Dodoala", "turtonator": "Boumata", "mimikyu": "Mimiqui", "bruxish": "Denticrisse", "drampa": "Draïeul", "dhelmise": "Sinistrail", "jangmoo": "Bébécaille", "hakamoo": "Écaïd", "kommoo": "Ékaïser", "tapu koko": "Tokorico", "tapu lele": "Tokopiyon", "tapu bulu": "Tokotoro", "tapu fini": "Tokopisco", "cosmoem": "Cosmovum", "nihilego": "Zéroïd", "buzzwole": "Mouscoto", "pheromosa": "Cancrelove", "xurkitree": "Câblifère", "celesteela": "Bamboiselle", "kartana": "Katagami", "guzzlord": "Engloutyran", "poipole": "Vémini", "naganadel": "Mandrillon", "stakataka": "Ama-Ama", "blacephalon": "Pierroteknik", "grookey": "Ouistempo", "thwackey": "Badabouin", "rillaboom": "Gorythmic", "scorbunny": "Flambino", "raboot": "Lapyro", "cinderace": "Pyrobut", "sobble": "Larméléon", "drizzile": "Arrozard", "inteleon": "Lézargus", "skwovet": "Rongourmand", "greedent": "Rongrigou", "rookidee": "Minisange", "corvisquire": "Bleuseille", "corviknight": "Corvaillus", "blipbug": "Larvadar", "dottler": "Coléodôme", "orbeetle": "Astronelle", "nickit": "Goupilou", "thievul": "Roublenard", "gossifleur": "Tournicoton", "eldegoss": "Blancoton", "wooloo": "Moumouton", "dubwool": "Moumouflon", "chewtle": "Khélocrok", "drednaw": "Torgamord", "yamper": "Voltoutou", "boltund": "Fulgudog", "rolycoly": "Charbi", "carkol": "Wagomine", "coalossal": "Monthracite", "applin": "Verpom", "flapple": "Pomdrapi", "appletun": "Dratatin", "silicobra": "Dunaja", "sandaconda": "Dunaconda", "cramorant": "Nigosier", "arrokuda": "Embrochet", "barraskewda": "Hastacuda", "toxel": "Toxizap", "toxtricity": "Salarsen", "sizzlipede": "Grillepattes", "centiskorch": "Scolocendre", "clobbopus": "Poulpaf", "grapploct": "Krakos", "sinistea": "Théffroi", "polteageist": "Polthégeist", "hatenna": "Bibichut", "hattrem": "Chapotus", "hatterene": "Sorcilence", "impidimp": "Grimalin", "morgrem": "Fourbelin", "grimmsnarl": "Angoliath", "obstagoon": "Ixon", "perrserker": "Berserkatt", "cursola": "Corayôme", "sirfetchd": "Palarticho", "mr rime": "M. Glaquette", "runerigus": "Tutétékri", "milcery": "Crèmy", "alcremie": "Charmilly", "falinks": "Hexadron", "pincurchin": "Wattapik", "snom": "Frissonille", "frosmoth": "Beldeneige", "stonjourner": "Dolman", "eiscue": "Bekaglaçon", "indeedee": "Wimessir", "cufant": "Charibari", "copperajah": "Pachyradjah", "dracozolt": "Galvagon", "arctozolt": "Galvagla", "dracovish": "Hydragon", "arctovish": "Hydragla", "duraludon": "Duralugon", "dreepy": "Fantyrm", "drakloak": "Dispareptil", "dragapult": "Lanssorien", "eternatus": "Éthernatos", "kubfu": "Wushours", "urshifu": "Shifours", "glastrier": "Blizzeval", "spectrier": "Spectreval", "calyrex": "Sylveroy", "wyrdeer": "Cerbyllin", "kleavor": "Hachécateur", "ursaluna": "Ursaking", "basculegion": "Paragruel", "sneasler": "Farfurex", "overqwil": "Qwilpik", "enamorus": "Amovénus", "sprigatito": "Poussacha", "floragato": "Matourgeon", "meowscarada": "Miascarade", "fuecoco": "Chochodile", "crocalor": "Crocogril", "skeledirge": "Flâmigator", "quaxly": "Coiffeton", "quaxwell": "Canarbello", "quaquaval": "Palmaval", "lechonk": "Gourmelet", "oinkologne": "Fragroin", "tarountula": "Tissenboule", "spidops": "Filentrappe", "nymble": "Lilliterelle", "lokix": "Gambex", "pawmi": "Pohm", "pawmo": "Pohmotte", "pawmot": "Pohmarmotte", "tandemaus": "Compagnol", "maushold": "Famignol", "fidough": "Pâtachiot", "dachsbun": "Briochien", "smoliv": "Olivini", "dolliv": "Olivado", "squawkabilly": "Tapatoès", "nacli": "Selutin", "naclstack": "Amassel", "garganacl": "Gigansel", "charcadet": "Charbambin", "armarouge": "Carmadura", "ceruledge": "Malvalame", "tadbulb": "Têtampoule", "bellibolt": "Ampibidou", "wattrel": "Zapétrel", "kilowattrel": "Fulgulairo", "maschiff": "Grondogue", "mabosstiff": "Dogrino", "shroodle": "Gribouraigne", "grafaiai": "Tag-Tag", "bramblin": "Virovent", "brambleghast": "Virevorreur", "toedscool": "Terracool", "toedscruel": "Terracruel", "klawf": "Craparoi", "capsakid": "Pimito", "scovillain": "Scovilain", "rellor": "Léboulérou", "rabsca": "Bérasca", "flittle": "Flotillon", "espathra": "Cléopsytra", "tinkatink": "Forgerette", "tinkatuff": "Forgella", "tinkaton": "Forgelina", "wiglett": "Taupikeau", "wugtrio": "Triopikeau", "bombirdier": "Lestombaile", "finizen": "Dofin", "palafin": "Superdofin", "varoom": "Vrombi", "revavroom": "Vrombotor", "cyclizar": "Motorizard", "orthworm": "Ferdeter", "glimmet": "Germéclat", "glimmora": "Floréclat", "greavard": "Toutombe", "houndstone": "Tomberro", "flamigo": "Flamenroule", "cetoddle": "Piétacé", "cetitan": "Balbalèze", "veluza": "Délestin", "dondozo": "Oyacata", "tatsugiri": "Nigirigon", "annihilape": "Courrousinge", "clodsire": "Terraiste", "dudunsparce": "Deusolourdo", "kingambit": "Scalpereur", "great tusk": "Fort-Ivoire", "scream tail": "Hurle-Queue", "brute bonnet": "Fongus-Furie", "flutter mane": "Flotte-Mèche", "slither wing": "Rampe-Ailes", "sandy shocks": "Pelage-Sablé", "iron treads": "Roue-de-Fer", "iron bundle": "Hotte-de-Fer", "iron hands": "Paume-de-Fer", "iron jugulis": "Têtes-de-Fer", "iron moth": "Mite-de-Fer", "iron thorns": "Épine-de-Fer", "frigibax": "Frigodo", "arctibax": "Cryodo", "baxcalibur": "Glaivodo", "gimmighoul": "Mordudor", "gholdengo": "Gromago", "wochien": "Chongjian", "chienpao": "Baojian", "tinglu": "Dinglu", "chiyu": "Yuyu", "roaring moon": "Rugit-Lune", "iron valiant": "Garde-de-Fer", "walking wake": "Serpente-Eau", "iron leaves": "Vert-de-Fer", "dipplin": "Pomdramour", "sinistcha": "Théffroyable", "okidogi": "Félicanis", "munkidori": "Fortusimia", "fezandipiti": "Favianos", "archaludon": "Pondralugon", "hydrapple": "Pomdorochi", "gouging fire": "Feu-Perçant", "raging bolt": "Ire-Foudre", "iron boulder": "Roc-de-Fer", "iron crown": "Chef-de-Fer", "pecharunt": "Pêchaminus", "nidoran f": "Nidoran femelle", "nidoran female": "Nidoran femelle", "nidoranf": "Nidoran femelle", "nidoran m": "Nidoran mâle", "nidoranm": "Nidoran mâle", "ash": "Sacha", "ash ketchum": "Sacha", "satoshi": "Sacha", "sacha ketchum": "Sacha", "misty": "Ondine", "kasumi": "Ondine", "brock": "Pierre", "takeshi": "Pierre", "gary": "Régis", "gary oak": "Régis", "shigeru": "Régis", "regis chen": "Régis", "professor oak": "Professeur Chen", "professeur oak": "Professeur Chen", "prof chen": "Professeur Chen", "chen": "Professeur Chen", "samuel oak": "Professeur Chen", "musashi": "Jessie", "kojiro": "James", "tracey sketchit": "Tracey", "may": "Flora", "haruka": "Flora", "masato": "Max", "dawn": "Aurore", "hikari": "Aurore", "cilan": "Rachid", "dent": "Rachid", "clemont": "Lem", "citron": "Lem", "bonnie": "Clem", "eureka": "Clem", "lillie": "Lilie", "mallow": "Barbara", "mao": "Barbara", "lana": "Néphie", "suiren": "Néphie", "sophocles": "Chrys", "mamane": "Chrys", "steven stone": "Pierre Rochard", "daigo": "Pierre Rochard", "lance": "Peter", "wataru": "Peter", "diantha": "Dianthéa", "carnet": "Dianthéa", "leon": "Goyah", "dande": "Goyah", "shinji": "Paul", "shirona": "Cynthia", "lt surge": "Major Bob", "lieutenant surge": "Major Bob", "matis": "Major Bob", "sabrina": "Morgane", "natsume": "Morgane", "blaine": "Auguste", "katsura": "Auguste", "lorelei": "Olga", "kanna": "Olga", "bruno": "Aldo", "shiba": "Aldo", "kikuko": "Agatha", "falkner": "Albert", "hayato": "Albert", "bugsy": "Hector", "tsukushi": "Hector", "whitney": "Blanche", "akane": "Blanche", "morty": "Mortimer", "matsuba": "Mortimer", "shijima": "Chuck", "mikan": "Jasmine", "pryce": "Frédo", "yanagi": "Frédo", "clair": "Sandra", "ibuki": "Sandra", "tsutsuji": "Roxanne", "brawly": "Bastien", "touki": "Bastien", "wattson": "Voltère", "tessen": "Voltère", "flannery": "Adriane", "asuna": "Adriane", "senri": "Norman", "winona": "Alizée", "nagi": "Alizée", "tate": "Lévy", "fu": "Lévy", "liza": "Tatia", "ran": "Tatia", "wallace": "Marc", "mikuri": "Marc", "adan": "Juan", "roark": "Pierrick", "hyouta": "Pierrick", "gardenia": "Flo", "natane": "Flo", "maylene": "Mélina", "sumomo": "Mélina", "crasher wake": "Lovis", "wake": "Lovis", "maxie mask": "Lovis", "fantina": "Kiméra", "melissa": "Kiméra", "byron": "Charles", "tougan": "Charles", "candice": "Gladys", "suzuna": "Gladys", "volkner": "Tanguy", "denji": "Tanguy", "cyrus": "Hélio", "akagi": "Hélio", "ghechis": "Ghetis", "natural harmonia gropius": "N", "flare": "Lysandre", "guzman": "Guzma", "lusamine": "Elsa-Mina", "elsa mina": "Elsa-Mina", "archie": "Arthur", "aogiri": "Arthur", "professor elm": "Professeur Orme", "prof orme": "Professeur Orme", "orme": "Professeur Orme", "utsugi": "Professeur Orme", "professor birch": "Professeur Seko", "prof seko": "Professeur Seko", "seko": "Professeur Seko", "odamaki": "Professeur Seko", "professor rowan": "Professeur Sorbier", "prof sorbier": "Professeur Sorbier", "sorbier": "Professeur Sorbier", "nanakamado": "Professeur Sorbier", "professor juniper": "Professeure Keteleeria", "keteleeria": "Professeure Keteleeria", "araragi": "Professeure Keteleeria", "professor sycamore": "Professeur Platane", "platane": "Professeur Platane", "prof platane": "Professeur Platane", "professor kukui": "Professeur Euphorbe", "kukui": "Professeur Euphorbe", "euphorbe": "Professeur Euphorbe", "professor magnolia": "Professeure Magnolia", "magnolia": "Professeure Magnolia", "nurse joy": "Infirmière Joy", "joy": "Infirmière Joy", "infirmiere joelle": "Infirmière Joy", "officer jenny": "Agent Jenny", "jenny": "Agent Jenny", "junsar": "Agent Jenny"}, "dragonball": {"73": "Seven-Three", "son goku": "Sangoku", "son gohan": "Sangohan", "majin buu": "Majin Boo", "vegito": "Vegeto", "goten": "Sangoten", "krillin": "Krilin", "tien shinhan": "Ten Shin Han", "chiaotzu": "Chaozu", "master roshi": "Tortue Géniale", "mr satan": "Hercule", "uub": "Oob", "bardock": "Baddack", "king vegeta": "Roi Vegeta", "king cold": "Roi Cold", "captain ginyu": "Capitaine Ginyu", "jeice": "Jeece", "burter": "Barta", "android 13": "C-13", "android 16": "C-16", "android 17": "C-17", "android 18": "C-18", "android 19": "C-19", "android 21": "C-21", "dr gero": "Docteur Gero", "supreme kai": "Kaioshin", "grand priest": "Grand Prêtre", "kami": "Tout-Puissant", "korin": "Maître Karin", "launch": "Lunch", "puar": "Plume", "king kai": "Maître Kaio", "guru": "Grand Chef", "omega shenron": "Li Shenron", "nuova shenron": "Su Shenron", "turles": "Tullece", "garlic jr": "Garlic Junior", "mercenary tao": "Tao Pai Pai", "hirudegarn": "Hildegarn", "dr hedo": "Docteur Hedo", "kakarot": "Sangoku", "kakarotto": "Sangoku", "goku ssj": "Sangoku", "super saiyan goku": "Sangoku", "great saiyaman": "Sangohan", "great saiyaman 1": "Sangohan", "petitcoeur": "Piccolo", "petitcur": "Piccolo", "ma junior": "Piccolo", "buu": "Majin Boo", "boo": "Majin Boo", "kid buu": "Majin Boo", "kid boo": "Majin Boo", "super buu": "Majin Boo", "super boo": "Majin Boo", "fat buu": "Majin Boo", "mr buu": "Majin Boo", "boubou": "Majin Boo", "vegetto": "Vegeto", "gogetto": "Gogeta", "son goten": "Sangoten", "kuririn": "Krilin", "tien": "Ten Shin Han", "tenshinhan": "Ten Shin Han", "chaoz": "Chaozu", "muten roshi": "Tortue Géniale", "kame sennin": "Tortue Géniale", "jackie chun": "Tortue Géniale", "mister satan": "Hercule", "majuub": "Oob", "ubb": "Oob", "burdock": "Baddack", "ginyu": "Capitaine Ginyu", "ginyu captain": "Capitaine Ginyu", "jheese": "Jeece", "jiisu": "Jeece", "bata": "Barta", "rikum": "Recoome", "rikoom": "Recoome", "gurdo": "Guldo", "android 14": "C-14", "android 15": "C-15", "android 8": "C-8", "cyborg 16": "C-16", "cyborg 17": "C-17", "cyborg 18": "C-18", "lapis": "C-17", "lazuli": "C-18", "super c17": "C-17", "majin 21": "C-21", "hatchan": "C-8", "eighter": "C-8", "doctor gero": "Docteur Gero", "android 20": "Docteur Gero", "c20": "Docteur Gero", "kaio shin": "Kaioshin", "kibito kai": "Kaioshin", "daishinkan": "Grand Prêtre", "zenoh": "Zeno", "zen oh": "Zeno", "omniking": "Zeno", "zeno sama": "Zeno", "kamisama": "Tout-Puissant", "karin": "Maître Karin", "ranchi": "Lunch", "kaio du nord": "Maître Kaio", "north kai": "Maître Kaio", "kaiosama": "Maître Kaio", "grand doyen": "Grand Chef", "grand elder guru": "Grand Chef", "saichoro": "Grand Chef", "syn shenron": "Li Shenron", "ii shenron": "Li Shenron", "yi xing long": "Li Shenron", "haze shenron": "Ryan Shenron", "eis shenron": "San Shenron", "oceanus shenron": "Uu Shenron", "naturon shenron": "Rō Shenron", "rage shenron": "Chi Shenron", "tarles": "Tullece", "tales": "Tullece", "slug": "Lord Slug", "seigneur slug": "Lord Slug", "taopaipai": "Tao Pai Pai", "cyborg tao": "Tao Pai Pai", "red": "Commander Red", "commandant red": "Commander Red", "blue": "General Blue", "frieza": "Freezer", "freeza": "Freezer", "mecha freezer": "Freezer", "golden freezer": "Freezer", "perfect cell": "Cell", "cell parfait": "Cell", "bills": "Beerus", "birusu": "Beerus", "wiss": "Whis", "broli": "Broly", "yamucha": "Yamcha", "radditz": "Raditz", "zabon": "Zarbon", "dabra": "Dabura", "dispo": "Dyspo", "zamas": "Zamasu", "paragas": "Paragus", "popo": "Mr. Popo", "shenlong": "Shenron", "shen long": "Shenron", "bojak": "Bojack", "kyabe": "Cabba", "kaulifla": "Caulifla", "brianne de chateau": "Ribrianne", "kusu": "Cus", "bulla": "Bra", "grandpa gohan": "Grand-père Sangohan", "son gohan grandpere": "Grand-père Sangohan", "ox king": "Gyumao", "oxsatan": "Gyumao", "satan petitcoeur": "Satan Petit-Cœur", "king piccolo": "Satan Petit-Cœur", "piccolo daimao": "Satan Petit-Cœur", "roi piccolo": "Satan Petit-Cœur", "uranai baba": "Baba", "fortuneteller baba": "Baba", "baba la voyante": "Baba", "arale": "Arale Norimaki", "senbei": "Senbei Norimaki", "snow": "Suno", "ninja murasaki": "Murasaki", "sergent murasaki": "Murasaki", "adjutant black": "Black", "staff officer black": "Black", "king chappa": "Roi Chappa", "master shen": "Tsuru Sennin", "ermite grue": "Tsuru Sennin", "crane hermit": "Tsuru Sennin", "dr brief": "Docteur Brief", "panchy": "Madame Brief", "bunny": "Madame Brief", "king yemma": "Enma", "roi enma": "Enma", "enma daio": "Enma", "princess snake": "Princesse Serpent", "grand kai": "Grand Kaio", "dai kaio": "Grand Kaio", "south kai": "Kaio du Sud", "east kai": "Kaio de l'Est", "west kai": "Kaio de l'Ouest", "old kai": "Vieux Kaioshin", "elder kai": "Vieux Kaioshin", "fasha": "Seripa", "borgos": "Toteppo", "shugesh": "Panbukin", "cui": "Kiwi", "dr wheelo": "Docteur Willow", "dr kochin": "Docteur Kochin", "giru": "Gil", "dr myuu": "Docteur Mu", "general rilldo": "Général Rild", "goku jr": "Sangoku Jr", "son goku jr": "Sangoku Jr", "chirai": "Cheelai", "dr arinsu": "Docteur Arinsu", "king gomah": "Roi Gomah", "monaito": "Monite"}, "hellsparadise": {"sagiri yamada asaemon": "Yamada Asaemon Sagiri", "asaemon sagiri": "Yamada Asaemon Sagiri", "yamada sagiri": "Yamada Asaemon Sagiri", "shion yamada asaemon": "Yamada Asaemon Shion", "asaemon shion": "Yamada Asaemon Shion", "yamada shion": "Yamada Asaemon Shion", "tenza yamada asaemon": "Yamada Asaemon Tenza", "asaemon tenza": "Yamada Asaemon Tenza", "yamada tenza": "Yamada Asaemon Tenza", "fuchi yamada asaemon": "Yamada Asaemon Fuchi", "asaemon fuchi": "Yamada Asaemon Fuchi", "yamada fuchi": "Yamada Asaemon Fuchi", "senta yamada asaemon": "Yamada Asaemon Senta", "asaemon senta": "Yamada Asaemon Senta", "yamada senta": "Yamada Asaemon Senta", "eizen yamada asaemon": "Yamada Asaemon Eizen", "asaemon eizen": "Yamada Asaemon Eizen", "yamada eizen": "Yamada Asaemon Eizen", "genji yamada asaemon": "Yamada Asaemon Genji", "asaemon genji": "Yamada Asaemon Genji", "yamada genji": "Yamada Asaemon Genji", "kisho yamada asaemon": "Yamada Asaemon Kisho", "asaemon kisho": "Yamada Asaemon Kisho", "yamada kisho": "Yamada Asaemon Kisho", "jikka yamada asaemon": "Yamada Asaemon Jikka", "asaemon jikka": "Yamada Asaemon Jikka", "yamada jikka": "Yamada Asaemon Jikka", "shugen yamada asaemon": "Yamada Asaemon Shugen", "asaemon shugen": "Yamada Asaemon Shugen", "yamada shugen": "Yamada Asaemon Shugen", "gabimaru the hollow": "Gabimaru", "gabimaru le vide": "Gabimaru", "chobei": "Aza Chobei", "chobei aza": "Aza Chobei", "toma aza": "Aza Toma", "yamada asaemon toma": "Aza Toma", "asaemon toma": "Aza Toma", "gantetsusai": "Tamiya Gantetsusai", "gantetsusai tamiya": "Tamiya Gantetsusai", "shukin": "Zhu Jin", "bokutan": "Mu Dan", "kikka": "Ju Fa", "toka": "Tao Fa", "keika": "Gui Fa", "ranban": "Lan Ban", "jofuku": "Xu Fu", "ienari": "Tokugawa Ienari"}, "gachiakuta": {"engine": "Enjin", "legto": "Regto Surebrec", "legto surebrec": "Regto Surebrec", "lemlin": "Remlin", "rudo": "Rudo Surebrec"}, "haikyuu": {"hinata shoyo": "Shoyo Hinata", "kageyama tobio": "Tobio Kageyama", "tsukishima kei": "Kei Tsukishima", "nishinoya yu": "Yu Nishinoya", "oikawa toru": "Toru Oikawa", "ushijima wakatoshi": "Wakatoshi Ushijima", "bokuto kotaro": "Kotaro Bokuto", "kozume kenma": "Kenma Kozume", "kuroo tetsuro": "Tetsuro Kuroo", "miya atsumu": "Atsumu Miya", "miya osamu": "Osamu Miya", "sakusa kiyoomi": "Kiyoomi Sakusa", "yamaguchi tadashi": "Tadashi Yamaguchi", "sawamura daichi": "Daichi Sawamura", "sugawara koshi": "Koshi Sugawara", "azumane asahi": "Asahi Azumane", "tanaka ryunosuke": "Ryunosuke Tanaka", "ennoshita chikara": "Chikara Ennoshita", "kinoshita hisashi": "Hisashi Kinoshita", "narita kazuhito": "Kazuhito Narita", "shimizu kiyoko": "Kiyoko Shimizu", "yachi hitoka": "Hitoka Yachi", "takeda ittetsu": "Ittetsu Takeda", "ukai keishin": "Keishin Ukai", "iwaizumi hajime": "Hajime Iwaizumi", "matsukawa issei": "Issei Matsukawa", "hanamaki takahiro": "Takahiro Hanamaki", "watari shinji": "Shinji Watari", "yahaba shigeru": "Shigeru Yahaba", "kindaichi yutaro": "Yutaro Kindaichi", "kunimi akira": "Akira Kunimi", "yaku morisuke": "Morisuke Yaku", "kai nobuyuki": "Nobuyuki Kai", "yamamoto taketora": "Taketora Yamamoto", "fukunaga shohei": "Shohei Fukunaga", "haiba lev": "Lev Haiba", "inuoka so": "So Inuoka", "teshiro tamahiko": "Tamahiko Teshiro", "akaashi keiji": "Keiji Akaashi", "konoha akinori": "Akinori Konoha", "tendo satori": "Satori Tendo", "goshiki tsutomu": "Tsutomu Goshiki", "shirabu kenjiro": "Kenjiro Shirabu", "semi eita": "Eita Semi", "ohira reon": "Reon Ohira", "kawanishi taichi": "Taichi Kawanishi", "kita shinsuke": "Shinsuke Kita", "suna rintaro": "Rintaro Suna", "ojiro aran": "Aran Ojiro", "hoshiumi korai": "Korai Hoshiumi", "hirugami sachiro": "Sachiro Hirugami", "komori motoya": "Motoya Komori", "aone takanobu": "Takanobu Aone", "futakuchi kenji": "Kenji Futakuchi", "koganegawa kanji": "Kanji Koganegawa", "terushima yuji": "Yuji Terushima", "daisho suguru": "Suguru Daisho", "kyotani kentaro": "Kentaro Kyotani", "takinoue yusuke": "Yusuke Takinoue", "shimada makoto": "Makoto Shimada", "tanaka saeko": "Saeko Tanaka", "tsukishima akiteru": "Akiteru Tsukishima", "hinata natsu": "Natsu Hinata", "ukai ikkei": "Ikkei Ukai", "nekomata yasufumi": "Yasufumi Nekomata", "shibayama yuki": "Yuki Shibayama", "washio tatsuki": "Tatsuki Washio", "komi haruki": "Haruki Komi", "onaga wataru": "Wataru Onaga", "sarukui yamato": "Yamato Sarukui", "yamagata hayato": "Hayato Yamagata", "washijo tanji": "Tanji Washijo", "ginjima hitoshi": "Hitoshi Ginjima", "akagi michinari": "Michinari Akagi", "omimi ren": "Ren Omimi", "hakuba gao": "Gao Hakuba", "moniwa kaname": "Kaname Moniwa", "kamasaki yasushi": "Yasushi Kamasaki", "nakashima takeru": "Takeru Nakashima", "hyakuzawa yudai": "Yudai Hyakuzawa", "michimiya yui": "Yui Michimiya", "yachi madoka": "Madoka Yachi", "kageyama miwa": "Miwa Kageyama", "kageyama kazuyo": "Kazuyo Kageyama", "izumi yukitaka": "Yukitaka Izumi", "sekimukai koji": "Koji Sekimukai", "irihata nobuteru": "Nobuteru Irihata", "mizoguchi sadayuki": "Sadayuki Mizoguchi", "oikawa takeru": "Takeru Oikawa", "sakunami kosuke": "Kosuke Sakunami", "obara yutaka": "Yutaka Obara", "sasaya takehito": "Takehito Sasaya", "oiwake takuro": "Takuro Oiwake", "saito soichiro": "Soichiro Saito", "naoi manabu": "Manabu Naoi", "haiba alisa": "Alisa Haiba", "yamamoto akane": "Akane Yamamoto", "suzumeda kaori": "Kaori Suzumeda", "shirofuku yukie": "Yukie Shirofuku", "kiryu wakatsu": "Wakatsu Kiryu", "udai tenma": "Tenma Udai", "meian shugo": "Shugo Meian", "inunaki shion": "Shion Inunaki", "shouyou hinata": "Shoyo Hinata", "hinata shouyou": "Shoyo Hinata", "toorou oikawa": "Toru Oikawa", "tooru oikawa": "Toru Oikawa", "oikawa tooru": "Toru Oikawa", "koutarou bokuto": "Kotaro Bokuto", "bokuto koutarou": "Kotaro Bokuto", "tetsurou kuroo": "Tetsuro Kuroo", "kuroo tetsurou": "Tetsuro Kuroo", "koushi sugawara": "Koshi Sugawara", "sugawara koushi": "Koshi Sugawara", "ryuunosuke tanaka": "Ryunosuke Tanaka", "yuu nishinoya": "Yu Nishinoya", "yuutarou kindaichi": "Yutaro Kindaichi", "kyoutani kentarou": "Kentaro Kyotani", "kentarou kyoutani": "Kentaro Kyotani", "kenjirou shirabu": "Kenjiro Shirabu", "rintarou suna": "Rintaro Suna", "kourai hoshiumi": "Korai Hoshiumi", "sachirou hirugami": "Sachiro Hirugami", "shouhei fukunaga": "Shohei Fukunaga", "sou inuoka": "So Inuoka", "yuuki shibayama": "Yuki Shibayama", "yuuji terushima": "Yuji Terushima", "yuusuke takinoue": "Yusuke Takinoue", "yuudai hyakuzawa": "Yudai Hyakuzawa", "grand roi": "Toru Oikawa", "great king": "Toru Oikawa", "roi du terrain": "Tobio Kageyama", "king of the court": "Tobio Kageyama", "guess monster": "Satori Tendo", "petit geant": "Tenma Udai", "little giant": "Tenma Udai", "ushiwaka": "Wakatoshi Ushijima", "noya": "Yu Nishinoya", "suga": "Koshi Sugawara", "tsukki": "Kei Tsukishima", "coach ukai": "Keishin Ukai"}, "jjk": {"itadori yuji": "Yuji Itadori", "yuuji itadori": "Yuji Itadori", "fushiguro megumi": "Megumi Fushiguro", "kugisaki nobara": "Nobara Kugisaki", "kugisaki": "Nobara Kugisaki", "gojo satoru": "Satoru Gojo", "satoru gojou": "Satoru Gojo", "gojo": "Satoru Gojo", "gojou": "Satoru Gojo", "okkotsu yuta": "Yuta Okkotsu", "yuuta okkotsu": "Yuta Okkotsu", "okkotsu": "Yuta Okkotsu", "zenin maki": "Maki Zenin", "maki zennin": "Maki Zenin", "geto suguru": "Suguru Geto", "suguru getou": "Suguru Geto", "geto": "Suguru Geto", "sukuna": "Ryomen Sukuna", "ryoumen sukuna": "Ryomen Sukuna", "sukuna ryomen": "Ryomen Sukuna", "nanami kento": "Kento Nanami", "nanami": "Kento Nanami", "toji zenin": "Toji Fushiguro", "touji fushiguro": "Toji Fushiguro", "fushiguro toji": "Toji Fushiguro", "todo aoi": "Aoi Todo", "aoi toudou": "Aoi Todo", "todo": "Aoi Todo", "inumaki toge": "Toge Inumaki", "inumaki": "Toge Inumaki", "yaga": "Masamichi Yaga", "ieiri shoko": "Shoko Ieiri", "ieiri": "Shoko Ieiri", "ijichi": "Kiyotaka Ijichi", "kusakabe": "Atsuya Kusakabe", "zenin mai": "Mai Zenin", "miwa": "Kasumi Miwa", "kamo": "Noritoshi Kamo", "nishimiya": "Momo Nishimiya", "iori utahime": "Utahime Iori", "faux geto": "Kenjaku", "kenjaku geto": "Kenjaku", "jougo": "Jogo", "chouso": "Choso", "meimei": "Mei Mei", "tsukumo": "Yuki Tsukumo", "hakari": "Kinji Hakari", "higuruma": "Hiromi Higuruma", "kashimo": "Hajime Kashimo", "gakuganji": "Yoshinobu Gakuganji", "takaba": "Fumihiko Takaba", "hazenoki": "Iori Hazenoki", "ishigori": "Ryu Ishigori", "uro": "Takako Uro", "rika": "Rika Orimoto", "ogami": "Granny Ogami", "mamie ogami": "Granny Ogami", "hosogami": "Smallpox Deity", "divinite de la variole": "Smallpox Deity", "dieu de la variole": "Smallpox Deity", "yoshino junpei": "Junpei Yoshino", "junpei": "Junpei Yoshino"}, "jojo": {"kujo jotaro": "Jotaro Kujo", "jotaro kujoh": "Jotaro Kujo", "kujou joutarou": "Jotaro Kujo", "higashikata josuke": "Josuke Higashikata", "haruno shiobana": "Giorno Giovanna", "jolyne kujo": "Jolyne Cujoh", "jolyne kujoh": "Jolyne Cujoh", "kujo jolyne": "Jolyne Cujoh", "gappy": "Josuke Higashikata Gappy", "josuke higashikata jojolion": "Josuke Higashikata Gappy", "dio": "Dio Brando", "speedwagon": "Robert E. O. Speedwagon", "zeppeli": "Will A. Zeppeli", "william antonio zeppeli": "Will A. Zeppeli", "baron zeppeli": "Will A. Zeppeli", "caesar anthonio zeppeli": "Caesar Zeppeli", "cesar zeppeli": "Caesar Zeppeli", "elizabeth joestar": "Lisa Lisa", "stroheim": "Rudol von Stroheim", "acdc": "Esidisi", "ecidisi": "Esidisi", "wham": "Wamuu", "cars": "Kars", "suzie q": "Suzi Q", "avdol": "Muhammad Avdol", "abdul": "Muhammad Avdol", "mohammed avdol": "Muhammad Avdol", "mohammed abdul": "Muhammad Avdol", "kakyoin": "Noriaki Kakyoin", "polnareff": "Jean Pierre Polnareff", "enya geil": "Enya", "holly kujo": "Holy Kujo", "holly joestar": "Holy Kujo", "daniel darby": "Daniel J. D'Arby", "terence darby": "Telence T. D'Arby", "terence t darby": "Telence T. D'Arby", "telence darby": "Telence T. D'Arby", "hirose koichi": "Koichi Hirose", "nijimura okuyasu": "Okuyasu Nijimura", "kishibe rohan": "Rohan Kishibe", "shigechi": "Shigekiyo Yangu", "otoishi": "Akira Otoishi", "kira yoshikage": "Yoshikage Kira", "angelo": "Anjuro Katagiri", "mikitakazo": "Mikitaka Hazekura", "bucellati": "Bruno Bucciarati", "buccellati": "Bruno Bucciarati", "bruno buccellati": "Bruno Bucciarati", "bruno bucellati": "Bruno Bucciarati", "mista": "Guido Mista", "panacotta fugo": "Pannacotta Fugo", "fugo": "Pannacotta Fugo", "abbacchio": "Leone Abbacchio", "narancia": "Narancia Ghirga", "doppio": "Vinegar Doppio", "mario zucchero": "Zucchero", "leakyeye luca": "Luca", "hermes costello": "Ermes Costello", "ff": "Foo Fighters", "wes bluemarine": "Weather Report", "weather": "Weather Report", "anasui": "Narciso Anasui", "emporio": "Emporio Alnino", "pucci": "Enrico Pucci", "pere pucci": "Enrico Pucci", "donatello versus": "Versus", "johngalli": "Johngalli A", "kenzo": "Kenzou", "julius caesar zeppeli": "Gyro Zeppeli", "gyro": "Gyro Zeppeli", "president valentine": "Funny Valentine", "soundman": "Sandman", "lucy pendleton": "Lucy Steel", "norisuke higashikata": "Norisuke Higashikata IV", "tooru": "Toru", "mister president": "Coco Jumbo", "mr president": "Coco Jumbo"}, "tensura": {"sakaguchi hinata": "Hinata Sakaguchi", "izawa shizue": "Shizue Izawa", "kagurazaka yuuki": "Yuuki Kagurazaka", "taguchi shogo": "Shogo Taguchi", "tachibana kyoya": "Kyoya Tachibana", "mizutani kirara": "Kirara Mizutani", "honjo masayuki": "Masayuki Honjo", "kondo tatsuya": "Tatsuya Kondo", "misaki kenya": "Kenya Misaki", "sekiguchi ryota": "Ryota Sekiguchi", "tanimura shinji": "Shinji Tanimura", "satoru mikami": "Rimuru Tempest", "mikami satoru": "Rimuru Tempest", "veldora": "Veldora Tempest", "storm dragon veldora": "Veldora Tempest", "dragon de la tempete": "Veldora Tempest", "shizu": "Shizue Izawa", "soei": "Souei", "hakurou": "Hakuro", "kurobee": "Kurobe", "gobuta": "Gobta", "kazalim": "Kagali", "noir": "Diablo", "blanc": "Testarossa", "jaune": "Carrera", "violet": "Ultima", "rouge": "Guy Crimson", "kaval": "Kabal", "cabal": "Kabal", "yuki kagurazaka": "Yuuki Kagurazaka", "kagurazaka yuki": "Yuuki Kagurazaka", "ruminas": "Luminous Valentine", "trainy": "Treyni", "mjurran": "Myulan", "gruecith": "Grucius", "fobio": "Phobio", "suphia": "Sufia", "dagrule": "Dagruel", "vesta": "Vester", "chloe obell": "Chloe Aubert", "myourmiles": "Mjöllmile", "mjollmire": "Mjöllmile", "reyhim": "Reyhiem", "middoray": "Middray", "gazel": "Gazel Dwargo", "roi gazel": "Gazel Dwargo", "masayuki honjou": "Masayuki Honjo", "kyouya tachibana": "Kyoya Tachibana", "shougo taguchi": "Shogo Taguchi", "elmesia": "Elmesia El-Ru Sarion", "elen": "Eren Grimwald", "elen grimwald": "Eren Grimwald", "abil": "Abiru", "souka": "Soka", "arnaud": "Arnaud Baumann"}, "opm": {"caped baldy": "Saitama", "chauve cape": "Saitama", "one punch man": "Saitama", "demon cyborg": "Genos", "cyborg demoniaque": "Genos", "cyborg demon": "Genos", "terrible tornado": "Tatsumaki", "tornade de la terreur": "Tatsumaki", "senritsu no tatsumaki": "Tatsumaki", "silver fang": "Bang", "croc dargent": "Bang", "blizzard of hell": "Fubuki", "hellish blizzard": "Fubuki", "blizzard infernal": "Fubuki", "hero hunter": "Garou", "chasseur de heros": "Garou", "lord boros": "Boros", "licenseless rider": "Mumen Rider", "mumen raider": "Mumen Rider", "cycliste sans licence": "Mumen Rider", "senko no flash": "Flashy Flash", "kamikaze": "Atomic Samurai", "empereur enfant": "Child Emperor", "bofoi": "Metal Knight", "dr bofoi": "Metal Knight", "zombie man": "Zombieman", "butagami": "Pig God", "darkshine": "Superalloy Darkshine", "watchdog": "Watchdog Man", "badd": "Metal Bat", "sweet mask": "Amai Mask", "sonic": "Speed-o'-Sound Sonic", "onsoku no sonic": "Speed-o'-Sound Sonic", "monster king orochi": "Orochi", "roi des monstres orochi": "Orochi", "senior centipede": "Elder Centipede", "snake": "Snek", "sneck": "Snek", "roi des profondeurs": "Deep Sea King", "genus": "Dr. Genus", "kabuto": "Carnage Kabuto", "three section staff lily": "Sansetsukon Lily", "rover": "Overgrown Rover", "kuseno": "Dr. Kuseno", "jet niceguy": "Jet Nice Guy", "chain n toad": "Chain'n'Toad"}, "sao": {"seijirou kikouka": "Seijirou Kikuoka", "seijiro kikuoka": "Seijirou Kikuoka", "kikuoka": "Seijirou Kikuoka", "black swordsman": "Kirito", "epeiste noir": "Kirito", "kirigaya kazuto": "Kirito", "asuna yuki": "Asuna Yuuki", "yuuki asuna": "Asuna Yuuki", "yuki asuna": "Asuna Yuuki", "stacia": "Asuna Yuuki", "asuna the flash": "Asuna Yuuki", "yuki konno": "Yuuki Konno", "konno yuuki": "Yuuki Konno", "konno yuki": "Yuuki Konno", "absolute sword": "Yuuki Konno", "zekken": "Yuuki Konno", "yuuki": "Yuuki Konno", "yuki": "Yuuki Konno", "solus": "Sinon", "asada shino": "Sinon", "terraria": "Leafa", "ryoutarou tsuboi": "Klein", "ryotaro tsuboi": "Klein", "andrew gilbert mills": "Agil", "alice synthesis thirty": "Alice Zuberg", "yui mhcp001": "Yui", "argo the rat": "Argo", "argo le rat": "Argo", "sterben": "Death Gun", "shouichi shinkawa": "XaXa", "shoichi shinkawa": "XaXa", "redeyed xaxa": "XaXa", "atsushi kanemoto": "Johnny Black", "prince of hell": "Vassago Casals", "vecta": "Gabriel Miller", "subtilizer": "Gabriel Miller", "quinnella": "Administrator", "bercouli": "Bercouli Synthesis One", "fanatio": "Fanatio Synthesis Two", "shinichi nagata": "Recon", "karen kohiruimaki": "Llenn", "elsa kanzaki": "Pitohui", "miyu shinohara": "Fukaziroh", "goushi asougi": "M", "nautilus": "Eiji", "eiji nochizawa": "Eiji", "yuna shigemura": "Yuna", "rinko kojiro": "Rinko Koujiro", "sheyta synthesis twelve": "Sheyta", "renly": "Renly Synthesis Twenty-Seven", "eldrie": "Eldrie Synthesis Thirty-One", "spiegel": "Kyouji Shinkawa", "kyoji shinkawa": "Kyouji Shinkawa", "tiese schtrinen": "Tiese Shtolienen", "ronye arabel": "Ronie Arabel", "ronye": "Ronie Arabel", "liena": "Sortiliena Serlut", "fizel synthesis twentynine": "Fizel", "linel synthesis twentyeight": "Linel", "boss": "Eva", "higa takeru": "Takeru Higa", "corvatz": "Kobatz", "kyoko yuuki": "Kyouko Yuuki", "koichiro yuuki": "Kouichirou Yuuki", "shozo yuuki": "Shouzou Yuuki", "goushi": "M"}, "tokyoghoul": {"kaneki ken": "Ken Kaneki", "centipede": "Ken Kaneki", "eyepatch": "Ken Kaneki", "oneeyed king": "Ken Kaneki", "roi borgne": "Ken Kaneki", "toka kirishima": "Touka Kirishima", "kirishima touka": "Touka Kirishima", "kisho arima": "Kishou Arima", "arima kishou": "Kishou Arima", "arima": "Kishou Arima", "juzo suzuya": "Juuzou Suzuya", "rei suzuya": "Juuzou Suzuya", "suzuya": "Juuzou Suzuya", "sen takatsuki": "Eto Yoshimura", "takatsuki sen": "Eto Yoshimura", "oneeyed owl": "Eto Yoshimura", "hibou borgne": "Eto Yoshimura", "shuu tsukiyama": "Shu Tsukiyama", "tsukiyama shuu": "Shu Tsukiyama", "black rabbit": "Ayato Kirishima", "koutarou amon": "Kotaro Amon", "amon": "Kotaro Amon", "nishio": "Nishiki Nishio", "nishio nishiki": "Nishiki Nishio", "binge eater": "Rize Kamishiro", "hide": "Hideyoshi Nagachika", "nagachika": "Hideyoshi Nagachika", "kuzen yoshimura": "Yoshimura", "yomo": "Renji Yomo", "no face": "Uta", "black dog": "Kaya Irimi", "devil ape": "Enji Koma", "mado akira": "Akira Mado", "seido takizawa": "Seidou Takizawa", "takizawa": "Seidou Takizawa", "shinohara": "Yukinori Shinohara", "yakumo omori": "Yakumo Oomori", "urie": "Kuki Urie", "shirazu": "Ginshi Shirazu", "toru mutsuki": "Tooru Mutsuki", "mutsuki": "Tooru Mutsuki", "saiko": "Saiko Yonebayashi", "kichimura washuu": "Nimura Furuta", "souta": "Nimura Furuta", "furuta": "Nimura Furuta", "karren von rosewald": "Kanae von Rosewald", "banjo": "Kazuichi Banjou", "kazuichi banjo": "Kazuichi Banjou", "kousuke houji": "Houji Kasuka", "kasuka houji": "Houji Kasuka", "hirako": "Take Hirako", "hsiao ching li": "Hsiao", "ching li hsiao": "Hsiao", "tsuneyoshi washu": "Tsuneyoshi Washuu", "matsuri washu": "Matsuri Washuu", "yoshitoki washu": "Yoshitoki Washuu", "marude": "Tokuyuki Marude", "kanou": "Akihiro Kanou", "akihiro kano": "Akihiro Kanou", "jason": "Yakumo Oomori"}, "tokyorevengers": {"takemitchy": "Takemichi Hanagaki", "takemicchi": "Takemichi Hanagaki", "hanagaki takemichi": "Takemichi Hanagaki", "sano manjiro": "Manjiro Sano", "ryuguji ken": "Ken Ryuguji", "ryuuguuji ken": "Ken Ryuguji", "baji": "Keisuke Baji", "baji keisuke": "Keisuke Baji", "matsuno chifuyu": "Chifuyu Matsuno", "mitsuya": "Takashi Mitsuya", "hanemiya kazutora": "Kazutora Hanemiya", "kisaki": "Tetta Kisaki", "kisaki tetta": "Tetta Kisaki", "kurokawa izana": "Izana Kurokawa", "shiba taiju": "Taiju Shiba", "akkun": "Atsushi Sendo", "hina": "Hinata Tachibana", "tachibana hinata": "Hinata Tachibana", "tachibana naoto": "Naoto Tachibana", "hanma": "Shuji Hanma", "osanai": "Nobutaka Osanai", "inupi": "Seishu Inui", "inui": "Seishu Inui", "koko": "Hajime Kokonoi", "kokonoi": "Hajime Kokonoi", "kakucho hitto": "Kakucho", "hitto kakucho": "Kakucho", "kakuchou": "Kakucho", "mochizuki": "Kanji Mochizuki", "madarame": "Shion Madarame", "sanzu": "Haruchiyo Sanzu", "haruchiyo akashi": "Haruchiyo Sanzu", "terano south": "South Terano", "senju akashi": "Senju Kawaragi", "waka": "Wakasa Imaushi", "benkei": "Keizo Arashi", "kiyomasa": "Masataka Kiyomizu", "yamagishi": "Kazushi Yamagishi"}};
...
// ================= AnimeDLE V20 — univers complétés =================
// ~500 fiches supplémentaires (6 catégories). "Inconnu(e)" = donnée jamais révélée dans l'œuvre.
const DLE_STATIC_PROFILES_V20 = {"bleach": {"Ganju Shiba": {"c0": "Âme", "c1": "Clan Shiba", "c2": "Aucune", "c3": "Kidō / feux d'artifice", "c4": "Non", "c5": "Rukongai"}, "Kukaku Shiba": {"c0": "Âme", "c1": "Clan Shiba", "c2": "Aucune", "c3": "Feux d'artifice", "c4": "Non", "c5": "Rukongai"}, "Hanataro Yamada": {"c0": "Shinigami", "c1": "Gotei 13", "c2": "4e", "c3": "Hisagomaru", "c4": "Non", "c5": "Soul Society"}, "Marechiyo Omaeda": {"c0": "Shinigami", "c1": "Gotei 13", "c2": "2e", "c3": "Gegetsuburi", "c4": "Non", "c5": "Soul Society"}, "Nemu Kurotsuchi": {"c0": "Âme artificielle", "c1": "Gotei 13", "c2": "12e", "c3": "Corps artificiel", "c4": "Non", "c5": "Soul Society"}, "Chojiro Sasakibe": {"c0": "Shinigami", "c1": "Gotei 13", "c2": "1re", "c3": "Gonryomaru", "c4": "Oui", "c5": "Soul Society"}, "Isane Kotetsu": {"c0": "Shinigami", "c1": "Gotei 13", "c2": "4e", "c3": "Itegumo", "c4": "Non", "c5": "Soul Society"}, "Love Aikawa": {"c0": "Shinigami", "c1": "Gotei 13", "c2": "7e", "c3": "Tengumaru", "c4": "Inconnu", "c5": "Soul Society"}, "Rōjūrō Otoribashi": {"c0": "Shinigami", "c1": "Gotei 13", "c2": "3e", "c3": "Kinshara", "c4": "Oui", "c5": "Soul Society"}, "Lisa Yadomaru": {"c0": "Shinigami", "c1": "Gotei 13", "c2": "8e", "c3": "Haguro Tonbo", "c4": "Inconnu", "c5": "Soul Society"}, "Hachigen Ushoda": {"c0": "Shinigami", "c1": "Visored", "c2": "Ex-8e", "c3": "Kidō", "c4": "Inconnu", "c5": "Soul Society"}, "Mashiro Kuna": {"c0": "Shinigami", "c1": "Visored", "c2": "Ex-9e", "c3": "Combat au corps à corps", "c4": "Inconnu", "c5": "Soul Society"}, "Kon": {"c0": "Âme modifiée", "c1": "Karakura", "c2": "Aucune", "c3": "Force des jambes", "c4": "Non", "c5": "Soul Society"}, "Yasutora Sado": {"c0": "Humain", "c1": "Karakura", "c2": "Aucune", "c3": "Brazo Derecha del Gigante", "c4": "Non", "c5": "Karakura"}, "Karin Kurosaki": {"c0": "Humaine", "c1": "Karakura", "c2": "Aucune", "c3": "Aucune", "c4": "Non", "c5": "Karakura"}, "Yuzu Kurosaki": {"c0": "Humaine", "c1": "Karakura", "c2": "Aucune", "c3": "Aucune", "c4": "Non", "c5": "Karakura"}, "Masaki Kurosaki": {"c0": "Quincy", "c1": "Karakura", "c2": "Aucune", "c3": "Arc", "c4": "Non", "c5": "Karakura"}, "Keigo Asano": {"c0": "Humain", "c1": "Karakura", "c2": "Aucune", "c3": "Aucune", "c4": "Non", "c5": "Karakura"}, "Soken Ishida": {"c0": "Quincy", "c1": "Karakura", "c2": "Aucune", "c3": "Arc", "c4": "Non", "c5": "Karakura"}, "Kugo Ginjo": {"c0": "Humain (Fullbringer)", "c1": "Xcution", "c2": "Aucune", "c3": "Cross of Scaffold", "c4": "Non", "c5": "Karakura"}, "Shukuro Tsukishima": {"c0": "Humain (Fullbringer)", "c1": "Xcution", "c2": "Aucune", "c3": "Book of the End", "c4": "Non", "c5": "Karakura"}, "Riruka Dokugamine": {"c0": "Humaine (Fullbringer)", "c1": "Xcution", "c2": "Aucune", "c3": "Dollhouse", "c4": "Non", "c5": "Karakura"}, "Grand Fisher": {"c0": "Hollow", "c1": "Hollow", "c2": "Aucune", "c3": "Leurre", "c4": "Non", "c5": "Hueco Mundo"}, "Yammy Llargo": {"c0": "Arrancar", "c1": "Espada", "c2": "10e Espada", "c3": "Ira", "c4": "Non", "c5": "Hueco Mundo"}, "Aaroniero Arruruerie": {"c0": "Arrancar", "c1": "Espada", "c2": "9e Espada", "c3": "Glotonería", "c4": "Non", "c5": "Hueco Mundo"}, "Zommari Rureaux": {"c0": "Arrancar", "c1": "Espada", "c2": "7e Espada", "c3": "Brujería", "c4": "Non", "c5": "Hueco Mundo"}, "Baraggan Louisenbairn": {"c0": "Arrancar", "c1": "Espada", "c2": "2e Espada", "c3": "Arrogante", "c4": "Non", "c5": "Hueco Mundo"}, "Wonderweiss Margela": {"c0": "Arrancar", "c1": "Arrancar", "c2": "Aucune", "c3": "Extinguir", "c4": "Non", "c5": "Hueco Mundo"}, "Nelliel Tu Odelschwanck": {"c0": "Arrancar", "c1": "Ex-Espada", "c2": "Ex-3e Espada", "c3": "Gamuza", "c4": "Non", "c5": "Hueco Mundo"}, "Luppi Antenor": {"c0": "Arrancar", "c1": "Espada", "c2": "Ex-6e Espada", "c3": "Trepadora", "c4": "Non", "c5": "Hueco Mundo"}, "Ggio Vega": {"c0": "Arrancar", "c1": "Fracción", "c2": "Aucune", "c3": "Tigre Estoque", "c4": "Non", "c5": "Hueco Mundo"}, "Lilynette Gingerbuck": {"c0": "Arrancar", "c1": "Espada", "c2": "1re Espada", "c3": "Los Lobos", "c4": "Non", "c5": "Hueco Mundo"}, "Bambietta Basterbine": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter E", "c3": "The Explode", "c4": "Non", "c5": "Empire Quincy"}, "Candice Catnipp": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter T", "c3": "The Thunderbolt", "c4": "Non", "c5": "Empire Quincy"}, "Liltotto Lamperd": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter G", "c3": "The Glutton", "c4": "Non", "c5": "Empire Quincy"}, "Giselle Gewelle": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter Z", "c3": "The Zombie", "c4": "Non", "c5": "Empire Quincy"}, "Meninas McAllon": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter P", "c3": "The Power", "c4": "Non", "c5": "Empire Quincy"}, "As Nodt": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter F", "c3": "The Fear", "c4": "Non", "c5": "Empire Quincy"}, "Mask De Masculine": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter S", "c3": "The Superstar", "c4": "Non", "c5": "Empire Quincy"}, "Gremmy Thoumeaux": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter V", "c3": "The Visionary", "c4": "Non", "c5": "Empire Quincy"}, "Pernida Parnkgjas": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter C", "c3": "The Compulsory", "c4": "Non", "c5": "Empire Quincy"}, "Quilge Opie": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter J", "c3": "The Jail", "c4": "Non", "c5": "Empire Quincy"}, "Cang Du": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter I", "c3": "The Iron", "c4": "Non", "c5": "Empire Quincy"}, "Driscoll Berci": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter O", "c3": "The Overkill", "c4": "Non", "c5": "Empire Quincy"}, "Robert Accutrone": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter N", "c3": "The Nightmare", "c4": "Non", "c5": "Empire Quincy"}, "NaNaNa Najahkoop": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter U", "c3": "The Underbelly", "c4": "Non", "c5": "Empire Quincy"}, "Berenice Gabrielli": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter Q", "c3": "The Question", "c4": "Non", "c5": "Empire Quincy"}, "Nianzol Weizol": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter W", "c3": "The Wind", "c4": "Non", "c5": "Empire Quincy"}, "Jerome Guizbatt": {"c0": "Quincy", "c1": "Wandenreich", "c2": "Sternritter R", "c3": "The Roar", "c4": "Non", "c5": "Empire Quincy"}, "Oetsu Nimaiya": {"c0": "Shinigami", "c1": "Division Zéro", "c2": "Zéro", "c3": "Forge des zanpakuto", "c4": "Inconnu", "c5": "Soul Society"}, "Senjumaru Shutara": {"c0": "Shinigami", "c1": "Division Zéro", "c2": "Zéro", "c3": "Shatatsu Karagara Shigarami no Tsuji", "c4": "Oui", "c5": "Soul Society"}, "Tenjiro Kirinji": {"c0": "Shinigami", "c1": "Division Zéro", "c2": "Zéro", "c3": "Kinpika", "c4": "Inconnu", "c5": "Soul Society"}, "Kirio Hikifune": {"c0": "Shinigami", "c1": "Division Zéro", "c2": "Zéro", "c3": "Nourriture spirituelle", "c4": "Inconnu", "c5": "Soul Society"}, "Tessai Tsukabishi": {"c0": "Shinigami", "c1": "Ex-Gotei 13", "c2": "Ex-Corps de Kidō", "c3": "Kidō", "c4": "Inconnu", "c5": "Soul Society"}}, "bluelock": {"Ryosuke Kira": {"c0": "Attaquant", "c1": "Droit", "c2": "Technique", "c3": "Aucune", "c4": "Non", "c5": "Brun"}, "Anri Teieri": {"c0": "Staff", "c1": "Aucun", "c2": "Organisation", "c3": "Blue Lock", "c4": "Non", "c5": "Brun"}}, "chainsaw": {"Bat Devil": {"c0": "Démon", "c1": "Aucune", "c2": "Bat", "c3": "Non", "c4": "Indéterminé", "c5": "Aucun"}, "Leech Devil": {"c0": "Démon", "c1": "Aucune", "c2": "Leech", "c3": "Non", "c4": "Femme", "c5": "Aucun"}, "Eternity Devil": {"c0": "Démon", "c1": "Aucune", "c2": "Eternity", "c3": "Non", "c4": "Indéterminé", "c5": "Aucun"}, "Tomato Devil": {"c0": "Démon", "c1": "Aucune", "c2": "Tomato", "c3": "Non", "c4": "Indéterminé", "c5": "Aucun"}, "Zombie Devil": {"c0": "Démon", "c1": "Aucune", "c2": "Zombie", "c3": "Non", "c4": "Indéterminé", "c5": "Aucun"}, "Gun Devil": {"c0": "Démon", "c1": "Gouvernements", "c2": "Gun", "c3": "Non", "c4": "Indéterminé", "c5": "Aucun"}, "Darkness Devil": {"c0": "Démon", "c1": "Primordiaux", "c2": "Darkness", "c3": "Non", "c4": "Indéterminé", "c5": "Aucun"}, "Falling Devil": {"c0": "Démon", "c1": "Primordiaux", "c2": "Falling", "c3": "Non", "c4": "Femme", "c5": "Aucun"}, "Justice Devil": {"c0": "Démon", "c1": "Contrats", "c2": "Justice", "c3": "Non", "c4": "Indéterminé", "c5": "Aucun"}, "Hell Devil": {"c0": "Démon", "c1": "Primordiaux", "c2": "Hell", "c3": "Non", "c4": "Indéterminé", "c5": "Aucun"}, "Aging Devil": {"c0": "Démon", "c1": "Primordiaux", "c2": "Aging", "c3": "Non", "c4": "Indéterminé", "c5": "Aucun"}, "Michiko Tendo": {"c0": "Humaine", "c1": "Public Safety", "c2": "Inconnu", "c3": "Non", "c4": "Femme", "c5": "Noir"}, "Yuko": {"c0": "Humaine / hôte de Justice", "c1": "Étudiante", "c2": "Justice", "c3": "Non", "c4": "Femme", "c5": "Noir"}, "Haruka Iseumi": {"c0": "Humain", "c1": "Club Chainsaw Man", "c2": "Aucun", "c3": "Non", "c4": "Homme", "c5": "Noir"}, "Nobana Higashiyama": {"c0": "Humaine", "c1": "Étudiante", "c2": "Aucun", "c3": "Non", "c4": "Femme", "c5": "Noir"}}, "clover": {"Kirsch Vermillion": {"c0": "Paon de Corail", "c1": "Fleurs", "c2": "3 feuilles", "c3": "Clover", "c4": "Royal", "c5": "Aucun"}, "Sol Marron": {"c0": "Rose Bleue", "c1": "Terre", "c2": "3 feuilles", "c3": "Clover", "c4": "Roturier", "c5": "Aucun"}, "Lolopechka": {"c0": "Royaume de Heart", "c1": "Eau", "c2": "4 feuilles", "c3": "Heart", "c4": "Royal", "c5": "Esprit"}, "Acier Silva": {"c0": "Chevaliers-mages", "c1": "Eau", "c2": "3 feuilles", "c3": "Clover", "c4": "Royal", "c5": "Aucun"}, "Zenon Zogratis": {"c0": "Triade Sombre", "c1": "Os / Espace", "c2": "5 feuilles", "c3": "Spade", "c4": "Royal usurpateur", "c5": "Zagred"}, "Megicula": {"c0": "Démon", "c1": "Malédiction", "c2": "Aucun", "c3": "Monde souterrain", "c4": "Aucun", "c5": "Démon supérieur"}, "Sally": {"c0": "Œil du Soleil de Minuit", "c1": "Bulle", "c2": "3 feuilles", "c3": "Clover", "c4": "Aucun", "c5": "Aucun"}, "Vetto": {"c0": "Œil du Soleil de Minuit", "c1": "Bête", "c2": "3 feuilles", "c3": "Clover", "c4": "Aucun", "c5": "Aucun"}}, "cote": {"Tomonari Mashima": {"c0": "Professeur classe A", "c1": "Professeur", "c2": "Non", "c3": "Non", "c4": "Homme", "c5": "Noir"}, "Hideo Sotomura": {"c0": "D", "c1": "1re", "c2": "Non", "c3": "Non", "c4": "Homme", "c5": "Noir"}}, "deathnote": {"Masahiko Kida": {"c0": "Yotsuba", "c1": "Membre du groupe Yotsuba", "c2": "Non", "c3": "Non", "c4": "Cadre Yotsuba", "c5": "Humain"}, "Shingo Mido": {"c0": "Yotsuba", "c1": "Membre du groupe Yotsuba", "c2": "Non", "c3": "Non", "c4": "Cadre Yotsuba", "c5": "Humain"}, "Arayoshi Hatori": {"c0": "Yotsuba", "c1": "Membre du groupe Yotsuba", "c2": "Non", "c3": "Non", "c4": "Cadre Yotsuba", "c5": "Humain"}, "Eiichi Takahashi": {"c0": "Yotsuba", "c1": "Membre du groupe Yotsuba", "c2": "Non", "c3": "Non", "c4": "Cadre Yotsuba", "c5": "Humain"}, "Suguru Shimura": {"c0": "Yotsuba", "c1": "Membre du groupe Yotsuba", "c2": "Non", "c3": "Non", "c4": "Cadre Yotsuba", "c5": "Humain"}, "Takeshi Ooi": {"c0": "Yotsuba", "c1": "Membre du groupe Yotsuba", "c2": "Non", "c3": "Non", "c4": "Cadre Yotsuba", "c5": "Humain"}, "Roger Ruvie": {"c0": "Enquête", "c1": "Directeur de la Wammy's House", "c2": "Non", "c3": "Non", "c4": "Directeur d'orphelinat", "c5": "Humain"}, "Jealous": {"c0": "Shinigami", "c1": "Shinigami", "c2": "Oui", "c3": "Oui natifs", "c4": "Shinigami", "c5": "Shinigami"}, "Gook": {"c0": "Shinigami", "c1": "Shinigami", "c2": "Oui", "c3": "Oui natifs", "c4": "Shinigami", "c5": "Shinigami"}, "Zellogi": {"c0": "Shinigami", "c1": "Shinigami", "c2": "Oui", "c3": "Oui natifs", "c4": "Shinigami", "c5": "Shinigami"}, "Calikarcha": {"c0": "Shinigami", "c1": "Shinigami", "c2": "Oui", "c3": "Oui natifs", "c4": "Shinigami", "c5": "Shinigami"}, "Deridovely": {"c0": "Shinigami", "c1": "Shinigami", "c2": "Oui", "c3": "Oui natifs", "c4": "Shinigami", "c5": "Shinigami"}, "Midora": {"c0": "Shinigami", "c1": "Shinigami", "c2": "Oui", "c3": "Oui natifs", "c4": "Shinigami", "c5": "Shinigami"}, "Nu": {"c0": "Shinigami", "c1": "Shinigami", "c2": "Oui", "c3": "Oui natifs", "c4": "Shinigami", "c5": "Shinigami"}}, "demonslayer": {"Aoi Kanzaki": {"c0": "Pourfendeurs", "c1": "Soigneuse", "c2": "Aucun", "c3": "Non", "c4": "Non", "c5": "Noir"}, "Murata": {"c0": "Pourfendeurs", "c1": "Membre", "c2": "Eau", "c3": "Non", "c4": "Non", "c5": "Noir"}, "Susamaru": {"c0": "Démons", "c1": "—", "c2": "Art du sang (temari)", "c3": "Non", "c4": "Oui", "c5": "Noir"}, "Nakime": {"c0": "Démons", "c1": "Lune Supérieure 4", "c2": "Art du sang (forteresse infinie)", "c3": "Non", "c4": "Oui", "c5": "Noir"}, "Hand Demon": {"c0": "Démons", "c1": "—", "c2": "Art du sang (mains)", "c3": "Non", "c4": "Oui", "c5": "Aucun"}, "Tanjuro Kamado": {"c0": "Famille Kamado", "c1": "Civil", "c2": "Danse du dieu du feu", "c3": "Non", "c4": "Non", "c5": "Rouge"}, "Kie Kamado": {"c0": "Famille Kamado", "c1": "Civile", "c2": "Aucun", "c3": "Non", "c4": "Non", "c5": "Noir"}, "Kiriya Ubuyashiki": {"c0": "Pourfendeurs", "c1": "Chef", "c2": "Aucun", "c3": "Non", "c4": "Non", "c5": "Blanc"}, "Kanata Ubuyashiki": {"c0": "Pourfendeurs", "c1": "Famille Ubuyashiki", "c2": "Aucun", "c3": "Non", "c4": "Non", "c5": "Blanc"}, "Amane Ubuyashiki": {"c0": "Pourfendeurs", "c1": "Famille Ubuyashiki", "c2": "Aucun", "c3": "Non", "c4": "Non", "c5": "Blanc"}, "Hinatsuru": {"c0": "Pourfendeurs", "c1": "Kunoichi", "c2": "Aucun", "c3": "Non", "c4": "Non", "c5": "Noir"}, "Makio": {"c0": "Pourfendeurs", "c1": "Kunoichi", "c2": "Aucun", "c3": "Non", "c4": "Non", "c5": "Noir"}, "Suma": {"c0": "Pourfendeurs", "c1": "Kunoichi", "c2": "Aucun", "c3": "Non", "c4": "Non", "c5": "Noir"}}, "dragonball": {"Kami": {"c0": "Namek", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Piccolo", "c5": "Aucun"}, "Mr. Popo": {"c0": "Djinn", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Chauve"}, "Korin": {"c0": "Chat", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Blanc"}, "Yajirobe": {"c0": "Humain", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Noir"}, "Launch": {"c0": "Humaine", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Bleu/Blond"}, "Oolong": {"c0": "Cochon", "c1": "Héros", "c2": "Métamorphose", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Puar": {"c0": "Chat", "c1": "Héros", "c2": "Métamorphose", "c3": "7", "c4": "Non", "c5": "Aucun"}, "King Kai": {"c0": "Kaio", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Guru": {"c0": "Namek", "c1": "Neutre", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Android 13": {"c0": "Androïde", "c1": "Antagoniste", "c2": "Super Android 13", "c3": "7", "c4": "Non", "c5": "Orange"}, "Android 19": {"c0": "Androïde", "c1": "Antagoniste", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Chauve"}, "Dr. Gero": {"c0": "Androïde", "c1": "Antagoniste", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Blanc"}, "Android 21": {"c0": "Androïde / Majin", "c1": "Variable", "c2": "Forme Majin", "c3": "7", "c4": "Non", "c5": "Brun"}, "Baby": {"c0": "Tsufurien", "c1": "Antagoniste", "c2": "Oozaru doré", "c3": "7", "c4": "Non", "c5": "Blanc"}, "Omega Shenron": {"c0": "Dragon maléfique", "c1": "Antagoniste", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Shenron": {"c0": "Dragon", "c1": "Neutre", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Porunga": {"c0": "Dragon", "c1": "Neutre", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Bojack": {"c0": "Extraterrestre", "c1": "Antagoniste", "c2": "Full Power", "c3": "7", "c4": "Non", "c5": "Rouge"}, "Turles": {"c0": "Saiyan", "c1": "Antagoniste", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Noir"}, "Lord Slug": {"c0": "Namek", "c1": "Antagoniste", "c2": "Géant", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Garlic Jr.": {"c0": "Makyan", "c1": "Antagoniste", "c2": "Forme géante", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Pilaf": {"c0": "Extraterrestre", "c1": "Antagoniste", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Mai": {"c0": "Humaine", "c1": "Antagoniste", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Noir"}, "Mercenary Tao": {"c0": "Humain / cyborg", "c1": "Antagoniste", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Noir"}, "Upa": {"c0": "Humain", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Noir"}, "Jaco": {"c0": "Extraterrestre", "c1": "Patrouille galactique", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Sorbet": {"c0": "Extraterrestre", "c1": "Armée de Freezer", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Tagoma": {"c0": "Extraterrestre", "c1": "Armée de Freezer", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Chauve"}, "Recoome": {"c0": "Extraterrestre", "c1": "Armée de Freezer", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Roux"}, "Jeice": {"c0": "Extraterrestre", "c1": "Armée de Freezer", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Blanc"}, "Burter": {"c0": "Extraterrestre", "c1": "Armée de Freezer", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Chauve"}, "Guldo": {"c0": "Extraterrestre", "c1": "Armée de Freezer", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Chauve"}, "Belmod": {"c0": "Dieu", "c1": "Univers 11", "c2": "Dieu de la destruction", "c3": "11", "c4": "Non", "c5": "Aucun"}, "Marcarita": {"c0": "Ange", "c1": "Univers 11", "c2": "Aucune", "c3": "11", "c4": "Non", "c5": "Blanc"}, "Quitela": {"c0": "Dieu", "c1": "Univers 4", "c2": "Dieu de la destruction", "c3": "4", "c4": "Non", "c5": "Aucun"}, "Heles": {"c0": "Dieu", "c1": "Univers 2", "c2": "Dieu de la destruction", "c3": "2", "c4": "Non", "c5": "Blanc"}, "Champa": {"c0": "Dieu", "c1": "Univers 6", "c2": "Dieu de la destruction", "c3": "6", "c4": "Non", "c5": "Aucun"}, "Vados": {"c0": "Ange", "c1": "Univers 6", "c2": "Aucune", "c3": "6", "c4": "Non", "c5": "Blanc"}, "Grand Priest": {"c0": "Ange", "c1": "Neutre", "c2": "Aucune", "c3": "Multivers", "c4": "Non", "c5": "Blanc"}, "Cabba": {"c0": "Saiyan", "c1": "Univers 6", "c2": "Super Saiyan 2", "c3": "6", "c4": "Non", "c5": "Noir"}, "Frost": {"c0": "Race de Freezer", "c1": "Univers 6", "c2": "Forme finale", "c3": "6", "c4": "Non", "c5": "Aucun"}, "Kefla": {"c0": "Fusion Saiyan", "c1": "Univers 6", "c2": "Super Saiyan 2", "c3": "6", "c4": "Oui", "c5": "Noir"}, "King Cold": {"c0": "Race de Freezer", "c1": "Antagoniste", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Aucun"}, "King Vegeta": {"c0": "Saiyan", "c1": "Armée de Freezer", "c2": "Oozaru", "c3": "7", "c4": "Non", "c5": "Noir"}, "Gine": {"c0": "Saiyan", "c1": "Neutre", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Noir"}, "Tarble": {"c0": "Saiyan", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Noir"}, "Paragus": {"c0": "Saiyan", "c1": "Antagoniste", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Noir"}, "Moro": {"c0": "Extraterrestre", "c1": "Antagoniste", "c2": "Forme ultime", "c3": "7", "c4": "Non", "c5": "Blanc"}, "Gas": {"c0": "Heeter", "c1": "Antagoniste", "c2": "Surpuissance", "c3": "7", "c4": "Non", "c5": "Aucun"}, "Kibito": {"c0": "Kaioshin", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Kibito Kai", "c5": "Blanc"}, "Bra": {"c0": "Saiyan/Humaine", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Bleu"}, "Marron": {"c0": "Humaine", "c1": "Héros", "c2": "Aucune", "c3": "7", "c4": "Non", "c5": "Blond"}}, "fairy": {"Bickslow": {"c0": "Fairy Tail", "c1": "Seith", "c2": "Non", "c3": "Âmes", "c4": "Homme", "c5": "Bleu"}, "Sherry Blendy": {"c0": "Lamia Scale", "c1": "Doll Play", "c2": "Non", "c3": "Poupées", "c4": "Femme", "c5": "Rose"}, "Sherria Blendy": {"c0": "Lamia Scale", "c1": "God Slayer", "c2": "Non", "c3": "Ciel", "c4": "Femme", "c5": "Rose"}, "Hibiki Lates": {"c0": "Blue Pegasus", "c1": "Archive", "c2": "Non", "c3": "Information", "c4": "Homme", "c5": "Blond"}, "Jenny Realight": {"c0": "Blue Pegasus", "c1": "Take Over", "c2": "Non", "c3": "Machine", "c4": "Femme", "c5": "Blond"}, "Rufus Lore": {"c0": "Sabertooth", "c1": "Memory-Make", "c2": "Non", "c3": "Mémoire", "c4": "Homme", "c5": "Blond"}, "Macao Conbolt": {"c0": "Fairy Tail", "c1": "Purple Flare", "c2": "Non", "c3": "Feu", "c4": "Homme", "c5": "Bleu"}, "Romeo Conbolt": {"c0": "Fairy Tail", "c1": "Rainbow Fire", "c2": "Non", "c3": "Feu", "c4": "Homme", "c5": "Noir"}, "Bisca Connell": {"c0": "Fairy Tail", "c1": "Requip", "c2": "Non", "c3": "Armes à feu", "c4": "Femme", "c5": "Vert"}, "Alzack Connell": {"c0": "Fairy Tail", "c1": "Guns Magic", "c2": "Non", "c3": "Armes à feu", "c4": "Homme", "c5": "Noir"}, "Mest Gryder": {"c0": "Fairy Tail", "c1": "Memory Control", "c2": "Non", "c3": "Mémoire", "c4": "Homme", "c5": "Noir"}, "Ur": {"c0": "Aucune", "c1": "Ice-Make", "c2": "Non", "c3": "Glace", "c4": "Femme", "c5": "Noir"}, "Deliora": {"c0": "Démon de Zeref", "c1": "Etherious", "c2": "Non", "c3": "Destruction", "c4": "Homme", "c5": "Aucun"}, "Igneel": {"c0": "Dragons", "c1": "Dragon", "c2": "Non", "c3": "Feu", "c4": "Homme", "c5": "Aucun"}, "Grandeeney": {"c0": "Dragons", "c1": "Dragon", "c2": "Non", "c3": "Ciel", "c4": "Femme", "c5": "Aucun"}, "Metalicana": {"c0": "Dragons", "c1": "Dragon", "c2": "Non", "c3": "Fer", "c4": "Homme", "c5": "Aucun"}, "Skiadrum": {"c0": "Dragons", "c1": "Dragon", "c2": "Non", "c3": "Ombre", "c4": "Homme", "c5": "Aucun"}, "Weisslogia": {"c0": "Dragons", "c1": "Dragon", "c2": "Non", "c3": "Lumière", "c4": "Homme", "c5": "Aucun"}, "Jose Porla": {"c0": "Phantom Lord", "c1": "Shade", "c2": "Non", "c3": "Ombres", "c4": "Homme", "c5": "Noir"}, "Brain": {"c0": "Oración Seis", "c1": "Dark Écriture", "c2": "Non", "c3": "Ténèbres", "c4": "Homme", "c5": "Gris"}, "Midnight": {"c0": "Oración Seis", "c1": "Reflector", "c2": "Non", "c3": "Distorsion", "c4": "Homme", "c5": "Noir/Blanc"}, "Sorano Agria": {"c0": "Oración Seis", "c1": "Angel Magic", "c2": "Non", "c3": "Anges", "c4": "Femme", "c5": "Blanc"}, "Zancrow": {"c0": "Grimoire Heart", "c1": "God Slayer", "c2": "Non", "c3": "Feu noir", "c4": "Homme", "c5": "Blond"}, "Seilah": {"c0": "Tartaros", "c1": "Macro", "c2": "Non", "c3": "Contrôle", "c4": "Femme", "c5": "Noir"}, "Kyoka": {"c0": "Tartaros", "c1": "Enhancement", "c2": "Non", "c3": "Sensations", "c4": "Femme", "c5": "Vert"}, "Jackal": {"c0": "Tartaros", "c1": "Explosion", "c2": "Non", "c3": "Explosion", "c4": "Homme", "c5": "Blond"}}, "fma": {"Heymans Breda": {"c0": "Humain", "c1": "Armée d'Amestris", "c2": "Non", "c3": "Aucune", "c4": "Non", "c5": "Roux"}, "Vato Falman": {"c0": "Humain", "c1": "Armée d'Amestris", "c2": "Non", "c3": "Aucune", "c4": "Non", "c5": "Gris"}, "Kain Fuery": {"c0": "Humain", "c1": "Armée d'Amestris", "c2": "Non", "c3": "Aucune", "c4": "Non", "c5": "Noir"}, "Gracia Hughes": {"c0": "Humaine", "c1": "Amestris", "c2": "Non", "c3": "Aucune", "c4": "Non", "c5": "Brun"}, "Elicia Hughes": {"c0": "Humaine", "c1": "Amestris", "c2": "Non", "c3": "Aucune", "c4": "Non", "c5": "Brun"}, "Yoki": {"c0": "Humain", "c1": "Amestris", "c2": "Non", "c3": "Aucune", "c4": "Non", "c5": "Noir"}, "Frank Archer": {"c0": "Humain", "c1": "Armée d'Amestris", "c2": "Non", "c3": "Aucune", "c4": "Non", "c5": "Noir"}, "Cornello": {"c0": "Humain", "c1": "Culte de Leto", "c2": "Non", "c3": "Fausse alchimie", "c4": "Oui", "c5": "Chauve"}, "Paninya": {"c0": "Humaine / automail", "c1": "Rush Valley", "c2": "Non", "c3": "Aucune", "c4": "Non", "c5": "Noir"}, "Chris Mustang": {"c0": "Humaine", "c1": "Amestris", "c2": "Non", "c3": "Aucune", "c4": "Non", "c5": "Noir"}, "Rebecca Catalina": {"c0": "Humaine", "c1": "Armée d'Amestris", "c2": "Non", "c3": "Aucune", "c4": "Non", "c5": "Noir"}}, "haikyuu": {"Hisashi Kinoshita": {"c0": "Karasuno", "c1": "Ailier", "c2": "2e", "c3": "8", "c4": "Droite", "c5": "Noir"}, "Kazuhito Narita": {"c0": "Karasuno", "c1": "Central", "c2": "2e", "c3": "7", "c4": "Droite", "c5": "Noir"}, "Kiyoko Shimizu": {"c0": "Karasuno", "c1": "Manager", "c2": "3e", "c3": "—", "c4": "Droite", "c5": "Noir"}, "Hitoka Yachi": {"c0": "Karasuno", "c1": "Manager", "c2": "1re", "c3": "—", "c4": "Droite", "c5": "Blond"}, "Keishin Ukai": {"c0": "Karasuno", "c1": "Entraîneur", "c2": "—", "c3": "—", "c4": "Droite", "c5": "Blond"}, "Ittetsu Takeda": {"c0": "Karasuno", "c1": "Conseiller", "c2": "—", "c3": "—", "c4": "Droite", "c5": "Noir"}, "Nobuyuki Kai": {"c0": "Nekoma", "c1": "Ailier", "c2": "3e", "c3": "2", "c4": "Droite", "c5": "Noir"}, "Shohei Fukunaga": {"c0": "Nekoma", "c1": "Ailier", "c2": "2e", "c3": "6", "c4": "Droite", "c5": "Noir"}}, "hellsparadise": {"Lan Ban": {"c0": "Tensen", "c1": "Tensen", "c2": "Tao", "c3": "Tao / plantes", "c4": "Variable", "c5": "Blanc"}, "Yui": {"c0": "Civile", "c1": "Civile", "c2": "Aucun", "c3": "Aucune", "c4": "Femme", "c5": "Noir"}}, "hxh": {"Alluka Zoldyck": {"c0": "Membre de la famille", "c1": "Inconnu", "c2": "Famille Zoldyck", "c3": "Vœux de Nanika", "c4": "Zoldyck", "c5": "Election"}, "Milluki Zoldyck": {"c0": "Assassin", "c1": "Inconnu", "c2": "Famille Zoldyck", "c3": "Technologie", "c4": "Zoldyck", "c5": "Zoldyck Family"}, "Satotz": {"c0": "Hunter", "c1": "Inconnu", "c2": "Association des Hunters", "c3": "Course d'endurance", "c4": "Aucune", "c5": "Examen Hunter"}, "Menchi": {"c0": "Hunter", "c1": "Inconnu", "c2": "Association des Hunters", "c3": "Cuisine", "c4": "Aucune", "c5": "Examen Hunter"}, "Hanzo": {"c0": "Hunter", "c1": "Inconnu", "c2": "Association des Hunters", "c3": "Ninjutsu", "c4": "Aucune", "c5": "Examen Hunter"}, "Tonpa": {"c0": "Candidat", "c1": "Aucun", "c2": "Aucune", "c3": "Sabotage", "c4": "Aucune", "c5": "Examen Hunter"}, "Pokkle": {"c0": "Hunter", "c1": "Inconnu", "c2": "Association des Hunters", "c3": "Arc", "c4": "Aucune", "c5": "Examen Hunter"}, "Shalnark": {"c0": "Brigand", "c1": "Manipulation", "c2": "Brigade Fantôme", "c3": "Black Voice", "c4": "Aucune", "c5": "Yorknew"}, "Bonolenov Ndongo": {"c0": "Brigand", "c1": "Inconnu", "c2": "Brigade Fantôme", "c3": "Battle Cantabile", "c4": "Aucune", "c5": "Yorknew"}, "Colt": {"c0": "Fourmi-Chimère", "c1": "Inconnu", "c2": "Fourmis-Chimères", "c3": "Combat aérien", "c4": "Fourmi-Chimère", "c5": "Chimera Ant"}, "Welfin": {"c0": "Fourmi-Chimère", "c1": "Inconnu", "c2": "Fourmis-Chimères", "c3": "Missile Man", "c4": "Fourmi-Chimère", "c5": "Chimera Ant"}, "Ikalgo": {"c0": "Fourmi-Chimère", "c1": "Inconnu", "c2": "Fourmis-Chimères", "c3": "Armes à feu", "c4": "Fourmi-Chimère", "c5": "Chimera Ant"}, "Meleoron": {"c0": "Fourmi-Chimère", "c1": "Inconnu", "c2": "Fourmis-Chimères", "c3": "Perfect Plan", "c4": "Fourmi-Chimère", "c5": "Chimera Ant"}, "Zazan": {"c0": "Fourmi-Chimère", "c1": "Inconnu", "c2": "Fourmis-Chimères", "c3": "Queen Shot", "c4": "Fourmi-Chimère", "c5": "Chimera Ant"}, "Cheetu": {"c0": "Fourmi-Chimère", "c1": "Inconnu", "c2": "Fourmis-Chimères", "c3": "Vitesse", "c4": "Fourmi-Chimère", "c5": "Chimera Ant"}, "Leol": {"c0": "Fourmi-Chimère", "c1": "Inconnu", "c2": "Fourmis-Chimères", "c3": "Hearing Aid", "c4": "Fourmi-Chimère", "c5": "Chimera Ant"}, "Rammot": {"c0": "Fourmi-Chimère", "c1": "Inconnu", "c2": "Fourmis-Chimères", "c3": "Force brute", "c4": "Fourmi-Chimère", "c5": "Chimera Ant"}, "Pariston Hill": {"c0": "Hunter", "c1": "Inconnu", "c2": "Association des Hunters", "c3": "Manipulation politique", "c4": "Aucune", "c5": "Election"}, "Cheadle Yorkshire": {"c0": "Hunter", "c1": "Inconnu", "c2": "Association des Hunters", "c3": "Médecine", "c4": "Aucune", "c5": "Election"}, "Mizaistom Nana": {"c0": "Hunter", "c1": "Inconnu", "c2": "Association des Hunters", "c3": "Criminal List", "c4": "Aucune", "c5": "Election"}, "Tsezguerra": {"c0": "Hunter", "c1": "Inconnu", "c2": "Greed Island", "c3": "Stratégie", "c4": "Aucune", "c5": "Greed Island"}, "Goreinu": {"c0": "Hunter", "c1": "Matérialisation", "c2": "Greed Island", "c3": "Gorilles de Nen", "c4": "Aucune", "c5": "Greed Island"}, "Senritsu": {"c0": "Hunter", "c1": "Inconnu", "c2": "Famille Nostrade", "c3": "Musique", "c4": "Aucune", "c5": "Yorknew"}, "Basho": {"c0": "Garde du corps", "c1": "Inconnu", "c2": "Famille Nostrade", "c3": "Haïkus", "c4": "Aucune", "c5": "Yorknew"}, "Squala": {"c0": "Garde du corps", "c1": "Inconnu", "c2": "Famille Nostrade", "c3": "Chiens", "c4": "Aucune", "c5": "Yorknew"}, "Light Nostrade": {"c0": "Civil", "c1": "Aucun", "c2": "Famille Nostrade", "c3": "Aucune", "c4": "Aucune", "c5": "Yorknew"}, "Canary": {"c0": "Majordome", "c1": "Inconnu", "c2": "Famille Zoldyck", "c3": "Bâton", "c4": "Aucune", "c5": "Zoldyck Family"}, "Tsubone": {"c0": "Majordome", "c1": "Inconnu", "c2": "Famille Zoldyck", "c3": "Transformation en monture", "c4": "Aucune", "c5": "Election"}, "Mito Freecss": {"c0": "Civile", "c1": "Aucun", "c2": "Aucune", "c3": "Aucune", "c4": "Freecss", "c5": "Examen Hunter"}, "Knov": {"c0": "Hunter", "c1": "Émission", "c2": "Association des Hunters", "c3": "Hide and Seek", "c4": "Aucune", "c5": "Chimera Ant"}}, "jjk": {"Kiyotaka Ijichi": {"c0": "Exorcistes", "c1": "Assistant", "c2": "Aucune", "c3": "Non", "c4": "Oui", "c5": "Noir"}, "Kechizu": {"c0": "Cursed Womb", "c1": "Inconnu", "c2": "Manipulation du sang", "c3": "Non", "c4": "Oui", "c5": "Aucun"}, "Ogi Zenin": {"c0": "Exorcistes", "c1": "Grade 1", "c2": "Sabre de flammes", "c3": "Non", "c4": "Oui", "c5": "Noir"}, "Yu Haibara": {"c0": "Exorcistes", "c1": "Grade 2", "c2": "Inconnue", "c3": "Non", "c4": "Oui", "c5": "Noir"}, "Rika Orimoto": {"c0": "Fléau", "c1": "Spécial", "c2": "Aucune", "c3": "Non", "c4": "Oui", "c5": "Noir"}, "Charles Bernard": {"c0": "Indépendant", "c1": "Inconnu", "c2": "G Warstaff", "c3": "Non", "c4": "Oui", "c5": "Blond"}, "Takuma Ino": {"c0": "Exorcistes", "c1": "Grade 2", "c2": "Shikigami (Kaichi)", "c3": "Non", "c4": "Oui", "c5": "Noir"}}, "jojo": {"George Joestar": {"c0": "1", "c1": "Héros", "c2": "Aucun", "c3": "—", "c4": "Non", "c5": "Oui"}, "Erina Pendleton": {"c0": "1/2", "c1": "Héros", "c2": "Aucun", "c3": "—", "c4": "Non", "c5": "Oui"}, "Dire": {"c0": "1", "c1": "Héros", "c2": "Aucun", "c3": "—", "c4": "Oui", "c5": "Non"}, "Straizo": {"c0": "1/2", "c1": "Ennemi", "c2": "Aucun", "c3": "—", "c4": "Oui", "c5": "Non"}, "Santana": {"c0": "2", "c1": "Ennemi", "c2": "Aucun", "c3": "—", "c4": "Non", "c5": "Non"}, "Suzi Q": {"c0": "2/3", "c1": "Héros", "c2": "Aucun", "c3": "—", "c4": "Non", "c5": "Oui"}, "Holy Kujo": {"c0": "3", "c1": "Héros", "c2": "Stand sans nom", "c3": "—", "c4": "Non", "c5": "Oui"}, "Enya": {"c0": "3", "c1": "Ennemi", "c2": "Justice", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Oingo": {"c0": "3", "c1": "Ennemi", "c2": "Khnum", "c3": "Transformation", "c4": "Non", "c5": "Non"}, "Boingo": {"c0": "3", "c1": "Ennemi", "c2": "Tohth", "c3": "Prophétie", "c4": "Non", "c5": "Non"}, "Mariah": {"c0": "3", "c1": "Ennemi", "c2": "Bastet", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Pet Shop": {"c0": "3", "c1": "Ennemi", "c2": "Horus", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "N'Doul": {"c0": "3", "c1": "Ennemi", "c2": "Geb", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Daniel J. D'Arby": {"c0": "3", "c1": "Ennemi", "c2": "Osiris", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Telence T. D'Arby": {"c0": "3", "c1": "Ennemi", "c2": "Atum", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Alessi": {"c0": "3", "c1": "Ennemi", "c2": "Sethan", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Rubber Soul": {"c0": "3", "c1": "Ennemi", "c2": "Yellow Temperance", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "J. Geil": {"c0": "3", "c1": "Ennemi", "c2": "Hanged Man", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Tonio Trussardi": {"c0": "4", "c1": "Héros", "c2": "Pearl Jam", "c3": "Automatique", "c4": "Non", "c5": "Non"}, "Reimi Sugimoto": {"c0": "4", "c1": "Héros", "c2": "Aucun", "c3": "—", "c4": "Non", "c5": "Non"}, "Tomoko Higashikata": {"c0": "4", "c1": "Héros", "c2": "Aucun", "c3": "—", "c4": "Non", "c5": "Non"}, "Toshikazu Hazamada": {"c0": "4", "c1": "Ennemi", "c2": "Surface", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Yuya Fungami": {"c0": "4", "c1": "Héros", "c2": "Highway Star", "c3": "Automatique", "c4": "Non", "c5": "Non"}, "Mikitaka Hazekura": {"c0": "4", "c1": "Héros", "c2": "Earth Wind and Fire", "c3": "Transformation", "c4": "Non", "c5": "Non"}, "Keicho Nijimura": {"c0": "4", "c1": "Ennemi", "c2": "Bad Company", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Hayato Kawajiri": {"c0": "4", "c1": "Héros", "c2": "Aucun", "c3": "—", "c4": "Non", "c5": "Non"}, "Terunosuke Miyamoto": {"c0": "4", "c1": "Ennemi", "c2": "Enigma", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Aya Tsuji": {"c0": "4", "c1": "Héros", "c2": "Cinderella", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Ken Oyanagi": {"c0": "4", "c1": "Ennemi", "c2": "Boy II Man", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Pesci": {"c0": "5", "c1": "Ennemi", "c2": "Beach Boy", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Melone": {"c0": "5", "c1": "Ennemi", "c2": "Baby Face", "c3": "Automatique", "c4": "Non", "c5": "Non"}, "Formaggio": {"c0": "5", "c1": "Ennemi", "c2": "Little Feet", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Illuso": {"c0": "5", "c1": "Ennemi", "c2": "Man in the Mirror", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Squalo": {"c0": "5", "c1": "Ennemi", "c2": "Clash", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Tiziano": {"c0": "5", "c1": "Ennemi", "c2": "Talking Head", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Cioccolata": {"c0": "5", "c1": "Ennemi", "c2": "Green Day", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Secco": {"c0": "5", "c1": "Ennemi", "c2": "Oasis", "c3": "Armure", "c4": "Non", "c5": "Non"}, "Polpo": {"c0": "5", "c1": "Ennemi", "c2": "Black Sabbath", "c3": "Automatique", "c4": "Non", "c5": "Non"}, "Gwess": {"c0": "6", "c1": "Ennemi", "c2": "Goo Goo Dolls", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Johngalli A": {"c0": "6", "c1": "Ennemi", "c2": "Manhattan Transfer", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Sports Maxx": {"c0": "6", "c1": "Ennemi", "c2": "Limp Bizkit", "c3": "Automatique", "c4": "Non", "c5": "Non"}, "Rikiel": {"c0": "6", "c1": "Ennemi", "c2": "Sky High", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Versus": {"c0": "6", "c1": "Ennemi", "c2": "Underworld", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Lucy Steel": {"c0": "7", "c1": "Héros", "c2": "Aucun", "c3": "—", "c4": "Non", "c5": "Non"}, "Sandman": {"c0": "7", "c1": "Ennemi", "c2": "In a Silent Way", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Mountain Tim": {"c0": "7", "c1": "Héros", "c2": "Oh! Lonesome Me", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Pocoloco": {"c0": "7", "c1": "Héros", "c2": "Hey Ya!", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Blackmore": {"c0": "7", "c1": "Ennemi", "c2": "Catch the Rainbow", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Ringo Roadagain": {"c0": "7", "c1": "Ennemi", "c2": "Mandom", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Wekapipo": {"c0": "7", "c1": "Héros", "c2": "Aucun", "c3": "—", "c4": "Non", "c5": "Non"}, "Magent Magent": {"c0": "7", "c1": "Ennemi", "c2": "20th Century Boy", "c3": "Armure", "c4": "Non", "c5": "Non"}, "Axl RO": {"c0": "7", "c1": "Ennemi", "c2": "Civil War", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Dr. Ferdinand": {"c0": "7", "c1": "Ennemi", "c2": "Scary Monsters", "c3": "Transformation", "c4": "Non", "c5": "Non"}, "Yasuho Hirose": {"c0": "8", "c1": "Héros", "c2": "Paisley Park", "c3": "Longue portée", "c4": "Non", "c5": "Non"}, "Norisuke Higashikata IV": {"c0": "8", "c1": "Héros", "c2": "King Nothing", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Tsurugi Higashikata": {"c0": "8", "c1": "Héros", "c2": "Paper Moon King", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Joshu Higashikata": {"c0": "8", "c1": "Héros", "c2": "Nut King Call", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Jobin Higashikata": {"c0": "8", "c1": "Ennemi", "c2": "Speed King", "c3": "Courte portée", "c4": "Non", "c5": "Non"}, "Toru": {"c0": "8", "c1": "Ennemi", "c2": "Wonder of U", "c3": "Automatique", "c4": "Non", "c5": "Non"}}, "naruto": {"Himawari Uzumaki": {"c0": "Konoha", "c1": "Genin", "c2": "Uzumaki / Hyūga", "c3": "Inconnue", "c4": "Byakugan", "c5": "Oui — Kurama"}, "Shizune": {"c0": "Konoha", "c1": "Jōnin / ninja médecin", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Anko Mitarashi": {"c0": "Konoha", "c1": "Jōnin", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Ibiki Morino": {"c0": "Konoha", "c1": "Jōnin / interrogateur", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Genma Shiranui": {"c0": "Konoha", "c1": "Jōnin", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Izumo Kamizuki": {"c0": "Konoha", "c1": "Chūnin", "c2": "Aucun", "c3": "Eau", "c4": "Aucun", "c5": "Non"}, "Kotetsu Hagane": {"c0": "Konoha", "c1": "Chūnin", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Jirōbō": {"c0": "Oto", "c1": "Quartet du Son", "c2": "Aucun", "c3": "Terre", "c4": "Aucun", "c5": "Non"}, "Kidōmaru": {"c0": "Oto", "c1": "Quartet du Son", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Tayuya": {"c0": "Oto", "c1": "Quartet du Son", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Sakon": {"c0": "Oto", "c1": "Quartet du Son", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Dosu Kinuta": {"c0": "Oto", "c1": "Genin", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Zaku Abumi": {"c0": "Oto", "c1": "Genin", "c2": "Aucun", "c3": "Vent", "c4": "Aucun", "c5": "Non"}, "Fū": {"c0": "Taki", "c1": "Jinchūriki", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Ancienne — Chōmei"}, "Sakumo Hatake": {"c0": "Konoha", "c1": "Jōnin", "c2": "Hatake", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Dan Katō": {"c0": "Konoha", "c1": "Jōnin", "c2": "Aucun", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Nawaki": {"c0": "Konoha", "c1": "Genin", "c2": "Senju", "c3": "Inconnue", "c4": "Aucun", "c5": "Non"}, "Shikaku Nara": {"c0": "Konoha", "c1": "Jōnin / conseiller", "c2": "Nara", "c3": "Yin", "c4": "Aucun", "c5": "Non"}, "Inoichi Yamanaka": {"c0": "Konoha", "c1": "Jōnin", "c2": "Yamanaka", "c3": "Yin", "c4": "Aucun", "c5": "Non"}, "Chōza Akimichi": {"c0": "Konoha", "c1": "Jōnin", "c2": "Akimichi", "c3": "Yang", "c4": "Aucun", "c5": "Non"}, "Hizashi Hyūga": {"c0": "Konoha", "c1": "Jōnin", "c2": "Hyūga", "c3": "Inconnue", "c4": "Byakugan", "c5": "Non"}}, "onepiece": {"Silvers Rayleigh": {"c0": "Roger Pirates", "c1": "Second", "c2": "Aucun", "c3": "Aucun", "c4": "Oui", "c5": "Inconnue"}, "Monkey D. Dragon": {"c0": "Armée révolutionnaire", "c1": "Commandant suprême", "c2": "Inconnu", "c3": "Inconnu", "c4": "Inconnu", "c5": "East Blue"}, "Issho": {"c0": "Marine", "c1": "Amiral", "c2": "Zushi Zushi no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Aramaki": {"c0": "Marine", "c1": "Amiral", "c2": "Mori Mori no Mi", "c3": "Logia", "c4": "Non", "c5": "Inconnue"}, "Sengoku": {"c0": "Marine", "c1": "Inspecteur général (ex-amiral en chef)", "c2": "Hito Hito no Mi, modèle Daibutsu", "c3": "Zoan mythique", "c4": "Non", "c5": "Inconnue"}, "Queen": {"c0": "Pirates aux Cent Bêtes", "c1": "All-Star", "c2": "Ryu Ryu no Mi, modèle Brachiosaure", "c3": "Zoan antique", "c4": "Non", "c5": "Inconnue"}, "Jack": {"c0": "Pirates aux Cent Bêtes", "c1": "All-Star", "c2": "Zo Zo no Mi, modèle Mammouth", "c3": "Zoan antique", "c4": "Non", "c5": "Inconnue"}, "Ulti": {"c0": "Pirates aux Cent Bêtes", "c1": "Tobi Roppo", "c2": "Ryu Ryu no Mi, modèle Pachycéphalosaure", "c3": "Zoan antique", "c4": "Non", "c5": "Inconnue"}, "Page One": {"c0": "Pirates aux Cent Bêtes", "c1": "Tobi Roppo", "c2": "Ryu Ryu no Mi, modèle Spinosaure", "c3": "Zoan antique", "c4": "Non", "c5": "Inconnue"}, "Who's Who": {"c0": "Pirates aux Cent Bêtes", "c1": "Tobi Roppo", "c2": "Neko Neko no Mi, modèle Tigre à dents de sabre", "c3": "Zoan antique", "c4": "Non", "c5": "Inconnue"}, "Black Maria": {"c0": "Pirates aux Cent Bêtes", "c1": "Tobi Roppo", "c2": "Kumo Kumo no Mi, modèle Rosamygale", "c3": "Zoan antique", "c4": "Non", "c5": "Inconnue"}, "Sasaki": {"c0": "Pirates aux Cent Bêtes", "c1": "Tobi Roppo", "c2": "Ryu Ryu no Mi, modèle Tricératops", "c3": "Zoan antique", "c4": "Non", "c5": "Inconnue"}, "Charlotte Smoothie": {"c0": "Big Mom Pirates", "c1": "Sweet Commander", "c2": "Shibo Shibo no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Grand Line"}, "Charlotte Perospero": {"c0": "Big Mom Pirates", "c1": "Ministre", "c2": "Pero Pero no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Grand Line"}, "Charlotte Oven": {"c0": "Big Mom Pirates", "c1": "Ministre", "c2": "Netsu Netsu no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Grand Line"}, "Vinsmoke Judge": {"c0": "Germa 66", "c1": "Roi", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "North Blue"}, "Vinsmoke Reiju": {"c0": "Germa 66", "c1": "Princesse", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "North Blue"}, "Vinsmoke Ichiji": {"c0": "Germa 66", "c1": "Prince", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "North Blue"}, "Vinsmoke Niji": {"c0": "Germa 66", "c1": "Prince", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "North Blue"}, "Vinsmoke Yonji": {"c0": "Germa 66", "c1": "Prince", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "North Blue"}, "Carrot": {"c0": "Minks", "c1": "Combattante", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Zou"}, "Nekomamushi": {"c0": "Minks", "c1": "Duc", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Zou"}, "Inuarashi": {"c0": "Minks", "c1": "Duc", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Zou"}, "Nefertari Cobra": {"c0": "Royaume d'Alabasta", "c1": "Roi", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Alabasta"}, "Wapol": {"c0": "Royaume de Drum", "c1": "Roi", "c2": "Baku Baku no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Grand Line"}, "Kuro": {"c0": "Pirates du Chat Noir", "c1": "Capitaine", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "East Blue"}, "Don Krieg": {"c0": "Pirates de Krieg", "c1": "Capitaine", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "East Blue"}, "Hody Jones": {"c0": "Nouveaux Hommes-Poissons", "c1": "Capitaine", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Île des Hommes-Poissons"}, "Hachi": {"c0": "Arlong Pirates", "c1": "Officier", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Île des Hommes-Poissons"}, "Bellamy": {"c0": "Bellamy Pirates", "c1": "Capitaine", "c2": "Bane Bane no Mi", "c3": "Paramecia", "c4": "Non", "c5": "North Blue"}, "Cavendish": {"c0": "Pirates Beautiful", "c1": "Capitaine", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Inconnue"}, "Bartolomeo": {"c0": "Barto Club", "c1": "Capitaine", "c2": "Bari Bari no Mi", "c3": "Paramecia", "c4": "Non", "c5": "East Blue"}, "Trebol": {"c0": "Donquixote Pirates", "c1": "Cadre", "c2": "Beta Beta no Mi", "c3": "Paramecia", "c4": "Non", "c5": "North Blue"}, "Diamante": {"c0": "Donquixote Pirates", "c1": "Cadre", "c2": "Hira Hira no Mi", "c3": "Paramecia", "c4": "Non", "c5": "North Blue"}, "Pica": {"c0": "Donquixote Pirates", "c1": "Cadre", "c2": "Ishi Ishi no Mi", "c3": "Paramecia", "c4": "Non", "c5": "North Blue"}, "Vergo": {"c0": "Donquixote Pirates", "c1": "Cadre (infiltré dans la Marine)", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "North Blue"}, "Monet": {"c0": "Donquixote Pirates", "c1": "Officière", "c2": "Yuki Yuki no Mi", "c3": "Logia", "c4": "Non", "c5": "Inconnue"}, "Sugar": {"c0": "Donquixote Pirates", "c1": "Officière", "c2": "Hobi Hobi no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Rebecca": {"c0": "Royaume de Dressrosa", "c1": "Princesse", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Grand Line"}, "Viola": {"c0": "Royaume de Dressrosa", "c1": "Princesse", "c2": "Giro Giro no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Grand Line"}, "Kyros": {"c0": "Royaume de Dressrosa", "c1": "Gladiateur", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Grand Line"}, "Kaku": {"c0": "CP0", "c1": "Agent", "c2": "Ushi Ushi no Mi, modèle Girafe", "c3": "Zoan", "c4": "Non", "c5": "Inconnue"}, "Kalifa": {"c0": "CP9", "c1": "Agent", "c2": "Awa Awa no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Jabra": {"c0": "CP9", "c1": "Agent", "c2": "Inu Inu no Mi, modèle Loup", "c3": "Zoan", "c4": "Non", "c5": "Inconnue"}, "Spandam": {"c0": "CP9", "c1": "Directeur", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Inconnue"}, "Galdino": {"c0": "Cross Guild", "c1": "Officier", "c2": "Doru Doru no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Bentham": {"c0": "Newkama", "c1": "Officier", "c2": "Mane Mane no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Daz Bonez": {"c0": "Cross Guild", "c1": "Officier", "c2": "Supa Supa no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Wyper": {"c0": "Shandia", "c1": "Guerrier", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Skypiea"}, "Foxy": {"c0": "Foxy Pirates", "c1": "Capitaine", "c2": "Noro Noro no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Iceburg": {"c0": "Galley-La Company", "c1": "Président", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Water Seven"}, "Paulie": {"c0": "Galley-La Company", "c1": "Contremaître", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Water Seven"}, "Absalom": {"c0": "Thriller Bark", "c1": "Officier", "c2": "Suke Suke no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Hogback": {"c0": "Thriller Bark", "c1": "Docteur", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Inconnue"}, "Boa Sandersonia": {"c0": "Pirates Kuja", "c1": "Combattante", "c2": "Hebi Hebi no Mi, modèle Anaconda", "c3": "Zoan", "c4": "Non", "c5": "Calm Belt"}, "Boa Marigold": {"c0": "Pirates Kuja", "c1": "Combattante", "c2": "Hebi Hebi no Mi, modèle Cobra royal", "c3": "Zoan", "c4": "Non", "c5": "Calm Belt"}, "Jozu": {"c0": "Pirates de Barbe Blanche", "c1": "Commandant", "c2": "Kira Kira no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Vista": {"c0": "Pirates de Barbe Blanche", "c1": "Commandant", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Inconnue"}, "Izo": {"c0": "Pirates de Barbe Blanche", "c1": "Commandant", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Wano"}, "Thatch": {"c0": "Pirates de Barbe Blanche", "c1": "Commandant", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Inconnue"}, "Shiryu": {"c0": "Pirates de Barbe Noire", "c1": "Capitaine", "c2": "Suke Suke no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Jesus Burgess": {"c0": "Pirates de Barbe Noire", "c1": "Capitaine", "c2": "Riki Riki no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Van Augur": {"c0": "Pirates de Barbe Noire", "c1": "Capitaine", "c2": "Wapu Wapu no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Doc Q": {"c0": "Pirates de Barbe Noire", "c1": "Médecin", "c2": "Shiku Shiku no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Catarina Devon": {"c0": "Pirates de Barbe Noire", "c1": "Capitaine", "c2": "Inu Inu no Mi, modèle Kyubi no Kitsune", "c3": "Zoan mythique", "c4": "Non", "c5": "Inconnue"}, "Benn Beckman": {"c0": "Pirates du Roux", "c1": "Second", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Inconnue"}, "Lucky Roux": {"c0": "Pirates du Roux", "c1": "Combattant", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Inconnue"}, "Yasopp": {"c0": "Pirates du Roux", "c1": "Tireur", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "East Blue"}, "Kozuki Momonosuke": {"c0": "Wano", "c1": "Shogun", "c2": "Uo Uo no Mi, modèle Seiryu (artificiel)", "c3": "Zoan mythique", "c4": "Non", "c5": "Wano"}, "Kozuki Hiyori": {"c0": "Wano", "c1": "Princesse", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Wano"}, "Denjiro": {"c0": "Allié de Wano", "c1": "Samouraï", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Wano"}, "Kawamatsu": {"c0": "Allié de Wano", "c1": "Samouraï", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Wano"}, "Raizo": {"c0": "Allié de Wano", "c1": "Ninja", "c2": "Maki Maki no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Wano"}, "Kikunojo": {"c0": "Allié de Wano", "c1": "Samouraï", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Wano"}, "Ashura Doji": {"c0": "Allié de Wano", "c1": "Samouraï", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Wano"}, "Kanjuro": {"c0": "Kurozumi (espion)", "c1": "Samouraï", "c2": "Fude Fude no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Wano"}, "Otama": {"c0": "Wano", "c1": "Kunoichi", "c2": "Kibi Kibi no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Wano"}, "Koala": {"c0": "Armée révolutionnaire", "c1": "Instructrice", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Inconnue"}, "Hina": {"c0": "Marine", "c1": "Contre-amiral", "c2": "Ori Ori no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Tsuru": {"c0": "Marine", "c1": "Vice-amiral", "c2": "Woshu Woshu no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Helmeppo": {"c0": "Marine", "c1": "Lieutenant", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "East Blue"}, "Zeff": {"c0": "Baratie", "c1": "Chef cuisinier", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "East Blue"}, "Kuina": {"c0": "Dojo de Shimotsuki", "c1": "Épéiste", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "East Blue"}, "Makino": {"c0": "Village de Fuchsia", "c1": "Tenancière", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "East Blue"}, "Shirahoshi": {"c0": "Royaume Ryugu", "c1": "Princesse", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Île des Hommes-Poissons"}, "Scratchmen Apoo": {"c0": "Pirates On Air", "c1": "Capitaine", "c2": "Oto Oto no Mi", "c3": "Paramecia", "c4": "Non", "c5": "Inconnue"}, "Basil Hawkins": {"c0": "Hawkins Pirates", "c1": "Capitaine", "c2": "Wara Wara no Mi", "c3": "Paramecia", "c4": "Non", "c5": "North Blue"}}, "opm": {"Snek": {"c0": "Héros", "c1": "Classe A", "c2": "38", "c3": "Arts martiaux", "c4": "Non", "c5": "Noir"}, "Carnage Kabuto": {"c0": "Monstre", "c1": "Aucune", "c2": "—", "c3": "Force / berserk", "c4": "Non", "c5": "Noir"}, "Vaccine Man": {"c0": "Monstre", "c1": "Aucune", "c2": "—", "c3": "Énergie", "c4": "Non", "c5": "Chauve"}, "Beast King": {"c0": "Monstre", "c1": "Aucune", "c2": "—", "c3": "Force", "c4": "Non", "c5": "Aucun"}, "Armored Gorilla": {"c0": "Monstre / cyborg", "c1": "Aucune", "c2": "—", "c3": "Force", "c4": "Oui", "c5": "Aucun"}, "Dr. Genus": {"c0": "Humain", "c1": "Maison de l'Évolution", "c2": "—", "c3": "Science", "c4": "Non", "c5": "Noir"}, "Homeless Emperor": {"c0": "Association des monstres", "c1": "Dragon", "c2": "—", "c3": "Énergie divine", "c4": "Non", "c5": "Noir"}, "Elder Centipede": {"c0": "Association des monstres", "c1": "Dragon", "c2": "—", "c3": "Force / carapace", "c4": "Non", "c5": "Aucun"}, "Gouketsu": {"c0": "Association des monstres", "c1": "Dragon", "c2": "—", "c3": "Arts martiaux / monstre", "c4": "Non", "c5": "Aucun"}, "Phoenix Man": {"c0": "Association des monstres", "c1": "Démon", "c2": "—", "c3": "Résurrection", "c4": "Non", "c5": "Aucun"}, "Nyan": {"c0": "Association des monstres", "c1": "Dragon", "c2": "—", "c3": "Vitesse / griffes", "c4": "Non", "c5": "Aucun"}, "Melzargard": {"c0": "Monstre", "c1": "Aucune", "c2": "—", "c3": "Régénération / fusion", "c4": "Non", "c5": "Aucun"}, "Geryuganshoop": {"c0": "Extraterrestre", "c1": "Aucune", "c2": "—", "c3": "Télékinésie", "c4": "Non", "c5": "Aucun"}, "Groribas": {"c0": "Extraterrestre", "c1": "Aucune", "c2": "—", "c3": "Fusion", "c4": "Non", "c5": "Aucun"}, "Tareo": {"c0": "Humain", "c1": "Aucune", "c2": "—", "c3": "Aucun", "c4": "Non", "c5": "Noir"}}, "rezero": {"Rom": {"c0": "Géant", "c1": "Felt", "c2": "Force", "c3": "Aucun", "c4": "Non", "c5": "Chauve"}, "Heinkel van Astrea": {"c0": "Humain", "c1": "Chevaliers royaux", "c2": "Aucun", "c3": "Aucun", "c4": "Non", "c5": "Rouge"}, "Joshua Juukulius": {"c0": "Humain", "c1": "Anastasia", "c2": "Magie", "c3": "Aucun", "c4": "Non", "c5": "Violet"}, "Hetaro": {"c0": "Demi-bête", "c1": "Anastasia", "c2": "Magie", "c3": "Aucun", "c4": "Non", "c5": "Orange"}, "Tivey": {"c0": "Demi-bête", "c1": "Anastasia", "c2": "Magie", "c3": "Aucun", "c4": "Non", "c5": "Orange"}}, "sao": {"Recon": {"c0": "ALO", "c1": "Sylphe", "c2": "Dague", "c3": "Sylphes", "c4": "Homme", "c5": "Vert"}, "Sakuya": {"c0": "ALO", "c1": "Sylphe", "c2": "Katana", "c3": "Sylphes", "c4": "Femme", "c5": "Noir"}, "Alicia Rue": {"c0": "ALO", "c1": "Cait Sith", "c2": "Griffes", "c3": "Cait Sith", "c4": "Femme", "c5": "Blond"}, "Eugene": {"c0": "ALO", "c1": "Salamandre", "c2": "Épée démoniaque", "c3": "Salamandres", "c4": "Homme", "c5": "Rouge"}, "Johnny Black": {"c0": "SAO", "c1": "Humain", "c2": "Dague", "c3": "Laughing Coffin", "c4": "Homme", "c5": "Noir"}, "Rosalia": {"c0": "SAO", "c1": "Humaine", "c2": "Lance", "c3": "Titan's Hand", "c4": "Femme", "c5": "Rouge"}, "Kizmel": {"c0": "SAO", "c1": "Elfe noire", "c2": "Sabre", "c3": "Elfes noirs", "c4": "Femme", "c5": "Violet"}, "Fukaziroh": {"c0": "GGO", "c1": "Humaine", "c2": "Lance-grenades", "c3": "LPFM", "c4": "Femme", "c5": "Blond"}, "M": {"c0": "GGO", "c1": "Humain", "c2": "Fusil / bouclier", "c3": "LPFM", "c4": "Homme", "c5": "Noir"}, "Sortiliena Serlut": {"c0": "Underworld", "c1": "Humaine d'Underworld", "c2": "Épée", "c3": "Académie impériale", "c4": "Femme", "c5": "Rouge"}, "Lilpilin": {"c0": "Underworld", "c1": "Orc", "c2": "Épée", "c3": "Dark Territory", "c4": "Homme", "c5": "Aucun"}, "Iskahn": {"c0": "Underworld", "c1": "Pugiliste", "c2": "Poings", "c3": "Dark Territory", "c4": "Homme", "c5": "Rouge"}, "Sheyta Synthesis Twelve": {"c0": "Underworld", "c1": "Humaine d'Underworld", "c2": "Black Lily Sword", "c3": "Integrity Knights", "c4": "Femme", "c5": "Gris"}}, "sds": {"Grayroad": {"c0": "Démon", "c1": "Dix Commandements", "c2": "Pacifisme", "c3": "Inconnu", "c4": "Aucun", "c5": "Oui"}, "Nerobasta": {"c0": "Déesse", "c1": "Clan des Déesses", "c2": "Aucun", "c3": "Inconnu", "c4": "Aucun", "c5": "Non"}, "Supreme Deity": {"c0": "Déesse", "c1": "Clan des Déesses", "c2": "Aucun", "c3": "Inconnu", "c4": "Aucun", "c5": "Non"}, "Demon King": {"c0": "Démon", "c1": "Clan des Démons", "c2": "Aucun", "c3": "Inconnu", "c4": "Aucun", "c5": "Oui"}, "Lancelot": {"c0": "Fée/Humain", "c1": "Chevaliers de l'Apocalypse", "c2": "Aucun", "c3": "Inconnu", "c4": "Aucun", "c5": "Non"}, "Tristan Liones": {"c0": "Déesse/Démon", "c1": "Chevaliers de l'Apocalypse", "c2": "Aucun", "c3": "Inconnu", "c4": "Aucun", "c5": "Oui"}, "Percival": {"c0": "Humain", "c1": "Chevaliers de l'Apocalypse", "c2": "Aucun", "c3": "Hope", "c4": "Aucun", "c5": "Non"}, "Gawain": {"c0": "Humaine", "c1": "Chevaliers de l'Apocalypse", "c2": "Aucun", "c3": "Sunshine", "c4": "Aucun", "c5": "Non"}, "Margaret Liones": {"c0": "Humaine", "c1": "Liones", "c2": "Aucun", "c3": "Aucune", "c4": "Aucun", "c5": "Non"}, "Veronica Liones": {"c0": "Humaine", "c1": "Liones", "c2": "Aucun", "c3": "Aucune", "c4": "Aucun", "c5": "Non"}, "Denzel Liones": {"c0": "Humain", "c1": "Chevaliers sacrés", "c2": "Aucun", "c3": "Inconnu", "c4": "Aucun", "c5": "Non"}, "Slader": {"c0": "Humain", "c1": "Chevaliers sacrés", "c2": "Aucun", "c3": "Inconnu", "c4": "Aucun", "c5": "Non"}, "Cain Barzad": {"c0": "Humain", "c1": "Chevaliers sacrés", "c2": "Aucun", "c3": "Inconnu", "c4": "Aucun", "c5": "Non"}, "Matrona": {"c0": "Géante", "c1": "Clan des Géants", "c2": "Aucun", "c3": "Creation", "c4": "Aucun", "c5": "Non"}, "Dogedo": {"c0": "Humain", "c1": "Armée des Pierres", "c2": "Aucun", "c3": "Inconnu", "c4": "Aucun", "c5": "Non"}, "Oslo": {"c0": "Créature", "c1": "Forêt du Roi des Fées", "c2": "Aucun", "c3": "Téléportation", "c4": "Aucun", "c5": "Non"}, "Jenna": {"c0": "Humaine", "c1": "Druides", "c2": "Aucun", "c3": "Magie druidique", "c4": "Aucun", "c5": "Non"}, "Zaneri": {"c0": "Humaine", "c1": "Druides", "c2": "Aucun", "c3": "Magie druidique", "c4": "Aucun", "c5": "Non"}, "Liz": {"c0": "Humaine", "c1": "Danafor", "c2": "Aucun", "c3": "Aucune", "c4": "Aucun", "c5": "Non"}, "Twigo": {"c0": "Humain", "c1": "Liones", "c2": "Aucun", "c3": "Aucune", "c4": "Aucun", "c5": "Non"}, "Deathpierce": {"c0": "Humain", "c1": "Chevaliers sacrés", "c2": "Aucun", "c3": "Inconnu", "c4": "Aucun", "c5": "Non"}}, "snk": {"Oluo Bozado": {"c0": "Paradis", "c1": "Bataillon / Escouade Levi", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Blond"}, "Eld Jinn": {"c0": "Paradis", "c1": "Bataillon / Escouade Levi", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Blond"}, "Gunther Schultz": {"c0": "Paradis", "c1": "Bataillon / Escouade Levi", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Brun"}, "Nanaba": {"c0": "Paradis", "c1": "Bataillon", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Blond"}, "Darius Zackly": {"c0": "Paradis", "c1": "Commandement suprême", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Gris"}, "Eren Kruger": {"c0": "Marley", "c1": "Restaurateurs eldiens", "c2": "Assaillant", "c3": "Marley", "c4": "Non", "c5": "Noir"}, "Niccolo": {"c0": "Marley", "c1": "Cuisinier", "c2": "Aucun", "c3": "Marley", "c4": "Non", "c5": "Blond"}, "Rico Brzenska": {"c0": "Paradis", "c1": "Garnison", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Blanc"}, "Mina Carolina": {"c0": "Paradis", "c1": "104e", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Noir"}, "Thomas Wagner": {"c0": "Paradis", "c1": "104e", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Blond"}, "Hitch Dreyse": {"c0": "Paradis", "c1": "Brigades spéciales", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Brun"}, "Marlowe Freudenberg": {"c0": "Paradis", "c1": "Brigades spéciales / Bataillon", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Noir"}, "Djel Sannes": {"c0": "Paradis", "c1": "Brigades spéciales", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Brun"}, "Traute Carven": {"c0": "Paradis", "c1": "Brigade anti-humaine", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Blond"}, "Kaya": {"c0": "Paradis", "c1": "Civile", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Noir"}, "Nifa": {"c0": "Paradis", "c1": "Bataillon", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Noir"}, "Ian Dietrich": {"c0": "Paradis", "c1": "Garnison", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Brun"}, "Mitabi Jarnach": {"c0": "Paradis", "c1": "Garnison", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Noir"}, "Ilse Langnar": {"c0": "Paradis", "c1": "Bataillon", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Noir"}, "Karl Fritz": {"c0": "Eldia", "c1": "Famille royale", "c2": "Originel", "c3": "Paradis", "c4": "Non", "c5": "Blond"}, "Tom Ksaver": {"c0": "Marley", "c1": "Guerriers", "c2": "Bestial", "c3": "Liberio", "c4": "Non", "c5": "Brun"}, "Faye Yeager": {"c0": "Marley", "c1": "Civile", "c2": "Aucun", "c3": "Liberio", "c4": "Non", "c5": "Brun"}, "Zofia": {"c0": "Marley", "c1": "Candidate guerrière", "c2": "Aucun", "c3": "Liberio", "c4": "Non", "c5": "Brun"}, "Udo": {"c0": "Marley", "c1": "Candidat guerrier", "c2": "Aucun", "c3": "Liberio", "c4": "Non", "c5": "Blond"}, "Artur Braus": {"c0": "Paradis", "c1": "Civil", "c2": "Aucun", "c3": "Dauper", "c4": "Non", "c5": "Brun"}, "Alma": {"c0": "Paradis", "c1": "Civile", "c2": "Aucun", "c3": "Mur Sina", "c4": "Non", "c5": "Blond"}, "Karina Braun": {"c0": "Marley", "c1": "Civile", "c2": "Aucun", "c3": "Liberio", "c4": "Non", "c5": "Noir"}, "Louise": {"c0": "Paradis", "c1": "Garnison / Jaegeristes", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Brun"}, "Samuel Linke-Jackson": {"c0": "Paradis", "c1": "Bataillon", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Brun"}, "Daz": {"c0": "Paradis", "c1": "104e", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Brun"}, "Franz Kefka": {"c0": "Paradis", "c1": "104e", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Brun"}, "Hannah Diamant": {"c0": "Paradis", "c1": "104e", "c2": "Aucun", "c3": "Paradis", "c4": "Non", "c5": "Roux"}}, "solo": {"Kamish": {"c0": "Dragon", "c1": "—", "c2": "Dragon", "c3": "Aucune", "c4": "Souffle de feu", "c5": "Non"}, "Kaisel": {"c0": "Ombre", "c1": "Chevalier", "c2": "Monture", "c3": "Armée des Ombres", "c4": "Vol", "c5": "Non"}, "Iron": {"c0": "Ombre", "c1": "Chevalier d'élite", "c2": "Tank", "c3": "Armée des Ombres", "c4": "Force / bouclier", "c5": "Non"}, "Tank": {"c0": "Ombre", "c1": "Chevalier d'élite", "c2": "Tank", "c3": "Armée des Ombres", "c4": "Force physique", "c5": "Non"}, "Architect": {"c0": "Construct", "c1": "—", "c2": "Architecte", "c3": "Système", "c4": "Création de donjons", "c5": "Non"}, "Vulcan": {"c0": "Démon", "c1": "Roi démon", "c2": "Démon", "c3": "Démons", "c4": "Feu", "c5": "Non"}, "Cerberus": {"c0": "Bête", "c1": "—", "c2": "Gardien", "c3": "Démons", "c4": "Feu", "c5": "Non"}, "Kasaka": {"c0": "Serpent", "c1": "—", "c2": "Boss de donjon", "c3": "Aucune", "c4": "Poison", "c5": "Non"}, "Barka": {"c0": "Elfe de glace", "c1": "—", "c2": "Chef", "c3": "Aucune", "c4": "Glace", "c5": "Non"}, "Lennart Niermann": {"c0": "Humain", "c1": "S", "c2": "Hunter", "c3": "Allemagne", "c4": "Inconnu", "c5": "Oui"}, "Siddharth Bachchan": {"c0": "Humain", "c1": "S", "c2": "Hunter", "c3": "Inde", "c4": "Inconnu", "c5": "Oui"}, "Adam White": {"c0": "Humain", "c1": "—", "c2": "Agent", "c3": "États-Unis", "c4": "Aucun", "c5": "Non"}, "Sung Il-Hwan": {"c0": "Humain", "c1": "S", "c2": "Hunter", "c3": "Souverains", "c4": "Fragment de Souverain", "c5": "Non"}, "Yogumunt": {"c0": "Monarque", "c1": "Monarque", "c2": "Monarque de la Transfiguration", "c3": "Monarques", "c4": "Magie noire / transformation", "c5": "Non"}}, "tensura": {"Velgrynd": {"c0": "True Dragon", "c1": "Empire de l'Est", "c2": "True Dragon", "c3": "Inconnue", "c4": "Non", "c5": "Rouge"}, "Velzard": {"c0": "True Dragon", "c1": "Continent du Nord", "c2": "True Dragon", "c3": "Inconnue", "c4": "Non", "c5": "Blanc"}, "Yuuki Kagurazaka": {"c0": "Humain", "c1": "Guilde / Moderate Harlequin", "c2": "Grand maître de guilde", "c3": "Inconnue", "c4": "Non", "c5": "Noir"}, "Masayuki Honjo": {"c0": "Humain", "c1": "Empire / Tempest", "c2": "Héros", "c3": "Inconnue", "c4": "Non", "c5": "Blond"}}, "tokyoghoul": {"Jason": {"c0": "Ghoul", "c1": "Aogiri", "c2": "Rinkaku", "c3": "Oui", "c4": "—", "c5": "Blanc"}, "Ginshi Shirazu": {"c0": "Humain/Ghoul artificiel", "c1": "CCG", "c2": "Rinkaku", "c3": "Non", "c4": "Inspecteur", "c5": "Blond"}, "Kazuichi Banjou": {"c0": "Ghoul", "c1": "Anteiku/Goat", "c2": "Rinkaku", "c3": "Non", "c4": "—", "c5": "Noir"}, "Akihiro Kanou": {"c0": "Humain", "c1": "Aogiri", "c2": "Aucun", "c3": "Non", "c4": "—", "c5": "Noir"}, "Hanbee Abara": {"c0": "Humain", "c1": "CCG", "c2": "Aucun", "c3": "Non", "c4": "Inspecteur", "c5": "Noir"}}, "tokyorevengers": {"Shion Madarame": {"c0": "Tenjiku", "c1": "Cadre", "c2": "—", "c3": "Non", "c4": "Homme", "c5": "Blond"}, "Takeomi Akashi": {"c0": "Black Dragon", "c1": "Cadre", "c2": "—", "c3": "Non", "c4": "Homme", "c5": "Noir"}, "Wakasa Imaushi": {"c0": "Black Dragon", "c1": "Cadre", "c2": "—", "c3": "Non", "c4": "Homme", "c5": "Blond"}}, "wakfu": {"Ogrest": {"c0": "Ogre", "c1": "Seul", "c2": "Wakfu", "c3": "Poings", "c4": "Homme", "c5": "Noir"}, "Rushu": {"c0": "Démon", "c1": "Shukrute", "c2": "Énergie démoniaque", "c3": "Magie", "c4": "Homme", "c5": "Aucun"}, "Toross Mordal": {"c0": "Iop", "c1": "Seul", "c2": "Force", "c3": "Épée", "c4": "Homme", "c5": "Blond"}}};

for (const [universeKey, profiles] of Object.entries(DLE_STATIC_PROFILES_V20)) {
    const existing = DLE_STATIC_PROFILES[universeKey] || (DLE_STATIC_PROFILES[universeKey] = {});
    const aliasOf = n => DLE_MASTER_ALIASES[universeKey]?.[n] || n;
    const existingNorms = new Set(Object.keys(existing).map(n => aliasOf(normalizeDle(n))));
    const current = DLE_TARGET_NAMES[universeKey] || (DLE_TARGET_NAMES[universeKey] = []);
    const seen = new Set(current.map(n => aliasOf(normalizeDle(n))));
    for (const [name, attrs] of Object.entries(profiles)) {
        const n = aliasOf(normalizeDle(name));
        if (existingNorms.has(n)) continue;
        existing[name] = attrs;
        existingNorms.add(n);
        if (!seen.has(n)) { current.push(name); seen.add(n); }
...
// ================= AnimeDLE — Pokédex complet (Gen 1 à 9, 1025 Pokémon) =================
// Données officielles (PokéAPI) : types, génération, légendaire/fabuleux, stade d'évolution, couleur.
// Noms affichés en français ; les noms anglais restent acceptés comme alias.
// [id, nom FR, nom EN, type 1, type 2, génération, légendaire, évolution, couleur]
const POKEDEX_FULL = [[1, "Bulbizarre", "Bulbasaur", "Plante", "Poison", 1, "Non", "Base", "Vert"], [2, "Herbizarre", "Ivysaur", "Plante", "Poison", 1, "Non", "Évolue", "Vert"], [3, "Florizarre", "Venusaur", "Plante", "Poison", 1, "Non", "Finale", "Vert"], [4, "Salamèche", "Charmander", "Feu", "—", 1, "Non", "Base", "Rouge"], [5, "Reptincel", "Charmeleon", "Feu", "—", 1, "Non", "Évolue", "Rouge"], [6, "Dracaufeu", "Charizard", "Feu", "Vol", 1, "Non", "Finale", "Rouge"], [7, "Carapuce", "Squirtle", "Eau", "—", 1, "Non", "Base", "Bleu"], [8, "Carabaffe", "Wartortle", "Eau", "—", 1, "Non", "Évolue", "Bleu"], [9, "Tortank", "Blastoise", "Eau", "—", 1, "Non", "Finale", "Bleu"], [10, "Chenipan", "Caterpie", "Insecte", "—", 1, "Non", "Base", "Vert"], [11, "Chrysacier", "Metapod", "Insecte", "—", 1, "Non", "Évolue", "Vert"], [12, "Papilusion", "Butterfree", "Insecte", "Vol", 1, "Non", "Finale", "Blanc"], [13, "Aspicot", "Weedle", "Insecte", "Poison", 1, "Non", "Base", "Brun"], [14, "Coconfort", "Kakuna", "Insecte", "Poison", 1, "Non", "Évolue", "Jaune"], [15, "Dardargnan", "Beedrill", "Insecte", "Poison", 1, "Non", "Finale", "Jaune"], [16, "Roucool", "Pidgey", "Normal", "Vol", 1, "Non", "Base", "Brun"], [17, "Roucoups", "Pidgeotto", "Normal", "Vol", 1, "Non", "Évolue", "Brun"], [18, "Roucarnage", "Pidgeot", "Normal", "Vol", 1, "Non", "Finale", "Brun"], [19, "Rattata", "Rattata", "Normal", "—", 1, "Non", "Base", "Violet"], [20, "Rattatac", "Raticate", "Normal", "—", 1, "Non", "Finale", "Brun"], [21, "Piafabec", "Spearow", "Normal", "Vol", 1, "Non", "Base", "Brun"], [22, "Rapasdepic", "Fearow", "Normal", "Vol", 1, "Non", "Finale", "Brun"], [23, "Abo", "Ekans", "Poison", "—", 1, "Non", "Base", "Violet"], [24, "Arbok", "Arbok", "Poison", "—", 1, "Non", "Finale", "Violet"], [25, "Pikachu", "Pikachu", "Électrik", "—", 1, "Non", "Évolue", "Jaune"], [26, "Raichu", "Raichu", "Électrik", "—", 1, "Non", "Finale", "Jaune"], [27, "Sabelette", "Sandshrew", "Sol", "—", 1, "Non", "Base", "Jaune"], [28, "Sablaireau", "Sandslash", "Sol", "—", 1, "Non", "Finale", "Jaune"], [29, "Nidoran femelle", "Nidoran female", "Poison", "—", 1, "Non", "Base", "Bleu"], [30, "Nidorina", "Nidorina", "Poison", "—", 1, "Non", "Évolue", "Bleu"], [31, "Nidoqueen", "Nidoqueen", "Poison", "Sol", 1, "Non", "Finale", "Bleu"], [32, "Nidoran mâle", "Nidoran male", "Poison", "—", 1, "Non", "Base", "Violet"], [33, "Nidorino", "Nidorino", "Poison", "—", 1, "Non", "Évolue", "Violet"], [34, "Nidoking", "Nidoking", "Poison", "Sol", 1, "Non", "Finale", "Violet"], [35, "Mélofée", "Clefairy", "Fée", "—", 1, "Non", "Évolue", "Rose"], [36, "Mélodelfe", "Clefable", "Fée", "—", 1, "Non", "Finale", "Rose"], [37, "Goupix", "Vulpix", "Feu", "—", 1, "Non", "Base", "Brun"], [38, "Feunard", "Ninetales", "Feu", "—", 1, "Non", "Finale", "Jaune"], [39, "Rondoudou", "Jigglypuff", "Normal", "Fée", 1, "Non", "Évolue", "Rose"], [40, "Grodoudou", "Wigglytuff", "Normal", "Fée", 1, "Non", "Finale", "Rose"], [41, "Nosferapti", "Zubat", "Poison", "Vol", 1, "Non", "Base", "Violet"], [42, "Nosferalto", "Golbat", "Poison", "Vol", 1, "Non", "Évolue", "Violet"], [43, "Mystherbe", "Oddish", "Plante", "Poison", 1, "Non", "Base", "Bleu"], [44, "Ortide", "Gloom", "Plante", "Poison", 1, "Non", "Évolue", "Bleu"], [45, "Rafflesia", "Vileplume", "Plante", "Poison", 1, "Non", "Finale", "Rouge"], [46, "Paras", "Paras", "Insecte", "Plante", 1, "Non", "Base", "Rouge"], [47, "Parasect", "Parasect", "Insecte", "Plante", 1, "Non", "Finale", "Rouge"], [48, "Mimitoss", "Venonat", "Insecte", "Poison", 1, "Non", "Base", "Violet"], [49, "Aéromite", "Venomoth", "Insecte", "Poison", 1, "Non", "Finale", "Violet"], [50, "Taupiqueur", "Diglett", "Sol", "—", 1, "Non", "Base", "Brun"], [51, "Triopikeur", "Dugtrio", "Sol", "—", 1, "Non", "Finale", "Brun"], [52, "Miaouss", "Meowth", "Normal", "—", 1, "Non", "Base", "Jaune"], [53, "Persian", "Persian", "Normal", "—", 1, "Non", "Finale", "Jaune"], [54, "Psykokwak", "Psyduck", "Eau", "—", 1, "Non", "Base", "Jaune"], [55, "Akwakwak", "Golduck", "Eau", "—", 1, "Non", "Finale", "Bleu"], [56, "Férosinge", "Mankey", "Combat", "—", 1, "Non", "Base", "Brun"], [57, "Colossinge", "Primeape", "Combat", "—", 1, "Non", "Évolue", "Brun"], [58, "Caninos", "Growlithe", "Feu", "—", 1, "Non", "Base", "Brun"], [59, "Arcanin", "Arcanine", "Feu", "—", 1, "Non", "Finale", "Brun"], [60, "Ptitard", "Poliwag", "Eau", "—", 1, "Non", "Base", "Bleu"], [61, "Têtarte", "Poliwhirl", "Eau", "—", 1, "Non", "Évolue", "Bleu"], [62, "Tartard", "Poliwrath", "Eau", "Combat", 1, "Non", "Finale", "Bleu"], [63, "Abra", "Abra", "Psy", "—", 1, "Non", "Base", "Brun"], [64, "Kadabra", "Kadabra", "Psy", "—", 1, "Non", "Évolue", "Brun"], [65, "Alakazam", "Alakazam", "Psy", "—", 1, "Non", "Finale", "Brun"], [66, "Machoc", "Machop", "Combat", "—", 1, "Non", "Base", "Gris"], [67, "Machopeur", "Machoke", "Combat", "—", 1, "Non", "Évolue", "Gris"], [68, "Mackogneur", "Machamp", "Combat", "—", 1, "Non", "Finale", "Gris"], [69, "Chétiflor", "Bellsprout", "Plante", "Poison", 1, "Non", "Base", "Vert"], [70, "Boustiflor", "Weepinbell", "Plante", "Poison", 1, "Non", "Évolue", "Vert"], [71, "Empiflor", "Victreebel", "Plante", "Poison", 1, "Non", "Finale", "Vert"], [72, "Tentacool", "Tentacool", "Eau", "Poison", 1, "Non", "Base", "Bleu"], [73, "Tentacruel", "Tentacruel", "Eau", "Poison", 1, "Non", "Finale", "Bleu"], [74, "Racaillou", "Geodude", "Roche", "Sol", 1, "Non", "Base", "Brun"], [75, "Gravalanch", "Graveler", "Roche", "Sol", 1, "Non", "Évolue", "Brun"], [76, "Grolem", "Golem", "Roche", "Sol", 1, "Non", "Finale", "Brun"], [77, "Ponyta", "Ponyta", "Feu", "—", 1, "Non", "Base", "Jaune"], [78, "Galopa", "Rapidash", "Feu", "—", 1, "Non", "Finale", "Jaune"], [79, "Ramoloss", "Slowpoke", "Eau", "Psy", 1, "Non", "Base", "Rose"], [80, "Flagadoss", "Slowbro", "Eau", "Psy", 1, "Non", "Finale", "Rose"], [81, "Magnéti", "Magnemite", "Électrik", "Acier", 1, "Non", "Base", "Gris"], [82, "Magnéton", "Magneton", "Électrik", "Acier", 1, "Non", "Évolue", "Gris"], [83, "Canarticho", "Farfetch’d", "Normal", "Vol", 1, "Non", "Base", "Brun"], [84, "Doduo", "Doduo", "Normal", "Vol", 1, "Non", "Base", "Brun"], [85, "Dodrio", "Dodrio", "Normal", "Vol", 1, "Non", "Finale", "Brun"], [86, "Otaria", "Seel", "Eau", "—", 1, "Non", "Base", "Blanc"], [87, "Lamantine", "Dewgong", "Eau", "Glace", 1, "Non", "Finale", "Blanc"], [88, "Tadmorv", "Grimer", "Poison", "—", 1, "Non", "Base", "Violet"], [89, "Grotadmorv", "Muk", "Poison", "—", 1, "Non", "Finale", "Violet"], [90, "Kokiyas", "Shellder", "Eau", "—", 1, "Non", "Base", "Violet"], [91, "Crustabri", "Cloyster", "Eau", "Glace", 1, "Non", "Finale", "Violet"], [92, "Fantominus", "Gastly", "Spectre", "Poison", 1, "Non", "Base", "Violet"], [93, "Spectrum", "Haunter", "Spectre", "Poison", 1, "Non", "Évolue", "Violet"], [94, "Ectoplasma", "Gengar", "Spectre", "Poison", 1, "Non", "Finale", "Violet"], [95, "Onix", "Onix", "Roche", "Sol", 1, "Non", "Base", "Gris"], [96, "Soporifik", "Drowzee", "Psy", "—", 1, "Non", "Base", "Jaune"], [97, "Hypnomade", "Hypno", "Psy", "—", 1, "Non", "Finale", "Jaune"], [98, "Krabby", "Krabby", "Eau", "—", 1, "Non", "Base", "Rouge"], [99, "Krabboss", "Kingler", "Eau", "—", 1, "Non", "Finale", "Rouge"], [100, "Voltorbe", "Voltorb", "Électrik", "—", 1, "Non", "Base", "Rouge"], [101, "Électrode", "Electrode", "Électrik", "—", 1, "Non", "Finale", "Rouge"], [102, "Noeunoeuf", "Exeggcute", "Plante", "Psy", 1, "Non", "Base", "Rose"], [103, "Noadkoko", "Exeggutor", "Plante", "Psy", 1, "Non", "Finale", "Jaune"], [104, "Osselait", "Cubone", "Sol", "—", 1, "Non", "Base", "Brun"], [105, "Ossatueur", "Marowak", "Sol", "—", 1, "Non", "Finale", "Brun"], [106, "Kicklee", "Hitmonlee", "Combat", "—", 1, "Non", "Finale", "Brun"], [107, "Tygnon", "Hitmonchan", "Combat", "—", 1, "Non", "Finale", "Brun"], [108, "Excelangue", "Lickitung", "Normal", "—", 1, "Non", "Base", "Rose"], [109, "Smogo", "Koffing", "Poison", "—", 1, "Non", "Base", "Violet"], [110, "Smogogo", "Weezing", "Poison", "—", 1, "Non", "Finale", "Violet"], [111, "Rhinocorne", "Rhyhorn", "Sol", "Roche", 1, "Non", "Base", "Gris"], [112, "Rhinoféros", "Rhydon", "Sol", "Roche", 1, "Non", "Évolue", "Gris"], [113, "Leveinard", "Chansey", "Normal", "—", 1, "Non", "Évolue", "Rose"], [114, "Saquedeneu", "Tangela", "Plante", "—", 1, "Non", "Base", "Bleu"], [115, "Kangourex", "Kangaskhan", "Normal", "—", 1, "Non", "Unique", "Brun"], [116, "Hypotrempe", "Horsea", "Eau", "—", 1, "Non", "Base", "Bleu"], [117, "Hypocéan", "Seadra", "Eau", "—", 1, "Non", "Évolue", "Bleu"], [118, "Poissirène", "Goldeen", "Eau", "—", 1, "Non", "Base", "Rouge"], [119, "Poissoroy", "Seaking", "Eau", "—", 1, "Non", "Finale", "Rouge"], [120, "Stari", "Staryu", "Eau", "—", 1, "Non", "Base", "Brun"], [121, "Staross", "Starmie", "Eau", "Psy", 1, "Non", "Finale", "Violet"], [122, "M. Mime", "Mr. Mime", "Psy", "Fée", 1, "Non", "Évolue", "Rose"], [123, "Insécateur", "Scyther", "Insecte", "Vol", 1, "Non", "Base", "Vert"], [124, "Lippoutou", "Jynx", "Glace", "Psy", 1, "Non", "Finale", "Rouge"], [125, "Élektek", "Electabuzz", "Électrik", "—", 1, "Non", "Évolue", "Jaune"], [126, "Magmar", "Magmar", "Feu", "—", 1, "Non", "Évolue", "Rouge"], [127, "Scarabrute", "Pinsir", "Insecte", "—", 1, "Non", "Unique", "Brun"], [128, "Tauros", "Tauros", "Normal", "—", 1, "Non", "Unique", "Brun"], [129, "Magicarpe", "Magikarp", "Eau", "—", 1, "Non", "Base", "Rouge"], [130, "Léviator", "Gyarados", "Eau", "Vol", 1, "Non", "Finale", "Bleu"], [131, "Lokhlass", "Lapras", "Eau", "Glace", 1, "Non", "Unique", "Bleu"], [132, "Métamorph", "Ditto", "Normal", "—", 1, "Non", "Unique", "Violet"], [133, "Évoli", "Eevee", "Normal", "—", 1, "Non", "Base", "Brun"], [134, "Aquali", "Vaporeon", "Eau", "—", 1, "Non", "Finale", "Bleu"], [135, "Voltali", "Jolteon", "Électrik", "—", 1, "Non", "Finale", "Jaune"], [136, "Pyroli", "Flareon", "Feu", "—", 1, "Non", "Finale", "Rouge"], [137, "Porygon", "Porygon", "Normal", "—", 1, "Non", "Base", "Rose"], [138, "Amonita", "Omanyte", "Roche", "Eau", 1, "Non", "Base", "Bleu"], [139, "Amonistar", "Omastar", "Roche", "Eau", 1, "Non", "Finale", "Bleu"], [140, "Kabuto", "Kabuto", "Roche", "Eau", 1, "Non", "Base", "Brun"], [141, "Kabutops", "Kabutops", "Roche", "Eau", 1, "Non", "Finale", "Brun"], [142, "Ptéra", "Aerodactyl", "Roche", "Vol", 1, "Non", "Unique", "Violet"], [143, "Ronflex", "Snorlax", "Normal", "—", 1, "Non", "Finale", "Noir"], [144, "Artikodin", "Articuno", "Glace", "Vol", 1, "Oui", "Unique", "Bleu"], [145, "Électhor", "Zapdos", "Électrik", "Vol", 1, "Oui", "Unique", "Jaune"], [146, "Sulfura", "Moltres", "Feu", "Vol", 1, "Oui", "Unique", "Jaune"], [147, "Minidraco", "Dratini", "Dragon", "—", 1, "Non", "Base", "Bleu"], [148, "Draco", "Dragonair", "Dragon", "—", 1, "Non", "Évolue", "Bleu"], [149, "Dracolosse", "Dragonite", "Dragon", "Vol", 1, "Non", "Finale", "Brun"], [150, "Mewtwo", "Mewtwo", "Psy", "—", 1, "Oui", "Unique", "Violet"], [151, "Mew", "Mew", "Psy", "—", 1, "Mythique", "Unique", "Rose"], [152, "Germignon", "Chikorita", "Plante", "—", 2, "Non", "Base", "Vert"], [153, "Macronium", "Bayleef", "Plante", "—", 2, "Non", "Évolue", "Vert"], [154, "Méganium", "Meganium", "Plante", "—", 2, "Non", "Finale", "Vert"], [155, "Héricendre", "Cyndaquil", "Feu", "—", 2, "Non", "Base", "Jaune"], [156, "Feurisson", "Quilava", "Feu", "—", 2, "Non", "Évolue", "Jaune"], [157, "Typhlosion", "Typhlosion", "Feu", "—", 2, "Non", "Finale", "Jaune"], [158, "Kaiminus", "Totodile", "Eau", "—", 2, "Non", "Base", "Bleu"], [159, "Crocrodil", "Croconaw", "Eau", "—", 2, "Non", "Évolue", "Bleu"], [160, "Aligatueur", "Feraligatr", "Eau", "—", 2, "Non", "Finale", "Bleu"], [161, "Fouinette", "Sentret", "Normal", "—", 2, "Non", "Base", "Brun"], [162, "Fouinar", "Furret", "Normal", "—", 2, "Non", "Finale", "Brun"], [163, "Hoothoot", "Hoothoot", "Normal", "Vol", 2, "Non", "Base", "Brun"], [164, "Noarfang", "Noctowl", "Normal", "Vol", 2, "Non", "Finale", "Brun"], [165, "Coxy", "Ledyba", "Insecte", "Vol", 2, "Non", "Base", "Rouge"], [166, "Coxyclaque", "Ledian", "Insecte", "Vol", 2, "Non", "Finale", "Rouge"], [167, "Mimigal", "Spinarak", "Insecte", "Poison", 2, "Non", "Base", "Vert"], [168, "Migalos", "Ariados", "Insecte", "Poison", 2, "Non", "Finale", "Rouge"], [169, "Nostenfer", "Crobat", "Poison", "Vol", 2, "Non", "Finale", "Violet"], [170, "Loupio", "Chinchou", "Eau", "Électrik", 2, "Non", "Base", "Bleu"], [171, "Lanturn", "Lanturn", "Eau", "Électrik", 2, "Non", "Finale", "Bleu"], [172, "Pichu", "Pichu", "Électrik", "—", 2, "Non", "Base", "Jaune"], [173, "Mélo", "Cleffa", "Fée", "—", 2, "Non", "Base", "Rose"], [174, "Toudoudou", "Igglybuff", "Normal", "Fée", 2, "Non", "Base", "Rose"], [175, "Togepi", "Togepi", "Fée", "—", 2, "Non", "Base", "Blanc"], [176, "Togetic", "Togetic", "Fée", "Vol", 2, "Non", "Évolue", "Blanc"], [177, "Natu", "Natu", "Psy", "Vol", 2, "Non", "Base", "Vert"], [178, "Xatu", "Xatu", "Psy", "Vol", 2, "Non", "Finale", "Vert"], [179, "Wattouat", "Mareep", "Électrik", "—", 2, "Non", "Base", "Blanc"], [180, "Lainergie", "Flaaffy", "Électrik", "—", 2, "Non", "Évolue", "Rose"], [181, "Pharamp", "Ampharos", "Électrik", "—", 2, "Non", "Finale", "Jaune"], [182, "Joliflor", "Bellossom", "Plante", "—", 2, "Non", "Finale", "Vert"], [183, "Marill", "Marill", "Eau", "Fée", 2, "Non", "Évolue", "Bleu"], [184, "Azumarill", "Azumarill", "Eau", "Fée", 2, "Non", "Finale", "Bleu"], [185, "Simularbre", "Sudowoodo", "Roche", "—", 2, "Non", "Finale", "Brun"], [186, "Tarpaud", "Politoed", "Eau", "—", 2, "Non", "Finale", "Vert"], [187, "Granivol", "Hoppip", "Plante", "Vol", 2, "Non", "Base", "Rose"], [188, "Floravol", "Skiploom", "Plante", "Vol", 2, "Non", "Évolue", "Vert"], [189, "Cotovol", "Jumpluff", "Plante", "Vol", 2, "Non", "Finale", "Bleu"], [190, "Capumain", "Aipom", "Normal", "—", 2, "Non", "Base", "Violet"], [191, "Tournegrin", "Sunkern", "Plante", "—", 2, "Non", "Base", "Jaune"], [192, "Héliatronc", "Sunflora", "Plante", "—", 2, "Non", "Finale", "Jaune"], [193, "Yanma", "Yanma", "Insecte", "Vol", 2, "Non", "Base", "Rouge"], [194, "Axoloto", "Wooper", "Eau", "Sol", 2, "Non", "Base", "Bleu"], [195, "Maraiste", "Quagsire", "Eau", "Sol", 2, "Non", "Finale", "Bleu"], [196, "Mentali", "Espeon", "Psy", "—", 2, "Non", "Finale", "Violet"], [197, "Noctali", "Umbreon", "Ténèbres", "—", 2, "Non", "Finale", "Noir"], [198, "Cornèbre", "Murkrow", "Ténèbres", "Vol", 2, "Non", "Base", "Noir"], [199, "Roigada", "Slowking", "Eau", "Psy", 2, "Non", "Finale", "Rose"], [200, "Feuforêve", "Misdreavus", "Spectre", "—", 2, "Non", "Base", "Gris"], [201, "Zarbi", "Unown", "Psy", "—", 2, "Non", "Unique", "Noir"], [202, "Qulbutoké", "Wobbuffet", "Psy", "—", 2, "Non", "Finale", "Bleu"], [203, "Girafarig", "Girafarig", "Normal", "Psy", 2, "Non", "Base", "Jaune"], [204, "Pomdepik", "Pineco", "Insecte", "—", 2, "Non", "Base", "Gris"], [205, "Foretress", "Forretress", "Insecte", "Acier", 2, "Non", "Finale", "Violet"], [206, "Insolourdo", "Dunsparce", "Normal", "—", 2, "Non", "Base", "Jaune"], [207, "Scorplane", "Gligar", "Sol", "Vol", 2, "Non", "Base", "Violet"], [208, "Steelix", "Steelix", "Acier", "Sol", 2, "Non", "Finale", "Gris"], [209, "Snubbull", "Snubbull", "Fée", "—", 2, "Non", "Base", "Rose"], [210, "Granbull", "Granbull", "Fée", "—", 2, "Non", "Finale", "Violet"], [211, "Qwilfish", "Qwilfish", "Eau", "Poison", 2, "Non", "Base", "Gris"], [212, "Cizayox", "Scizor", "Insecte", "Acier", 2, "Non", "Finale", "Rouge"], [213, "Caratroc", "Shuckle", "Insecte", "Roche", 2, "Non", "Unique", "Jaune"], [214, "Scarhino", "Heracross", "Insecte", "Combat", 2, "Non", "Unique", "Bleu"], [215, "Farfuret", "Sneasel", "Ténèbres", "Glace", 2, "Non", "Base", "Noir"], [216, "Teddiursa", "Teddiursa", "Normal", "—", 2, "Non", "Base", "Brun"], [217, "Ursaring", "Ursaring", "Normal", "—", 2, "Non", "Évolue", "Brun"], [218, "Limagma", "Slugma", "Feu", "—", 2, "Non", "Base", "Rouge"], [219, "Volcaropod", "Magcargo", "Feu", "Roche", 2, "Non", "Finale", "Rouge"], [220, "Marcacrin", "Swinub", "Glace", "Sol", 2, "Non", "Base", "Brun"], [221, "Cochignon", "Piloswine", "Glace", "Sol", 2, "Non", "Évolue", "Brun"], [222, "Corayon", "Corsola", "Eau", "Roche", 2, "Non", "Base", "Rose"], [223, "Rémoraid", "Remoraid", "Eau", "—", 2, "Non", "Base", "Gris"], [224, "Octillery", "Octillery", "Eau", "—", 2, "Non", "Finale", "Rouge"], [225, "Cadoizo", "Delibird", "Glace", "Vol", 2, "Non", "Unique", "Rouge"], [226, "Démanta", "Mantine", "Eau", "Vol", 2, "Non", "Finale", "Violet"], [227, "Airmure", "Skarmory", "Acier", "Vol", 2, "Non", "Unique", "Gris"], [228, "Malosse", "Houndour", "Ténèbres", "Feu", 2, "Non", "Base", "Noir"], [229, "Démolosse", "Houndoom", "Ténèbres", "Feu", 2, "Non", "Finale", "Noir"], [230, "Hyporoi", "Kingdra", "Eau", "Dragon", 2, "Non", "Finale", "Bleu"], [231, "Phanpy", "Phanpy", "Sol", "—", 2, "Non", "Base", "Bleu"], [232, "Donphan", "Donphan", "Sol", "—", 2, "Non", "Finale", "Gris"], [233, "Porygon2", "Porygon2", "Normal", "—", 2, "Non", "Évolue", "Rouge"], [234, "Cerfrousse", "Stantler", "Normal", "—", 2, "Non", "Base", "Brun"], [235, "Queulorior", "Smeargle", "Normal", "—", 2, "Non", "Unique", "Blanc"], [236, "Debugant", "Tyrogue", "Combat", "—", 2, "Non", "Base", "Violet"], [237, "Kapoera", "Hitmontop", "Combat", "—", 2, "Non", "Finale", "Brun"], [238, "Lippouti", "Smoochum", "Glace", "Psy", 2, "Non", "Base", "Rose"], [239, "Élekid", "Elekid", "Électrik", "—", 2, "Non", "Base", "Jaune"], [240, "Magby", "Magby", "Feu", "—", 2, "Non", "Base", "Rouge"], [241, "Écrémeuh", "Miltank", "Normal", "—", 2, "Non", "Unique", "Rose"], [242, "Leuphorie", "Blissey", "Normal", "—", 2, "Non", "Finale", "Rose"], [243, "Raikou", "Raikou", "Électrik", "—", 2, "Oui", "Unique", "Jaune"], [244, "Entei", "Entei", "Feu", "—", 2, "Oui", "Unique", "Brun"], [245, "Suicune", "Suicune", "Eau", "—", 2, "Oui", "Unique", "Bleu"], [246, "Embrylex", "Larvitar", "Roche", "Sol", 2, "Non", "Base", "Vert"], [247, "Ymphect", "Pupitar", "Roche", "Sol", 2, "Non", "Évolue", "Gris"], [248, "Tyranocif", "Tyranitar", "Roche", "Ténèbres", 2, "Non", "Finale", "Vert"], [249, "Lugia", "Lugia", "Psy", "Vol", 2, "Oui", "Unique", "Blanc"], [250, "Ho-Oh", "Ho-Oh", "Feu", "Vol", 2, "Oui", "Unique", "Rouge"], [251, "Celebi", "Celebi", "Psy", "Plante", 2, "Mythique", "Unique", "Vert"], [252, "Arcko", "Treecko", "Plante", "—", 3, "Non", "Base", "Vert"], [253, "Massko", "Grovyle", "Plante", "—", 3, "Non", "Évolue", "Vert"], [254, "Jungko", "Sceptile", "Plante", "—", 3, "Non", "Finale", "Vert"], [255, "Poussifeu", "Torchic", "Feu", "—", 3, "Non", "Base", "Rouge"], [256, "Galifeu", "Combusken", "Feu", "Combat", 3, "Non", "Évolue", "Rouge"], [257, "Braségali", "Blaziken", "Feu", "Combat", 3, "Non", "Finale", "Rouge"], [258, "Gobou", "Mudkip", "Eau", "—", 3, "Non", "Base", "Bleu"], [259, "Flobio", "Marshtomp", "Eau", "Sol", 3, "Non", "Évolue", "Bleu"], [260, "Laggron", "Swampert", "Eau", "Sol", 3, "Non", "Finale", "Bleu"], [261, "Medhyèna", "Poochyena", "Ténèbres", "—", 3, "Non", "Base", "Gris"], [262, "Grahyèna", "Mightyena", "Ténèbres", "—", 3, "Non", "Finale", "Gris"], [263, "Zigzaton", "Zigzagoon", "Normal", "—", 3, "Non", "Base", "Brun"], [264, "Linéon", "Linoone", "Normal", "—", 3, "Non", "Évolue", "Blanc"], [265, "Chenipotte", "Wurmple", "Insecte", "—", 3, "Non", "Base", "Rouge"], [266, "Armulys", "Silcoon", "Insecte", "—", 3, "Non", "Évolue", "Blanc"], [267, "Charmillon", "Beautifly", "Insecte", "Vol", 3, "Non", "Finale", "Jaune"], [268, "Blindalys", "Cascoon", "Insecte", "—", 3, "Non", "Évolue", "Violet"], [269, "Papinox", "Dustox", "Insecte", "Poison", 3, "Non", "Finale", "Vert"], [270, "Nénupiot", "Lotad", "Eau", "Plante", 3, "Non", "Base", "Vert"], [271, "Lombre", "Lombre", "Eau", "Plante", 3, "Non", "Évolue", "Vert"], [272, "Ludicolo", "Ludicolo", "Eau", "Plante", 3, "Non", "Finale", "Vert"], [273, "Grainipiot", "Seedot", "Plante", "—", 3, "Non", "Base", "Brun"], [274, "Pifeuil", "Nuzleaf", "Plante", "Ténèbres", 3, "Non", "Évolue", "Brun"], [275, "Tengalice", "Shiftry", "Plante", "Ténèbres", 3, "Non", "Finale", "Brun"], [276, "Nirondelle", "Taillow", "Normal", "Vol", 3, "Non", "Base", "Bleu"], [277, "Hélédelle", "Swellow", "Normal", "Vol", 3, "Non", "Finale", "Bleu"], [278, "Goélise", "Wingull", "Eau", "Vol", 3, "Non", "Base", "Blanc"], [279, "Bekipan", "Pelipper", "Eau", "Vol", 3, "Non", "Finale", "Jaune"], [280, "Tarsal", "Ralts", "Psy", "Fée", 3, "Non", "Base", "Blanc"], [281, "Kirlia", "Kirlia", "Psy", "Fée", 3, "Non", "Évolue", "Blanc"], [282, "Gardevoir", "Gardevoir", "Psy", "Fée", 3, "Non", "Finale", "Blanc"], [283, "Arakdo", "Surskit", "Insecte", "Eau", 3, "Non", "Base", "Bleu"], [284, "Maskadra", "Masquerain", "Insecte", "Vol", 3, "Non", "Finale", "Bleu"], [285, "Balignon", "Shroomish", "Plante", "—", 3, "Non", "Base", "Brun"], [286, "Chapignon", "Breloom", "Plante", "Combat", 3, "Non", "Finale", "Vert"], [287, "Parecool", "Slakoth", "Normal", "—", 3, "Non", "Base", "Brun"], [288, "Vigoroth", "Vigoroth", "Normal", "—", 3, "Non", "Évolue", "Blanc"], [289, "Monaflèmit", "Slaking", "Normal", "—", 3, "Non", "Finale", "Brun"], [290, "Ningale", "Nincada", "Insecte", "Sol", 3, "Non", "Base", "Gris"], [291, "Ninjask", "Ninjask", "Insecte", "Vol", 3, "Non", "Finale", "Jaune"], [292, "Munja", "Shedinja", "Insecte", "Spectre", 3, "Non", "Finale", "Brun"], [293, "Chuchmur", "Whismur", "Normal", "—", 3, "Non", "Base", "Rose"], [294, "Ramboum", "Loudred", "Normal", "—", 3, "Non", "Évolue", "Bleu"], [295, "Brouhabam", "Exploud", "Normal", "—", 3, "Non", "Finale", "Bleu"], [296, "Makuhita", "Makuhita", "Combat", "—", 3, "Non", "Base", "Jaune"], [297, "Hariyama", "Hariyama", "Combat", "—", 3, "Non", "Finale", "Brun"], [298, "Azurill", "Azurill", "Normal", "Fée", 3, "Non", "Base", "Bleu"], [299, "Tarinor", "Nosepass", "Roche", "—", 3, "Non", "Base", "Gris"], [300, "Skitty", "Skitty", "Normal", "—", 3, "Non", "Base", "Rose"], [301, "Delcatty", "Delcatty", "Normal", "—", 3, "Non", "Finale", "Violet"], [302, "Ténéfix", "Sableye", "Ténèbres", "Spectre", 3, "Non", "Unique", "Violet"], [303, "Mysdibule", "Mawile", "Acier", "Fée", 3, "Non", "Unique", "Noir"], [304, "Galekid", "Aron", "Acier", "Roche", 3, "Non", "Base", "Gris"], [305, "Galegon", "Lairon", "Acier", "Roche", 3, "Non", "Évolue", "Gris"], [306, "Galeking", "Aggron", "Acier", "Roche", 3, "Non", "Finale", "Gris"], [307, "Méditikka", "Meditite", "Combat", "Psy", 3, "Non", "Base", "Bleu"], [308, "Charmina", "Medicham", "Combat", "Psy", 3, "Non", "Finale", "Rouge"], [309, "Dynavolt", "Electrike", "Électrik", "—", 3, "Non", "Base", "Vert"], [310, "Élecsprint", "Manectric", "Électrik", "—", 3, "Non", "Finale", "Jaune"], [311, "Posipi", "Plusle", "Électrik", "—", 3, "Non", "Unique", "Jaune"], [312, "Négapi", "Minun", "Électrik", "—", 3, "Non", "Unique", "Jaune"], [313, "Muciole", "Volbeat", "Insecte", "—", 3, "Non", "Unique", "Gris"], [314, "Lumivole", "Illumise", "Insecte", "—", 3, "Non", "Unique", "Violet"], [315, "Rosélia", "Roselia", "Plante", "Poison", 3, "Non", "Évolue", "Vert"], [316, "Gloupti", "Gulpin", "Poison", "—", 3, "Non", "Base", "Vert"], [317, "Avaltout", "Swalot", "Poison", "—", 3, "Non", "Finale", "Violet"], [318, "Carvanha", "Carvanha", "Eau", "Ténèbres", 3, "Non", "Base", "Rouge"], [319, "Sharpedo", "Sharpedo", "Eau", "Ténèbres", 3, "Non", "Finale", "Bleu"], [320, "Wailmer", "Wailmer", "Eau", "—", 3, "Non", "Base", "Bleu"], [321, "Wailord", "Wailord", "Eau", "—", 3, "Non", "Finale", "Bleu"], [322, "Chamallot", "Numel", "Feu", "Sol", 3, "Non", "Base", "Jaune"], [323, "Camérupt", "Camerupt", "Feu", "Sol", 3, "Non", "Finale", "Rouge"], [324, "Chartor", "Torkoal", "Feu", "—", 3, "Non", "Unique", "Brun"], [325, "Spoink", "Spoink", "Psy", "—", 3, "Non", "Base", "Noir"], [326, "Groret", "Grumpig", "Psy", "—", 3, "Non", "Finale", "Violet"], [327, "Spinda", "Spinda", "Normal", "—", 3, "Non", "Unique", "Brun"], [328, "Kraknoix", "Trapinch", "Sol", "—", 3, "Non", "Base", "Brun"], [329, "Vibraninf", "Vibrava", "Sol", "Dragon", 3, "Non", "Évolue", "Vert"], [330, "Libégon", "Flygon", "Sol", "Dragon", 3, "Non", "Finale", "Vert"], [331, "Cacnea", "Cacnea", "Plante", "—", 3, "Non", "Base", "Vert"], [332, "Cacturne", "Cacturne", "Plante", "Ténèbres", 3, "Non", "Finale", "Vert"], [333, "Tylton", "Swablu", "Normal", "Vol", 3, "Non", "Base", "Bleu"], [334, "Altaria", "Altaria", "Dragon", "Vol", 3, "Non", "Finale", "Bleu"], [335, "Mangriff", "Zangoose", "Normal", "—", 3, "Non", "Unique", "Blanc"], [336, "Séviper", "Seviper", "Poison", "—", 3, "Non", "Unique", "Noir"], [337, "Séléroc", "Lunatone", "Roche", "Psy", 3, "Non", "Unique", "Jaune"], [338, "Solaroc", "Solrock", "Roche", "Psy", 3, "Non", "Unique", "Rouge"], [339, "Barloche", "Barboach", "Eau", "Sol", 3, "Non", "Base", "Gris"], [340, "Barbicha", "Whiscash", "Eau", "Sol", 3, "Non", "Finale", "Bleu"], [341, "Écrapince", "Corphish", "Eau", "—", 3, "Non", "Base", "Rouge"], [342, "Colhomard", "Crawdaunt", "Eau", "Ténèbres", 3, "Non", "Finale", "Rouge"], [343, "Balbuto", "Baltoy", "Sol", "Psy", 3, "Non", "Base", "Brun"], [344, "Kaorine", "Claydol", "Sol", "Psy", 3, "Non", "Finale", "Noir"], [345, "Lilia", "Lileep", "Roche", "Plante", 3, "Non", "Base", "Violet"], [346, "Vacilys", "Cradily", "Roche", "Plante", 3, "Non", "Finale", "Vert"], [347, "Anorith", "Anorith", "Roche", "Insecte", 3, "Non", "Base", "Gris"], [348, "Armaldo", "Armaldo", "Roche", "Insecte", 3, "Non", "Finale", "Gris"], [349, "Barpau", "Feebas", "Eau", "—", 3, "Non", "Base", "Brun"], [350, "Milobellus", "Milotic", "Eau", "—", 3, "Non", "Finale", "Rose"], [351, "Morphéo", "Castform", "Normal", "—", 3, "Non", "Unique", "Gris"], [352, "Kecleon", "Kecleon", "Normal", "—", 3, "Non", "Unique", "Vert"], [353, "Polichombr", "Shuppet", "Spectre", "—", 3, "Non", "Base", "Noir"], [354, "Branette", "Banette", "Spectre", "—", 3, "Non", "Finale", "Noir"], [355, "Skelénox", "Duskull", "Spectre", "—", 3, "Non", "Base", "Noir"], [356, "Téraclope", "Dusclops", "Spectre", "—", 3, "Non", "Évolue", "Noir"], [357, "Tropius", "Tropius", "Plante", "Vol", 3, "Non", "Unique", "Vert"], [358, "Éoko", "Chimecho", "Psy", "—", 3, "Non", "Finale", "Bleu"], [359, "Absol", "Absol", "Ténèbres", "—", 3, "Non", "Unique", "Blanc"], [360, "Okéoké", "Wynaut", "Psy", "—", 3, "Non", "Base", "Bleu"], [361, "Stalgamin", "Snorunt", "Glace", "—", 3, "Non", "Base", "Gris"], [362, "Oniglali", "Glalie", "Glace", "—", 3, "Non", "Finale", "Gris"], [363, "Obalie", "Spheal", "Glace", "Eau", 3, "Non", "Base", "Bleu"], [364, "Phogleur", "Sealeo", "Glace", "Eau", 3, "Non", "Évolue", "Bleu"], [365, "Kaimorse", "Walrein", "Glace", "Eau", 3, "Non", "Finale", "Bleu"], [366, "Coquiperl", "Clamperl", "Eau", "—", 3, "Non", "Base", "Bleu"], [367, "Serpang", "Huntail", "Eau", "—", 3, "Non", "Finale", "Bleu"], [368, "Rosabyss", "Gorebyss", "Eau", "—", 3, "Non", "Finale", "Rose"], [369, "Relicanth", "Relicanth", "Eau", "Roche", 3, "Non", "Unique", "Gris"], [370, "Lovdisc", "Luvdisc", "Eau", "—", 3, "Non", "Unique", "Rose"], [371, "Draby", "Bagon", "Dragon", "—", 3, "Non", "Base", "Bleu"], [372, "Drackhaus", "Shelgon", "Dragon", "—", 3, "Non", "Évolue", "Blanc"], [373, "Drattak", "Salamence", "Dragon", "Vol", 3, "Non", "Finale", "Bleu"], [374, "Terhal", "Beldum", "Acier", "Psy", 3, "Non", "Base", "Bleu"], [375, "Métang", "Metang", "Acier", "Psy", 3, "Non", "Évolue", "Bleu"], [376, "Métalosse", "Metagross", "Acier", "Psy", 3, "Non", "Finale", "Bleu"], [377, "Regirock", "Regirock", "Roche", "—", 3, "Oui", "Unique", "Brun"], [378, "Regice", "Regice", "Glace", "—", 3, "Oui", "Unique", "Bleu"], [379, "Registeel", "Registeel", "Acier", "—", 3, "Oui", "Unique", "Gris"], [380, "Latias", "Latias", "Dragon", "Psy", 3, "Oui", "Unique", "Rouge"], [381, "Latios", "Latios", "Dragon", "Psy", 3, "Oui", "Unique", "Bleu"], [382, "Kyogre", "Kyogre", "Eau", "—", 3, "Oui", "Unique", "Bleu"], [383, "Groudon", "Groudon", "Sol", "—", 3, "Oui", "Unique", "Rouge"], [384, "Rayquaza", "Rayquaza", "Dragon", "Vol", 3, "Oui", "Unique", "Vert"], [385, "Jirachi", "Jirachi", "Acier", "Psy", 3, "Mythique", "Unique", "Jaune"], [386, "Deoxys", "Deoxys", "Psy", "—", 3, "Mythique", "Unique", "Rouge"], [387, "Tortipouss", "Turtwig", "Plante", "—", 4, "Non", "Base", "Vert"], [388, "Boskara", "Grotle", "Plante", "—", 4, "Non", "Évolue", "Vert"], [389, "Torterra", "Torterra", "Plante", "Sol", 4, "Non", "Finale", "Vert"], [390, "Ouisticram", "Chimchar", "Feu", "—", 4, "Non", "Base", "Brun"], [391, "Chimpenfeu", "Monferno", "Feu", "Combat", 4, "Non", "Évolue", "Brun"], [392, "Simiabraz", "Infernape", "Feu", "Combat", 4, "Non", "Finale", "Brun"], [393, "Tiplouf", "Piplup", "Eau", "—", 4, "Non", "Base", "Bleu"], [394, "Prinplouf", "Prinplup", "Eau", "—", 4, "Non", "Évolue", "Bleu"], [395, "Pingoléon", "Empoleon", "Eau", "Acier", 4, "Non", "Finale", "Bleu"], [396, "Étourmi", "Starly", "Normal", "Vol", 4, "Non", "Base", "Brun"], [397, "Étourvol", "Staravia", "Normal", "Vol", 4, "Non", "Évolue", "Brun"], [398, "Étouraptor", "Staraptor", "Normal", "Vol", 4, "Non", "Finale", "Brun"], [399, "Keunotor", "Bidoof", "Normal", "—", 4, "Non", "Base", "Brun"], [400, "Castorno", "Bibarel", "Normal", "Eau", 4, "Non", "Finale", "Brun"], [401, "Crikzik", "Kricketot", "Insecte", "—", 4, "Non", "Base", "Rouge"], [402, "Mélokrik", "Kricketune", "Insecte", "—", 4, "Non", "Finale", "Rouge"], [403, "Lixy", "Shinx", "Électrik", "—", 4, "Non", "Base", "Bleu"], [404, "Luxio", "Luxio", "Électrik", "—", 4, "Non", "Évolue", "Bleu"], [405, "Luxray", "Luxray", "Électrik", "—", 4, "Non", "Finale", "Bleu"], [406, "Rozbouton", "Budew", "Plante", "Poison", 4, "Non", "Base", "Vert"], [407, "Roserade", "Roserade", "Plante", "Poison", 4, "Non", "Finale", "Vert"], [408, "Kranidos", "Cranidos", "Roche", "—", 4, "Non", "Base", "Bleu"], [409, "Charkos", "Rampardos", "Roche", "—", 4, "Non", "Finale", "Bleu"], [410, "Dinoclier", "Shieldon", "Roche", "Acier", 4, "Non", "Base", "Gris"], [411, "Bastiodon", "Bastiodon", "Roche", "Acier", 4, "Non", "Finale", "Gris"], [412, "Cheniti", "Burmy", "Insecte", "—", 4, "Non", "Base", "Vert"], [413, "Cheniselle", "Wormadam", "Insecte", "Plante", 4, "Non", "Finale", "Vert"], [414, "Papilord", "Mothim", "Insecte", "Vol", 4, "Non", "Finale", "Jaune"], [415, "Apitrini", "Combee", "Insecte", "Vol", 4, "Non", "Base", "Jaune"], [416, "Apireine", "Vespiquen", "Insecte", "Vol", 4, "Non", "Finale", "Jaune"], [417, "Pachirisu", "Pachirisu", "Électrik", "—", 4, "Non", "Unique", "Blanc"], [418, "Mustébouée", "Buizel", "Eau", "—", 4, "Non", "Base", "Brun"], [419, "Mustéflott", "Floatzel", "Eau", "—", 4, "Non", "Finale", "Brun"], [420, "Ceribou", "Cherubi", "Plante", "—", 4, "Non", "Base", "Rose"], [421, "Ceriflor", "Cherrim", "Plante", "—", 4, "Non", "Finale", "Violet"], [422, "Sancoki", "Shellos", "Eau", "—", 4, "Non", "Base", "Violet"], [423, "Tritosor", "Gastrodon", "Eau", "Sol", 4, "Non", "Finale", "Violet"], [424, "Capidextre", "Ambipom", "Normal", "—", 4, "Non", "Finale", "Violet"], [425, "Baudrive", "Drifloon", "Spectre", "Vol", 4, "Non", "Base", "Violet"], [426, "Grodrive", "Drifblim", "Spectre", "Vol", 4, "Non", "Finale", "Violet"], [427, "Laporeille", "Buneary", "Normal", "—", 4, "Non", "Base", "Brun"], [428, "Lockpin", "Lopunny", "Normal", "—", 4, "Non", "Finale", "Brun"], [429, "Magirêve", "Mismagius", "Spectre", "—", 4, "Non", "Finale", "Violet"], [430, "Corboss", "Honchkrow", "Ténèbres", "Vol", 4, "Non", "Finale", "Noir"], [431, "Chaglam", "Glameow", "Normal", "—", 4, "Non", "Base", "Gris"], [432, "Chaffreux", "Purugly", "Normal", "—", 4, "Non", "Finale", "Gris"], [433, "Korillon", "Chingling", "Psy", "—", 4, "Non", "Base", "Jaune"], [434, "Moufouette", "Stunky", "Poison", "Ténèbres", 4, "Non", "Base", "Violet"], [435, "Moufflair", "Skuntank", "Poison", "Ténèbres", 4, "Non", "Finale", "Violet"], [436, "Archéomire", "Bronzor", "Acier", "Psy", 4, "Non", "Base", "Vert"], [437, "Archéodong", "Bronzong", "Acier", "Psy", 4, "Non", "Finale", "Vert"], [438, "Manzaï", "Bonsly", "Roche", "—", 4, "Non", "Base", "Brun"], [439, "Mime Jr.", "Mime Jr.", "Psy", "Fée", 4, "Non", "Base", "Rose"], [440, "Ptiravi", "Happiny", "Normal", "—", 4, "Non", "Base", "Rose"], [441, "Pijako", "Chatot", "Normal", "Vol", 4, "Non", "Unique", "Noir"], [442, "Spiritomb", "Spiritomb", "Spectre", "Ténèbres", 4, "Non", "Unique", "Violet"], [443, "Griknot", "Gible", "Dragon", "Sol", 4, "Non", "Base", "Bleu"], [444, "Carmache", "Gabite", "Dragon", "Sol", 4, "Non", "Évolue", "Bleu"], [445, "Carchacrok", "Garchomp", "Dragon", "Sol", 4, "Non", "Finale", "Bleu"], [446, "Goinfrex", "Munchlax", "Normal", "—", 4, "Non", "Base", "Noir"], [447, "Riolu", "Riolu", "Combat", "—", 4, "Non", "Base", "Bleu"], [448, "Lucario", "Lucario", "Combat", "Acier", 4, "Non", "Finale", "Bleu"], [449, "Hippopotas", "Hippopotas", "Sol", "—", 4, "Non", "Base", "Brun"], [450, "Hippodocus", "Hippowdon", "Sol", "—", 4, "Non", "Finale", "Brun"], [451, "Rapion", "Skorupi", "Poison", "Insecte", 4, "Non", "Base", "Violet"], [452, "Drascore", "Drapion", "Poison", "Ténèbres", 4, "Non", "Finale", "Violet"], [453, "Cradopaud", "Croagunk", "Poison", "Combat", 4, "Non", "Base", "Bleu"], [454, "Coatox", "Toxicroak", "Poison", "Combat", 4, "Non", "Finale", "Bleu"], [455, "Vortente", "Carnivine", "Plante", "—", 4, "Non", "Unique", "Vert"], [456, "Écayon", "Finneon", "Eau", "—", 4, "Non", "Base", "Bleu"], [457, "Luminéon", "Lumineon", "Eau", "—", 4, "Non", "Finale", "Bleu"], [458, "Babimanta", "Mantyke", "Eau", "Vol", 4, "Non", "Base", "Bleu"], [459, "Blizzi", "Snover", "Plante", "Glace", 4, "Non", "Base", "Blanc"], [460, "Blizzaroi", "Abomasnow", "Plante", "Glace", 4, "Non", "Finale", "Blanc"], [461, "Dimoret", "Weavile", "Ténèbres", "Glace", 4, "Non", "Finale", "Noir"], [462, "Magnézone", "Magnezone", "Électrik", "Acier", 4, "Non", "Finale", "Gris"], [463, "Coudlangue", "Lickilicky", "Normal", "—", 4, "Non", "Finale", "Rose"], [464, "Rhinastoc", "Rhyperior", "Sol", "Roche", 4, "Non", "Finale", "Gris"], [465, "Bouldeneu", "Tangrowth", "Plante", "—", 4, "Non", "Finale", "Bleu"], [466, "Élekable", "Electivire", "Électrik", "—", 4, "Non", "Finale", "Jaune"], [467, "Maganon", "Magmortar", "Feu", "—", 4, "Non", "Finale", "Rouge"], [468, "Togekiss", "Togekiss", "Fée", "Vol", 4, "Non", "Finale", "Blanc"], [469, "Yanmega", "Yanmega", "Insecte", "Vol", 4, "Non", "Finale", "Vert"], [470, "Phyllali", "Leafeon", "Plante", "—", 4, "Non", "Finale", "Vert"], [471, "Givrali", "Glaceon", "Glace", "—", 4, "Non", "Finale", "Bleu"], [472, "Scorvol", "Gliscor", "Sol", "Vol", 4, "Non", "Finale", "Violet"], [473, "Mammochon", "Mamoswine", "Glace", "Sol", 4, "Non", "Finale", "Brun"], [474, "Porygon-Z", "Porygon-Z", "Normal", "—", 4, "Non", "Finale", "Rouge"], [475, "Gallame", "Gallade", "Psy", "Combat", 4, "Non", "Finale", "Blanc"], [476, "Tarinorme", "Probopass", "Roche", "Acier", 4, "Non", "Finale", "Gris"], [477, "Noctunoir", "Dusknoir", "Spectre", "—", 4, "Non", "Finale", "Noir"], [478, "Momartik", "Froslass", "Glace", "Spectre", 4, "Non", "Finale", "Blanc"], [479, "Motisma", "Rotom", "Électrik", "Spectre", 4, "Non", "Unique", "Rouge"], [480, "Créhelf", "Uxie", "Psy", "—", 4, "Oui", "Unique", "Jaune"], [481, "Créfollet", "Mesprit", "Psy", "—", 4, "Oui", "Unique", "Rose"], [482, "Créfadet", "Azelf", "Psy", "—", 4, "Oui", "Unique", "Bleu"], [483, "Dialga", "Dialga", "Acier", "Dragon", 4, "Oui", "Unique", "Blanc"], [484, "Palkia", "Palkia", "Eau", "Dragon", 4, "Oui", "Unique", "Violet"], [485, "Heatran", "Heatran", "Feu", "Acier", 4, "Oui", "Unique", "Brun"], [486, "Regigigas", "Regigigas", "Normal", "—", 4, "Oui", "Unique", "Blanc"], [487, "Giratina", "Giratina", "Spectre", "Dragon", 4, "Oui", "Unique", "Noir"], [488, "Cresselia", "Cresselia", "Psy", "—", 4, "Oui", "Unique", "Jaune"], [489, "Phione", "Phione", "Eau", "—", 4, "Mythique", "Unique", "Bleu"], [490, "Manaphy", "Manaphy", "Eau", "—", 4, "Mythique", "Unique", "Bleu"], [491, "Darkrai", "Darkrai", "Ténèbres", "—", 4, "Mythique", "Unique", "Noir"], [492, "Shaymin", "Shaymin", "Plante", "—", 4, "Mythique", "Unique", "Vert"], [493, "Arceus", "Arceus", "Normal", "—", 4, "Mythique", "Unique", "Blanc"], [494, "Victini", "Victini", "Psy", "Feu", 5, "Mythique", "Unique", "Jaune"], [495, "Vipélierre", "Snivy", "Plante", "—", 5, "Non", "Base", "Vert"], [496, "Lianaja", "Servine", "Plante", "—", 5, "Non", "Évolue", "Vert"], [497, "Majaspic", "Serperior", "Plante", "—", 5, "Non", "Finale", "Vert"], [498, "Gruikui", "Tepig", "Feu", "—", 5, "Non", "Base", "Rouge"], [499, "Grotichon", "Pignite", "Feu", "Combat", 5, "Non", "Évolue", "Rouge"], [500, "Roitiflam", "Emboar", "Feu", "Combat", 5, "Non", "Finale", "Rouge"], [501, "Moustillon", "Oshawott", "Eau", "—", 5, "Non", "Base", "Bleu"], [502, "Mateloutre", "Dewott", "Eau", "—", 5, "Non", "Évolue", "Bleu"], [503, "Clamiral", "Samurott", "Eau", "—", 5, "Non", "Finale", "Bleu"], [504, "Ratentif", "Patrat", "Normal", "—", 5, "Non", "Base", "Brun"], [505, "Miradar", "Watchog", "Normal", "—", 5, "Non", "Finale", "Brun"], [506, "Ponchiot", "Lillipup", "Normal", "—", 5, "Non", "Base", "Brun"], [507, "Ponchien", "Herdier", "Normal", "—", 5, "Non", "Évolue", "Gris"], [508, "Mastouffe", "Stoutland", "Normal", "—", 5, "Non", "Finale", "Gris"], [509, "Chacripan", "Purrloin", "Ténèbres", "—", 5, "Non", "Base", "Violet"], [510, "Léopardus", "Liepard", "Ténèbres", "—", 5, "Non", "Finale", "Violet"], [511, "Feuillajou", "Pansage", "Plante", "—", 5, "Non", "Base", "Vert"], [512, "Feuiloutan", "Simisage", "Plante", "—", 5, "Non", "Finale", "Vert"], [513, "Flamajou", "Pansear", "Feu", "—", 5, "Non", "Base", "Rouge"], [514, "Flamoutan", "Simisear", "Feu", "—", 5, "Non", "Finale", "Rouge"], [515, "Flotajou", "Panpour", "Eau", "—", 5, "Non", "Base", "Bleu"], [516, "Flotoutan", "Simipour", "Eau", "—", 5, "Non", "Finale", "Bleu"], [517, "Munna", "Munna", "Psy", "—", 5, "Non", "Base", "Rose"], [518, "Mushana", "Musharna", "Psy", "—", 5, "Non", "Finale", "Rose"], [519, "Poichigeon", "Pidove", "Normal", "Vol", 5, "Non", "Base", "Gris"], [520, "Colombeau", "Tranquill", "Normal", "Vol", 5, "Non", "Évolue", "Gris"], [521, "Déflaisan", "Unfezant", "Normal", "Vol", 5, "Non", "Finale", "Gris"], [522, "Zébibron", "Blitzle", "Électrik", "—", 5, "Non", "Base", "Noir"], [523, "Zéblitz", "Zebstrika", "Électrik", "—", 5, "Non", "Finale", "Noir"], [524, "Nodulithe", "Roggenrola", "Roche", "—", 5, "Non", "Base", "Bleu"], [525, "Géolithe", "Boldore", "Roche", "—", 5, "Non", "Évolue", "Bleu"], [526, "Gigalithe", "Gigalith", "Roche", "—", 5, "Non", "Finale", "Bleu"], [527, "Chovsourir", "Woobat", "Psy", "Vol", 5, "Non", "Base", "Bleu"], [528, "Rhinolove", "Swoobat", "Psy", "Vol", 5, "Non", "Finale", "Bleu"], [529, "Rototaupe", "Drilbur", "Sol", "—", 5, "Non", "Base", "Gris"], [530, "Minotaupe", "Excadrill", "Sol", "Acier", 5, "Non", "Finale", "Gris"], [531, "Nanméouïe", "Audino", "Normal", "—", 5, "Non", "Unique", "Rose"], [532, "Charpenti", "Timburr", "Combat", "—", 5, "Non", "Base", "Gris"], [533, "Ouvrifier", "Gurdurr", "Combat", "—", 5, "Non", "Évolue", "Gris"], [534, "Bétochef", "Conkeldurr", "Combat", "—", 5, "Non", "Finale", "Brun"], [535, "Tritonde", "Tympole", "Eau", "—", 5, "Non", "Base", "Bleu"], [536, "Batracné", "Palpitoad", "Eau", "Sol", 5, "Non", "Évolue", "Bleu"], [537, "Crapustule", "Seismitoad", "Eau", "Sol", 5, "Non", "Finale", "Bleu"], [538, "Judokrak", "Throh", "Combat", "—", 5, "Non", "Unique", "Rouge"], [539, "Karaclée", "Sawk", "Combat", "—", 5, "Non", "Unique", "Bleu"], [540, "Larveyette", "Sewaddle", "Insecte", "Plante", 5, "Non", "Base", "Jaune"], [541, "Couverdure", "Swadloon", "Insecte", "Plante", 5, "Non", "Évolue", "Vert"], [542, "Manternel", "Leavanny", "Insecte", "Plante", 5, "Non", "Finale", "Jaune"], [543, "Venipatte", "Venipede", "Insecte", "Poison", 5, "Non", "Base", "Rouge"], [544, "Scobolide", "Whirlipede", "Insecte", "Poison", 5, "Non", "Évolue", "Gris"], [545, "Brutapode", "Scolipede", "Insecte", "Poison", 5, "Non", "Finale", "Rouge"], [546, "Doudouvet", "Cottonee", "Plante", "Fée", 5, "Non", "Base", "Vert"], [547, "Farfaduvet", "Whimsicott", "Plante", "Fée", 5, "Non", "Finale", "Vert"], [548, "Chlorobule", "Petilil", "Plante", "—", 5, "Non", "Base", "Vert"], [549, "Fragilady", "Lilligant", "Plante", "—", 5, "Non", "Finale", "Vert"], [550, "Bargantua", "Basculin", "Eau", "—", 5, "Non", "Base", "Vert"], [551, "Mascaïman", "Sandile", "Sol", "Ténèbres", 5, "Non", "Base", "Brun"], [552, "Escroco", "Krokorok", "Sol", "Ténèbres", 5, "Non", "Évolue", "Brun"], [553, "Crocorible", "Krookodile", "Sol", "Ténèbres", 5, "Non", "Finale", "Rouge"], [554, "Darumarond", "Darumaka", "Feu", "—", 5, "Non", "Base", "Rouge"], [555, "Darumacho", "Darmanitan", "Feu", "—", 5, "Non", "Finale", "Rouge"], [556, "Maracachi", "Maractus", "Plante", "—", 5, "Non", "Unique", "Vert"], [557, "Crabicoque", "Dwebble", "Insecte", "Roche", 5, "Non", "Base", "Rouge"], [558, "Crabaraque", "Crustle", "Insecte", "Roche", 5, "Non", "Finale", "Rouge"], [559, "Baggiguane", "Scraggy", "Ténèbres", "Combat", 5, "Non", "Base", "Jaune"], [560, "Baggaïd", "Scrafty", "Ténèbres", "Combat", 5, "Non", "Finale", "Rouge"], [561, "Cryptéro", "Sigilyph", "Psy", "Vol", 5, "Non", "Unique", "Noir"], [562, "Tutafeh", "Yamask", "Spectre", "—", 5, "Non", "Base", "Noir"], [563, "Tutankafer", "Cofagrigus", "Spectre", "—", 5, "Non", "Finale", "Jaune"], [564, "Carapagos", "Tirtouga", "Eau", "Roche", 5, "Non", "Base", "Bleu"], [565, "Mégapagos", "Carracosta", "Eau", "Roche", 5, "Non", "Finale", "Bleu"], [566, "Arkéapti", "Archen", "Roche", "Vol", 5, "Non", "Base", "Jaune"], [567, "Aéroptéryx", "Archeops", "Roche", "Vol", 5, "Non", "Finale", "Jaune"], [568, "Miamiasme", "Trubbish", "Poison", "—", 5, "Non", "Base", "Vert"], [569, "Miasmax", "Garbodor", "Poison", "—", 5, "Non", "Finale", "Vert"], [570, "Zorua", "Zorua", "Ténèbres", "—", 5, "Non", "Base", "Gris"], [571, "Zoroark", "Zoroark", "Ténèbres", "—", 5, "Non", "Finale", "Gris"], [572, "Chinchidou", "Minccino", "Normal", "—", 5, "Non", "Base", "Gris"], [573, "Pashmilla", "Cinccino", "Normal", "—", 5, "Non", "Finale", "Gris"], [574, "Scrutella", "Gothita", "Psy", "—", 5, "Non", "Base", "Violet"], [575, "Mesmérella", "Gothorita", "Psy", "—", 5, "Non", "Évolue", "Violet"], [576, "Sidérella", "Gothitelle", "Psy", "—", 5, "Non", "Finale", "Violet"], [577, "Nucléos", "Solosis", "Psy", "—", 5, "Non", "Base", "Vert"], [578, "Méios", "Duosion", "Psy", "—", 5, "Non", "Évolue", "Vert"], [579, "Symbios", "Reuniclus", "Psy", "—", 5, "Non", "Finale", "Vert"], [580, "Couaneton", "Ducklett", "Eau", "Vol", 5, "Non", "Base", "Bleu"], [581, "Lakmécygne", "Swanna", "Eau", "Vol", 5, "Non", "Finale", "Blanc"], [582, "Sorbébé", "Vanillite", "Glace", "—", 5, "Non", "Base", "Blanc"], [583, "Sorboul", "Vanillish", "Glace", "—", 5, "Non", "Évolue", "Blanc"], [584, "Sorbouboul", "Vanilluxe", "Glace", "—", 5, "Non", "Finale", "Blanc"], [585, "Vivaldaim", "Deerling", "Normal", "Plante", 5, "Non", "Base", "Rose"], [586, "Haydaim", "Sawsbuck", "Normal", "Plante", 5, "Non", "Finale", "Brun"], [587, "Emolga", "Emolga", "Électrik", "Vol", 5, "Non", "Unique", "Blanc"], [588, "Carabing", "Karrablast", "Insecte", "—", 5, "Non", "Base", "Bleu"], [589, "Lançargot", "Escavalier", "Insecte", "Acier", 5, "Non", "Finale", "Gris"], [590, "Trompignon", "Foongus", "Plante", "Poison", 5, "Non", "Base", "Blanc"], [591, "Gaulet", "Amoonguss", "Plante", "Poison", 5, "Non", "Finale", "Blanc"], [592, "Viskuse", "Frillish", "Eau", "Spectre", 5, "Non", "Base", "Blanc"], [593, "Moyade", "Jellicent", "Eau", "Spectre", 5, "Non", "Finale", "Blanc"], [594, "Mamanbo", "Alomomola", "Eau", "—", 5, "Non", "Unique", "Rose"], [595, "Statitik", "Joltik", "Insecte", "Électrik", 5, "Non", "Base", "Jaune"], [596, "Mygavolt", "Galvantula", "Insecte", "Électrik", 5, "Non", "Finale", "Jaune"], [597, "Grindur", "Ferroseed", "Plante", "Acier", 5, "Non", "Base", "Gris"], [598, "Noacier", "Ferrothorn", "Plante", "Acier", 5, "Non", "Finale", "Gris"], [599, "Tic", "Klink", "Acier", "—", 5, "Non", "Base", "Gris"], [600, "Clic", "Klang", "Acier", "—", 5, "Non", "Évolue", "Gris"], [601, "Cliticlic", "Klinklang", "Acier", "—", 5, "Non", "Finale", "Gris"], [602, "Anchwatt", "Tynamo", "Électrik", "—", 5, "Non", "Base", "Blanc"], [603, "Lampéroie", "Eelektrik", "Électrik", "—", 5, "Non", "Évolue", "Bleu"], [604, "Ohmassacre", "Eelektross", "Électrik", "—", 5, "Non", "Finale", "Bleu"], [605, "Lewsor", "Elgyem", "Psy", "—", 5, "Non", "Base", "Bleu"], [606, "Neitram", "Beheeyem", "Psy", "—", 5, "Non", "Finale", "Brun"], [607, "Funécire", "Litwick", "Spectre", "Feu", 5, "Non", "Base", "Blanc"], [608, "Mélancolux", "Lampent", "Spectre", "Feu", 5, "Non", "Évolue", "Noir"], [609, "Lugulabre", "Chandelure", "Spectre", "Feu", 5, "Non", "Finale", "Noir"], [610, "Coupenotte", "Axew", "Dragon", "—", 5, "Non", "Base", "Vert"], [611, "Incisache", "Fraxure", "Dragon", "—", 5, "Non", "Évolue", "Vert"], [612, "Tranchodon", "Haxorus", "Dragon", "—", 5, "Non", "Finale", "Jaune"], [613, "Polarhume", "Cubchoo", "Glace", "—", 5, "Non", "Base", "Blanc"], [614, "Polagriffe", "Beartic", "Glace", "—", 5, "Non", "Finale", "Blanc"], [615, "Hexagel", "Cryogonal", "Glace", "—", 5, "Non", "Unique", "Bleu"], [616, "Escargaume", "Shelmet", "Insecte", "—", 5, "Non", "Base", "Rouge"], [617, "Limaspeed", "Accelgor", "Insecte", "—", 5, "Non", "Finale", "Rouge"], [618, "Limonde", "Stunfisk", "Sol", "Électrik", 5, "Non", "Unique", "Brun"], [619, "Kungfouine", "Mienfoo", "Combat", "—", 5, "Non", "Base", "Jaune"], [620, "Shaofouine", "Mienshao", "Combat", "—", 5, "Non", "Finale", "Violet"], [621, "Drakkarmin", "Druddigon", "Dragon", "—", 5, "Non", "Unique", "Rouge"], [622, "Gringolem", "Golett", "Sol", "Spectre", 5, "Non", "Base", "Vert"], [623, "Golemastoc", "Golurk", "Sol", "Spectre", 5, "Non", "Finale", "Vert"], [624, "Scalpion", "Pawniard", "Ténèbres", "Acier", 5, "Non", "Base", "Rouge"], [625, "Scalproie", "Bisharp", "Ténèbres", "Acier", 5, "Non", "Évolue", "Rouge"], [626, "Frison", "Bouffalant", "Normal", "—", 5, "Non", "Unique", "Brun"], [627, "Furaiglon", "Rufflet", "Normal", "Vol", 5, "Non", "Base", "Blanc"], [628, "Gueriaigle", "Braviary", "Normal", "Vol", 5, "Non", "Finale", "Rouge"], [629, "Vostourno", "Vullaby", "Ténèbres", "Vol", 5, "Non", "Base", "Brun"], [630, "Vaututrice", "Mandibuzz", "Ténèbres", "Vol", 5, "Non", "Finale", "Brun"], [631, "Aflamanoir", "Heatmor", "Feu", "—", 5, "Non", "Unique", "Rouge"], [632, "Fermite", "Durant", "Insecte", "Acier", 5, "Non", "Unique", "Gris"], [633, "Solochi", "Deino", "Ténèbres", "Dragon", 5, "Non", "Base", "Bleu"], [634, "Diamat", "Zweilous", "Ténèbres", "Dragon", 5, "Non", "Évolue", "Bleu"], [635, "Trioxhydre", "Hydreigon", "Ténèbres", "Dragon", 5, "Non", "Finale", "Bleu"], [636, "Pyronille", "Larvesta", "Insecte", "Feu", 5, "Non", "Base", "Blanc"], [637, "Pyrax", "Volcarona", "Insecte", "Feu", 5, "Non", "Finale", "Blanc"], [638, "Cobaltium", "Cobalion", "Acier", "Combat", 5, "Oui", "Unique", "Bleu"], [639, "Terrakium", "Terrakion", "Roche", "Combat", 5, "Oui", "Unique", "Gris"], [640, "Viridium", "Virizion", "Plante", "Combat", 5, "Oui", "Unique", "Vert"], [641, "Boréas", "Tornadus", "Vol", "—", 5, "Oui", "Unique", "Vert"], [642, "Fulguris", "Thundurus", "Électrik", "Vol", 5, "Oui", "Unique", "Bleu"], [643, "Reshiram", "Reshiram", "Dragon", "Feu", 5, "Oui", "Unique", "Blanc"], [644, "Zekrom", "Zekrom", "Dragon", "Électrik", 5, "Oui", "Unique", "Noir"], [645, "Démétéros", "Landorus", "Sol", "Vol", 5, "Oui", "Unique", "Brun"], [646, "Kyurem", "Kyurem", "Dragon", "Glace", 5, "Oui", "Unique", "Gris"], [647, "Keldeo", "Keldeo", "Eau", "Combat", 5, "Mythique", "Unique", "Jaune"], [648, "Meloetta", "Meloetta", "Normal", "Psy", 5, "Mythique", "Unique", "Blanc"], [649, "Genesect", "Genesect", "Insecte", "Acier", 5, "Mythique", "Unique", "Violet"], [650, "Marisson", "Chespin", "Plante", "—", 6, "Non", "Base", "Vert"], [651, "Boguérisse", "Quilladin", "Plante", "—", 6, "Non", "Évolue", "Vert"], [652, "Blindépique", "Chesnaught", "Plante", "Combat", 6, "Non", "Finale", "Vert"], [653, "Feunnec", "Fennekin", "Feu", "—", 6, "Non", "Base", "Rouge"], [654, "Roussil", "Braixen", "Feu", "—", 6, "Non", "Évolue", "Rouge"], [655, "Goupelin", "Delphox", "Feu", "Psy", 6, "Non", "Finale", "Rouge"], [656, "Grenousse", "Froakie", "Eau", "—", 6, "Non", "Base", "Bleu"], [657, "Croâporal", "Frogadier", "Eau", "—", 6, "Non", "Évolue", "Bleu"], [658, "Amphinobi", "Greninja", "Eau", "Ténèbres", 6, "Non", "Finale", "Bleu"], [659, "Sapereau", "Bunnelby", "Normal", "—", 6, "Non", "Base", "Brun"], [660, "Excavarenne", "Diggersby", "Normal", "Sol", 6, "Non", "Finale", "Brun"], [661, "Passerouge", "Fletchling", "Normal", "Vol", 6, "Non", "Base", "Rouge"], [662, "Braisillon", "Fletchinder", "Feu", "Vol", 6, "Non", "Évolue", "Rouge"], [663, "Flambusard", "Talonflame", "Feu", "Vol", 6, "Non", "Finale", "Rouge"], [664, "Lépidonille", "Scatterbug", "Insecte", "—", 6, "Non", "Base", "Noir"], [665, "Pérégrain", "Spewpa", "Insecte", "—", 6, "Non", "Évolue", "Noir"], [666, "Prismillon", "Vivillon", "Insecte", "Vol", 6, "Non", "Finale", "Blanc"], [667, "Hélionceau", "Litleo", "Feu", "Normal", 6, "Non", "Base", "Brun"], [668, "Némélios", "Pyroar", "Feu", "Normal", 6, "Non", "Finale", "Brun"], [669, "Flabébé", "Flabébé", "Fée", "—", 6, "Non", "Base", "Blanc"], [670, "Floette", "Floette", "Fée", "—", 6, "Non", "Évolue", "Blanc"], [671, "Florges", "Florges", "Fée", "—", 6, "Non", "Finale", "Blanc"], [672, "Cabriolaine", "Skiddo", "Plante", "—", 6, "Non", "Base", "Brun"], [673, "Chevroum", "Gogoat", "Plante", "—", 6, "Non", "Finale", "Brun"], [674, "Pandespiègle", "Pancham", "Combat", "—", 6, "Non", "Base", "Blanc"], [675, "Pandarbare", "Pangoro", "Combat", "Ténèbres", 6, "Non", "Finale", "Blanc"], [676, "Couafarel", "Furfrou", "Normal", "—", 6, "Non", "Unique", "Blanc"], [677, "Psystigri", "Espurr", "Psy", "—", 6, "Non", "Base", "Gris"], [678, "Mistigrix", "Meowstic", "Psy", "—", 6, "Non", "Finale", "Bleu"], [679, "Monorpale", "Honedge", "Acier", "Spectre", 6, "Non", "Base", "Brun"], [680, "Dimoclès", "Doublade", "Acier", "Spectre", 6, "Non", "Évolue", "Brun"], [681, "Exagide", "Aegislash", "Acier", "Spectre", 6, "Non", "Finale", "Brun"], [682, "Fluvetin", "Spritzee", "Fée", "—", 6, "Non", "Base", "Rose"], [683, "Cocotine", "Aromatisse", "Fée", "—", 6, "Non", "Finale", "Rose"], [684, "Sucroquin", "Swirlix", "Fée", "—", 6, "Non", "Base", "Blanc"], [685, "Cupcanaille", "Slurpuff", "Fée", "—", 6, "Non", "Finale", "Blanc"], [686, "Sepiatop", "Inkay", "Ténèbres", "Psy", 6, "Non", "Base", "Bleu"], [687, "Sepiatroce", "Malamar", "Ténèbres", "Psy", 6, "Non", "Finale", "Bleu"], [688, "Opermine", "Binacle", "Roche", "Eau", 6, "Non", "Base", "Brun"], [689, "Golgopathe", "Barbaracle", "Roche", "Eau", 6, "Non", "Finale", "Brun"], [690, "Venalgue", "Skrelp", "Poison", "Eau", 6, "Non", "Base", "Brun"], [691, "Kravarech", "Dragalge", "Poison", "Dragon", 6, "Non", "Finale", "Brun"], [692, "Flingouste", "Clauncher", "Eau", "—", 6, "Non", "Base", "Bleu"], [693, "Gamblast", "Clawitzer", "Eau", "—", 6, "Non", "Finale", "Bleu"], [694, "Galvaran", "Helioptile", "Électrik", "Normal", 6, "Non", "Base", "Jaune"], [695, "Iguolta", "Heliolisk", "Électrik", "Normal", 6, "Non", "Finale", "Jaune"], [696, "Ptyranidur", "Tyrunt", "Roche", "Dragon", 6, "Non", "Base", "Brun"], [697, "Rexillius", "Tyrantrum", "Roche", "Dragon", 6, "Non", "Finale", "Rouge"], [698, "Amagara", "Amaura", "Roche", "Glace", 6, "Non", "Base", "Bleu"], [699, "Dragmara", "Aurorus", "Roche", "Glace", 6, "Non", "Finale", "Bleu"], [700, "Nymphali", "Sylveon", "Fée", "—", 6, "Non", "Finale", "Rose"], [701, "Brutalibré", "Hawlucha", "Combat", "Vol", 6, "Non", "Unique", "Vert"], [702, "Dedenne", "Dedenne", "Électrik", "Fée", 6, "Non", "Unique", "Jaune"], [703, "Strassie", "Carbink", "Roche", "Fée", 6, "Non", "Unique", "Gris"], [704, "Mucuscule", "Goomy", "Dragon", "—", 6, "Non", "Base", "Violet"], [705, "Colimucus", "Sliggoo", "Dragon", "—", 6, "Non", "Évolue", "Violet"], [706, "Muplodocus", "Goodra", "Dragon", "—", 6, "Non", "Finale", "Violet"], [707, "Trousselin", "Klefki", "Acier", "Fée", 6, "Non", "Unique", "Gris"], [708, "Brocélôme", "Phantump", "Spectre", "Plante", 6, "Non", "Base", "Brun"], [709, "Desséliande", "Trevenant", "Spectre", "Plante", 6, "Non", "Finale", "Brun"], [710, "Pitrouille", "Pumpkaboo", "Spectre", "Plante", 6, "Non", "Base", "Brun"], [711, "Banshitrouye", "Gourgeist", "Spectre", "Plante", 6, "Non", "Finale", "Brun"], [712, "Grelaçon", "Bergmite", "Glace", "—", 6, "Non", "Base", "Bleu"], [713, "Séracrawl", "Avalugg", "Glace", "—", 6, "Non", "Finale", "Bleu"], [714, "Sonistrelle", "Noibat", "Vol", "Dragon", 6, "Non", "Base", "Violet"], [715, "Bruyverne", "Noivern", "Vol", "Dragon", 6, "Non", "Finale", "Violet"], [716, "Xerneas", "Xerneas", "Fée", "—", 6, "Oui", "Unique", "Bleu"], [717, "Yveltal", "Yveltal", "Ténèbres", "Vol", 6, "Oui", "Unique", "Rouge"], [718, "Zygarde", "Zygarde", "Dragon", "Sol", 6, "Oui", "Unique", "Vert"], [719, "Diancie", "Diancie", "Roche", "Fée", 6, "Mythique", "Unique", "Rose"], [720, "Hoopa", "Hoopa", "Psy", "Spectre", 6, "Mythique", "Unique", "Violet"], [721, "Volcanion", "Volcanion", "Feu", "Eau", 6, "Mythique", "Unique", "Brun"], [722, "Brindibou", "Rowlet", "Plante", "Vol", 7, "Non", "Base", "Brun"], [723, "Efflèche", "Dartrix", "Plante", "Vol", 7, "Non", "Évolue", "Brun"], [724, "Archéduc", "Decidueye", "Plante", "Spectre", 7, "Non", "Finale", "Brun"], [725, "Flamiaou", "Litten", "Feu", "—", 7, "Non", "Base", "Rouge"], [726, "Matoufeu", "Torracat", "Feu", "—", 7, "Non", "Évolue", "Rouge"], [727, "Félinferno", "Incineroar", "Feu", "Ténèbres", 7, "Non", "Finale", "Rouge"], [728, "Otaquin", "Popplio", "Eau", "—", 7, "Non", "Base", "Bleu"], [729, "Otarlette", "Brionne", "Eau", "—", 7, "Non", "Évolue", "Bleu"], [730, "Oratoria", "Primarina", "Eau", "Fée", 7, "Non", "Finale", "Bleu"], [731, "Picassaut", "Pikipek", "Normal", "Vol", 7, "Non", "Base", "Noir"], [732, "Piclairon", "Trumbeak", "Normal", "Vol", 7, "Non", "Évolue", "Noir"], [733, "Bazoucan", "Toucannon", "Normal", "Vol", 7, "Non", "Finale", "Noir"], [734, "Manglouton", "Yungoos", "Normal", "—", 7, "Non", "Base", "Brun"], [735, "Argouste", "Gumshoos", "Normal", "—", 7, "Non", "Finale", "Brun"], [736, "Larvibule", "Grubbin", "Insecte", "—", 7, "Non", "Base", "Gris"], [737, "Chrysapile", "Charjabug", "Insecte", "Électrik", 7, "Non", "Évolue", "Vert"], [738, "Lucanon", "Vikavolt", "Insecte", "Électrik", 7, "Non", "Finale", "Bleu"], [739, "Crabagarre", "Crabrawler", "Combat", "—", 7, "Non", "Base", "Violet"], [740, "Crabominable", "Crabominable", "Combat", "Glace", 7, "Non", "Finale", "Blanc"], [741, "Plumeline", "Oricorio", "Feu", "Vol", 7, "Non", "Unique", "Rouge"], [742, "Bombydou", "Cutiefly", "Insecte", "Fée", 7, "Non", "Base", "Jaune"], [743, "Rubombelle", "Ribombee", "Insecte", "Fée", 7, "Non", "Finale", "Jaune"], [744, "Rocabot", "Rockruff", "Roche", "—", 7, "Non", "Base", "Brun"], [745, "Lougaroc", "Lycanroc", "Roche", "—", 7, "Non", "Finale", "Brun"], [746, "Froussardine", "Wishiwashi", "Eau", "—", 7, "Non", "Unique", "Bleu"], [747, "Vorastérie", "Mareanie", "Poison", "Eau", 7, "Non", "Base", "Bleu"], [748, "Prédastérie", "Toxapex", "Poison", "Eau", 7, "Non", "Finale", "Bleu"], [749, "Tiboudet", "Mudbray", "Sol", "—", 7, "Non", "Base", "Brun"], [750, "Bourrinos", "Mudsdale", "Sol", "—", 7, "Non", "Finale", "Brun"], [751, "Araqua", "Dewpider", "Eau", "Insecte", 7, "Non", "Base", "Vert"], [752, "Tarenbulle", "Araquanid", "Eau", "Insecte", 7, "Non", "Finale", "Vert"], [753, "Mimantis", "Fomantis", "Plante", "—", 7, "Non", "Base", "Rose"], [754, "Floramantis", "Lurantis", "Plante", "—", 7, "Non", "Finale", "Rose"], [755, "Spododo", "Morelull", "Plante", "Fée", 7, "Non", "Base", "Violet"], [756, "Lampignon", "Shiinotic", "Plante", "Fée", 7, "Non", "Finale", "Violet"], [757, "Tritox", "Salandit", "Poison", "Feu", 7, "Non", "Base", "Noir"], [758, "Malamandre", "Salazzle", "Poison", "Feu", 7, "Non", "Finale", "Noir"], [759, "Nounourson", "Stufful", "Normal", "Combat", 7, "Non", "Base", "Rose"], [760, "Chelours", "Bewear", "Normal", "Combat", 7, "Non", "Finale", "Rose"], [761, "Croquine", "Bounsweet", "Plante", "—", 7, "Non", "Base", "Violet"], [762, "Candine", "Steenee", "Plante", "—", 7, "Non", "Évolue", "Violet"], [763, "Sucreine", "Tsareena", "Plante", "—", 7, "Non", "Finale", "Violet"], [764, "Guérilande", "Comfey", "Fée", "—", 7, "Non", "Unique", "Vert"], [765, "Gouroutan", "Oranguru", "Normal", "Psy", 7, "Non", "Unique", "Blanc"], [766, "Quartermac", "Passimian", "Combat", "—", 7, "Non", "Unique", "Blanc"], [767, "Sovkipou", "Wimpod", "Insecte", "Eau", 7, "Non", "Base", "Gris"], [768, "Sarmuraï", "Golisopod", "Insecte", "Eau", 7, "Non", "Finale", "Gris"], [769, "Bacabouh", "Sandygast", "Spectre", "Sol", 7, "Non", "Base", "Brun"], [770, "Trépassable", "Palossand", "Spectre", "Sol", 7, "Non", "Finale", "Brun"], [771, "Concombaffe", "Pyukumuku", "Eau", "—", 7, "Non", "Unique", "Noir"], [772, "Type:0", "Type: Null", "Normal", "—", 7, "Oui", "Base", "Gris"], [773, "Silvallié", "Silvally", "Normal", "—", 7, "Oui", "Finale", "Gris"], [774, "Météno", "Minior", "Roche", "Vol", 7, "Non", "Unique", "Brun"], [775, "Dodoala", "Komala", "Normal", "—", 7, "Non", "Unique", "Bleu"], [776, "Boumata", "Turtonator", "Feu", "Dragon", 7, "Non", "Unique", "Rouge"], [777, "Togedemaru", "Togedemaru", "Électrik", "Acier", 7, "Non", "Unique", "Gris"], [778, "Mimiqui", "Mimikyu", "Spectre", "Fée", 7, "Non", "Unique", "Jaune"], [779, "Denticrisse", "Bruxish", "Eau", "Psy", 7, "Non", "Unique", "Rose"], [780, "Draïeul", "Drampa", "Normal", "Dragon", 7, "Non", "Unique", "Blanc"], [781, "Sinistrail", "Dhelmise", "Spectre", "Plante", 7, "Non", "Unique", "Vert"], [782, "Bébécaille", "Jangmo-o", "Dragon", "—", 7, "Non", "Base", "Gris"], [783, "Écaïd", "Hakamo-o", "Dragon", "Combat", 7, "Non", "Évolue", "Gris"], [784, "Ékaïser", "Kommo-o", "Dragon", "Combat", 7, "Non", "Finale", "Gris"], [785, "Tokorico", "Tapu Koko", "Électrik", "Fée", 7, "Oui", "Unique", "Jaune"], [786, "Tokopiyon", "Tapu Lele", "Psy", "Fée", 7, "Oui", "Unique", "Rose"], [787, "Tokotoro", "Tapu Bulu", "Plante", "Fée", 7, "Oui", "Unique", "Rouge"], [788, "Tokopisco", "Tapu Fini", "Eau", "Fée", 7, "Oui", "Unique", "Violet"], [789, "Cosmog", "Cosmog", "Psy", "—", 7, "Oui", "Base", "Bleu"], [790, "Cosmovum", "Cosmoem", "Psy", "—", 7, "Oui", "Évolue", "Bleu"], [791, "Solgaleo", "Solgaleo", "Psy", "Acier", 7, "Oui", "Finale", "Blanc"], [792, "Lunala", "Lunala", "Psy", "Spectre", 7, "Oui", "Finale", "Violet"], [793, "Zéroïd", "Nihilego", "Roche", "Poison", 7, "Non", "Unique", "Blanc"], [794, "Mouscoto", "Buzzwole", "Insecte", "Combat", 7, "Non", "Unique", "Rouge"], [795, "Cancrelove", "Pheromosa", "Insecte", "Combat", 7, "Non", "Unique", "Blanc"], [796, "Câblifère", "Xurkitree", "Électrik", "—", 7, "Non", "Unique", "Noir"], [797, "Bamboiselle", "Celesteela", "Acier", "Vol", 7, "Non", "Unique", "Vert"], [798, "Katagami", "Kartana", "Plante", "Acier", 7, "Non", "Unique", "Blanc"], [799, "Engloutyran", "Guzzlord", "Ténèbres", "Dragon", 7, "Non", "Unique", "Noir"], [800, "Necrozma", "Necrozma", "Psy", "—", 7, "Oui", "Unique", "Noir"], [801, "Magearna", "Magearna", "Acier", "Fée", 7, "Mythique", "Unique", "Gris"], [802, "Marshadow", "Marshadow", "Combat", "Spectre", 7, "Mythique", "Unique", "Gris"], [803, "Vémini", "Poipole", "Poison", "—", 7, "Non", "Base", "Violet"], [804, "Mandrillon", "Naganadel", "Poison", "Dragon", 7, "Non", "Finale", "Violet"], [805, "Ama-Ama", "Stakataka", "Roche", "Acier", 7, "Non", "Unique", "Gris"], [806, "Pierroteknik", "Blacephalon", "Feu", "Spectre", 7, "Non", "Unique", "Blanc"], [807, "Zeraora", "Zeraora", "Électrik", "—", 7, "Mythique", "Unique", "Jaune"], [808, "Meltan", "Meltan", "Acier", "—", 7, "Mythique", "Base", "Gris"], [809, "Melmetal", "Melmetal", "Acier", "—", 7, "Mythique", "Finale", "Gris"], [810, "Ouistempo", "Grookey", "Plante", "—", 8, "Non", "Base", "Vert"], [811, "Badabouin", "Thwackey", "Plante", "—", 8, "Non", "Évolue", "Vert"], [812, "Gorythmic", "Rillaboom", "Plante", "—", 8, "Non", "Finale", "Vert"], [813, "Flambino", "Scorbunny", "Feu", "—", 8, "Non", "Base", "Blanc"], [814, "Lapyro", "Raboot", "Feu", "—", 8, "Non", "Évolue", "Gris"], [815, "Pyrobut", "Cinderace", "Feu", "—", 8, "Non", "Finale", "Blanc"], [816, "Larméléon", "Sobble", "Eau", "—", 8, "Non", "Base", "Bleu"], [817, "Arrozard", "Drizzile", "Eau", "—", 8, "Non", "Évolue", "Bleu"], [818, "Lézargus", "Inteleon", "Eau", "—", 8, "Non", "Finale", "Bleu"], [819, "Rongourmand", "Skwovet", "Normal", "—", 8, "Non", "Base", "Brun"], [820, "Rongrigou", "Greedent", "Normal", "—", 8, "Non", "Finale", "Brun"], [821, "Minisange", "Rookidee", "Vol", "—", 8, "Non", "Base", "Bleu"], [822, "Bleuseille", "Corvisquire", "Vol", "—", 8, "Non", "Évolue", "Bleu"], [823, "Corvaillus", "Corviknight", "Vol", "Acier", 8, "Non", "Finale", "Violet"], [824, "Larvadar", "Blipbug", "Insecte", "—", 8, "Non", "Base", "Bleu"], [825, "Coléodôme", "Dottler", "Insecte", "Psy", 8, "Non", "Évolue", "Jaune"], [826, "Astronelle", "Orbeetle", "Insecte", "Psy", 8, "Non", "Finale", "Rouge"], [827, "Goupilou", "Nickit", "Ténèbres", "—", 8, "Non", "Base", "Brun"], [828, "Roublenard", "Thievul", "Ténèbres", "—", 8, "Non", "Finale", "Brun"], [829, "Tournicoton", "Gossifleur", "Plante", "—", 8, "Non", "Base", "Vert"], [830, "Blancoton", "Eldegoss", "Plante", "—", 8, "Non", "Finale", "Vert"], [831, "Moumouton", "Wooloo", "Normal", "—", 8, "Non", "Base", "Blanc"], [832, "Moumouflon", "Dubwool", "Normal", "—", 8, "Non", "Finale", "Blanc"], [833, "Khélocrok", "Chewtle", "Eau", "—", 8, "Non", "Base", "Vert"], [834, "Torgamord", "Drednaw", "Eau", "Roche", 8, "Non", "Finale", "Vert"], [835, "Voltoutou", "Yamper", "Électrik", "—", 8, "Non", "Base", "Jaune"], [836, "Fulgudog", "Boltund", "Électrik", "—", 8, "Non", "Finale", "Jaune"], [837, "Charbi", "Rolycoly", "Roche", "—", 8, "Non", "Base", "Noir"], [838, "Wagomine", "Carkol", "Roche", "Feu", 8, "Non", "Évolue", "Noir"], [839, "Monthracite", "Coalossal", "Roche", "Feu", 8, "Non", "Finale", "Noir"], [840, "Verpom", "Applin", "Plante", "Dragon", 8, "Non", "Base", "Vert"], [841, "Pomdrapi", "Flapple", "Plante", "Dragon", 8, "Non", "Finale", "Vert"], [842, "Dratatin", "Appletun", "Plante", "Dragon", 8, "Non", "Finale", "Vert"], [843, "Dunaja", "Silicobra", "Sol", "—", 8, "Non", "Base", "Vert"], [844, "Dunaconda", "Sandaconda", "Sol", "—", 8, "Non", "Finale", "Vert"], [845, "Nigosier", "Cramorant", "Vol", "Eau", 8, "Non", "Unique", "Bleu"], [846, "Embrochet", "Arrokuda", "Eau", "—", 8, "Non", "Base", "Brun"], [847, "Hastacuda", "Barraskewda", "Eau", "—", 8, "Non", "Finale", "Brun"], [848, "Toxizap", "Toxel", "Électrik", "Poison", 8, "Non", "Base", "Violet"], [849, "Salarsen", "Toxtricity", "Électrik", "Poison", 8, "Non", "Finale", "Violet"], [850, "Grillepattes", "Sizzlipede", "Feu", "Insecte", 8, "Non", "Base", "Rouge"], [851, "Scolocendre", "Centiskorch", "Feu", "Insecte", 8, "Non", "Finale", "Rouge"], [852, "Poulpaf", "Clobbopus", "Combat", "—", 8, "Non", "Base", "Brun"], [853, "Krakos", "Grapploct", "Combat", "—", 8, "Non", "Finale", "Bleu"], [854, "Théffroi", "Sinistea", "Spectre", "—", 8, "Non", "Base", "Violet"], [855, "Polthégeist", "Polteageist", "Spectre", "—", 8, "Non", "Finale", "Violet"], [856, "Bibichut", "Hatenna", "Psy", "—", 8, "Non", "Base", "Rose"], [857, "Chapotus", "Hattrem", "Psy", "—", 8, "Non", "Évolue", "Rose"], [858, "Sorcilence", "Hatterene", "Psy", "Fée", 8, "Non", "Finale", "Rose"], [859, "Grimalin", "Impidimp", "Ténèbres", "Fée", 8, "Non", "Base", "Rose"], [860, "Fourbelin", "Morgrem", "Ténèbres", "Fée", 8, "Non", "Évolue", "Rose"], [861, "Angoliath", "Grimmsnarl", "Ténèbres", "Fée", 8, "Non", "Finale", "Violet"], [862, "Ixon", "Obstagoon", "Ténèbres", "Normal", 8, "Non", "Finale", "Gris"], [863, "Berserkatt", "Perrserker", "Acier", "—", 8, "Non", "Finale", "Brun"], [864, "Corayôme", "Cursola", "Spectre", "—", 8, "Non", "Finale", "Blanc"], [865, "Palarticho", "Sirfetch’d", "Combat", "—", 8, "Non", "Finale", "Blanc"], [866, "M. Glaquette", "Mr. Rime", "Glace", "Psy", 8, "Non", "Finale", "Violet"], [867, "Tutétékri", "Runerigus", "Sol", "Spectre", 8, "Non", "Finale", "Gris"], [868, "Crèmy", "Milcery", "Fée", "—", 8, "Non", "Base", "Blanc"], [869, "Charmilly", "Alcremie", "Fée", "—", 8, "Non", "Finale", "Blanc"], [870, "Hexadron", "Falinks", "Combat", "—", 8, "Non", "Unique", "Jaune"], [871, "Wattapik", "Pincurchin", "Électrik", "—", 8, "Non", "Unique", "Violet"], [872, "Frissonille", "Snom", "Glace", "Insecte", 8, "Non", "Base", "Blanc"], [873, "Beldeneige", "Frosmoth", "Glace", "Insecte", 8, "Non", "Finale", "Blanc"], [874, "Dolman", "Stonjourner", "Roche", "—", 8, "Non", "Unique", "Gris"], [875, "Bekaglaçon", "Eiscue", "Glace", "—", 8, "Non", "Unique", "Bleu"], [876, "Wimessir", "Indeedee", "Psy", "Normal", 8, "Non", "Unique", "Violet"], [877, "Morpeko", "Morpeko", "Électrik", "Ténèbres", 8, "Non", "Unique", "Jaune"], [878, "Charibari", "Cufant", "Acier", "—", 8, "Non", "Base", "Jaune"], [879, "Pachyradjah", "Copperajah", "Acier", "—", 8, "Non", "Finale", "Vert"], [880, "Galvagon", "Dracozolt", "Électrik", "Dragon", 8, "Non", "Unique", "Vert"], [881, "Galvagla", "Arctozolt", "Électrik", "Glace", 8, "Non", "Unique", "Bleu"], [882, "Hydragon", "Dracovish", "Eau", "Dragon", 8, "Non", "Unique", "Vert"], [883, "Hydragla", "Arctovish", "Eau", "Glace", 8, "Non", "Unique", "Bleu"], [884, "Duralugon", "Duraludon", "Acier", "Dragon", 8, "Non", "Base", "Blanc"], [885, "Fantyrm", "Dreepy", "Dragon", "Spectre", 8, "Non", "Base", "Vert"], [886, "Dispareptil", "Drakloak", "Dragon", "Spectre", 8, "Non", "Évolue", "Vert"], [887, "Lanssorien", "Dragapult", "Dragon", "Spectre", 8, "Non", "Finale", "Vert"], [888, "Zacian", "Zacian", "Fée", "—", 8, "Oui", "Unique", "Bleu"], [889, "Zamazenta", "Zamazenta", "Combat", "—", 8, "Oui", "Unique", "Rouge"], [890, "Éthernatos", "Eternatus", "Poison", "Dragon", 8, "Oui", "Unique", "Violet"], [891, "Wushours", "Kubfu", "Combat", "—", 8, "Oui", "Base", "Gris"], [892, "Shifours", "Urshifu", "Combat", "Ténèbres", 8, "Oui", "Finale", "Gris"], [893, "Zarude", "Zarude", "Ténèbres", "Plante", 8, "Mythique", "Unique", "Vert"], [894, "Regieleki", "Regieleki", "Électrik", "—", 8, "Oui", "Unique", "Jaune"], [895, "Regidrago", "Regidrago", "Dragon", "—", 8, "Oui", "Unique", "Vert"], [896, "Blizzeval", "Glastrier", "Glace", "—", 8, "Oui", "Unique", "Blanc"], [897, "Spectreval", "Spectrier", "Spectre", "—", 8, "Oui", "Unique", "Noir"], [898, "Sylveroy", "Calyrex", "Psy", "Plante", 8, "Oui", "Unique", "Vert"], [899, "Cerbyllin", "Wyrdeer", "Normal", "Psy", 8, "Non", "Finale", "Gris"], [900, "Hachécateur", "Kleavor", "Insecte", "Roche", 8, "Non", "Finale", "Brun"], [901, "Ursaking", "Ursaluna", "Sol", "Normal", 8, "Non", "Finale", "Brun"], [902, "Paragruel", "Basculegion", "Eau", "Spectre", 8, "Non", "Finale", "Vert"], [903, "Farfurex", "Sneasler", "Combat", "Poison", 8, "Non", "Finale", "Gris"], [904, "Qwilpik", "Overqwil", "Ténèbres", "Poison", 8, "Non", "Finale", "Noir"], [905, "Amovénus", "Enamorus", "Fée", "Vol", 8, "Oui", "Unique", "Rose"], [906, "Poussacha", "Sprigatito", "Plante", "—", 9, "Non", "Base", "Vert"], [907, "Matourgeon", "Floragato", "Plante", "—", 9, "Non", "Évolue", "Vert"], [908, "Miascarade", "Meowscarada", "Plante", "Ténèbres", 9, "Non", "Finale", "Vert"], [909, "Chochodile", "Fuecoco", "Feu", "—", 9, "Non", "Base", "Rouge"], [910, "Crocogril", "Crocalor", "Feu", "—", 9, "Non", "Évolue", "Rouge"], [911, "Flâmigator", "Skeledirge", "Feu", "Spectre", 9, "Non", "Finale", "Rouge"], [912, "Coiffeton", "Quaxly", "Eau", "—", 9, "Non", "Base", "Blanc"], [913, "Canarbello", "Quaxwell", "Eau", "—", 9, "Non", "Évolue", "Bleu"], [914, "Palmaval", "Quaquaval", "Eau", "Combat", 9, "Non", "Finale", "Bleu"], [915, "Gourmelet", "Lechonk", "Normal", "—", 9, "Non", "Base", "Gris"], [916, "Fragroin", "Oinkologne", "Normal", "—", 9, "Non", "Finale", "Gris"], [917, "Tissenboule", "Tarountula", "Insecte", "—", 9, "Non", "Base", "Blanc"], [918, "Filentrappe", "Spidops", "Insecte", "—", 9, "Non", "Finale", "Vert"], [919, "Lilliterelle", "Nymble", "Insecte", "—", 9, "Non", "Base", "Gris"], [920, "Gambex", "Lokix", "Insecte", "Ténèbres", 9, "Non", "Finale", "Gris"], [921, "Pohm", "Pawmi", "Électrik", "—", 9, "Non", "Base", "Jaune"], [922, "Pohmotte", "Pawmo", "Électrik", "Combat", 9, "Non", "Évolue", "Jaune"], [923, "Pohmarmotte", "Pawmot", "Électrik", "Combat", 9, "Non", "Finale", "Jaune"], [924, "Compagnol", "Tandemaus", "Normal", "—", 9, "Non", "Base", "Blanc"], [925, "Famignol", "Maushold", "Normal", "—", 9, "Non", "Finale", "Blanc"], [926, "Pâtachiot", "Fidough", "Fée", "—", 9, "Non", "Base", "Jaune"], [927, "Briochien", "Dachsbun", "Fée", "—", 9, "Non", "Finale", "Brun"], [928, "Olivini", "Smoliv", "Plante", "Normal", 9, "Non", "Base", "Vert"], [929, "Olivado", "Dolliv", "Plante", "Normal", 9, "Non", "Évolue", "Vert"], [930, "Arboliva", "Arboliva", "Plante", "Normal", 9, "Non", "Finale", "Vert"], [931, "Tapatoès", "Squawkabilly", "Normal", "Vol", 9, "Non", "Unique", "Vert"], [932, "Selutin", "Nacli", "Roche", "—", 9, "Non", "Base", "Brun"], [933, "Amassel", "Naclstack", "Roche", "—", 9, "Non", "Évolue", "Brun"], [934, "Gigansel", "Garganacl", "Roche", "—", 9, "Non", "Finale", "Brun"], [935, "Charbambin", "Charcadet", "Feu", "—", 9, "Non", "Base", "Rouge"], [936, "Carmadura", "Armarouge", "Feu", "Psy", 9, "Non", "Finale", "Rouge"], [937, "Malvalame", "Ceruledge", "Feu", "Spectre", 9, "Non", "Finale", "Bleu"], [938, "Têtampoule", "Tadbulb", "Électrik", "—", 9, "Non", "Base", "Jaune"], [939, "Ampibidou", "Bellibolt", "Électrik", "—", 9, "Non", "Finale", "Vert"], [940, "Zapétrel", "Wattrel", "Électrik", "Vol", 9, "Non", "Base", "Noir"], [941, "Fulgulairo", "Kilowattrel", "Électrik", "Vol", 9, "Non", "Finale", "Jaune"], [942, "Grondogue", "Maschiff", "Ténèbres", "—", 9, "Non", "Base", "Brun"], [943, "Dogrino", "Mabosstiff", "Ténèbres", "—", 9, "Non", "Finale", "Gris"], [944, "Gribouraigne", "Shroodle", "Poison", "Normal", 9, "Non", "Base", "Gris"], [945, "Tag-Tag", "Grafaiai", "Poison", "Normal", 9, "Non", "Finale", "Gris"], [946, "Virovent", "Bramblin", "Plante", "Spectre", 9, "Non", "Base", "Brun"], [947, "Virevorreur", "Brambleghast", "Plante", "Spectre", 9, "Non", "Finale", "Brun"], [948, "Terracool", "Toedscool", "Sol", "Plante", 9, "Non", "Base", "Jaune"], [949, "Terracruel", "Toedscruel", "Sol", "Plante", 9, "Non", "Finale", "Noir"], [950, "Craparoi", "Klawf", "Roche", "—", 9, "Non", "Unique", "Rouge"], [951, "Pimito", "Capsakid", "Plante", "—", 9, "Non", "Base", "Vert"], [952, "Scovilain", "Scovillain", "Plante", "Feu", 9, "Non", "Finale", "Vert"], [953, "Léboulérou", "Rellor", "Insecte", "—", 9, "Non", "Base", "Brun"], [954, "Bérasca", "Rabsca", "Insecte", "Psy", 9, "Non", "Finale", "Vert"], [955, "Flotillon", "Flittle", "Psy", "—", 9, "Non", "Base", "Jaune"], [956, "Cléopsytra", "Espathra", "Psy", "—", 9, "Non", "Finale", "Jaune"], [957, "Forgerette", "Tinkatink", "Fée", "Acier", 9, "Non", "Base", "Rose"], [958, "Forgella", "Tinkatuff", "Fée", "Acier", 9, "Non", "Évolue", "Rose"], [959, "Forgelina", "Tinkaton", "Fée", "Acier", 9, "Non", "Finale", "Rose"], [960, "Taupikeau", "Wiglett", "Eau", "—", 9, "Non", "Base", "Blanc"], [961, "Triopikeau", "Wugtrio", "Eau", "—", 9, "Non", "Finale", "Rouge"], [962, "Lestombaile", "Bombirdier", "Vol", "Ténèbres", 9, "Non", "Unique", "Blanc"], [963, "Dofin", "Finizen", "Eau", "—", 9, "Non", "Base", "Bleu"], [964, "Superdofin", "Palafin", "Eau", "—", 9, "Non", "Finale", "Bleu"], [965, "Vrombi", "Varoom", "Acier", "Poison", 9, "Non", "Base", "Gris"], [966, "Vrombotor", "Revavroom", "Acier", "Poison", 9, "Non", "Finale", "Gris"], [967, "Motorizard", "Cyclizar", "Dragon", "Normal", 9, "Non", "Unique", "Vert"], [968, "Ferdeter", "Orthworm", "Acier", "—", 9, "Non", "Unique", "Rose"], [969, "Germéclat", "Glimmet", "Roche", "Poison", 9, "Non", "Base", "Bleu"], [970, "Floréclat", "Glimmora", "Roche", "Poison", 9, "Non", "Finale", "Bleu"], [971, "Toutombe", "Greavard", "Spectre", "—", 9, "Non", "Base", "Blanc"], [972, "Tomberro", "Houndstone", "Spectre", "—", 9, "Non", "Finale", "Blanc"], [973, "Flamenroule", "Flamigo", "Vol", "Combat", 9, "Non", "Unique", "Rose"], [974, "Piétacé", "Cetoddle", "Glace", "—", 9, "Non", "Base", "Blanc"], [975, "Balbalèze", "Cetitan", "Glace", "—", 9, "Non", "Finale", "Blanc"], [976, "Délestin", "Veluza", "Eau", "Psy", 9, "Non", "Unique", "Gris"], [977, "Oyacata", "Dondozo", "Eau", "—", 9, "Non", "Unique", "Bleu"], [978, "Nigirigon", "Tatsugiri", "Dragon", "Eau", 9, "Non", "Unique", "Rose"], [979, "Courrousinge", "Annihilape", "Combat", "Spectre", 9, "Non", "Finale", "Gris"], [980, "Terraiste", "Clodsire", "Poison", "Sol", 9, "Non", "Finale", "Brun"], [981, "Farigiraf", "Farigiraf", "Normal", "Psy", 9, "Non", "Finale", "Brun"], [982, "Deusolourdo", "Dudunsparce", "Normal", "—", 9, "Non", "Finale", "Jaune"], [983, "Scalpereur", "Kingambit", "Ténèbres", "Acier", 9, "Non", "Finale", "Noir"], [984, "Fort-Ivoire", "Great Tusk", "Sol", "Combat", 9, "Non", "Unique", "Violet"], [985, "Hurle-Queue", "Scream Tail", "Fée", "Psy", 9, "Non", "Unique", "Rose"], [986, "Fongus-Furie", "Brute Bonnet", "Plante", "Ténèbres", 9, "Non", "Unique", "Gris"], [987, "Flotte-Mèche", "Flutter Mane", "Spectre", "Fée", 9, "Non", "Unique", "Gris"], [988, "Rampe-Ailes", "Slither Wing", "Insecte", "Combat", 9, "Non", "Unique", "Rouge"], [989, "Pelage-Sablé", "Sandy Shocks", "Électrik", "Sol", 9, "Non", "Unique", "Gris"], [990, "Roue-de-Fer", "Iron Treads", "Sol", "Acier", 9, "Non", "Unique", "Gris"], [991, "Hotte-de-Fer", "Iron Bundle", "Glace", "Eau", 9, "Non", "Unique", "Rouge"], [992, "Paume-de-Fer", "Iron Hands", "Combat", "Électrik", 9, "Non", "Unique", "Gris"], [993, "Têtes-de-Fer", "Iron Jugulis", "Ténèbres", "Vol", 9, "Non", "Unique", "Bleu"], [994, "Mite-de-Fer", "Iron Moth", "Feu", "Poison", 9, "Non", "Unique", "Jaune"], [995, "Épine-de-Fer", "Iron Thorns", "Roche", "Électrik", 9, "Non", "Unique", "Vert"], [996, "Frigodo", "Frigibax", "Dragon", "Glace", 9, "Non", "Base", "Gris"], [997, "Cryodo", "Arctibax", "Dragon", "Glace", 9, "Non", "Évolue", "Gris"], [998, "Glaivodo", "Baxcalibur", "Dragon", "Glace", 9, "Non", "Finale", "Gris"], [999, "Mordudor", "Gimmighoul", "Spectre", "—", 9, "Non", "Base", "Brun"], [1000, "Gromago", "Gholdengo", "Acier", "Spectre", 9, "Non", "Finale", "Jaune"], [1001, "Chongjian", "Wo-Chien", "Ténèbres", "Plante", 9, "Oui", "Unique", "Brun"], [1002, "Baojian", "Chien-Pao", "Ténèbres", "Glace", 9, "Oui", "Unique", "Blanc"], [1003, "Dinglu", "Ting-Lu", "Ténèbres", "Sol", 9, "Oui", "Unique", "Brun"], [1004, "Yuyu", "Chi-Yu", "Ténèbres", "Feu", 9, "Oui", "Unique", "Rouge"], [1005, "Rugit-Lune", "Roaring Moon", "Dragon", "Ténèbres", 9, "Non", "Unique", "Vert"], [1006, "Garde-de-Fer", "Iron Valiant", "Fée", "Combat", 9, "Non", "Unique", "Blanc"], [1007, "Koraidon", "Koraidon", "Combat", "Dragon", 9, "Oui", "Unique", "Rouge"], [1008, "Miraidon", "Miraidon", "Électrik", "Dragon", 9, "Oui", "Unique", "Bleu"], [1009, "Serpente-Eau", "Walking Wake", "Eau", "Dragon", 9, "Non", "Unique", "Bleu"], [1010, "Vert-de-Fer", "Iron Leaves", "Plante", "Psy", 9, "Non", "Unique", "Vert"], [1011, "Pomdramour", "Dipplin", "Plante", "Dragon", 9, "Non", "Évolue", "Vert"], [1012, "Poltchageist", "Poltchageist", "Plante", "Spectre", 9, "Non", "Base", "Vert"], [1013, "Théffroyable", "Sinistcha", "Plante", "Spectre", 9, "Non", "Finale", "Vert"], [1014, "Félicanis", "Okidogi", "Poison", "Combat", 9, "Oui", "Unique", "Noir"], [1015, "Fortusimia", "Munkidori", "Poison", "Psy", 9, "Oui", "Unique", "Noir"], [1016, "Favianos", "Fezandipiti", "Poison", "Fée", 9, "Oui", "Unique", "Noir"], [1017, "Ogerpon", "Ogerpon", "Plante", "—", 9, "Oui", "Unique", "Vert"], [1018, "Pondralugon", "Archaludon", "Acier", "Dragon", 9, "Non", "Finale", "Blanc"], [1019, "Pomdorochi", "Hydrapple", "Plante", "Dragon", 9, "Non", "Finale", "Vert"], [1020, "Feu-Perçant", "Gouging Fire", "Feu", "Dragon", 9, "Non", "Unique", "Brun"], [1021, "Ire-Foudre", "Raging Bolt", "Électrik", "Dragon", 9, "Non", "Unique", "Jaune"], [1022, "Roc-de-Fer", "Iron Boulder", "Roche", "Psy", 9, "Non", "Unique", "Gris"], [1023, "Chef-de-Fer", "Iron Crown", "Acier", "Psy", 9, "Non", "Unique", "Bleu"], [1024, "Terapagos", "Terapagos", "Normal", "—", 9, "Oui", "Unique", "Bleu"], [1025, "Pêchaminus", "Pecharunt", "Poison", "Spectre", 9, "Mythique", "Unique", "Violet"]];

const POKEMON_IMAGE_BY_NAME = {};
(function installFullPokedex() {
    const profiles = {};
    const names = [];
    const aliases = {};
    for (const [id, fr, en, t1, t2, gen, leg, evo, color] of POKEDEX_FULL) {
        profiles[fr] = { c0: t1, c1: t2, c2: String(gen), c3: leg, c4: evo, c5: color };
        names.push(fr);
        POKEMON_IMAGE_BY_NAME[fr] = `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/official-artwork/${id}.png`;
        for (const norm of [normalizeDle, normalizeRG]) {
            const e = norm(en), f = norm(fr);
            if (e && f && e !== f) aliases[e] = f;
        }
    }
    // Le Pokédex remplace entièrement l'ancienne liste Pokémon (mélange FR/EN, ~80 fiches)
    DLE_STATIC_PROFILES.pokemon = profiles;
    DLE_TARGET_NAMES.pokemon = names.slice();
    DLE_MASTER_NAMES.pokemon = names.slice();
    DLE_MASTER_ALIASES.pokemon = aliases;
    if (DLE_UNIVERSES.pokemon) {
        DLE_UNIVERSES.pokemon.characters = [];
        const gen = DLE_UNIVERSES.pokemon.categories.find(c => c.key === 'c2');
        if (gen) gen.label = 'Génération';
        const leg = DLE_UNIVERSES.pokemon.categories.find(c => c.key === 'c3');
        if (leg) leg.label = 'Légendaire';
        const evo = DLE_UNIVERSES.pokemon.categories.find(c => c.key === 'c4');
        if (evo) evo.label = 'Stade d\'évolution';
        const col = DLE_UNIVERSES.pokemon.categories.find(c => c.key === 'c5');
        if (col) col.label = 'Couleur';
    }
})();
...
function dleExpandedUniverse(key) {
    ...
    // Les fiches statiques détaillées priment sur les fiches de base (plus anciennes et simplifiées :
    // ex. Naruto « Vent » seul au lieu de toutes ses natures de chakra).
    const staticCanon = new Set(Object.entries(DLE_STATIC_PROFILES[key] || {})
        .filter(([, attrs]) => isCompleteDleAttrs(attrs, base.categories))
        .map(([n]) => dleMasterCanonicalNorm(key, n)).filter(Boolean));

    for (const c of (base.characters || [])) {
        const canonical = dleMasterCanonicalNorm(key, c.name);
        if (!canonical || byCanonical.has(canonical) || staticCanon.has(canonical)) continue;
    ...
    // V19 : corrections vérifiées + retrait des doublons de forme (Goku Black = Zamasu, Estarossa = Mael...)
    const excluded = new Set((DLE_EXCLUDE[key] || []).map(n => dleMasterCanonicalNorm(key, n) || normalizeDle(n)));
    const fixes = DLE_PROFILE_FIXES[key] || {};
    const fixByCanon = new Map(Object.entries(fixes).map(([n, f]) => [dleMasterCanonicalNorm(key, n) || normalizeDle(n), f]));
    const characters = [];
    for (const [canon, c] of byCanonical) {
        if (excluded.has(canon)) continue;
        const f = fixByCanon.get(canon);
        characters.push(f ? { ...c, attrs: { ...c.attrs, ...f } } : c);
    }

    return { name: base.name, categories: [...(base.categories || [])], customCategoryCount:(base.categories||[]).length, characters };
}

// ================= AnimeDLE V19 — fiches corrigées =================
// Valeurs revues à la main (natures de chakra complètes, affiliations homogènes, numéros, etc.)
const DLE_PROFILE_FIXES = {"naruto": {"Naruto Uzumaki": {"c3": "Vent, Feu, Foudre, Terre, Eau, Yin, Yang", "c5": "Oui — Kurama"}, "Sasuke Uchiha": {"c3": "Feu, Foudre, Vent, Terre, Eau, Yin, Yang", "c4": "Sharingan, Rinnegan", "c1": "Shinobi indépendant"}, "Sakura Haruno": {"c3": "Terre, Eau, Yin, Yang"}, "Kakashi Hatake": {"c3": "Foudre, Terre, Eau, Feu, Vent, Yin, Yang", "c4": "Sharingan"}, "Itachi Uchiha": {"c3": "Feu, Eau, Vent, Yin, Yang", "c4": "Mangekyō Sharingan"}, "Gaara": {"c3": "Vent, Terre, Magnétisme", "c5": "Ancien — Shukaku"}, "Minato Namikaze": {"c3": "Vent, Foudre, Feu, Yang"}, "Madara Uchiha": {"c3": "Feu, Vent, Foudre, Terre, Eau, Bois, Yin, Yang", "c4": "Sharingan, Rinnegan", "c5": "Ancien — Jūbi"}, "Hashirama Senju": {"c3": "Terre, Eau, Bois, Yang"}, "Jiraiya": {"c3": "Feu, Vent, Terre, Eau, Yang"}, "Orochimaru": {"c3": "Vent, Feu, Foudre, Terre, Eau, Yin, Yang"}, "Tsunade": {"c3": "Foudre, Terre, Eau, Feu, Yin, Yang"}, "Rock Lee": {"c3": "Aucune"}, "Metal Lee": {"c3": "Aucune"}, "Mifune": {"c3": "Aucune"}, "Hidan": {"c3": "Aucune"}, "Kimimaro": {"c3": "Aucune"}, "Hiruzen Sarutobi": {"c3": "Feu, Vent, Foudre, Terre, Eau, Yin, Yang"}, "Yamato": {"c3": "Eau, Terre, Bois"}, "Danzō Shimura": {"c3": "Vent, Terre, Eau, Feu, Bois, Yin, Yang"}, "Obito Uchiha": {"c3": "Feu, Vent, Foudre, Terre, Eau, Bois, Yin, Yang"}, "Nagato": {"c3": "Feu, Vent, Foudre, Terre, Eau, Yin, Yang"}, "Deidara": {"c3": "Terre, Foudre, Explosion"}, "Kakuzu": {"c3": "Feu, Vent, Foudre, Terre, Eau"}, "Kabuto Yakushi": {"c3": "Feu, Vent, Foudre, Terre, Eau, Yin, Yang"}, "Darui": {"c3": "Foudre, Eau, Tempête"}, "Rasa": {"c3": "Vent, Terre, Magnétisme"}, "Mei Terumī": {"c3": "Eau, Feu, Terre, Ébullition, Lave"}, "Haku": {"c3": "Eau, Vent, Glace"}, "Mū": {"c3": "Vent, Terre, Feu, Poussière"}, "Ōnoki": {"c3": "Vent, Terre, Feu, Poussière"}, "Kurotsuchi": {"c3": "Terre, Feu, Eau, Lave"}, "Gengetsu Hōzuki": {"c3": "Eau, Feu, Ébullition"}, "Tobirama Senju": {"c3": "Eau, Feu, Vent, Foudre, Terre, Yin, Yang"}, "Mito Uzumaki": {"c3": "Yin, Yang"}, "Hagoromo Ōtsutsuki": {"c3": "Feu, Vent, Foudre, Terre, Eau, Yin, Yang"}, "Kaguya Ōtsutsuki": {"c3": "Feu, Vent, Foudre, Terre, Eau, Yin, Yang"}, "Momoshiki Ōtsutsuki": {"c3": "Feu, Vent, Foudre, Terre, Eau, Yin, Yang"}, "Kurama": {"c3": "Feu, Vent, Yin, Yang"}, "Shukaku": {"c3": "Vent, Terre, Magnétisme"}, "Son Gokū": {"c3": "Feu, Terre, Lave"}, "Rōshi": {"c3": "Feu, Terre, Lave"}, "Han": {"c3": "Feu, Eau, Ébullition"}, "Kokuō": {"c3": "Feu, Eau, Ébullition"}, "Jūgo": {"c3": "Terre, Yang, Senjutsu"}, "Mitsuki": {"c3": "Vent, Foudre, Senjutsu"}}, "onepiece": {"Monkey D. Luffy": {"c2": "Hito Hito no Mi, modèle Nika", "c3": "Zoan mythique"}, "Kaido": {"c0": "Pirates aux Cent Bêtes", "c1": "Capitaine"}, "King": {"c0": "Pirates aux Cent Bêtes"}, "Boa Hancock": {"c0": "Pirates Kuja"}, "Shanks": {"c0": "Pirates du Roux"}, "Portgas D. Ace": {"c0": "Pirates de Barbe Blanche", "c2": "Mera Mera no Mi"}, "Marco": {"c0": "Pirates de Barbe Blanche"}, "Edward Newgate": {"c0": "Pirates de Barbe Blanche"}, "Marshall D. Teach": {"c0": "Pirates de Barbe Noire"}, "Sabo": {"c2": "Mera Mera no Mi"}, "Nico Robin": {"c2": "Hana Hana no Mi"}, "Kuzan": {"c0": "Pirates de Barbe Noire / ex-Marine"}, "Koby": {"c1": "Capitaine de la Marine"}, "Tashigi": {"c1": "Capitaine de la Marine"}}, "bleach": {"Sosuke Aizen": {"c1": "Hueco Mundo / Aizen", "c2": "Ex-5e", "c4": "Oui"}, "Uryu Ishida": {"c1": "Karakura / Wandenreich"}, "Yhwach": {"c5": "Empire Quincy"}, "Coyote Starrk": {"c2": "1re Espada"}, "Tier Harribel": {"c2": "3e Espada"}, "Nnoitra Gilga": {"c2": "5e Espada"}, "Szayelaporro Granz": {"c2": "8e Espada"}, "Jugram Haschwalth": {"c2": "Sternritter B"}, "Bazz-B": {"c2": "Sternritter H"}, "Isshin Kurosaki": {"c1": "Ex-Gotei 13"}, "Jushiro Ukitake": {"c4": "Non"}, "Kisuke Urahara": {"c2": "Ex-12e"}}, "hxh": {"Hisoka Morow": {"c2": "Brigade Fantôme (infiltré)"}, "Chrollo Lucilfer": {"c0": "Brigand"}, "Illumi Zoldyck": {"c2": "Famille Zoldyck / Brigade Fantôme"}, "Menthuthuyoupi": {"c1": "Transformation", "c4": "Fourmi-Chimère"}, "Shaiapouf": {"c4": "Fourmi-Chimère"}, "Knuckle Bine": {"c1": "Émission"}, "Zeno Zoldyck": {"c5": "Zoldyck Family"}, "Leorio Paradinight": {"c2": "Association des Hunters"}, "Isaac Netero": {"c2": "Association des Hunters"}}, "snk": {"Eren Yeager": {"c2": "Assaillant, Originel, Marteau d'armes"}, "Annie Leonhart": {"c3": "Liberio"}, "Bertholdt Hoover": {"c3": "Liberio"}, "Pieck Finger": {"c3": "Liberio"}, "Porco Galliard": {"c3": "Liberio"}, "Historia Reiss": {"c1": "104e / Bataillon"}, "Marco Bott": {"c1": "104e"}, "Floch Forster": {"c5": "Roux"}, "Keith Shadis": {"c5": "Chauve"}, "Frieda Reiss": {"c5": "Noir"}, "Grisha Yeager": {"c2": "Assaillant, Originel"}}, "deathnote": {"Mello": {"c0": "Mello", "c2": "Oui"}}, "cote": {"Suzune Horikita": {"c2": "Oui"}, "Miyabi Nagumo": {"c1": "2e"}, "Arisu Sakayanagi": {"c5": "Violet"}, "Ichika Amasawa": {"c0": "A"}}, "solo": {"Bellion": {"c1": "Grand Maréchal"}, "Antares": {"c1": "Monarque"}, "Ashborn": {"c5": "Non"}}, "clover": {"Luck Voltia": {"c4": "Roturier"}, "Asta": {"c4": "Roturier"}, "Magna Swing": {"c4": "Roturier"}, "Noelle Silva": {"c4": "Royal"}, "Mereoleona Vermillion": {"c4": "Royal"}, "Nebra Silva": {"c4": "Royal"}, "Julius Novachrono": {"c5": "Aucun"}}, "mushoku": {"Zenith Greyrat": {"c0": "Humaine"}, "Lilia Greyrat": {"c0": "Humaine"}, "Norn Greyrat": {"c0": "Humaine"}, "Aisha Greyrat": {"c0": "Humaine"}, "Ghislaine Dedoldia": {"c0": "Bestiale"}, "Perugius Dola": {"c0": "Dragon"}, "Eris Boreas Greyrat": {"c2": "Aucune"}}, "rezero": {"Felix Argyle": {"c5": "Brun"}, "Anastasia Hoshin": {"c0": "Humaine"}, "Felt": {"c0": "Humaine"}}, "fairy": {"Acnologia": {"c0": "Aucune", "c1": "Dragon Slayer", "c2": "Oui"}, "Zeref Dragneel": {"c0": "Alvarez"}}, "bluelock": {"Yoichi Isagi": {"c5": "Noir"}, "Julian Loki": {"c4": "Oui"}, "Charles Chevalier": {"c4": "Oui"}}, "fma": {"Edward Elric": {"c1": "Armée d'Amestris"}, "Roy Mustang": {"c1": "Armée d'Amestris"}, "Riza Hawkeye": {"c1": "Armée d'Amestris"}, "King Bradley": {"c1": "Armée d'Amestris / Homunculi"}, "Lust": {"c1": "Homunculi"}, "Gluttony": {"c1": "Homunculi"}, "Sloth": {"c1": "Homunculi"}, "Selim Bradley": {"c1": "Homunculi"}, "Olivier Mira Armstrong": {"c0": "Humaine", "c1": "Armée d'Amestris / Fort Briggs"}, "Buccaneer": {"c1": "Armée d'Amestris / Fort Briggs"}, "Miles": {"c1": "Armée d'Amestris / Fort Briggs"}, "Barry the Chopper": {"c5": "Aucun"}, "Scar": {"c4": "Non"}}, "chainsaw": {"Kishibe": {"c5": "Gris"}, "Pochita": {"c4": "Homme"}, "Asa Mitaka": {"c0": "Humaine / hôte de Yoru"}}, "wakfu": {"Oropo": {"c0": "Eliatrope"}}, "demonslayer": {"Giyu Tomioka": {"c1": "Pilier"}, "Kyojuro Rengoku": {"c1": "Pilier", "c5": "Blond/Rouge"}, "Shinobu Kocho": {"c1": "Pilier"}, "Kokushibo": {"c1": "Lune Supérieure 1"}, "Akaza": {"c1": "Lune Supérieure 3"}, "Rui": {"c1": "Lune Inférieure 5"}, "Enmu": {"c1": "Lune Inférieure 1"}}, "dragonball": {"Freezer": {"c1": "Antagoniste"}, "Cell": {"c1": "Antagoniste"}, "Jiren": {"c0": "Extraterrestre"}, "Babidi": {"c0": "Extraterrestre"}}, "haikyuu": {"Kenji Futakuchi": {"c3": "2"}}, "jjk": {"Ryomen Sukuna": {"c0": "Fléaux"}, "Mahito": {"c0": "Fléaux"}, "Suguru Geto": {"c0": "Antagoniste"}, "Kenjaku": {"c0": "Antagoniste"}}, "jojo": {"Joseph Joestar": {"c3": "Courte portée"}, "Yukako Yamagishi": {"c3": "Longue portée"}, "Risotto Nero": {"c3": "Longue portée"}, "Emporio Alnino": {"c3": "Longue portée"}}, "tensura": {"Testarossa": {"c0": "Démon primordial"}, "Carrera": {"c0": "Démon primordial"}, "Ultima": {"c0": "Démon primordial"}}, "sao": {"Asuna Yuuki": {"c5": "Brun"}, "Heathcliff": {"c5": "Gris"}, "Eugeo": {"c1": "Humain d'Underworld", "c3": "Académie impériale"}, "Alice Zuberg": {"c3": "Integrity Knights"}, "Chudelkin": {"c0": "Underworld"}}, "tokyoghoul": {"Kishou Arima": {"c4": "Inspecteur spécial"}, "Juuzou Suzuya": {"c4": "Inspecteur spécial"}, "Nimura Furuta": {"c4": "Inspecteur spécial"}, "Kotaro Amon": {"c4": "Inspecteur 1re classe"}}, "tokyorevengers": {"Takemichi Hanagaki": {"c1": "Capitaine"}}};
// Personnages retirés car ce sont des formes/doublons d'un autre personnage déjà présent
const DLE_EXCLUDE = {"sds": ["Estarossa"], "dragonball": ["Goku Black", "Future Trunks"], "sao": ["Death Gun"]};
...
const DLE_TOKEN_EXPANSIONS = {
    'cinq natures': ['feu', 'vent', 'foudre', 'terre', 'eau'],
    'yin yang': ['yin', 'yang'],
    // masculin / féminin / pluriel : "Humaine" = "Humain", "Condamnée" = "Condamné"...
    'humaine': ['humain'], 'geante': ['geant'], 'bestiale': ['bestial'], 'beastfolk': ['bestial'],
    'combattante': ['combattant'], 'guerriere': ['guerrier'], 'aventuriere': ['aventurier'],
    'etudiante': ['etudiant'], 'civile': ['civil'], 'independante': ['independant'], 'royale': ['royal'],
    'soldate du feu': ['soldat du feu'], 'candidate guerriere': ['candidat guerrier'], 'executrice': ['executeur'],
    'condamnee': ['condamne'], 'condamnes': ['condamne'], 'humaine d underworld': ['humain d underworld'],
    'ghoul artificielle': ['ghoul artificiel'], 'humaine modifiee': ['humain modifie'], 'apprentie': ['apprenti'],
    'fleaux': ['fleau'], 'alien': ['extraterrestre'], 'ennemi': ['antagoniste'], 'hashira': ['pilier'],
    'primordial demon': ['demon primordial'], 'homunculus': ['homunculi'], 'armee': ['armee d amestris'],
    'hunter association': ['association des hunters'], 'association hunter': ['association des hunters'],
    'zoldyck': ['famille zoldyck'], 'chatain': ['brun'], 'paysan': ['roturier'], 'commun': ['roturier']
};

function dleTokenSet(value) {
    const out = [];
    String(value ?? '')
        .replace(/\([^)]*\)/g, ' ')          // "(anime)", "(greffé)"… ne comptent pas comme jetons
        .split(/[\/,+&|;]| et | puis /)
        .map(x => normalizeDle(x))
        .filter(Boolean)
        .forEach(t => {
            const list = [...(DLE_TOKEN_EXPANSIONS[t] || [t])];
            // "Ancien Hokage", "Ex-12e" partagent aussi "Hokage", "12e" (orange)
            const stripped = t.replace(/^(ancien|ancienne|ex|futur|future)\s+/, '');
            if (stripped !== t) list.push(...(DLE_TOKEN_EXPANSIONS[stripped] || [stripped]));
            list.forEach(x => { if (!out.includes(x)) out.push(x); });
        });
    return out;
}

// "1re", "2e", "12" -> nombre ; sinon null
function dleNumberValue(value) {
    const m = String(value ?? '').trim().match(/^(\d+)\s*(re|er|e|ème|eme)?$/i);
    ...
    for (const cat of categories) {
        ...
        if (known) {
            if (cat.type === "number") {
                ...
                // Mêmes éléments dans un ordre différent ("Feu, Foudre" / "Foudre, Feu"), ou même valeur
                // au masculin/féminin ("Humain" / "Humaine") = bonne réponse
                const noDetail = !/[()]/.test(String(mine)) && !/[()]/.test(String(wanted));
                if (!match && a.length && a.length === b.size && a.every(x => b.has(x)) && (a.length > 1 || noDetail)) match = true;
                if (!match && !cat.meta) close = a.some(x => b.has(x));

                // Valeurs numériques (génération, année, numéro, rang...) : flèche plus haut / plus bas
                const na = dleNumberValue(mine), nb = dleNumberValue(wanted);
                if (!match && na !== null && nb !== null) {
                    direction = na < nb ? "up" : "down";
                    close = Math.abs(na - nb) <= 1;
                }
            }
        }
...
io.on('connection', (socket) => {
    ...
    socket.on('dle_guess', async ({ roomCode, guess }) => {
        ...
        if (room.dle.guesses.some(g => normalizeDle(g.name) === normalizeDle(character.name))) {
            socket.emit('dle_feedback', { ok:false, message:"Ce personnage a déjà été proposé." });
            return;
        }
