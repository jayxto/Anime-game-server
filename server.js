...
// ⚠️ Remplace par le chemin de ton fichier de configuration Firebase Admin (téléchargé depuis la console Firebase)
...
// Liste Undercover Normal (Paires) - VERROUILLÉE
...
// Liste Undercover Hardcore (Tous les persos avec leur anime) - VERROUILLÉE
...
io.on('connection', (socket) => {
    ...
    socket.on('join_room', async ({ roomCode, username, mode, subMode, firebaseToken }) => {
        let finalUsername = username;

        if (firebaseToken) {
            try {
                const decodedToken = await admin.auth().verifyIdToken(firebaseToken);
                console.log(`Utilisateur authentifié Firebase : ${decodedToken.uid}`);
            } catch (e) {
                console.log("Token Firebase invalide ou expiré, utilisation du pseudo simple.");
            }
        }
